...
这个通过只证明固定样本的来源、版本与公式可用，不是因子收益或选股证据。新的全市场来源与无收益协议已经冻结在 [`a_share_tushare_contract_liability_backlog_no_return_preregistration.json`](a_share_tushare_contract_liability_backlog_no_return_preregistration.json)（SHA‑256 `7528d5ab17c24b4c0904f6d213311a0a5132ab7a0c897fa572223e9de1455dc8`），但真实全量尚未运行。协议要求复用验收三股，不得重请求已消费的 12 次调用；其余 5,448 股四切片共 21,792 次，只能由 `sync-tushare-contract-liability-backlog --allow-large` 原子执行。任何失败删除完整临时快照并消耗协议。

全量下载入口需要按第 4 节把 `TUSHARE_TOKEN` 只注入该 Python 子进程。成功后生成的全量清单则交给 `python scripts/a_share_short_horizon_factor_research.py tushare-contract-liability-backlog-no-return-audit --manifest <全量清单路径>`；无收益命令不读取 Token、不访问 Tushare，也不应再套 Token 包装器。它先做 200-cohort、五年份容量门，只有通过后才加载 48 个稠密比较字段和毛利率终止语义证据。两个入口都已本地实现并测试，但真实全量与无收益审计均未运行。无收益通过后仍须另行冻结收益诊断；本文的 Token 配置步骤本身不构成运行全量、读取收益、聚合、评分、选股或下单的许可。
