...
这个通过只证明固定样本的来源、版本与公式可用，不是因子收益或选股证据。全市场来源与无收益协议冻结在 [`a_share_tushare_contract_liability_backlog_no_return_preregistration.json`](a_share_tushare_contract_liability_backlog_no_return_preregistration.json)（SHA‑256 `7528d5ab17c24b4c0904f6d213311a0a5132ab7a0c897fa572223e9de1455dc8`）。唯一全量已经复用验收三股，并对其余 5,448 股四切片完成 21,792 / 21,792 次请求，得到 217,539 条来源行和 76,511 个开发期完整事件。事件数、七个信号年份和 78.10% 的报告期覆盖中位数通过，但 P05 覆盖只有 2.12%，低于冻结的 30% 门槛；本次冻结快照中 2019Q1–2020Q4 单期覆盖只有 0.19%–4.03%。因此来源门终止，临时快照已删除，没有年度分区或成功全量清单。

跨克隆终止记录为 [`a_share_tushare_contract_liability_backlog_research_record.json`](a_share_tushare_contract_liability_backlog_research_record.json)（SHA‑256 `982d3014b82b13ef85017e68ed8e4a2fa2e23336b752d3d488169b1a2daf2519`）。不得再次运行 `sync-tushare-contract-liability-backlog --allow-large`；入口会在协议、合同、Token 和供应商访问前拒绝。因为没有成功全量清单，也不得运行 `tushare-contract-liability-backlog-no-return-audit`；该入口会在清单、价格基座、质量数据和比较字段之前拒绝。不要局部续传、删除 2019–2020 历史、降低覆盖门槛或改变字段/切片/版本规则/公式。Token 的配置、更新或轮换不会恢复该分支，也不授权容量、唯一性、收益、聚合、评分、选股或下单。
