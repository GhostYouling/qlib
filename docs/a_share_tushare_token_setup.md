...
该命令不会把 Token 作为 Python 参数或文件内容保存。`--allow-large` 只是对已经冻结的长任务进行显式确认，不会放宽数据合同。运行期间不要再启动第二个相同同步；若出现锁，先确认现有进程，不要直接删除锁文件。全量来源成功后，容量阶段仍不得读取价格或收益；只有容量门通过，唯一性阶段才可按预注册临时计算当日收盘已知和历史收盘已知的比较字段，并且仍不得读取未来价格或收益。来源成功不代表可以直接做收益诊断、聚合、选股或下单。
