...
`fina_audit` 的三股票一次性来源验收已经成功并永久消费，跟踪记录是 [`a_share_tushare_audit_opinion_source_acceptance_record.json`](a_share_tushare_audit_opinion_source_acceptance_record.json)。不要再次运行 `acceptance-tushare-audit-opinion`，也不要换股票、日期、字段或文本映射重验；生产入口会先读取跟踪记录并在访问 Token 或供应商之前拒绝。

该验收只证明当前账户权限、四字段响应、公告日可用性和严格二值公式成立，没有读取价格或收益，也没有证明因子有效。当前下一步是先实现和本地验证冻结的 2019–2025 全量来源同步；仓库尚未提供获准的审计意见全量命令，因此不要从聊天记录自行拼接请求。等全量实现、测试和来源覆盖门都通过后，仍须先做无收益容量与唯一性检查，才可能另行冻结收益诊断协议。
