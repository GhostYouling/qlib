...
实现入口是 `python scripts/a_share_rich_data.py sync-tushare-contract-liability-backlog --allow-large`。该命令目前只完成了合同加载、验收复用、精确请求计数、原子发布和消费守卫的本地实现与测试；本文记录它不等于授权立即运行，实际请求前还必须确认完整测试、磁盘/进程状态和当前研究决定。`TUSHARE_TOKEN` 只从进程环境读取，不能写入命令、文档或清单。
