...
### Tushare 龙虎榜机构席位（仅冻结数据合同）

下一个独立候选是 `top_inst` 机构专用席位净买入机制。数据合同 `docs/a_share_tushare_top_inst_data_contract.json`（SHA‑256 `0520cd8bac454f14c2434cf7b8092aaaf3ff94cdc09b5404a6b55323bf0e7461`）已在任何 `top_inst` 权限结果、数据行、全历史、因子值或因子收益被观察前冻结。唯一候选固定为同股票同日所有唯一机构席位的 `(sum(buy)-sum(sell))/(sum(buy)+sum(sell))`，高值方向固定为更好；供应商 `net_buy` 只用于逐行完整性对账，缺失事件不补零、不向前填充。

当前只允许针对已收盘的 `2026-07-13` 运行一次无收益、单调用权限/字段/键/量纲验收，并要求机构席位股票全部存在于同日已验收的原始 `top_list` 快照。尚未发出该 API 请求，也尚未授权全历史同步、容量/唯一性门、收益诊断、聚合、当前评分、选股、仓位或订单。单日验收若失败，必须保留一份不读收益的拒绝记录并停止，不得换日期、改字段或换席位类型重试。
