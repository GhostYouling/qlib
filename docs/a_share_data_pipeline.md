...
跨克隆验收记录为 [`a_share_tushare_contract_liability_backlog_source_acceptance_record.json`](a_share_tushare_contract_liability_backlog_source_acceptance_record.json)（SHA‑256 `5a06db91c904c38bf0415cbff7cec6987e72212d8349a3805ca4a7749911295b`）。验收入口会在合同、Token 和供应商访问前拒绝重跑；不得换股票、切片、字段、版本规则、公式或方向。该结果只证明固定样本的来源、版本和公式可用，不是收益、执行或当前选股证据。下一步必须先另行冻结全市场来源与无收益协议，绑定本次 manifest/Parquet、点时来源宇宙、日历、精确调用数和切片、原子性、覆盖率、三日非重叠容量以及包括资产负债表韧性、核心利润、PB 和毛利率在内的稠密近邻唯一性；在这些门禁通过前不得请求全量、读取价格/收益、聚合、评分、选股、定仓、下单或采购 Level‑2。
