...
## Campaign301 Alpha158/日线残余前沿终局（2026-08-25）

Campaign301 在候选、比较器、日线价格和 forward return 值之前，对当前仍看似可执行的 Alpha158 与本地日线 OHLCV 路线做了 11 项有限审计。K 线几何、趋势/残差、分位/极值、量价依赖、涨跌符号矩、成交活跃度、peer/multi-horizon 几何、完整 Alpha158 模型以及日线缺口/日内指标均已有明确终止记录；对旧因子做子集、配对、调权、阈值、过滤或模型搜索属于禁止的曝光后组合。

10 条路线因此在值前关闭。Alpha360 继续标记为基础设施延期而非预测失败：授权工作区只剩 11,008,628 KiB，不能安全容纳五年 360 维稠密快照、原子临时副本和模型输出；不得缩短历史、曝光后选择股票或通过外部 `data` symlink 绕开写边界。

Campaign301 选中因子、完整因子尝试、收益读取开发试验和 2024–2025 压力试验均为 0。11 条值前科学尝试使累计历史尝试更新为 3,107，累计收益读取开发试验保持 331；重复控制库保持 `172/153`，v431 不追加。Candidate49 仍是唯一前瞻候选且账本 0/0，2026-08-25 不得同日重试；没有 provider 请求、历史回填、当前评分、选股、仓位、订单或投资建议。Campaign302 只能从真正新的已接受信息通道或单独冻结的资源安全机制开始。

## Campaign302 完整 Alpha360 时序模型实现终止（2026-08-25）

Campaign302 采用单独冻结的资源安全机制，不等待新增日线或 16:30。完整 Qlib Alpha360 按固定顺序重塑为 60×6 时序；唯一配置是 8 维、2 头、1 层 Transformer，AdamW 固定 5 epoch、seed 302、无早停或参数搜索。训练会话先按不可变身份哈希抽 64 只，270/360 有限特征门槛、训练中位数/IQR 预处理、三组 2019–2023 expanding folds、三信号会话清除、三日持有、Top3、成本和合取 survivor 门均在特征与收益值前冻结。

零收益设计三折全部完成：预选/支持行为 8,512/8,369、13,696/13,451、18,816/18,492，共持久化 54,197,216 字节训练特征。第一折随后读取训练收益，在 8,369 行上完成 5 epoch，训练损失从 `0.208604` 降至 `0.092226`。2021 Alpha360 验证特征完成流式评分，但在生成分数覆盖门禁时，冻结 runner 的 `DatetimeIndex.eq()` 调用触发 `AttributeError`；分数快照、验证指标和任何验证收益均未产生。退出码为 1，失败记录已保留，同 Campaign 未重试且冻结 runner 未修改。

这是一项实现失败，不是预测失败或验证通过。第二、三折模型未训练，验证折收益读取数为 0，2024–2025 未打开，survivor 为 0。8 条值前概念、3 次基础设施/实现失败与 1 次完整模型尝试使 Campaign302 有效尝试为 12；累计历史尝试更新为 3,119、累计收益读取开发试验为 332。v432 保持重复控制库 `172/153`，不追加 Campaign302。Candidate49 仍是唯一前瞻候选且账本 0/0；无 provider 请求、凭据加载、历史回填、当前评分、选股、仓位、订单或投资建议。未来只能使用新标识、单独预注册且科学选择完全不变的实现恢复 Campaign，或从真正新信息开始。
