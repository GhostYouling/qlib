#!/usr/bin/env python3
"""Build and verify Campaign070 disclosure-delay improvement ranks."""
...
TEMPLATE_RUNNER = REPO_ROOT / "scripts/a_share_three_day_walkforward_campaign069_features.py"
TEMPLATE_RUNNER_SHA256 = "f72cbd4b88aca872d7255fee60151e5344ae72b25b41c0e808c1a2e1698e0b75"
FACTOR_NAME = "quarterly_announcement_delay_improvement_yoy_rank_1y"
FACTOR_FORMULA = (
    "prior-year same-quarter announcement delay minus current announcement "
    "delay, strict-next-session activation, finite-state forward fill, then "
    "same-stock-day average-tie percentile rank"
)
PROTOCOL_SHA256 = "5474879454fbc3113fb386b4ebc4a1e1e0643a168511ebfe39d8bd9fc9a96952"
MECHANISM_AUDIT_SHA256 = (
    "b561bd2b53051d25caeaf987345d87829623d7ca8c57481eb982158e86dfc9ec"
)
NUMERIC_POLICY_SHA256 = (
    "afdb235a9aac5d6cde01b5e4edf57cdd3da15fd3adac035c69f284408bead913"
)
FULL_DEFINITION_COUNT = 101
FULL_DEFINITION_ORDER_SHA256 = (
    "0bc65c0bcae2dd089ceb8c67ea606be483673caac5cbce4ce9dc9e89b04fd124"
)
COMPARISON_COUNT = 100
COMPARISON_ORDER_SHA256 = (
    "d1c58576dcdef7980c279426f135440d05f8484d2f42d4b4866539b963a0b7ee"
)
...
C68_SNAPSHOT_DATASET_SHA256 = (
    "bcd03a5e2122b64b072985b3a9a9ada880cad337dae4766a00cf52bf51f4f449"
)
C69_SNAPSHOT_MANIFEST_SHA256 = (
    "6023becea579d0f6ed6ac27316421294f13fd6759d6d51790a31940526e363b7"
)
C69_SNAPSHOT_DATASET_SHA256 = (
    "7c4b2e446bf4ebb64b4d3e992510610b1f0059fde868dff0bf02a06b7ce75960"
)
C69_SNAPSHOT_BINDING_SHA256 = (
    "b98b14a2ad5b0c95a5a69b48b46fc751df0e877f8daf4cc495ff447193e6eec0"
)
C69_RESEARCH_RECORD_SHA256 = (
    "85f1161e86beba49a1b5841dec335d627171702a633270a47b52899dc11f24bd"
)
...
class Campaign070FeatureError(RuntimeError):
    """Fail-closed Campaign070 feature error."""
...
if not TEMPLATE_RUNNER.is_file() or _sha256(TEMPLATE_RUNNER) != TEMPLATE_RUNNER_SHA256:
    raise RuntimeError("frozen Campaign069 feature runner changed")


_source = TEMPLATE_RUNNER.read_text(encoding="utf-8")
for _old, _new in (
    ("Campaign069", "Campaign070"),
    ("campaign069", "campaign070"),
    ("campaign_069", "campaign_070"),
    ("quarterly_profit_growth_roe_transition_gap_2r", FACTOR_NAME),
    ("r_profit_yoy-r_roe", FACTOR_FORMULA),
    ("c044776cff0031ac3e65a4a3de51c06b90e1c1c0c1ba94895f79e900628dffe1", PROTOCOL_SHA256),
    ("c3856cc3d89632cc502747ef44602247e2afe1bd526a62a53995aff91df782db", MECHANISM_AUDIT_SHA256),
    ("FULL_DEFINITION_COUNT = 100", "FULL_DEFINITION_COUNT = 101"),
    ("ba1d04cb281f0ed64ae792cc68be6257e2e53716d2c294138d0336ca01d59e7d", FULL_DEFINITION_ORDER_SHA256),
    ("COMPARISON_COUNT = 99", "COMPARISON_COUNT = 100"),
    ("7f1ed85abfeb4688c0b07f892b497942a202f09ac8806ea095b6c9050f670538", COMPARISON_ORDER_SHA256),
    ('EVENT_FIELDS = ("instrument", "report_date", "announcement_date", "roe", "profit_yoy")', 'EVENT_FIELDS = ("instrument", "report_date", "announcement_date")'),
    ('STATE_FIELDS = ("profit_yoy", "roe")', 'STATE_FIELDS = ("improvement_days",)'),
    ("all_99_numeric_comparators_must_pass", "all_100_numeric_comparators_must_pass"),
    ("wf069_", "wf070_"),
):
...
_namespace: dict[str, Any] = {
    ...
    "__name__": "a_share_three_day_walkforward_campaign070_features_generated",
}
...
DEFAULT_PROTOCOL = (
    REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_070_no_return_preregistration.json"
)
DEFAULT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_070_feature_implementation_freeze_20260806.json"
)


def _require_link(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign070FeatureError(f"Campaign070 {label} changed")
...
def reconstruct_comparisons(
    ...
    if (
        ...
        raise Campaign070FeatureError("Campaign070 comparison order changed")
    return items
...
def _validate_comparator_sources(spec: dict[str, Any]) -> None:
    ...
    for raw_path, expected, label in (
        ...
        (c69.get("research_record_path"), C69_RESEARCH_RECORD_SHA256, "Campaign069 record"),
        (c69.get("snapshot_binding_path"), C69_SNAPSHOT_BINDING_SHA256, "Campaign069 snapshot binding"),
    ...
    cache_manifest = json.loads(Path(str(cache["manifest_path"])).read_text(encoding="utf-8"))
    c68_manifest = json.loads(Path(str(c68["snapshot_manifest_path"])).read_text(encoding="utf-8"))
    c69_manifest = json.loads(Path(str(c69["snapshot_manifest_path"])).read_text(encoding="utf-8"))
    if not (
        cache_manifest.get("dataset_sha256") == CACHE_DATASET_SHA256
        ...
        and c69_manifest.get("dataset_sha256") == C69_SNAPSHOT_DATASET_SHA256
        and c69_manifest.get("factor_names") == [C69_FACTOR]
        and (c69_manifest.get("factor_directions") or {}).get(C69_FACTOR) == "higher"
    ):
        raise Campaign070FeatureError("Campaign070 comparator source semantics changed")
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign070FeatureError("Campaign070 protocol binding failed")
    ...
    if not (
        spec.get("kind")
        == "a_share_three_day_walkforward_campaign070_no_return_preregistration"
        and spec.get("status")
        == "frozen_before_campaign070_source_candidate_comparison_daily_price_or_return_values"
        ...
        and candidate.get("prior_event_rule")
        == "exact_same_instrument_and_report_date_minus_one_calendar_year"
        ...
        == COMPARISON_ORDER_SHA256
        and unique.get("all_100_numeric_comparators_must_pass") is True
        and len(comparisons) == COMPARISON_COUNT
        and finite.get("trial_id")
        == "wf070_quarterly_announcement_delay_improvement_yoy_rank_1y_single_higher"
        ...
        and boundary.get("quarterly_or_stock_day_source_rows_read_before_this_freeze")
        is False
        ...
        raise Campaign070FeatureError("Campaign070 protocol semantics changed")
    return spec


def compute_improvement_events(events: pd.DataFrame) -> pd.DataFrame:
    if tuple(events.columns) != EVENT_FIELDS:
        raise Campaign070FeatureError("quarterly source projection changed")
    work = events.copy()
    ...
    if (
        ...
        raise Campaign070FeatureError("quarterly event identities changed")
    valid_quarter = (
        ((work["report_date"].dt.month == 3) & (work["report_date"].dt.day == 31))
        | ((work["report_date"].dt.month == 6) & (work["report_date"].dt.day == 30))
        | ((work["report_date"].dt.month == 9) & (work["report_date"].dt.day == 30))
        | ((work["report_date"].dt.month == 12) & (work["report_date"].dt.day == 31))
    )
    if not valid_quarter.all():
        raise Campaign070FeatureError("quarterly report dates escaped Q-DEC")
    prior = work.rename(
        columns={
            "report_date": "prior_report_date",
            "announcement_date": "prior_announcement_date",
        }
    )
    current = work.copy()
    current["prior_report_date"] = current["report_date"] - pd.DateOffset(years=1)
    joined = current.merge(
        prior[["instrument", "prior_report_date", "prior_announcement_date"]],
        on=["instrument", "prior_report_date"],
        how="left",
        validate="many_to_one",
    )
    current_delay = (joined["announcement_date"] - joined["report_date"]).dt.days
    prior_delay = (
        joined["prior_announcement_date"] - joined["prior_report_date"]
    ).dt.days
    valid = (
        current_delay.notna()
        & prior_delay.notna()
        & current_delay.ge(0)
        & prior_delay.ge(0)
        & joined["prior_announcement_date"].lt(joined["announcement_date"])
    )
    joined["improvement_days"] = np.where(
        valid,
        prior_delay.to_numpy(dtype=float) - current_delay.to_numpy(dtype=float),
        np.nan,
    )
    return joined.loc[
        :,
        ["instrument", "report_date", "announcement_date", "improvement_days"],
    ]
...
def prepare_events(
    ...
    work = compute_improvement_events(events)
    work = work.loc[np.isfinite(work["improvement_days"])].copy()
    ...
    for symbol, group in work.groupby("instrument", sort=False):
        result[str(symbol)] = (
            ...
            group[["improvement_days"]].to_numpy(dtype=np.float64),
        )
    return result
...
def attach_states(
    ...
    *,
    ...
    if tuple(frame.columns) != ("trade_date", "symbol", "provider"):
        raise Campaign070FeatureError("joint-clean identity projection changed")
    out = frame.copy()
    out["trade_date"] = pd.to_datetime(out["trade_date"], errors="coerce").dt.normalize()
    out["symbol"] = out["symbol"].astype(str).str.upper()
    out["provider"] = out["provider"].astype(str).str.lower()
    if out.empty:
        out["improvement_days"] = pd.Series(dtype="float64")
        return out
    if (
        ...
        raise Campaign070FeatureError(f"joint-clean identity changed for {symbol}")
    calendar_values = calendar.to_numpy(dtype="datetime64[ns]")
    ...
    if (
        ...
        raise Campaign070FeatureError(f"joint-clean dates escaped calendar for {symbol}")
    ...
    out["improvement_days"] = states
    return out


def rank_improvement_year_frame(frame: pd.DataFrame) -> pd.DataFrame:
    work = frame.copy()
    ranks = work.groupby("trade_date", sort=False)["improvement_days"].rank(
    ...
    work["provider"] = "eastmoney_disclosure_timing"
    return work


def validate_value_semantics(frame: pd.DataFrame) -> tuple[int, int]:
    if tuple(frame.columns) != OUTPUT_COLUMNS:
        raise Campaign070FeatureError("Campaign070 partition columns changed")
    ...
    if (
        ...
        raise Campaign070FeatureError("Campaign070 value semantics changed")
...
def verify_snapshot_files(
    ...
    if manifest_path != expected:
        raise Campaign070FeatureError("Campaign070 manifest path changed")
    ...
    def verify(item: dict[str, Any]) -> tuple[int, int]:
        ...
        try:
            ...
            raise Campaign070FeatureError("partition escaped output root") from exc
        if _sha256(path) != item["output_byte_sha256"]:
            raise Campaign070FeatureError(f"partition byte hash changed: {path}")
        frame = pd.read_parquet(path)
        if (
            len(frame) != item["rows"]
            or _runtime["_frame_sha256"](frame) != item["output_frame_sha256"]
        ):
            raise Campaign070FeatureError(f"partition frame changed: {path}")
    ...
    if not (
        ...
        raise Campaign070FeatureError("Campaign070 aggregate identity changed")
    return {
        "status": "verified",
...
for _name, _value in {
    ...
    "rank_and_balance_year_frame": rank_improvement_year_frame,
