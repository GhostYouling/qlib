#!/usr/bin/env python3
"""Run Campaign099's frozen coverage-first all-127-comparator no-return audit."""
...
from scripts import (
    a_share_three_day_walkforward_campaign098_no_return_audit as c98_audit,
)
from scripts import a_share_three_day_walkforward_campaign099_features as definitions
...
SNAPSHOT_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_099_feature_snapshot_binding_20260807.json"
)
SNAPSHOT_BINDING_SHA256 = (
    "e90f88f2fd3f102ecfc20610978e125320d2f51e8d155cb9b82e2759824c59f8"
)
AUDIT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_099_no_return_audit_implementation_freeze_v3_20260807.json"
)
AUDIT_ACTIVATION_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_099_no_return_audit_activation_binding_v3_20260807.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign099_no_return_audit.py"
)
...
C98_MANIFEST_PATH = c98_audit.SNAPSHOT_MANIFEST_PATH
C98_MANIFEST_SHA256 = c98_audit.SNAPSHOT_MANIFEST_SHA256
C98_DATASET_SHA256 = c98_audit.SNAPSHOT_DATASET_SHA256
C98_FACTOR_NAME = c98_audit.FACTOR_NAME
...
EXPECTED_COMPARISON_COUNT = 127
EXPECTED_COMPLETE_DEFINITION_COUNT = 130
...
class Campaign099NoReturnAuditError(RuntimeError):
    """Fail closed on an altered Campaign099 no-return audit boundary."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign099NoReturnAuditError(f"Campaign099 {label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        and comparisons[-1] == {"name": C98_FACTOR_NAME, "score_direction": "higher"}
        ...
        raise Campaign099NoReturnAuditError("Campaign099 definition order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign099NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign099_no_return_audit_implementation_freeze"
        and record.get("status")
        == "postvalue_atomic_writer_repair_frozen_before_identical_retry"
        ...
        and record.get("comparison_values_read_before_freeze_conservative") is True
        and record.get(
        ...
        raise Campaign099NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign099NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign099_no_return_audit_activation_binding"
        and record.get("status")
        == "postvalue_atomic_writer_repair_frozen_before_identical_retry"
        ...
        is True
        and record.get("comparison_values_read_before_activation_conservative") is True
        and record.get(
        ...
        raise Campaign099NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int) -> dict[str, int]:
    ...
    result = definitions.verify_snapshot_files(SNAPSHOT_MANIFEST_PATH, workers=workers)
    if not (
        result.get("partitions_verified") == EXPECTED_RAW_PARTITIONS
        and result.get("rows_verified") == EXPECTED_RAW_ROWS
        and result.get("eligible_rows_verified") == EXPECTED_RAW_ELIGIBLE_ROWS
    ):
        raise Campaign099NoReturnAuditError("candidate snapshot verification changed")
    return result
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(C98_MANIFEST_PATH, C98_MANIFEST_SHA256, "Campaign098 comparator")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign099NoReturnAuditError("snapshot binding validation failed")
    return {
...
def align_candidate_year_with_missing(
    *,
    ...
    """Left-align the frozen denominator and retain absent candidates as NaN."""
    ...
    if not (
        ...
        raise Campaign099NoReturnAuditError("candidate alignment inputs changed")
    ...
    aligned_values[matched] = sorted_values[positions[matched]]
    finite = np.isfinite(aligned_values)
    if np.any((aligned_values[finite] < -1.0) | (aligned_values[finite] > 1.0)):
        raise Campaign099NoReturnAuditError("candidate values left frozen range")
    stats = {
        "year": year,
        "quality_listing_keys": int(len(frozen_keys)),
        "matched_candidate_keys": int(matched.sum()),
        "missing_candidate_keys_retained_as_nan": int((~matched).sum()),
        "candidate_keys_outside_quality_listing_denominator": int(
            len(observed_keys) - matched.sum()
        ),
    }
    return (
        frozen_keys,
...
def load_candidate_arrays() -> (
    ...
    """Align candidate values to frozen identity keys without comparator columns."""
    ...
    candidate_by_year = c97_audit._records_by_year(
        candidate_manifest.get("files") or []
    )
    eligibility_by_year = {
    ...
    if not (
        ...
        raise Campaign099NoReturnAuditError(
    ...
    alignment_by_year: list[dict[str, int]] = []
    raw_rows_read = 0
    for year in range(2019, 2026):
        ...
        for record in candidate_by_year[year]:
            ...
            year_keys.append(
                c97_audit.compact_stock_day_keys(frame["trade_date"], frame["symbol"])
            )
            year_values.append(frame[FACTOR_NAME].to_numpy(dtype=np.float64))
            raw_rows_read += len(frame)
        keys, values, years, alignment_stats = align_candidate_year_with_missing(
        ...
        alignment_by_year.append(alignment_stats)
    ...
    if not (
        ...
        raise Campaign099NoReturnAuditError("aligned candidate arrays changed")
    receipt = {
        ...
        "matched_candidate_identities": int(
            sum(item["matched_candidate_keys"] for item in alignment_by_year)
        ),
        "missing_candidate_identities_retained_as_nan": int(
            sum(
                item["missing_candidate_keys_retained_as_nan"]
                for item in alignment_by_year
            )
        ),
        "identity_alignment_by_year": alignment_by_year,
...
def _install_frozen_ranges(engine: Any) -> None:
    c98_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (-1.0, 1.0):
        raise Campaign099NoReturnAuditError("conflicting Campaign099 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c98_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    if len(comparisons) != 126:
        raise Campaign099NoReturnAuditError("first 126 comparison order changed")
    result, receipt = compact_helper._compact_snapshot_comparison(
        manifest_path=C98_MANIFEST_PATH,
        expected_manifest_sha256=C98_MANIFEST_SHA256,
        factor=C98_FACTOR_NAME,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        comparison_engine=comparison_engine,
        verifier=c98_audit.verify_candidate_snapshot,
    )
    comparisons.append(result)
    receipts["campaign098_compact_snapshot"] = receipt
    receipts.pop("all_126_sources_loaded_in_frozen_order", None)
    receipts["all_127_sources_loaded_in_frozen_order"] = True
    if not (
        len(comparisons) == EXPECTED_COMPARISON_COUNT
        and comparisons[-1]["comparison_factor"] == C98_FACTOR_NAME
    ):
        raise Campaign099NoReturnAuditError("Campaign099 comparison order changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int, confirm_run: bool
    ...
    if not confirm_run:
        raise Campaign099NoReturnAuditError("Campaign099 audit requires --confirm-run")
    static = verify_static_bindings()
    spec = load_protocol()
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign099NoReturnAuditError("Campaign099 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign099_no_return_audit.json")):
        raise Campaign099NoReturnAuditError("Campaign099 audit already exists")
    ...
    payload = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign099_no_return_audit",
        "status": (
        ...
        "run_id": f"{run_id}_campaign099_no_return_audit",
        "protocol": {
    ...
    definitions.c97.base.foundation.atomic_write_json(payload, path)
    return path
    ...
    return {
        ...
        "audit_count": len(
            list(
                ...
                .resolve()
                .glob("*_campaign099_no_return_audit.json")
            )
        ),
