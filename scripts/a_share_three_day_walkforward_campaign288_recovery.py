#!/usr/bin/env python3
"""Recover Campaign288 by applying the frozen ordinal map before market filtering."""
...
RECOVERY_OUTPUT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_288/ordinal_rff_ridge_recovery_v1"
)
FAILURE_RECORD_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_288_training_projection_center_failure_20260825.json"
)
FAILURE_RECORD_SHA256 = "8cbc1980bd80fc50ee924b9e6dc79a0a9a3ae8f5730cb8843d075fb52b985645"
ORIGINAL_INTENT_PATH = ORIGINAL_OUTPUT_ROOT / "development_intent.json"
ORIGINAL_INTENT_SHA256 = "1ae95e3b17cc5ca9e2b642a6e522b4fbc3b647af679baff846fe176b3465733b"
ORIGINAL_FAILURE_PATH = ORIGINAL_OUTPUT_ROOT / "development_failure.json"
ORIGINAL_FAILURE_SHA256 = "073491dce2511d7f082ace0f26dc47ea526a101f4e4b754e40f5c87d53810286"
RECOVERY_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_288_projection_peer_recovery_implementation_freeze_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign288_recovery.py"
)


class Campaign288RecoveryError(RuntimeError):
    """Fail closed when the bounded Campaign288 recovery changes."""


def validate_failure_bindings() -> None:
    bindings = (
        (FAILURE_RECORD_PATH, FAILURE_RECORD_SHA256, "Campaign288 failure record"),
        (ORIGINAL_INTENT_PATH, ORIGINAL_INTENT_SHA256, "Campaign288 original intent"),
        (ORIGINAL_FAILURE_PATH, ORIGINAL_FAILURE_SHA256, "Campaign288 original failure"),
    )
    ...
    if existing != ["design_evidence.json", "development_failure.json", "development_intent.json"]:
        raise Campaign288RecoveryError("original failed output root changed")
    failure = campaign.load_json(ORIGINAL_FAILURE_PATH)
    if not (
        failure.get("error")
        == "training projection center differs from zero-return design"
        and failure.get("lockbox_2024_2025_return_fields_read") is False
        and failure.get("candidate49_ledgers_changed") is False
    ):
        raise Campaign288RecoveryError("original failure semantics changed")


def validate_recovery_freeze() -> dict[str, Any]:
    if not RECOVERY_FREEZE_PATH.is_file():
        raise Campaign288RecoveryError("Campaign288 recovery freeze missing")
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign288_projection_peer_recovery_implementation_freeze"
        ...
        raise Campaign288RecoveryError("Campaign288 recovery freeze changed")
    return record


def factor_frame(
    identities: pd.DataFrame, ordinal_matrix: np.ndarray, eligible: np.ndarray
) -> pd.DataFrame:
    values = np.asarray(ordinal_matrix, dtype=np.float32)
    ...
    if not (
        ...
        raise Campaign288RecoveryError("precomputed ordinal factor frame changed")
...
def build_training_panel_with_frozen_peer_set(
    ...
    ordinal_stats = campaign.session_ordinal_transform_inplace(
...
def fit_training_fold_recovery(
    ...
    dict[str, Any],
    campaign.RandomFourierRidgeRegressor,
    dict[str, Any],
]:
    ...
    if ordinal_stats != bundle["record"]["session_ordinal_transform"]:
        raise Campaign288RecoveryError("recovered ordinal peer-set evidence changed")
    ...
    if len(sample_counts) < campaign.MINIMUM_RETAINED_TRAINING_SESSIONS:
        raise Campaign288RecoveryError("recovered sampled panel coverage changed")
    ...
    model = campaign.RandomFourierRidgeRegressor(
        dict(campaign.MODEL_PARAMETERS), bundle["projection_center"]
    ).fit(matrix[valid], target[valid], target_weights[valid])
    model_path = campaign.OUTPUT_ROOT / f"fold_{fold['fold']}_model" / f"{campaign.TRIAL_ID}.npz"
    ...
    fit = {
        ...
        "frozen_peer_set_recovery": {
            "ordinal_map_computed_before_market_quality_filter": True,
            ...
            "frozen_design_projection_center_used_unchanged": True,
...
def plan_recovery() -> dict[str, Any]:
    ...
    return {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign288_recovery_plan",
        "status": "ready_for_bounded_recovery" if ready else "not_ready_preserve_existing_recovery_output",
        "ready": ready,
...
def run_recovery(confirm: bool, batch_size: int) -> dict[str, Any]:
    if not confirm:
        raise Campaign288RecoveryError("run-recovery requires --confirm-run")
    payload = plan_recovery()
    if payload["ready"] is not True:
        raise Campaign288RecoveryError("Campaign288 recovery plan is not ready")
    RECOVERY_OUTPUT_ROOT.mkdir(parents=True, exist_ok=False)
    recovery_intent = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign288_projection_peer_recovery_intent",
        ...
        "frozen_design_sample_projection_and_model_unchanged": True,
...
def main() -> int:
    ...
    if args.command == "plan-recovery":
        ...
        raise Campaign288RecoveryError(f"unsupported command: {args.command}")
