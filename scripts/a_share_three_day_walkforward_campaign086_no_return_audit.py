#!/usr/bin/env python3
"""Run Campaign086's frozen coverage-first no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign085_no_return_audit as c85_audit
from scripts import a_share_three_day_walkforward_campaign086_features as definitions
from scripts import a_share_three_day_walkforward_campaign086_features_v4 as candidate
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_086/no_return"
)
...
AUDIT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_086_no_return_audit_implementation_freeze_20260807.json"
)
AUDIT_ACTIVATION_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_086_no_return_audit_activation_binding_20260807.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign086_no_return_audit.py"
)
EXPECTED_ROWS = 1_331_759
EXPECTED_PARTITIONS = 7
EXPECTED_ELIGIBLE_ROWS = 1_193_690
EXPECTED_SESSIONS = 1_632
EXPECTED_COMPARISON_COUNT = 115
EXPECTED_COMPLETE_DEFINITION_COUNT = 117
...
C85_SNAPSHOT_MANIFEST_PATH = c85_audit.SNAPSHOT_MANIFEST_PATH


class Campaign086NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign086 no-return audit error."""
...
def _json_digest(value: Any) -> str:
    return definitions._json_digest(value)


def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign086NoReturnAuditError(f"Campaign086 {label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign086NoReturnAuditError("Campaign086 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign086NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign086_no_return_audit_implementation_freeze"
        and record.get("status")
        == "frozen_before_campaign086_coverage_or_comparison_values"
        ...
        raise Campaign086NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign086NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign086_no_return_audit_activation_binding"
        and record.get("status")
        == "frozen_before_campaign086_coverage_or_comparison_values"
        ...
        raise Campaign086NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign086NoReturnAuditError("candidate snapshot verification changed")
    return result
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign086NoReturnAuditError("snapshot binding validation failed")
    return {
        "audit_implementation_freeze_sha256": _sha256(
            AUDIT_IMPLEMENTATION_FREEZE
        ),
...
def load_candidate_arrays() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not (
        ...
        raise Campaign086NoReturnAuditError("candidate arrays changed")
...
def coverage_and_capacity(
    ...
    return c85_audit.coverage_and_capacity(keys, values, years, spec)


def _install_frozen_ranges(engine: Any) -> None:
    c85_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign086NoReturnAuditError("conflicting Campaign086 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    helper = (
        c85_audit.c84_audit.c83_audit.c82_audit.c81_audit.c80_audit.c78_audit.c77_audit.c76_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    )
    result, receipt = helper(
        manifest_path=C85_SNAPSHOT_MANIFEST_PATH,
        factor=c85_audit.candidate.FACTOR_NAME,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: c85_audit.verify_candidate_snapshot(),
    )
    comparisons.append(result)
    receipts["campaign085_snapshot"] = receipt
    receipts.pop("all_114_sources_loaded_in_frozen_order", None)
    receipts["all_115_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign086NoReturnAuditError("Campaign086 comparison count changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int, confirm_run: bool
    ...
    if not confirm_run:
        raise Campaign086NoReturnAuditError("Campaign086 audit requires --confirm-run")
    static = verify_static_bindings()
    spec = load_protocol()
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign086NoReturnAuditError("Campaign086 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign086_no_return_audit.json")):
        raise Campaign086NoReturnAuditError("Campaign086 audit already exists")
    ...
    payload = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign086_no_return_audit",
        "status": (
        ...
        "run_id": f"{run_id}_campaign086_no_return_audit",
        "protocol": {
    ...
    return {
        ...
        "audit_count": len(
            list(
                ...
                .resolve()
                .glob("*_campaign086_no_return_audit.json")
            )
        ),
