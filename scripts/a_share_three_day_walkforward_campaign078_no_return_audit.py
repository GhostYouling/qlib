#!/usr/bin/env python3
"""Run Campaign078's frozen coverage-first no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign077_no_return_audit as c77_audit
from scripts import a_share_three_day_walkforward_campaign077_no_return_audit_v2 as c77_audit_v2
from scripts import a_share_three_day_walkforward_campaign078_features as candidate
...
SNAPSHOT_MANIFEST_SHA256 = "a8d2f2a68dba3f7f3a3f34ac8d203f1381e429612ded6daf6c6447e0ed983071"
SNAPSHOT_DATASET_SHA256 = "225c539824978df0cc3e4b34a9520aa919ffa77ba73246c21e1bdb6122a0227d"
SNAPSHOT_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_078_feature_snapshot_binding_20260806.json"
SNAPSHOT_BINDING_SHA256 = "75a9fe0927308ddcbeccf128b89145e5f82e5c8272c52404494c6c95f4a9b1dc"
AUDIT_IMPLEMENTATION_FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_078_no_return_audit_implementation_freeze_20260806.json"
AUDIT_ACTIVATION_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_078_no_return_audit_activation_binding_20260806.json"
TEST_PATH = REPO_ROOT / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign078_no_return_audit.py"
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 7_724_498
EXPECTED_COMPARISON_COUNT = 108
EXPECTED_COMPLETE_DEFINITION_COUNT = 109
...
C77_SNAPSHOT_MANIFEST_PATH = candidate.C77_MANIFEST_PATH
STRUCTURALLY_NONNUMERIC_FACTOR = "intraday_cross_sectional_standardized_return_state_stability_236p"


class Campaign078NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign078 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign078NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign078NoReturnAuditError("Campaign078 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign078NoReturnAuditError("audit implementation freeze is absent")
    ...
    if not (
        ...
        raise Campaign078NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign078NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign078_no_return_audit_activation_binding"
        ...
        raise Campaign078NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int = 4) -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign078NoReturnAuditError("candidate snapshot verification changed")
    return result


def verify_campaign077_snapshot(*, workers: int = 4) -> dict[str, Any]:
    return c77_audit.verify_candidate_snapshot(workers=workers)
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(SNAPSHOT_MANIFEST_PATH, SNAPSHOT_MANIFEST_SHA256, "candidate snapshot")
    _require(C77_SNAPSHOT_MANIFEST_PATH, candidate.C77_MANIFEST_SHA256, "Campaign077 snapshot")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign078NoReturnAuditError("snapshot binding validation failed")
    ...
    if not (
        ...
        raise Campaign078NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    c77_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign078NoReturnAuditError("conflicting Campaign078 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c77_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    helper = c77_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    result, receipt = helper(
        manifest_path=C77_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C77_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: verify_campaign077_snapshot(
            workers=workers
        ),
    )
    ...
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign078NoReturnAuditError("Campaign078 comparison count changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign078NoReturnAuditError("Campaign078 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign078_no_return_audit.json")):
        raise Campaign078NoReturnAuditError("Campaign078 no-return audit already exists")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    run_id = f"{timestamp}_campaign078_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign078_no_return_audit",
        ...
        "next_action": "freeze and run the exact one Campaign078 development trial"
        if admitted
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    """Run once with Campaign077's frozen legacy-verifier runtime binding."""

    c77_audit_v2.load_repair_protocol()
    with c77_audit_v2._temporary_runtime_version_binding():
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        experiment_root.expanduser().resolve().glob("*_campaign078_no_return_audit.json")
    )
    return {
