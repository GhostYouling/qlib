#!/usr/bin/env python3
"""Run Campaign145's frozen coverage-first no-return audit."""
...
for _old, _new in (
    ("Campaign117", "Campaign145"),
    ("campaign117", "campaign145"),
    ("campaign_117", "campaign_145"),
    ("20260813", "20260814"),
    ("all_134", "all_141"),
    ("all-134", "all-141"),
    ("all 134", "all 141"),
    ('numeric_comparator_count") == 134', 'numeric_comparator_count") == 141'),
    (">= candidate.EXPECTED_ROWS", ">= candidate.EXPECTED_ROWS"),
):
    _source = _source.replace(_old, _new)
_source = _source.replace("(finite_values < 0.0).any()", "(finite_values < -1.0).any()")
_source = _source.replace("(finite < 0.0).any()", "(finite < -1.0).any()")

_generated: dict[str, Any] = {
    "__file__": str(Path(__file__).resolve()),
    "__name__": "a_share_three_day_walkforward_campaign145_no_return_audit_generated",
}
exec(compile(_source, str(BASE_AUDIT), "exec"), _generated)

Campaign145NoReturnAuditError = _generated["Campaign145NoReturnAuditError"]
