#!/usr/bin/env python3
"""Build Campaign145's frozen return-amount cumulative path-area snapshot."""
...
FACTOR_SHORT_NAME = "return_amount_cumulative_path_signed_area"
FACTOR_FORMULA = (
    "mean of the two fixed-half open-path oriented areas formed by normalized "
    "signed log-close-return increments and normalized destination-amount increments"
)
RAW_COLUMNS = ("datetime", "symbol", "provider", "close", "amount")
OUTPUT_RUN_ID = (
    "tushare_stk_mins_1m_2019_2025_ea0cbb8f_"
    "walkforward_campaign145_feature_library_v1"
)
PROTOCOL_SHA256 = "0c422531fc1d5ceffb4193d4bcd6b42227555d3cff0ec58f04c81a3aab19edd2"
MECHANISM_AUDIT_SHA256 = (
    "bc45603ced25c8717dd8d3289e16b7be177859295501096e70c2d4dc57e74680"
)
NUMERIC_POLICY_SHA256 = (
    "d6e8b359bd7b8af6678b91b0b8c10bbee40af0b06d2373c0897c5e3f159034ac"
)
...
for _old, _new in (
    ("Campaign105", "Campaign145"),
    ("campaign105", "campaign145"),
    ("campaign_105", "campaign_145"),
    ("activity-clock occupancy", "return-amount cumulative path signed area"),
    (
    ...
    (
        ...
        "extract_return_amount_cumulative_path_signed_area",
    ),
    ("attach_activity_values", "attach_path_area_values"),
    ("valid_activity_sessions", "valid_path_area_sessions"),
    ("invalid_numeric_sessions", "invalid_close_or_amount_sessions"),
    ("one_sided_zero_sessions", "missing_half_denominator_sessions"),
    ("active_bars", "return_amount_pair_observations"),
    ("joint_zero_bars", "zero_amount_pair_observations"),
    ("activity_rule", "path_area_rule"),
    ("minimum_active_bar_count", "half_session_pair_count"),
    ("positive_total_activity_required", "positive_half_denominators_required"),
    (
...
_source = _source.replace(
    '"half_session_pair_count": 0', '"half_session_pair_count": 119'
)
_source = _source.replace(
    '"positive_half_denominators_required": False',
    '"positive_half_denominators_required": True',
)
_source = _source.replace(
...
_generated: dict[str, Any] = {
    ...
    "__name__": "a_share_three_day_walkforward_campaign145_features_generated",
}
exec(compile(_source, str(BASE_RUNNER), "exec"), _generated)

Campaign145FeatureError = _generated["Campaign145FeatureError"]

from scripts import (  # noqa: E402
    a_share_three_day_walkforward_campaign128_features as c128,
)
from scripts import (  # noqa: E402
    a_share_three_day_walkforward_campaign136_formula as c136,
)
from scripts import (  # noqa: E402
    a_share_three_day_walkforward_campaign145_formula as formula,
)
...
FEATURE_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign145_features.py"
)
FORMULA_PATH = (
    REPO_ROOT / "scripts/a_share_three_day_walkforward_campaign145_formula.py"
)
FORMULA_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign145_formula.py"
)
AUDIT_RUNNER_PATH = (
...
AUDIT_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign145_no_return_audit.py"
)
...
def reconstruct_comparisons() -> list[dict[str, str]]:
    ...
    items.extend(
        (
            ...
            {"name": c136.FACTOR_NAME, "score_direction": "higher"},
        )
    )
    if (
        ...
        raise Campaign145FeatureError("Campaign145 numeric comparison order changed")
    return items


def reconstruct_prior_complete_definitions() -> list[dict[str, str]]:
    items = [dict(item) for item in c128.reconstruct_complete_definitions()]
    items.extend(
        (
            {"name": c136.FACTOR_NAME, "score_direction": "higher"},
            {
    ...
    if (
        ...
        raise Campaign145FeatureError("Campaign145 prior definition order changed")
    return items
...
def reconstruct_complete_definitions() -> list[dict[str, str]]:
    ...
    if (
        ...
        raise Campaign145FeatureError("Campaign145 complete definition order changed")
    return items
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if target != DEFAULT_PROTOCOL.resolve():
        raise Campaign145FeatureError("Campaign145 protocol path changed")
    try:
        spec = formula.load_protocol(target)
    except formula.Campaign145FormulaError as exc:
        raise Campaign145FeatureError(str(exc)) from exc
    inputs = spec.get("authoritative_inputs") or {}
    policy = inputs.get("numeric_policy_v190") or {}
    ...
    if not (
        ...
        raise Campaign145FeatureError("Campaign145 protocol bindings changed")
    return spec


def extract_return_amount_cumulative_path_signed_area(
    raw: pd.DataFrame, *, symbol: str
) -> tuple[pd.DataFrame, dict[str, int]]:
    if tuple(raw.columns) != RAW_COLUMNS:
        raise Campaign145FeatureError(
    ...
    quality = {
        "source_rows": 0,
        "source_sessions": 0,
        "valid_path_area_sessions": 0,
        "invalid_close_or_amount_sessions": 0,
        "missing_half_denominator_sessions": 0,
        "return_amount_pair_observations": 0,
        "zero_amount_pair_observations": 0,
    }
    if raw.empty:
    ...
    if (
        ...
        raise Campaign145FeatureError(f"raw minute identity changed for {symbol}")
    ...
    if (
        ...
        raise Campaign145FeatureError(f"raw minute grid changed for {symbol}")
    selected = work.loc[
    ...
    if len(selected) != len(dates) * formula.SELECTED_BAR_COUNT:
        raise Campaign145FeatureError(f"continuous minute grid changed for {symbol}")
    ...
    values, eligible, _half_areas, formula_quality = (
        formula.compute_return_amount_path_signed_area(close, amount)
    )
    finite_positive_close = np.isfinite(close).all(axis=1) & (close > 0.0).all(axis=1)
    finite_nonnegative_amount = np.isfinite(amount).all(axis=1) & (amount >= 0.0).all(
        axis=1
    )
    source_valid = finite_positive_close & finite_nonnegative_amount
    endpoint_amount = np.concatenate((amount[:, 1:120], amount[:, 121:240]), axis=1)
    missing_denominator = (
        formula_quality["morning_zero_absolute_return_total_rows"]
        + formula_quality["afternoon_zero_absolute_return_total_rows"]
        + formula_quality["morning_nonpositive_endpoint_amount_total_rows"]
        + formula_quality["afternoon_nonpositive_endpoint_amount_total_rows"]
    )
    return pd.DataFrame({"trade_date": dates, FACTOR_SHORT_NAME: values}), {
        "source_rows": int(len(work)),
        "source_sessions": int(len(dates)),
        "valid_path_area_sessions": int(eligible.sum()),
        "invalid_close_or_amount_sessions": int((~source_valid).sum()),
        "missing_half_denominator_sessions": int(missing_denominator),
        "return_amount_pair_observations": int(source_valid.sum() * 238),
        "zero_amount_pair_observations": int(
            ((endpoint_amount == 0.0) & source_valid[:, None]).sum()
        ),
    }
...
def validate_value_semantics(frame: pd.DataFrame) -> tuple[int, int]:
    ...
    if tuple(frame.columns) != expected:
        raise Campaign145FeatureError("Campaign145 output columns changed")
    ...
    if (
        ...
        raise Campaign145FeatureError("Campaign145 value semantics changed")
...
def _validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise Campaign145FeatureError("Campaign145 implementation freeze is absent")
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign145_no_return_implementation_freeze"
        ...
        and boundary.get("campaign145_historical_source_rows_read") is False
        and boundary.get("campaign145_candidate_values_computed_or_read") is False
        and boundary.get("campaign145_comparator_values_read") is False
        and boundary.get("historical_daily_price_or_forward_return_values_read")
        is False
        and boundary.get("provider_request_issued") is False
    ):
        raise Campaign145FeatureError("Campaign145 implementation freeze changed")
    return record
...
for _name, _value in {
    ...
    "extract_return_amount_cumulative_path_signed_area": extract_return_amount_cumulative_path_signed_area,
...
attach_path_area_values = _generated["attach_path_area_values"]
