#!/usr/bin/env python3
"""Resume Campaign063 while accepting only manifest-declared empty raw partitions.

This additive launcher leaves the frozen Campaign063 implementation unchanged.
...
from scripts import a_share_three_day_walkforward_campaign063_features as core


REPO_ROOT = Path(__file__).resolve().parents[1]
CORE_RUNNER = REPO_ROOT / "scripts/a_share_three_day_walkforward_campaign063_features.py"
CORE_RUNNER_SHA256 = "c6987c8cb59d1a6091eeceed923406fcee4c00d943da5665d0abef674b17ab1b"
CORE_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_063_feature_implementation_freeze_20260805.json"
)
CORE_IMPLEMENTATION_FREEZE_SHA256 = "1cc8ce929ec540eb23fac617fe8fe426824fb82ec144b863a972b78a6af5918a"
FAILURE_RECORD = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_063_empty_partition_build_failure_20260805.json"
)
FAILURE_RECORD_SHA256 = "d431746a5bb1b5b314b893973b8bacc9fb4d8e957360163795f7ce054bcb5fd4"
...
class Campaign063EmptyPartitionRepairError(RuntimeError):
    """Fail-closed Campaign063 compatibility-repair error."""
...
def _require_file(path: Path, expected_sha256: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected_sha256:
        raise Campaign063EmptyPartitionRepairError(f"{label} changed: {path}")
...
def verify_repair_bindings(
    ...
    if data_root != core.DEFAULT_DATA_ROOT.resolve():
        raise Campaign063EmptyPartitionRepairError("Campaign063 data root changed")
    ...
    if empty != EXPECTED_EMPTY_PARTITIONS:
        raise Campaign063EmptyPartitionRepairError(
            "manifest-declared Campaign063 empty partitions changed"
        )
    return {
...
def install_repair(data_root: Path = core.DEFAULT_DATA_ROOT) -> dict[str, Any]:
    ...
    if current not in (_ORIGINAL_EXTRACT_PROFILES, extract_profiles_compat):
        raise Campaign063EmptyPartitionRepairError(
            "Campaign063 extractor was already changed by another caller"
        )
