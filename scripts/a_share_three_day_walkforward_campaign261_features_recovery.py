#!/usr/bin/env python3
"""Recover Campaign261's snapshot build with two manifest-only counters."""
...
from scripts import a_share_three_day_walkforward_campaign261_features as base
...
FAILURE_RECORD_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_261_initial_snapshot_build_failure_20260816.json"
)
RECOVERY_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_261_snapshot_build_recovery_freeze_20260816.json"
)
RECOVERY_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign261_features_recovery.py"
)
...
if _sha256(BASE_BUILDER_PATH) != BASE_BUILDER_SHA256:
    raise RuntimeError("frozen Campaign261 base builder changed")


_original_extract = base.extract_amount_lorenz_gini
...
def extract_amount_lorenz_gini(
    ...
    amounts = (
        ...
        .reshape(len(dates), base.formula.PROFILE_POSITIONS)
    )
...
def _validate_recovery_freeze() -> dict[str, Any]:
    if not RECOVERY_FREEZE_PATH.is_file():
        raise base.Campaign261FeatureError("Campaign261 recovery freeze missing")
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign261_snapshot_build_recovery_freeze"
        and record.get("status") == "counter_schema_only_recovery_frozen_before_retry"
        and code.get("base_builder_sha256") == BASE_BUILDER_SHA256
        and code.get("recovery_runner_sha256") == _sha256(Path(__file__).resolve())
        and code.get("failure_record_sha256") == _sha256(FAILURE_RECORD_PATH)
        ...
        raise base.Campaign261FeatureError("Campaign261 recovery freeze changed")
    return record


base._engine["extract_amount_lorenz_gini"] = extract_amount_lorenz_gini
