#!/usr/bin/env python3
"""Build and audit the preregistered intraday amount-profile persistence.

The candidate reads only minute identity, timestamp, and amount from the
immutable Tushare source.  It measures lag-1 correlation on the 240-bar
continuous-session log-amount path.  Build and audit never read a price or a
forward return, and comparison values load only after coverage passes.
"""
...
import a_share_tushare_intraday_amount_participation_entropy as previous
...
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_profile_serial_persistence_no_return_preregistration.json"
)
PREREGISTRATION_SHA256 = (
    "f0eddc1fd2fe9586618d1375c53a0725d14f5a906157573e75ec59e4537b406d"
)
DEFAULT_DIAGNOSTIC_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_profile_serial_persistence_diagnostic_preregistration.json"
)
DIAGNOSTIC_PREREGISTRATION_SHA256 = (
    "a86f51d939da2b0221f16783d470fd528c595c54eccd9079f83064b41cc062c6"
)
NO_RETURN_AUDIT_SHA256 = (
    "a95d03c6aef98f49dcd31f1ba3f3e6c7223f706b65e1283deba8a6689907345c"
)
CANDIDATE_MANIFEST_SHA256 = (
    "8c362725187c9e6021e64a07feadc6958e4334c65642dca0e0dc1eb4fccf7186"
)
DIAGNOSTIC_PURPOSE = (
    "development_only_preregistered_tushare_intraday_amount_profile_serial_"
    "persistence_three_session_diagnostic_research_not_investment_advice"
)
CONSUMPTION_FILENAME = (
    "tushare_intraday_amount_profile_serial_persistence_v1_consumption.json"
)
...
AMOUNT_ENTROPY_MANIFEST_SHA256 = previous.CANDIDATE_MANIFEST_SHA256
AMOUNT_ENTROPY_DATASET_SHA256 = (
    "8d34ac56836fe1cc22157ac07f29e6a3ef94429008376e22e851730992b3891d"
)
CURRENT_STATUS_SHA256 = (
    "c2bbefdae0f87e6d6bba10d9efe2b16a5cea263df62cb5f3a1429cd5bc412754"
)
SOURCE_RUN_ID = foundation.SOURCE_RUN_ID
OUTPUT_RUN_ID = f"{SOURCE_RUN_ID}_intraday_amount_profile_serial_persistence_v1"
FACTOR_NAME = "intraday_amount_profile_serial_persistence_240m"
FACTOR_FORMULA = (
    "corr(log1p(a_t) for t=1..239, log1p(a_t) for t=2..240), using the "
    "240 continuous-session amount observations at 09:31-11:30 and "
    "13:01-15:00"
)
...
RAW_COLUMNS = previous.RAW_COLUMNS
OUTPUT_COLUMNS = (
    "trade_date",
    "symbol",
    "provider",
...
class AmountProfilePersistenceError(RuntimeError):
...
def _require_file(path: Path, expected_sha256: str, label: str) -> None:
    ...
    if observed != expected_sha256:
        raise AmountProfilePersistenceError(
...
def load_preregistration(path: Path = DEFAULT_PREREGISTRATION) -> dict[str, Any]:
    ...
    _require_file(path, PREREGISTRATION_SHA256, "amount-persistence protocol")
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_profile_serial_persistence_no_return_preregistration",
    )
    ...
    if not (
        ...
        == "aggregation_blocked_after_intraday_amount_participation_entropy_terminal_rejection_zero_dual_gate_factors"
        and current.get("terminal_mechanism_count_before_this_candidate") == 31
        ...
        == 0.8
        and uniqueness.get("all_seven_comparisons_must_pass") is True
        ...
        raise AmountProfilePersistenceError(
            "amount-profile-persistence protocol no longer matches its frozen definition"
        )
    return spec
...
def _validate_entropy_manifest(
    ...
    link = (spec.get("source_chain") or {}).get("amount_entropy_comparison_manifest") or {}
    path = (data_root / str(link.get("path_below_data_root"))).resolve()
    _require_file(path, AMOUNT_ENTROPY_MANIFEST_SHA256, "amount-entropy manifest")
    manifest = research.load_json_record(
        path, kind="a_share_tushare_intraday_amount_participation_entropy_snapshot"
    )
    if not (
        manifest.get("status")
        ...
        and manifest.get("dataset_sha256") == AMOUNT_ENTROPY_DATASET_SHA256
        ...
        raise AmountProfilePersistenceError("amount-entropy manifest identity is rejected")
...
def validate_external_chain(
    ...
    entropy, entropy_path = _validate_entropy_manifest(spec, data_root)
    return (
        raw,
        joint,
        raw_path,
...
def compute_partition_frame(
    ...
    *,
    ...
    """Compute lag-1 Pearson correlation of the continuous log-amount path."""
    ...
    if (
        ...
        raise AmountProfilePersistenceError(f"joint base identity is invalid for {symbol}")
    ...
    if raw_work["datetime"].isna().any():
        raise AmountProfilePersistenceError(f"raw timestamps are invalid for {symbol}")
    ...
    if set(raw_work["symbol"]) != {symbol} or set(raw_work["provider"]) != {"tushare"}:
        raise AmountProfilePersistenceError(f"raw identity is invalid for {symbol}")
    ...
    if not np.array_equal(codes, np.broadcast_to(SOURCE_MINUTE_CODES, codes.shape)):
        raise AmountProfilePersistenceError(f"raw source grid order changed for {symbol}")
    if len(CONTINUOUS_MINUTE_CODES) != 240:
        raise AmountProfilePersistenceError("continuous amount support changed")
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        logged = np.log1p(amounts)
        left = logged[:, :-1]
        right = logged[:, 1:]
        left_centered = left - left.mean(axis=1, keepdims=True)
        right_centered = right - right.mean(axis=1, keepdims=True)
        left_ss = np.square(left_centered).sum(axis=1)
        right_ss = np.square(right_centered).sum(axis=1)
        denominator = np.sqrt(left_ss * right_ss)
        values = (left_centered * right_centered).sum(axis=1) / denominator
    positive_norms = (
        np.isfinite(left_ss)
        & np.isfinite(right_ss)
        & (left_ss > 0.0)
        & (right_ss > 0.0)
    ...
    if range_violation.any():
        raise AmountProfilePersistenceError(
            f"lag correlation escaped [-1, 1] beyond tolerance for {symbol}"
        )
    eligible = (
        required_valid
        & positive_norms
    ...
    return output, {
        ...
        "constant_profile_rows": int((required_valid & ~positive_norms).sum()),
        "invalid_required_value_rows": int((~required_valid).sum()),
        "ieee_boundary_canonicalization_rows": int(canonicalizable.sum()),
        "correlation_range_violation_rows": int(range_violation.sum()),
    }
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_intraday_amount_profile_serial_persistence"
        / OUTPUT_RUN_ID
    )
...
def _load_checkpoint(
    ...
    valid = (
        record.get("kind")
        == "a_share_tushare_intraday_amount_profile_serial_persistence_partition"
    ...
    if not valid:
        raise AmountProfilePersistenceError(
            f"completed amount-persistence checkpoint changed: {paths.partial_sidecar}"
        )
    ...
    if len(output) != int(record.get("rows", -1)) or foundation.frame_digest(output) != record.get(
        ...
        raise AmountProfilePersistenceError(
            f"completed amount-persistence frame changed: {paths.partial_data}"
        )
    dates = Counter(
...
def _process_partition(
    ...
    *,
    ...
    if foundation.file_digest(raw_path) != raw_record.get("byte_sha256"):
        raise AmountProfilePersistenceError(f"raw partition changed: {raw_path}")
    if foundation.file_digest(base_path) != joint_record.get("output_byte_sha256"):
        raise AmountProfilePersistenceError(f"joint-base partition changed: {base_path}")
    ...
    if len(raw) != int(raw_record.get("rows", -1)):
        raise AmountProfilePersistenceError(f"raw row count changed: {raw_path}")
    if len(base) != int(joint_record.get("rows", -1)):
        raise AmountProfilePersistenceError(f"joint-base row count changed: {base_path}")
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_amount_profile_serial_persistence_partition",
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    if final_root.exists():
        if not final_manifest.is_file():
            raise AmountProfilePersistenceError(
                f"published amount-persistence root has no manifest: {final_root}"
            )
        manifest = research.load_json_record(
            final_manifest,
        ...
        if not (
            ...
            raise AmountProfilePersistenceError(
                f"published amount-persistence snapshot changed: {final_root}"
            )
        return final_manifest
    if shutil.disk_usage(data_root).free < 5 * 1024**3:
        raise AmountProfilePersistenceError("external data root has less than 5 GiB free")
    ...
    lock_path = data_root / ".a_share_tushare_intraday_amount_persistence.lock"
    with foundation.ProcessLock(lock_path):
        ...
        completed_symbols = 0
        print(
            f"building {len(joint_records):,} amount-persistence partitions "
        ...
        if len(all_records) != 33_015:
            raise AmountProfilePersistenceError(
                "not every source partition produced an amount-persistence checkpoint"
            )
...
def verify_snapshot_files(
    ...
    if len(records) != 33_015:
        raise AmountProfilePersistenceError("candidate snapshot partition count changed")
    ...
    if rows != manifest.get("rows") or eligible != manifest.get("eligible_rows"):
        raise AmountProfilePersistenceError("candidate snapshot aggregate counts changed")
    return {
...
def load_candidate_frame(manifest_path: Path, manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(frame) != int(manifest.get("rows", -1)):
        raise AmountProfilePersistenceError("candidate frame row count changed")
    ...
    if (
        ...
        raise AmountProfilePersistenceError("candidate frame values or keys are invalid")
...
def uniqueness_audit(
    ...
    print("coverage passed; loading seven terminal comparison factors", flush=True)
    ...
    if (
        ...
        raise AmountProfilePersistenceError("base comparison frame identity changed")
    ...
    entropy = previous.load_candidate_frame(entropy_manifest_path, entropy_manifest)[
        ["trade_date", "symbol", previous.FACTOR_NAME]
    ]
    for frame in (efficiency, recovery_frame, entropy):
    ...
    comparisons = (
        ...
        .merge(entropy, on=["trade_date", "symbol"], how="left", validate="one_to_one")
    )
    del efficiency, recovery_frame, entropy
    ...
    all_passed = len(results) == 7 and all(item["gate_passed"] for item in results)
    return {
        ...
        "all_seven_comparisons_passed": all_passed,
    }
...
def _find_existing_audit(experiment_root: Path, manifest_sha256: str) -> Path | None:
    for path in sorted(
        experiment_root.glob(
            "*_intraday_amount_profile_serial_persistence_no_return_audit.json"
        )
    ):
        record = research.load_json_record(path)
        if (
            record.get("kind")
            == "a_share_tushare_intraday_amount_profile_serial_persistence_no_return_audit"
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if not (
        ...
        raise AmountProfilePersistenceError("candidate snapshot identity is rejected")
    ...
    uniqueness: dict[str, Any] = {
        ...
        "all_seven_comparisons_passed": False,
        "comparisons": [],
    }
    ...
    passed = bool(
        ...
        and uniqueness["all_seven_comparisons_passed"]
    )
    status = (
    ...
    audit = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_amount_profile_serial_persistence_no_return_audit",
        "status": status,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "purpose": "ordered_candidate_coverage_capacity_then_seven_terminal_factor_uniqueness_without_prices_or_forward_returns",
        "preregistration": {
    ...
    path = (
        experiment_root
        / f"{run_id}_intraday_amount_profile_serial_persistence_no_return_audit.json"
    )
...
def load_diagnostic_preregistration(
    ...
    _require_file(
        path,
        DIAGNOSTIC_PREREGISTRATION_SHA256,
        "amount-persistence diagnostic protocol",
    )
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_profile_serial_persistence_diagnostic_preregistration",
    )
    ...
    if not (
        ...
        raise AmountProfilePersistenceError(
            "amount-persistence diagnostic protocol no longer matches its frozen definition"
        )
    return spec
...
def require_diagnostic_unconsumed(experiment_root: Path) -> None:
    ...
    if marker.exists():
        raise AmountProfilePersistenceError(
            f"amount-persistence historical diagnostic is already consumed: {marker}"
        )
    for path in sorted(experiment_root.glob("*_factor_diagnostic.json")):
        record = research.load_json_record(path)
        if record.get("purpose") == DIAGNOSTIC_PURPOSE:
            raise AmountProfilePersistenceError(
                f"amount-persistence historical diagnostic already exists: {path}"
            )
...
def run_diagnostic(args: argparse.Namespace) -> Path:
    ...
    marker = {
        "kind": "a_share_tushare_intraday_amount_profile_serial_persistence_historical_consumption",
