#!/usr/bin/env python3
"""Run Campaign297's frozen Alpha158 envelope-occupancy campaign."""
...
from scripts import a_share_three_day_walkforward_campaign296 as prior


REPO_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_297_preregistration_20260825.json"
)
ORIGINAL_IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_297_implementation_freeze_20260825.json"
)
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_297_implementation_freeze_v2_20260825.json"
)
INFRASTRUCTURE_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_297_coverage_key_mapping_failure_20260825.json"
)
CONCEPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_297_concept_scouting_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign297.py"
)
...
C296_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
SIGNAL_LEDGER_PATH = prior.SIGNAL_LEDGER_PATH
EXECUTION_LEDGER_PATH = prior.EXECUTION_LEDGER_PATH

FAILED_OUTPUT_ROOT = (
    REPO_ROOT
    / ".local-research/campaign_297/alpha158_interquantile_envelope_occupancy_20d_v1"
)
...
OUTPUT_ROOT = (
    REPO_ROOT
    / ".local-research/campaign_297/alpha158_interquantile_envelope_occupancy_20d_v2"
)
...
FACTOR_NAME = "alpha158_interquantile_envelope_occupancy_20d"
TRIAL_ID = "wf297_alpha158_interquantile_envelope_occupancy_20d_single_higher"
UPPER_FEATURE = "QTLU20"
LOWER_FEATURE = "QTLD20"
MAX_FEATURE = "MAX20"
MIN_FEATURE = "MIN20"
INPUT_TOLERANCE = 1e-6
FEATURE_COUNT = 158
MINIMUM_ALPHA158_FINITE_FEATURES = 119
EXPECTED_COMPARATOR_COUNT = 149
...
CAMPAIGN296_FACTOR = prior.FACTOR_NAME
...
class Campaign297Error(RuntimeError):
    """Fail closed when a frozen Campaign297 invariant changes."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Campaign297Error(f"JSON binding unreadable: {path}") from exc
    if not isinstance(value, dict):
        raise Campaign297Error(f"JSON binding is not an object: {path}")
    return value


def require_file(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or file_sha256(path) != expected:
        raise Campaign297Error(f"{label} fingerprint changed")
...
def validate_protocol() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        protocol.get("kind")
        == "a_share_three_day_walkforward_campaign297_preregistration"
        and protocol.get("status")
        == "fully_frozen_before_campaign297_candidate_comparator_daily_price_or_forward_return_values"
        and len(protocol.get("factor_library") or []) == 1
        and factor.get("name") == FACTOR_NAME
        and factor.get("direction") == "higher"
        and parameters.get("upper_close_quantile_feature") == UPPER_FEATURE
        and parameters.get("lower_close_quantile_feature") == LOWER_FEATURE
        and parameters.get("rolling_high_feature") == MAX_FEATURE
        and parameters.get("rolling_low_feature") == MIN_FEATURE
        and parameters.get("fixed_horizon_sessions") == 20
        and parameters.get("score") == "(QTLU20 - QTLD20) / (MAX20 - MIN20)"
        ...
        and uniqueness.get("all_149_must_pass") is True
        ...
        == {"complete_definitions": 168, "numeric_comparators": 149}
        and boundary.get(
            "campaign297_candidate_or_comparator_values_read_before_protocol"
        )
        is False
        and boundary.get(
        ...
        raise Campaign297Error("Campaign297 protocol semantics changed")
    ...
    required_features = {UPPER_FEATURE, LOWER_FEATURE, MAX_FEATURE, MIN_FEATURE}
    if not (
        ...
        raise Campaign297Error("Campaign297 source or Candidate49 semantics changed")
...
def validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign297_implementation_freeze_v2"
        and freeze.get("status")
        == "scientifically_identical_coverage_key_mapping_repair_frozen_before_comparator_or_return_values"
        ...
        and frozen.get("formula") == "(QTLU20 - QTLD20) / (MAX20 - MIN20)"
        ...
        and boundary.get("candidate_values_read_before_freeze") is True
        ...
        raise Campaign297Error("Campaign297 implementation freeze changed")
    return freeze


def interquantile_envelope_occupancy(
    *,
    keys: np.ndarray,
    upper: np.ndarray,
    lower: np.ndarray,
    ...
    if not (
        ...
        raise Campaign297Error("Alpha158 row support semantics changed")
    finite_inputs = (
        np.isfinite(upper)
        & np.isfinite(lower)
        & np.isfinite(maximum)
    ...
    central_width = upper - lower
    envelope_width = maximum - minimum
    valid_inputs = (
        finite_inputs
        & (envelope_width > 0.0)
        & (central_width >= -INPUT_TOLERANCE)
        & (central_width <= envelope_width + INPUT_TOLERANCE)
    )
    score = np.full(len(keys), np.nan, dtype=np.float64)
    calculable = model_support & valid_inputs
    score[calculable] = central_width[calculable] / envelope_width[calculable]
    selected = np.flatnonzero(quality_listing)
    selected_scores = score[selected]
    input_counts = (
        np.isfinite(upper[selected]).astype(np.uint8)
        + np.isfinite(lower[selected]).astype(np.uint8)
        + np.isfinite(maximum[selected]).astype(np.uint8)
        + np.isfinite(minimum[selected]).astype(np.uint8)
    )
    eligible = (
...
def build_candidate_snapshot(alpha: Mapping[str, Any]) -> dict[str, Any]:
    if CANDIDATE_ROOT.exists():
        raise Campaign297Error("candidate output root already exists; refuse overwrite")
    columns = [
        "stock_day_key",
        UPPER_FEATURE,
        LOWER_FEATURE,
    ...
    for year in YEARS:
        ...
        for _, frame in base.iter_session_frames(source_path, columns):
            parts.append(
                interquantile_envelope_occupancy(
                    keys=frame["stock_day_key"].to_numpy(dtype=np.int64),
                    upper=frame[UPPER_FEATURE].to_numpy(dtype=np.float64),
                    lower=frame[LOWER_FEATURE].to_numpy(dtype=np.float64),
                    maximum=frame[MAX_FEATURE].to_numpy(dtype=np.float64),
                    minimum=frame[MIN_FEATURE].to_numpy(dtype=np.float64),
        ...
        if len(np.unique(annual["stock_day_key"])) != len(annual):
            raise Campaign297Error(f"candidate annual keys duplicated: {year}")
    ...
    if [item["sessions"] for item in records] != [244, 243, 243, 242, 242]:
        raise Campaign297Error("candidate session counts changed")
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign297_candidate_snapshot",
        "status": "immutable_candidate_ready_for_coverage_before_comparator_values",
        "created_at": base.utc_now(),
        "factor_name": FACTOR_NAME,
        "direction": "higher",
        "formula_parameters": {
            "upper_close_quantile_feature": UPPER_FEATURE,
            "lower_close_quantile_feature": LOWER_FEATURE,
            "rolling_high_feature": MAX_FEATURE,
            "rolling_low_feature": MIN_FEATURE,
            "fixed_horizon_sessions": 20,
            "score": "(QTLU20 - QTLD20) / (MAX20 - MIN20)",
...
def load_candidate_snapshot(
    ...
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign297_candidate_snapshot"
        ...
        raise Campaign297Error("candidate manifest semantics changed")
    ...
    if canonical_sha256(digest_rows) != manifest.get("dataset_sha256"):
        raise Campaign297Error("candidate dataset digest changed")
    keys = np.concatenate(key_parts)
    values = np.concatenate(value_parts)
    eligible = np.concatenate(eligible_parts)
    if len(np.unique(keys)) != len(keys) or np.any(keys[1:] < keys[:-1]):
        raise Campaign297Error("candidate snapshot key order changed")
...
def comparison_definitions(numeric_manifest: Mapping[str, Any]) -> list[str]:
    ...
    definitions.append(CAMPAIGN296_FACTOR)
    if len(definitions) != EXPECTED_COMPARATOR_COUNT:
        raise Campaign297Error("comparator name order changed")
    ...
    if observed != EXPECTED_COMPARATOR_ORDER_SHA256:
        raise Campaign297Error("comparator order digest changed")
    return definitions
    ...
    *,
    ...
    for year in YEARS:
        ...
        if not np.array_equal(source_keys, candidate_keys):
            raise Campaign297Error(f"{factor} candidate alignment changed: {year}")
        finite = values[np.isfinite(values)]
        if finite.size and minimum is not None and (finite < minimum).any():
            raise Campaign297Error(f"{factor} comparator minimum changed")
        if finite.size and maximum is not None and (finite > maximum).any():
            raise Campaign297Error(f"{factor} comparator maximum changed")
...
def run_ordered_uniqueness(
    *, numeric_manifest: Mapping[str, Any], candidate_manifest: Mapping[str, Any]
    ...
    return {
        "status": (
            "all_149_uniqueness_gates_passed"
            if all_passed
...
def write_trial_ledger(
    *, no_return: Mapping[str, Any], terminal: Mapping[str, Any]
    ...
    entries.append(
        {
            ...
            "phase": failure["phase"],
            "name": "coverage_key_mapping_failure",
    ...
    entries.append(
        {
            "attempt_id": "campaign297_factor_001",
            "phase": "coverage_uniqueness_and_conditional_development",
            "name": FACTOR_NAME,
            "formula": "(QTLU20 - QTLD20) / (MAX20 - MIN20)",
            "direction": "higher",
            "parameters": {
                "fixed_horizon_sessions": 20,
                "input_numerical_tolerance": INPUT_TOLERANCE,
    ...
    ledger = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign297_terminal_trial_ledger",
...
def run_campaign(*, batch_size: int) -> dict[str, Any]:
    ...
    all_unique = bool(uniqueness and uniqueness["all_149_passed"])
    no_return = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign297_no_return_audit",
        "status": (
    ...
    if all_unique:
        ...
        for fold in protocol["development_trial"]["walkforward_folds"]:
            ...
            binding = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign297_prevalidation_scores",
    ...
    terminal = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign297_terminal_result",
        "status": (
        ...
        "prior_definition_count": 168,
        "prior_numeric_comparator_count": 149,
...
def verify_terminal() -> dict[str, Any]:
    ...
    for ordinal, entry in enumerate(ledger.get("entries", []), start=1):
        ...
        if not (
            ...
            raise Campaign297Error("terminal trial ledger chain changed")
        previous = observed
    if not (
        terminal.get("kind")
        == "a_share_three_day_walkforward_campaign297_terminal_result"
        ...
        and ledger.get("infrastructure_failure_count") == 1
        ...
        raise Campaign297Error("Campaign297 terminal semantics changed")
    return {
        "status": "verified_campaign297_terminal",
...
def plan() -> dict[str, Any]:
    ...
    for manifest_path, manifest, label in (
        ...
        for year in YEARS:
            ...
            path = resolve_record(manifest_path, record)
            if not path.is_file():
                raise Campaign297Error(f"{label} annual partition missing: {year}")
            schemas.append(
                {
                    "source": label,
                    "year": year,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        if not args.confirm_run:
            raise Campaign297Error("run requires --confirm-run")
        print(
            json.dumps(
