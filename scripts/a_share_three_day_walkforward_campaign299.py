#!/usr/bin/env python3
"""Run Campaign299's frozen Alpha158 close-distribution asymmetry campaign."""
...
from scripts import a_share_three_day_walkforward_campaign298 as prior


REPO_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_preregistration_20260825.json"
)
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_implementation_freeze_v2_20260825.json"
)
ORIGINAL_IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_implementation_freeze_20260825.json"
)
CONCEPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_concept_scouting_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign299.py"
)
PYTEST_ENTRYPOINT_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_pytest_entrypoint_collection_failure_20260825.json"
)
DIRECT_PLAN_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_299_direct_script_plan_import_failure_20260825.json"
)
...
C298_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
SIGNAL_LEDGER_PATH = prior.SIGNAL_LEDGER_PATH
EXECUTION_LEDGER_PATH = prior.EXECUTION_LEDGER_PATH

OUTPUT_ROOT = (
    REPO_ROOT
    / ".local-research/campaign_299/alpha158_close_distribution_upper_tail_asymmetry_20d_v1"
)
...
FACTOR_NAME = "alpha158_close_distribution_upper_tail_asymmetry_20d"
TRIAL_ID = (
    "wf299_alpha158_close_distribution_upper_tail_asymmetry_20d_single_higher"
)
MEAN_FEATURE = "MA20"
UPPER_QUANTILE_FEATURE = "QTLU20"
LOWER_QUANTILE_FEATURE = "QTLD20"
FEATURE_COUNT = 158
MINIMUM_ALPHA158_FINITE_FEATURES = 119
EXPECTED_COMPARATOR_COUNT = 151
...
CAMPAIGN298_FACTOR = prior.FACTOR_NAME
...
class Campaign299Error(RuntimeError):
    """Fail closed when a frozen Campaign299 invariant changes."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Campaign299Error(f"JSON binding unreadable: {path}") from exc
    if not isinstance(value, dict):
        raise Campaign299Error(f"JSON binding is not an object: {path}")
    return value


def require_file(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or file_sha256(path) != expected:
        raise Campaign299Error(f"{label} fingerprint changed")
...
def validate_protocol() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        protocol.get("kind")
        == "a_share_three_day_walkforward_campaign299_preregistration"
        and protocol.get("status")
        == "fully_frozen_before_campaign299_candidate_comparator_daily_price_or_forward_return_values"
        and len(protocol.get("factor_library") or []) == 1
        and factor.get("name") == FACTOR_NAME
        and factor.get("direction") == "higher"
        and parameters.get("mean_close_feature") == MEAN_FEATURE
        and parameters.get("upper_close_quantile_feature")
        == UPPER_QUANTILE_FEATURE
        and parameters.get("lower_close_quantile_feature")
        == LOWER_QUANTILE_FEATURE
        ...
        and uniqueness.get("all_151_must_pass") is True
        ...
        == {"complete_definitions": 170, "numeric_comparators": 151}
        and boundary.get(
            "campaign299_candidate_or_comparator_values_read_before_protocol"
        )
        is False
        and boundary.get(
        ...
        raise Campaign299Error("Campaign299 protocol semantics changed")
    ...
    if not (
        ...
        raise Campaign299Error("Campaign299 source or Candidate49 semantics changed")
...
def validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign299_implementation_freeze_v2"
        and freeze.get("status")
        == "module_entrypoint_repair_frozen_before_campaign299_candidate_comparator_or_return_values"
        ...
        and frozen.get("pytest_entrypoint_failure_sha256")
        == file_sha256(PYTEST_ENTRYPOINT_FAILURE_PATH)
        and frozen.get("direct_plan_failure_sha256")
        == file_sha256(DIRECT_PLAN_FAILURE_PATH)
        ...
        raise Campaign299Error("Campaign299 implementation freeze changed")
    return freeze


def close_distribution_upper_tail_asymmetry(
    *,
    keys: np.ndarray,
    ...
    if not (
        ...
        raise Campaign299Error("Alpha158 row support semantics changed")
    finite_inputs = (
        np.isfinite(mean_close)
        & np.isfinite(upper_quantile)
        & np.isfinite(lower_quantile)
    )
    width = upper_quantile - lower_quantile
    valid_inputs = finite_inputs & np.isfinite(width) & (width > 0.0)
    score = np.full(len(keys), np.nan, dtype=np.float64)
    calculable = model_support & valid_inputs
    score[calculable] = (
        2.0 * mean_close[calculable]
        - upper_quantile[calculable]
        - lower_quantile[calculable]
    ) / width[calculable]
    ...
    input_counts = (
        np.isfinite(mean_close[selected]).astype(np.uint8)
        + np.isfinite(upper_quantile[selected]).astype(np.uint8)
        + np.isfinite(lower_quantile[selected]).astype(np.uint8)
    )
...
def build_candidate_snapshot(alpha: Mapping[str, Any]) -> dict[str, Any]:
    if CANDIDATE_ROOT.exists():
        raise Campaign299Error("candidate output root already exists; refuse overwrite")
    columns = [
        "stock_day_key",
        MEAN_FEATURE,
        UPPER_QUANTILE_FEATURE,
    ...
    for year in YEARS:
        ...
        for _, frame in base.iter_session_frames(source_path, columns):
            parts.append(
                close_distribution_upper_tail_asymmetry(
                    keys=frame["stock_day_key"].to_numpy(dtype=np.int64),
                    mean_close=frame[MEAN_FEATURE].to_numpy(dtype=np.float64),
                    upper_quantile=frame[UPPER_QUANTILE_FEATURE].to_numpy(
                        dtype=np.float64
                    ),
                    lower_quantile=frame[LOWER_QUANTILE_FEATURE].to_numpy(
                        dtype=np.float64
                    ),
        ...
        if len(np.unique(annual["stock_day_key"])) != len(annual):
            raise Campaign299Error(f"candidate annual keys duplicated: {year}")
    ...
    if [item["sessions"] for item in records] != [244, 243, 243, 242, 242]:
        raise Campaign299Error("candidate session counts changed")
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign299_candidate_snapshot",
        "status": "immutable_candidate_ready_for_coverage_before_comparator_values",
        "created_at": base.utc_now(),
        "factor_name": FACTOR_NAME,
        "direction": "higher",
        "formula_parameters": {
            "mean_close_feature": MEAN_FEATURE,
            "upper_close_quantile_feature": UPPER_QUANTILE_FEATURE,
            "lower_close_quantile_feature": LOWER_QUANTILE_FEATURE,
            "fixed_horizon_sessions": 20,
            "score": "(2 * MA20 - QTLU20 - QTLD20) / (QTLU20 - QTLD20)",
...
def load_candidate_snapshot(
    ...
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign299_candidate_snapshot"
        ...
        raise Campaign299Error("candidate manifest semantics changed")
    ...
    if canonical_sha256(digest_rows) != manifest.get("dataset_sha256"):
        raise Campaign299Error("candidate dataset digest changed")
    keys = np.concatenate(key_parts)
    values = np.concatenate(value_parts)
    eligible = np.concatenate(eligible_parts)
    if len(np.unique(keys)) != len(keys) or np.any(keys[1:] < keys[:-1]):
        raise Campaign299Error("candidate snapshot key order changed")
...
def comparison_definitions(numeric_manifest: Mapping[str, Any]) -> list[str]:
    ...
    definitions.append(CAMPAIGN298_FACTOR)
    if len(definitions) != EXPECTED_COMPARATOR_COUNT:
        raise Campaign299Error("comparator name order changed")
    ...
    if observed != EXPECTED_COMPARATOR_ORDER_SHA256:
        raise Campaign299Error("comparator order digest changed")
    return definitions
    ...
    *,
    ...
    for year in YEARS:
        ...
        if not np.array_equal(source_keys, candidate_keys):
            raise Campaign299Error(f"{factor} candidate alignment changed: {year}")
        finite = values[np.isfinite(values)]
        if finite.size and minimum is not None and (finite < minimum).any():
            raise Campaign299Error(f"{factor} comparator minimum changed")
        if finite.size and maximum is not None and (finite > maximum).any():
            raise Campaign299Error(f"{factor} comparator maximum changed")
...
def run_ordered_uniqueness(
    *, numeric_manifest: Mapping[str, Any], candidate_manifest: Mapping[str, Any]
    ...
    return {
        "status": (
            "all_151_uniqueness_gates_passed"
            if all_passed
...
def write_trial_ledger(
    *, no_return: Mapping[str, Any], terminal: Mapping[str, Any]
    ...
    failure = load_json(PYTEST_ENTRYPOINT_FAILURE_PATH)
    entries.append(
        {
            "attempt_id": failure["attempt_id"],
            "phase": failure["phase"],
            "name": "standalone_pytest_collection_entrypoint",
            "decision": failure["status"],
            "error_type": failure["error_type"],
            "error_summary": failure["error_summary"],
            "failure_record_sha256": file_sha256(PYTEST_ENTRYPOINT_FAILURE_PATH),
            "candidate_values_read": False,
            "comparator_values_read": False,
            "historical_daily_price_or_forward_return_values_read": False,
        }
    )
    direct_plan_failure = load_json(DIRECT_PLAN_FAILURE_PATH)
    entries.append(
        {
            "attempt_id": direct_plan_failure["attempt_id"],
            "phase": direct_plan_failure["phase"],
            "name": "direct_script_plan_entrypoint",
            "decision": direct_plan_failure["status"],
            "error_type": direct_plan_failure["error_type"],
            "error_summary": direct_plan_failure["error_summary"],
            "failure_record_sha256": file_sha256(DIRECT_PLAN_FAILURE_PATH),
            "candidate_values_read": False,
            "comparator_values_read": False,
            "historical_daily_price_or_forward_return_values_read": False,
        }
    ...
    entries.append(
        {
            "attempt_id": "campaign299_factor_001",
    ...
    ledger = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign299_terminal_trial_ledger",
...
def run_campaign(*, batch_size: int) -> dict[str, Any]:
    ...
    all_unique = bool(uniqueness and uniqueness["all_151_passed"])
    no_return = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign299_no_return_audit",
        "status": (
    ...
    if all_unique:
        ...
        for fold in protocol["development_trial"]["walkforward_folds"]:
            ...
            binding = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign299_prevalidation_scores",
    ...
    terminal = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign299_terminal_result",
        "status": (
        ...
        "prior_definition_count": 170,
        "prior_numeric_comparator_count": 151,
...
def verify_terminal() -> dict[str, Any]:
    ...
    for ordinal, entry in enumerate(ledger.get("entries", []), start=1):
        ...
        if not (
            ...
            raise Campaign299Error("terminal trial ledger chain changed")
        previous = observed
    if not (
        terminal.get("kind")
        == "a_share_three_day_walkforward_campaign299_terminal_result"
        ...
        raise Campaign299Error("Campaign299 terminal semantics changed")
    return {
        "status": "verified_campaign299_terminal",
...
def plan() -> dict[str, Any]:
    ...
    for manifest_path, manifest, label in (
        ...
        for year in YEARS:
            ...
            path = resolve_record(manifest_path, record)
            if not path.is_file():
                raise Campaign299Error(f"{label} annual partition missing: {year}")
            schemas.append(
                {
                    "source": label,
                    "year": year,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        if not args.confirm_run:
            raise Campaign299Error("run requires --confirm-run")
        print(
            json.dumps(
