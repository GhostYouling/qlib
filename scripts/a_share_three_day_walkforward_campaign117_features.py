...
FACTOR_SHORT_NAME = "amount_weak_order_entropy"
FACTOR_FORMULA = (
...
OUTPUT_RUN_ID = (
    ...
    "walkforward_campaign117_feature_library_v1"
)
...
for _old, _new in (
    ("Campaign105", "Campaign117"),
    ("campaign105", "campaign117"),
    ("campaign_105", "campaign_117"),
    ("activity-clock occupancy", "amount weak-order entropy"),
    (
    ...
    ("extract_active_trading_bar_share", "extract_amount_weak_order_entropy"),
    ("attach_activity_values", "attach_entropy_values"),
    ("valid_activity_sessions", "valid_entropy_sessions"),
    ("invalid_numeric_sessions", "invalid_amount_sessions"),
    ("one_sided_zero_sessions", "nonpositive_total_amount_sessions"),
    ("active_bars", "recognized_state_observations"),
    ("joint_zero_bars", "exact_tie_triple_observations"),
    ("activity_rule", "entropy_rule"),
    ("minimum_active_bar_count", "weak_order_state_count"),
    ("positive_total_activity_required", "positive_total_amount_required"),
    ("activity_magnitude_used", "amount_magnitude_used_after_ordinal_encoding"),
    (
...
_source = _source.replace('"weak_order_state_count": 0', '"weak_order_state_count": 13')
...
_generated: dict[str, Any] = {
    ...
    "__name__": "a_share_three_day_walkforward_campaign117_features_generated",
}
exec(compile(_source, str(BASE_RUNNER), "exec"), _generated)

Campaign117FeatureError = _generated["Campaign117FeatureError"]

from scripts import (  # noqa: E402
    a_share_three_day_walkforward_campaign116_features as c116,
)
from scripts import (  # noqa: E402
    a_share_three_day_walkforward_campaign117_formula as formula,
)
...
FEATURE_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign117_features.py"
)
FORMULA_PATH = (
    REPO_ROOT / "scripts/a_share_three_day_walkforward_campaign117_formula.py"
)
FORMULA_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign117_formula.py"
)
AUDIT_RUNNER_PATH = (
...
AUDIT_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign117_no_return_audit.py"
)
...
def reconstruct_comparisons() -> list[dict[str, str]]:
    items = [dict(item) for item in c116.reconstruct_comparisons()]
    if (
        len(items) != NUMERIC_COMPARATOR_COUNT
        or _order_digest(items) != NUMERIC_COMPARATOR_ORDER_SHA256
    ):
        raise Campaign117FeatureError("Campaign117 numeric comparison order changed")
    return items


def reconstruct_prior_complete_definitions() -> list[dict[str, str]]:
    items = [dict(item) for item in c116.reconstruct_complete_definitions()]
    if (
        len(items) != PRIOR_COMPLETE_DEFINITION_COUNT
        or _order_digest(items) != PRIOR_COMPLETE_DEFINITION_ORDER_SHA256
    ):
        raise Campaign117FeatureError("Campaign117 prior definition order changed")
    return items
...
def reconstruct_complete_definitions() -> list[dict[str, str]]:
    ...
    if (
        ...
        raise Campaign117FeatureError("Campaign117 complete definition order changed")
    return items
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if target != DEFAULT_PROTOCOL.resolve():
        raise Campaign117FeatureError("Campaign117 protocol path changed")
    try:
        spec = formula.load_protocol(target)
    except formula.Campaign117FormulaError as exc:
        raise Campaign117FeatureError(str(exc)) from exc
    inputs = spec.get("authoritative_inputs") or {}
    policy = inputs.get("numeric_policy_v104") or {}
    ...
    if not (
        ...
        raise Campaign117FeatureError("Campaign117 protocol bindings changed")
    return spec


def extract_amount_weak_order_entropy(
    raw: pd.DataFrame, *, symbol: str
) -> tuple[pd.DataFrame, dict[str, int]]:
    if tuple(raw.columns) != RAW_COLUMNS:
        raise Campaign117FeatureError(
    ...
    if (
        ...
        raise Campaign117FeatureError(f"raw minute identity changed for {symbol}")
    ...
    if (
        ...
        raise Campaign117FeatureError(f"raw minute grid changed for {symbol}")
    selected = work.loc[
    ...
    if len(selected) != len(dates) * formula.SELECTED_BAR_COUNT:
        raise Campaign117FeatureError(f"continuous minute grid changed for {symbol}")
    ...
    values, eligible, _state_counts, _state_codes, formula_quality = (
        formula.compute_amount_weak_order_entropy(amount)
    )
    return pd.DataFrame({"trade_date": dates, FACTOR_SHORT_NAME: values}), {
        ...
        "exact_tie_triple_observations": int(
            formula_quality["exact_tie_triple_observations"]
        ),
    }
...
def validate_value_semantics(frame: pd.DataFrame) -> tuple[int, int]:
    ...
    if tuple(frame.columns) != expected:
        raise Campaign117FeatureError("Campaign117 output columns changed")
    ...
    if (
        ...
        raise Campaign117FeatureError("Campaign117 value semantics changed")
...
def _validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not path.is_file():
        raise Campaign117FeatureError("Campaign117 implementation freeze is absent")
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign117_no_return_implementation_freeze"
        ...
        and tests.get("passed") == 16
        and boundary.get("campaign117_historical_source_rows_read") is False
        and boundary.get("campaign117_candidate_values_computed_or_read") is False
        and boundary.get("campaign117_comparator_values_read") is False
        and boundary.get("historical_daily_price_or_forward_return_values_read")
        is False
        and boundary.get("provider_request_issued") is False
    ):
        raise Campaign117FeatureError("Campaign117 implementation freeze changed")
    return record
...
for _name, _value in {
    ...
    "extract_amount_weak_order_entropy": extract_amount_weak_order_entropy,
...
attach_entropy_values = _generated["attach_entropy_values"]
