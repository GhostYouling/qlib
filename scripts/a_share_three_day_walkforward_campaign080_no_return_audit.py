#!/usr/bin/env python3
"""Run Campaign080's frozen coverage-first no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign078_no_return_audit as c78_audit
from scripts import a_share_three_day_walkforward_campaign078_no_return_audit_v2 as c78_audit_v2
from scripts import a_share_three_day_walkforward_campaign080_features as candidate
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_080/no_return"
)
SNAPSHOT_MANIFEST_PATH = candidate.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
SNAPSHOT_MANIFEST_SHA256 = "64c883cc669425f9b8ab49ad2885de7c4239d7dc49f9944dab0ee3241f2c84b7"
SNAPSHOT_DATASET_SHA256 = "53c89fa15b46e21269c8956284707fe6bde09e77ca308d3506a3336d40e6ef62"
SNAPSHOT_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_080_feature_snapshot_binding_20260806.json"
SNAPSHOT_BINDING_SHA256 = "2e12b961571639f12768687d53408b5f4d48e8b63148a4950a5cd2b54d4e2887"
AUDIT_IMPLEMENTATION_FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_080_no_return_audit_implementation_freeze_20260806.json"
AUDIT_ACTIVATION_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_080_no_return_audit_activation_binding_20260806.json"
TEST_PATH = REPO_ROOT / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign080_no_return_audit.py"
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 7_724_498
EXPECTED_COMPARISON_COUNT = 109
EXPECTED_COMPLETE_DEFINITION_COUNT = 111
...
C78_SNAPSHOT_MANIFEST_PATH = candidate.C78_MANIFEST_PATH
STRUCTURALLY_NONNUMERIC_FACTOR = "intraday_cross_sectional_standardized_return_state_stability_236p"
PREVALUE_TERMINAL_NONNUMERIC_FACTOR = candidate.C79_FACTOR


class Campaign080NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign080 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign080NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign080NoReturnAuditError("Campaign080 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign080NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign080_no_return_audit_implementation_freeze"
        ...
        raise Campaign080NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign080NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign080_no_return_audit_activation_binding"
        ...
        raise Campaign080NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int = 4) -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign080NoReturnAuditError("candidate snapshot verification changed")
    return result


def verify_campaign078_snapshot(*, workers: int = 4) -> dict[str, Any]:
    return c78_audit.verify_candidate_snapshot(workers=workers)
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(SNAPSHOT_MANIFEST_PATH, SNAPSHOT_MANIFEST_SHA256, "candidate snapshot")
    _require(C78_SNAPSHOT_MANIFEST_PATH, candidate.C78_MANIFEST_SHA256, "Campaign078 snapshot")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign080NoReturnAuditError("snapshot binding validation failed")
    ...
    if not (
        ...
        raise Campaign080NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    c78_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign080NoReturnAuditError("conflicting Campaign080 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c78_audit_v2._fixed_load_comparisons_after_coverage(
        coverage=coverage,
    ...
    helper = c78_audit.c77_audit.c76_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    result, receipt = helper(
        manifest_path=C78_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C78_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: verify_campaign078_snapshot(
            workers=workers
        ),
    )
    comparisons.append(result)
    receipts["campaign078_snapshot"] = receipt
    receipts.pop("all_108_sources_loaded_in_frozen_order", None)
    receipts["all_109_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign080NoReturnAuditError("Campaign080 comparison count changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign080NoReturnAuditError("Campaign080 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign080_no_return_audit.json")):
        raise Campaign080NoReturnAuditError("Campaign080 no-return audit already exists")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    run_id = f"{timestamp}_campaign080_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign080_no_return_audit",
        ...
        "next_action": "freeze and run the exact one Campaign080 development trial"
        if admitted
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    c78_audit.c77_audit_v2.load_repair_protocol()
    c78_audit_v2.load_repair_protocol()
    with c78_audit.c77_audit_v2._temporary_runtime_version_binding():
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        experiment_root.expanduser().resolve().glob("*_campaign080_no_return_audit.json")
    )
    return {
