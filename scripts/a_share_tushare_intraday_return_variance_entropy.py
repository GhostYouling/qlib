#!/usr/bin/env python3
"""Build and audit the preregistered intraday return-variance entropy.

The candidate reads only minute identity and close from the immutable Tushare
source. It normalizes the 238 within-half squared log returns into realized-
variance shares and computes their Shannon entropy. The build and ordered
...
import a_share_tushare_intraday_amount_volatility_coupling as previous  # noqa: E402
...
diffusive = previous.previous
up_move = diffusive.previous
REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_return_variance_entropy_no_return_preregistration.json"
)
PREREGISTRATION_SHA256 = (
    "0493b2855e9e9c8d01e3a233cf5e7198fbb8b8ae303cf4f3d1d02be488cce5d3"
)
...
DIFFUSIVE_VARIATION_MANIFEST_SHA256 = previous.DIFFUSIVE_VARIATION_MANIFEST_SHA256
DIFFUSIVE_VARIATION_DATASET_SHA256 = previous.DIFFUSIVE_VARIATION_DATASET_SHA256
AMOUNT_VOLATILITY_COUPLING_MANIFEST_SHA256 = previous.CANDIDATE_MANIFEST_SHA256
AMOUNT_VOLATILITY_COUPLING_DATASET_SHA256 = previous.CANDIDATE_DATASET_SHA256
...
OUTPUT_RUN_ID = f"{SOURCE_RUN_ID}_intraday_return_variance_entropy_v1"
FACTOR_NAME = "intraday_return_variance_entropy_238m"
FACTOR_FORMULA = (
    "-Sum(q_hj * log(q_hj)) / log(238), q_hj = "
    "log(close_hj/close_h,j-1)^2 / "
    "Sum_all_238(log(close/previous_close)^2), "
...
class IntradayReturnVarianceEntropyError(RuntimeError):
...
def load_preregistration(path: Path = DEFAULT_PREREGISTRATION) -> dict[str, Any]:
    ...
    _require_file(path, PREREGISTRATION_SHA256, "return-variance-entropy protocol")
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_return_variance_entropy_no_return_preregistration",
    )
    ...
    if not (
        ...
        and current.get("terminal_mechanism_count_before_this_candidate") == 41
        ...
        and grid.get("variance_contributions") == 238
        ...
        and validity.get("strictly_positive_finite_total_realized_variance_required")
        is True
        and validity.get(
            "zero_variance_contribution_kept_in_support_with_zero_entropy_term"
        )
        is True
        and validity.get("entropy_finite_required") is True
        and validity.get("theoretical_closed_interval") == [0, 1]
        and validity.get("zero_total_realized_variance_policy") == "missing"
        and validity.get("numerical_endpoint_canonicalization_tolerance") == 1e-12
        and validity.get(
            "statistical_clipping_imputation_winsorization_power_change_sign_split_amount_weighting_or_daily_substitution_allowed"
        )
        is False
        ...
        == 0.8
        and uniqueness.get("all_seventeen_comparisons_must_pass") is True
        ...
        raise IntradayReturnVarianceEntropyError(
            "return-variance-entropy protocol no longer matches its frozen definition"
        )
    return spec
...
def validate_repository_chain(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    append_only_successor_state = (
        research.terminal_mechanism_is_preserved_in_append_only_status(
            state,
            mechanism="tushare_intraday_return_variance_entropy_238m",
            terminal_ordinal=42,
        )
    )
    if not (
    ...
    evidence["terminal_amount_volatility_coupling_record"] = {
...
def load_terminal_record_if_present() -> dict[str, Any] | None:
    ...
    _require_file(
        DEFAULT_TERMINAL_RECORD,
        TERMINAL_RECORD_SHA256,
        "intraday-return-variance-entropy research record",
    )
    record = research.load_json_record(
        DEFAULT_TERMINAL_RECORD,
        kind="a_share_tushare_intraday_return_variance_entropy_research_record",
    )
    ...
    if not (
        ...
        raise IntradayReturnVarianceEntropyError(
            "intraday-return-variance-entropy research record is inconsistent"
        )
    return record


def _validate_amount_volatility_coupling_manifest(
    spec: dict[str, Any], data_root: Path
) -> tuple[dict[str, Any], Path]:
    link = (spec.get("source_chain") or {}).get(
        "amount_volatility_coupling_comparison_manifest"
    ) or {}
    path = (data_root / str(link.get("path_below_data_root"))).resolve()
    _require_file(
        path,
        AMOUNT_VOLATILITY_COUPLING_MANIFEST_SHA256,
        "amount-volatility-coupling manifest",
    )
...
def validate_external_chain(spec: dict[str, Any], data_root: Path) -> tuple[Any, ...]:
    ...
    coupling, coupling_path = _validate_amount_volatility_coupling_manifest(
        spec, data_root
    )
    return (*chain, coupling, coupling_path)
...
def compute_partition_frame(
    ...
    *,
    ...
    """Compute the frozen entropy of 238 squared within-half returns."""
    ...
    empty_quality = {
        ...
        "nonfinite_entropy_component_rows": 0,
        "nonfinite_entropy_rows": 0,
        "numerical_endpoint_canonicalization_rows": 0,
        "return_variance_entropy_range_violation_rows": 0,
    }
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        realized_variance = squared_returns.sum(axis=1)
        variance_shares = squared_returns / realized_variance[:, None]
        entropy_components = np.where(
            variance_shares > 0.0,
            variance_shares * np.log(variance_shares),
            0.0,
        )
        values = -entropy_components.sum(axis=1) / math.log(238.0)
    log_returns_finite = np.isfinite(log_returns).all(axis=1)
    required_valid = required_closes_valid & log_returns_finite
    realized_variance_positive = np.isfinite(realized_variance) & (
        realized_variance > 0.0
    )
    entropy_components_finite = np.isfinite(entropy_components).all(axis=1)
    entropy_finite = np.isfinite(values)
    tolerance = 1e-12
    near_low = (
        required_valid
    ...
    if range_violation.any():
        raise IntradayReturnVarianceEntropyError(
            f"intraday return variance entropy escaped [0, 1] for {symbol}"
        )
    eligible = (
        required_valid
        & realized_variance_positive
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_intraday_return_variance_entropy"
        / OUTPUT_RUN_ID
    )
...
def _load_checkpoint(
    ...
    if not valid:
        raise IntradayReturnVarianceEntropyError(
            f"completed return-variance-entropy checkpoint changed: {paths.partial_sidecar}"
        )
    ...
    if len(output) != int(record.get("rows", -1)) or foundation.frame_digest(
        ...
        raise IntradayReturnVarianceEntropyError(
            f"completed return-variance-entropy frame changed: {paths.partial_data}"
        )
    dates = Counter(
...
def _process_partition(
    ...
    *,
    ...
    if foundation.file_digest(raw_path) != raw_record.get("byte_sha256"):
        raise IntradayReturnVarianceEntropyError(f"raw partition changed: {raw_path}")
    ...
    if len(raw) != int(raw_record.get("rows", -1)):
        raise IntradayReturnVarianceEntropyError(f"raw row count changed: {raw_path}")
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_return_variance_entropy_partition",
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    if final_root.exists():
        if not final_manifest.is_file():
            raise IntradayReturnVarianceEntropyError(
                f"published return-variance-entropy root has no manifest: {final_root}"
            )
        _require_file(
            final_manifest,
            CANDIDATE_MANIFEST_SHA256,
            "published return-variance-entropy manifest",
        )
        ...
        if not (
            ...
            == 0
            and (manifest.get("quality") or {}).get(
                "return_variance_entropy_range_violation_rows"
            )
            == 0
            and manifest.get("source_close_read") is True
            ...
            raise IntradayReturnVarianceEntropyError(
                f"published return-variance-entropy snapshot changed: {final_root}"
            )
    ...
    lock_path = data_root / ".a_share_tushare_intraday_return_variance_entropy.lock"
    with foundation.ProcessLock(lock_path):
        ...
        completed_symbols = 0
        print(
            f"building {len(joint_records):,} return-variance-entropy partitions "
        ...
        if len(all_records) != 33_015:
            raise IntradayReturnVarianceEntropyError(
                "not every source partition produced a return-variance-entropy checkpoint"
            )
...
def load_candidate_frame(manifest_path: Path, manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(frame) != int(manifest.get("rows", -1)):
        raise IntradayReturnVarianceEntropyError("candidate frame row count changed")
...
def uniqueness_audit(
    ...
    coupling_manifest_path: Path,
    spec: dict[str, Any],
    workers: int,
) -> dict[str, Any]:
    print("coverage passed; loading seventeen terminal comparison factors", flush=True)
    ...
    up_move_verification = up_move.verify_snapshot_files(
        up_move_manifest, up_move_manifest_path, workers
    )
    diffusive_verification = diffusive.verify_snapshot_files(
        diffusive_manifest, diffusive_manifest_path, workers
    )
    coupling_verification = previous.verify_snapshot_files(
        coupling_manifest, coupling_manifest_path, workers
    )
    dataset = pa_dataset.dataset(
    ...
    up_move_frame = up_move.load_candidate_frame(
        up_move_manifest_path, up_move_manifest
    )[["trade_date", "symbol", up_move.FACTOR_NAME]]
    diffusive_frame = diffusive.load_candidate_frame(
        diffusive_manifest_path, diffusive_manifest
    )[["trade_date", "symbol", diffusive.FACTOR_NAME]]
    coupling_frame = previous.load_candidate_frame(
        coupling_manifest_path, coupling_manifest
    ...
    all_passed = len(results) == 17 and all(item["gate_passed"] for item in results)
    return {
        ...
        "prior_candidate_snapshot_file_verification": {
            ...
            "amount_volatility_coupling": coupling_verification,
        },
        ...
        "all_seventeen_comparisons_passed": all_passed,
    }


def _find_existing_audit(experiment_root: Path, manifest_sha256: str) -> Path | None:
    for path in sorted(
        experiment_root.glob("*_intraday_return_variance_entropy_no_return_audit.json")
    ):
        record = research.load_json_record(path)
        if (
            record.get("kind")
            == "a_share_tushare_intraday_return_variance_entropy_no_return_audit"
    ...
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if terminal_record is not None:
        ...
        audit = research.load_json_record(
            audit_path,
            kind="a_share_tushare_intraday_return_variance_entropy_no_return_audit",
        )
        if not (
            audit.get("status")
            == "passed_no_return_coverage_capacity_and_uniqueness_pending_separate_return_diagnostic_preregistration"
    ...
    uniqueness: dict[str, Any] = {
        ...
        "all_seventeen_comparisons_passed": False,
        "comparisons": [],
    }
    ...
    passed = bool(
        ...
        and uniqueness["all_seventeen_comparisons_passed"]
    )
    status = (
        "passed_no_return_coverage_capacity_and_uniqueness_pending_separate_return_diagnostic_preregistration"
        if passed
    ...
    audit = {
        ...
        "status": status,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "purpose": "ordered_candidate_coverage_capacity_then_seventeen_terminal_factor_uniqueness_without_daily_prices_or_forward_returns",
        "preregistration": {
        ...
        "source_chain": {
            ...
            "amount_volatility_coupling_manifest_sha256": AMOUNT_VOLATILITY_COUPLING_MANIFEST_SHA256,
            "repository_evidence": repository_evidence,
...
def load_diagnostic_preregistration(
    ...
    _require_file(
        path,
        DIAGNOSTIC_PREREGISTRATION_SHA256,
        "intraday return-variance-entropy diagnostic protocol",
    )
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_return_variance_entropy_diagnostic_preregistration",
    )
    ...
    if not (
        ...
        raise IntradayReturnVarianceEntropyError(
            "intraday return-variance-entropy diagnostic protocol no longer matches its frozen definition"
        )
    return spec
...
def require_diagnostic_unconsumed(experiment_root: Path) -> None:
    ...
    if marker.exists():
        raise IntradayReturnVarianceEntropyError(
            f"intraday return-variance-entropy historical diagnostic is already consumed: {marker}"
        )
    for path in sorted(experiment_root.glob("*_factor_diagnostic.json")):
        record = research.load_json_record(path)
        if record.get("purpose") == DIAGNOSTIC_PURPOSE:
            raise IntradayReturnVarianceEntropyError(
                f"intraday return-variance-entropy historical diagnostic already exists: {path}"
            )
...
def run_diagnostic(args: argparse.Namespace) -> Path:
    ...
    marker = {
        "kind": "a_share_tushare_intraday_return_variance_entropy_historical_consumption",
    ...
    diagnostic = {
        ...
        "strategy_timing": {
            ...
            "signal_time": "full-session intraday return-variance-entropy factor known after signal-session close",
