#!/usr/bin/env python3
"""Build the frozen Campaign060 consecutive-session sign-recurrence factor.

The factor aligns the same stock's fixed 238 within-half close returns on the
signal session and its exact accepted-calendar predecessor.  It reads no daily
price, forward return, provider API, or Candidate49 historical outcome.
"""
...
FACTOR_FORMULA = (
    ...
    "fixed within-half signed log-close returns for each session; among "
    "same-position pairs where both returns are exactly nonzero, return the "
    "equal-sign count divided by the jointly nonzero count."
)
PROTOCOL_SHA256 = "1d8d41bcfde2cd0ae81e69eae0abb4fd7ef92fed9ec63dd8187e510cd02b92a3"
MECHANISM_AUDIT_SHA256 = "508d079eb00a59039538dcc686ee3bfeb383fbe6395870f2322fe30c82e26606"
COMPARISON_COUNT = 91
COMPARISON_ORDER_SHA256 = "abbf08eef9022adec5dfa0be4d15ae79a2007f4d424b7aa1d29522d8319e9312"
...
OUTPUT_RUN_ID = (
    ...
    "walkforward_campaign060_feature_library_v1"
)
...
for _old, _new in (
    ("Campaign057", "Campaign060"),
    ("campaign057", "campaign060"),
    ("campaign_057", "campaign_060"),
    ...
    ("COMPARISON_COUNT = 80", "COMPARISON_COUNT = 91"),
    ("all_80_must_pass", "all_91_must_pass"),
):
    _source = _source.replace(_old, _new)

_runtime: dict[str, Any] = {
    "__file__": str(Path(__file__).resolve()),
    "__name__": "a_share_three_day_walkforward_campaign060_features_runtime",
}
exec(compile(_source, str(BASE_RUNNER), "exec"), _runtime)
_engine: dict[str, Any] = _runtime["_generated"]

Campaign060FeatureError = _runtime["Campaign060FeatureError"]
...
def reconstruct_comparisons(spec: dict[str, Any]) -> list[dict[str, str]]:
    ...
    if (
        ...
        raise Campaign060FeatureError("Campaign059 comparison protocol changed")
    prior = json.loads(prior_path.read_text(encoding="utf-8"))
    inherited = campaign059.reconstruct_comparisons(prior)
    appended = list(
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if not path.is_file() or _local_sha256(path) != PROTOCOL_SHA256:
        raise Campaign060FeatureError(f"Campaign060 no-return protocol changed: {path}")
    report = bindings.validate_record(path, data_root=DEFAULT_DATA_ROOT)
    if report.get("all_bindings_passed") is not True:
        raise Campaign060FeatureError("Campaign060 protocol binding failed")
    ...
    if not (
        spec.get("version") == 1
        and spec.get("kind")
        == "a_share_three_day_walkforward_campaign060_no_return_preregistration"
        and spec.get("status")
        == "frozen_before_campaign060_source_candidate_comparison_daily_price_or_return_values"
        ...
        and candidate.get("minimum_informative_positions")
        ...
        and uniqueness.get("all_91_must_pass") is True
        and uniqueness.get("maximum_allowed_absolute_median_daily_rank_correlation")
        == 0.8
        and uniqueness.get("minimum_pairwise_names_per_session") == 50
        and uniqueness.get("minimum_pairwise_sessions_per_comparison") == 100
        and finite.get("trial_id") == f"wf060_{FACTOR_NAME}_single_higher"
        ...
        raise Campaign060FeatureError("Campaign060 protocol semantics changed")
    return spec


def compute_directional_agreement_values(
    current_returns: np.ndarray,
    prior_returns: np.ndarray,
) -> tuple[np.ndarray, np.ndarray, dict[str, int]]:
    """Compute the frozen equal-vote same-clock sign recurrence."""
    ...
    if (
        ...
        raise Campaign060FeatureError("return matrices must share shape (n, 238)")
    current_finite = np.isfinite(current).all(axis=1)
    prior_finite = np.isfinite(prior).all(axis=1)
    informative = (current != 0.0) & (prior != 0.0)
    informative_count = informative.sum(axis=1)
    agreement_count = (informative & (np.signbit(current) == np.signbit(prior))).sum(
        axis=1
    )
    support = informative_count >= MINIMUM_INFORMATIVE_POSITIONS
    input_valid = current_finite & prior_finite & support
    denominator = np.where(support, informative_count, 1)
    raw_score = agreement_count / denominator
    ...
    quality = {
        ...
        "insufficient_informative_position_rows": int(
            (current_finite & prior_finite & ~support).sum()
        ),
        "eligible_informative_positions": int(informative[eligible].sum()),
        "eligible_directional_agreements": int(
            (informative & (np.signbit(current) == np.signbit(prior)))[eligible].sum()
        ),
...
def extract_signed_return_profiles(
    ...
    *,
    ...
    if (
        ...
        raise Campaign060FeatureError(f"raw identity changed for {symbol}")
    ...
    if not codes.eq(SOURCE_MINUTE_CODE_SET).all():
        raise Campaign060FeatureError(f"raw minute grid changed for {symbol}")
    continuous = work.loc[
    ...
    if len(continuous) != len(dates) * SELECTED_CLOSE_COUNT:
        raise Campaign060FeatureError(f"continuous grid changed for {symbol}")
...
for _name, _value in {
    ...
    "compute_similarity_values": compute_directional_agreement_values,
    "compute_directional_agreement_values": compute_directional_agreement_values,
