#!/usr/bin/env python3
"""Run Campaign296's frozen Alpha158 linear-trend-quality campaign."""
...
from scripts import a_share_three_day_walkforward_campaign295 as prior


REPO_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_preregistration_20260825.json"
)
ORIGINAL_IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_implementation_freeze_20260825.json"
)
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_implementation_freeze_v3_20260825.json"
)
PREVIOUS_IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_implementation_freeze_v2_20260825.json"
)
CONCEPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_concept_scouting_20260825.json"
)
SHELL_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_zsh_sha_verification_split_failure_20260825.json"
)
COMPARATOR_DIRECTION_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_comparator_direction_digest_failure_20260825.json"
)
LOCAL_RECONSTRUCTION_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_local_reconstruction_import_failure_20260825.json"
)
COMPARATOR_DIRECTION_RECEIPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_comparator_direction_reconstruction_receipt_20260825.json"
)
CAMPAIGN295_COMPARATOR_RANGE_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_296_campaign295_comparator_range_failure_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign296.py"
)
...
C295_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
SIGNAL_LEDGER_PATH = prior.SIGNAL_LEDGER_PATH
EXECUTION_LEDGER_PATH = prior.EXECUTION_LEDGER_PATH

FAILED_OUTPUT_ROOT = (
    REPO_ROOT / ".local-research/campaign_296/alpha158_linear_trend_quality_10d_v1"
)
FAILED_CANDIDATE_MANIFEST_PATH = (
    FAILED_OUTPUT_ROOT / "candidate_snapshot/snapshot_manifest.json"
)
OUTPUT_ROOT = (
    REPO_ROOT / ".local-research/campaign_296/alpha158_linear_trend_quality_10d_v2"
)
...
FACTOR_NAME = "alpha158_linear_trend_quality_10d"
TRIAL_ID = "wf296_alpha158_linear_trend_quality_10d_single_higher"
SLOPE_FEATURE = "BETA10"
LINEARITY_FEATURE = "RSQR10"
INPUT_TOLERANCE = 1e-6
FEATURE_COUNT = 158
MINIMUM_ALPHA158_FINITE_FEATURES = 119
EXPECTED_COMPARATOR_COUNT = 148
...
CAMPAIGN295_FACTOR = prior.FACTOR_NAME
...
class Campaign296Error(RuntimeError):
    """Fail closed when a frozen Campaign296 invariant changes."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Campaign296Error(f"JSON binding unreadable: {path}") from exc
    if not isinstance(value, dict):
        raise Campaign296Error(f"JSON binding is not an object: {path}")
    return value


def require_file(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or file_sha256(path) != expected:
        raise Campaign296Error(f"{label} fingerprint changed")


def validate_protocol() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    prior.verify_terminal()
    ...
    if not (
        protocol.get("kind")
        == "a_share_three_day_walkforward_campaign296_preregistration"
        and protocol.get("status")
        == "fully_frozen_before_campaign296_candidate_comparator_daily_price_or_forward_return_values"
        and len(protocol.get("factor_library") or []) == 1
        and factor.get("name") == FACTOR_NAME
        and factor.get("direction") == "higher"
        and parameters.get("signed_slope_feature") == SLOPE_FEATURE
        and parameters.get("linearity_feature") == LINEARITY_FEATURE
        and parameters.get("fixed_horizon_sessions") == 10
        and parameters.get("score") == "BETA10 * RSQR10"
        ...
        and uniqueness.get("all_148_must_pass") is True
        ...
        == {"complete_definitions": 167, "numeric_comparators": 148}
        and boundary.get(
            "campaign296_candidate_or_comparator_values_read_before_protocol"
        )
        is False
        and boundary.get(
        ...
        raise Campaign296Error("Campaign296 protocol semantics changed")
    ...
    if not (
        ...
        and SLOPE_FEATURE in alpha["feature_names"]
        and LINEARITY_FEATURE in alpha["feature_names"]
        ...
        raise Campaign296Error("Campaign296 source or Candidate49 semantics changed")
...
def validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign296_implementation_freeze_v3"
        and freeze.get("status")
        == "scientifically_identical_comparator_range_correction_refrozen_before_return_values"
        ...
        and frozen.get("prevalue_infrastructure_failure_sha256")
        == file_sha256(SHELL_FAILURE_PATH)
        and frozen.get("comparator_direction_failure_sha256")
        == file_sha256(COMPARATOR_DIRECTION_FAILURE_PATH)
        and frozen.get("local_reconstruction_failure_sha256")
        == file_sha256(LOCAL_RECONSTRUCTION_FAILURE_PATH)
        and frozen.get("comparator_direction_reconstruction_receipt_sha256")
        == file_sha256(COMPARATOR_DIRECTION_RECEIPT_PATH)
        and frozen.get("campaign295_comparator_range_failure_sha256")
        == file_sha256(CAMPAIGN295_COMPARATOR_RANGE_FAILURE_PATH)
        and frozen.get("failed_candidate_manifest_sha256")
        == file_sha256(FAILED_CANDIDATE_MANIFEST_PATH)
        and frozen.get("scientific_semantics_changed") is False
        and frozen.get("factor_name") == FACTOR_NAME
        and frozen.get("formula") == "BETA10 * RSQR10"
        and frozen.get("fixed_horizon_sessions") == 10
        ...
        and boundary.get("campaign296_candidate_values_read_before_freeze") is True
        ...
        raise Campaign296Error("Campaign296 implementation freeze changed")
    return freeze


def linear_trend_quality(
    *,
    keys: np.ndarray,
    beta: np.ndarray,
    rsquare: np.ndarray,
    ...
    if not (
        ...
        raise Campaign296Error("Alpha158 row support semantics changed")
    valid_inputs = (
        np.isfinite(beta)
        & np.isfinite(rsquare)
        & (rsquare >= -INPUT_TOLERANCE)
        & (rsquare <= 1.0 + INPUT_TOLERANCE)
    )
    score = np.full(len(keys), np.nan, dtype=np.float64)
    calculable = model_support & valid_inputs
    score[calculable] = beta[calculable] * rsquare[calculable]
    selected = np.flatnonzero(quality_listing)
    selected_scores = score[selected]
    input_counts = np.isfinite(beta[selected]).astype(np.uint8) + np.isfinite(
        rsquare[selected]
    ).astype(np.uint8)
    eligible = (
        model_support[selected] & valid_inputs[selected] & np.isfinite(selected_scores)
    )
    return pd.DataFrame(
        {
...
def build_candidate_snapshot(alpha: Mapping[str, Any]) -> dict[str, Any]:
    if CANDIDATE_ROOT.exists():
        raise Campaign296Error("candidate output root already exists; refuse overwrite")
    columns = [
        "stock_day_key",
        SLOPE_FEATURE,
        LINEARITY_FEATURE,
    ...
    for year in YEARS:
        ...
        for _, frame in base.iter_session_frames(source_path, columns):
            parts.append(
                linear_trend_quality(
                    keys=frame["stock_day_key"].to_numpy(dtype=np.int64),
                    beta=frame[SLOPE_FEATURE].to_numpy(dtype=np.float64),
                    rsquare=frame[LINEARITY_FEATURE].to_numpy(dtype=np.float64),
        ...
        if len(np.unique(annual["stock_day_key"])) != len(annual):
            raise Campaign296Error(f"candidate annual keys duplicated: {year}")
    ...
    if [item["sessions"] for item in records] != [244, 243, 243, 242, 242]:
        raise Campaign296Error("candidate session counts changed")
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign296_candidate_snapshot",
        "status": "immutable_candidate_ready_for_coverage_before_comparator_values",
        "created_at": base.utc_now(),
        "factor_name": FACTOR_NAME,
        "direction": "higher",
        "formula_parameters": {
            "signed_slope_feature": SLOPE_FEATURE,
            "linearity_feature": LINEARITY_FEATURE,
            "fixed_horizon_sessions": 10,
            "score": "BETA10 * RSQR10",
    ...
    failed_manifest = load_json(FAILED_CANDIDATE_MANIFEST_PATH)
    replay_signature = [
        [item["year"], item["sha256"], item["rows"], item["eligible_rows"]]
        for item in records
    ]
    failed_signature = [
        [item["year"], item["sha256"], item["rows"], item["eligible_rows"]]
        for item in failed_manifest.get("files", [])
    ]
    if not (
        manifest["dataset_sha256"] == failed_manifest.get("dataset_sha256")
        and replay_signature == failed_signature
        and totals == failed_manifest.get("totals")
    ):
        raise Campaign296Error("candidate replay changed after infrastructure failure")
    manifest["failed_v1_candidate_manifest_sha256"] = file_sha256(
        FAILED_CANDIDATE_MANIFEST_PATH
    )
    manifest["scientifically_identical_candidate_replay"] = True
...
def load_candidate_snapshot(
    ...
    for record in manifest.get("files", []):
        ...
        require_file(
            path, str(record["sha256"]), f"candidate partition {record['year']}"
        )
        ...
        if not (np.all(np.diff(keys) >= 0) and np.all(np.isfinite(values[eligible]))):
            raise Campaign296Error("candidate snapshot values changed")
    ...
    if len(np.unique(keys)) != len(keys) or np.any(keys[1:] < keys[:-1]):
        raise Campaign296Error("candidate snapshot key order changed")
...
def comparison_definitions(numeric_manifest: Mapping[str, Any]) -> list[str]:
    ...
    if len(definitions) != EXPECTED_COMPARATOR_COUNT:
        raise Campaign296Error("comparator name order changed")
    ...
    if observed != EXPECTED_COMPARATOR_ORDER_SHA256:
        raise Campaign296Error("comparator order digest changed")
    return definitions


def prior_campaign_comparison_with_range(
    *,
    manifest_path: Path,
    factor: str,
    source: str,
    ...
    for year in YEARS:
        ...
        frame = pd.read_parquet(
            path,
            columns=["stock_day_key", factor, f"{factor}_eligible"],
        )
        ...
        if not np.array_equal(source_keys, candidate_keys):
            raise Campaign296Error(f"{factor} candidate alignment changed: {year}")
        finite = values[np.isfinite(values)]
        if finite.size and ((finite < minimum).any() or (finite > maximum).any()):
            raise Campaign296Error(f"{factor} comparator range changed")
    ...
    *, numeric_manifest: Mapping[str, Any], candidate_manifest: Mapping[str, Any]
    ...
    if failed_ordinal is None:
        ...
        for ordinal, (manifest_path, factor, source, minimum, maximum) in enumerate(
            ...
            result, receipt = prior_campaign_comparison_with_range(
    ...
    return {
        "status": (
            "all_148_uniqueness_gates_passed"
            if all_passed
...
def write_trial_ledger(
    *, no_return: Mapping[str, Any], terminal: Mapping[str, Any]
    ...
    infrastructure_failures = [
        (SHELL_FAILURE_PATH, "zsh_sha_verification_scalar_split"),
        (COMPARATOR_DIRECTION_FAILURE_PATH, "comparator_direction_digest"),
        (LOCAL_RECONSTRUCTION_FAILURE_PATH, "local_reconstruction_import"),
        (
            CAMPAIGN295_COMPARATOR_RANGE_FAILURE_PATH,
            "campaign295_comparator_range",
        ),
    ]
    for path, name in infrastructure_failures:
        ...
        entries.append(
            {
                ...
                "failure_record_sha256": file_sha256(path),
                "historical_forward_return_values_read": False,
            }
        )
    entries.append(
        {
            "attempt_id": "campaign296_factor_001",
            "phase": "coverage_uniqueness_and_conditional_development",
            "name": FACTOR_NAME,
            "formula": "BETA10 * RSQR10",
            "direction": "higher",
            "parameters": {
                "fixed_horizon_sessions": 10,
                "input_numerical_tolerance": INPUT_TOLERANCE,
    ...
    ledger = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign296_terminal_trial_ledger",
...
def run_campaign(*, batch_size: int) -> dict[str, Any]:
    if batch_size != 8192:
        raise Campaign296Error("Campaign296 batch size changed")
    protocol, alpha, numeric = validate_protocol()
    validate_implementation_freeze()
    verify_source_files(alpha, numeric)
    if OUTPUT_ROOT.exists():
        raise Campaign296Error(
            "Campaign296 output root already exists; refuse overwrite"
        )
    ...
    all_unique = bool(uniqueness and uniqueness["all_148_passed"])
    no_return = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign296_no_return_audit",
        "status": (
    ...
    if all_unique:
        ...
        for fold in protocol["development_trial"]["walkforward_folds"]:
            ...
            binding = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign296_prevalidation_scores",
    ...
    terminal = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign296_terminal_result",
        "status": (
        ...
        "prior_definition_count": 167,
        "prior_numeric_comparator_count": 148,
...
def verify_terminal() -> dict[str, Any]:
    ...
    for ordinal, entry in enumerate(ledger.get("entries", []), start=1):
        ...
        if not (
            ...
            raise Campaign296Error("terminal trial ledger chain changed")
        previous = observed
    if not (
        terminal.get("kind")
        == "a_share_three_day_walkforward_campaign296_terminal_result"
        ...
        raise Campaign296Error("Campaign296 terminal semantics changed")
    return {
        "status": "verified_campaign296_terminal",
...
def plan() -> dict[str, Any]:
    ...
    for manifest_path, manifest, label in (
        ...
        for year in YEARS:
            ...
            path = resolve_record(manifest_path, record)
            if not path.is_file():
                raise Campaign296Error(f"{label} annual partition missing: {year}")
            schemas.append(
                {
                    "source": label,
                    "year": year,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        if not args.confirm_run:
            raise Campaign296Error("run requires --confirm-run")
        print(
            json.dumps(
