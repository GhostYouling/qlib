#!/usr/bin/env python3
"""Run Campaign081's frozen coverage-first no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign080_no_return_audit as c80_audit
from scripts import a_share_three_day_walkforward_campaign081_features as candidate
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_081/no_return"
)
SNAPSHOT_MANIFEST_PATH = candidate.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
SNAPSHOT_MANIFEST_SHA256 = "fb8bed1defb00cffb4508e78b74ad79a00e4d9dad64c7a9b48c5de206051f1ab"
SNAPSHOT_DATASET_SHA256 = "7fecf302d703fd040ae710a31f5fb78949d60a021751cf94414aec607d73eeac"
SNAPSHOT_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_081_feature_snapshot_binding_20260806.json"
SNAPSHOT_BINDING_SHA256 = "d7de059bb486189b3a29ebda76c42362120cabaac9b23460df444cf781393fad"
AUDIT_IMPLEMENTATION_FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_081_no_return_audit_implementation_freeze_20260806.json"
AUDIT_ACTIVATION_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_081_no_return_audit_activation_binding_20260806.json"
TEST_PATH = REPO_ROOT / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign081_no_return_audit.py"
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 7_724_498
EXPECTED_COMPARISON_COUNT = 110
EXPECTED_COMPLETE_DEFINITION_COUNT = 112
...
C80_SNAPSHOT_MANIFEST_PATH = candidate.C80_MANIFEST_PATH
...
class Campaign081NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign081 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign081NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign081NoReturnAuditError("Campaign081 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign081NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign081_no_return_audit_implementation_freeze"
        ...
        raise Campaign081NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign081NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign081_no_return_audit_activation_binding"
        ...
        raise Campaign081NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int = 4) -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign081NoReturnAuditError("candidate snapshot verification changed")
    return result


def verify_campaign080_snapshot(*, workers: int = 4) -> dict[str, Any]:
    return c80_audit.verify_candidate_snapshot(workers=workers)
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(SNAPSHOT_MANIFEST_PATH, SNAPSHOT_MANIFEST_SHA256, "candidate snapshot")
    _require(C80_SNAPSHOT_MANIFEST_PATH, candidate.C80_MANIFEST_SHA256, "Campaign080 snapshot")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign081NoReturnAuditError("snapshot binding validation failed")
    ...
    if not (
        ...
        raise Campaign081NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    c80_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign081NoReturnAuditError("conflicting Campaign081 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c80_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    helper = c80_audit.c78_audit.c77_audit.c76_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    result, receipt = helper(
        manifest_path=C80_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C80_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: verify_campaign080_snapshot(
            workers=workers
        ),
    )
    comparisons.append(result)
    receipts["campaign080_snapshot"] = receipt
    receipts.pop("all_109_sources_loaded_in_frozen_order", None)
    receipts["all_110_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign081NoReturnAuditError("Campaign081 comparison count changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign081NoReturnAuditError("Campaign081 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign081_no_return_audit.json")):
        raise Campaign081NoReturnAuditError("Campaign081 no-return audit already exists")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    run_id = f"{timestamp}_campaign081_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign081_no_return_audit",
        ...
        "next_action": "freeze and run the exact one Campaign081 development trial"
        if admitted
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    c80_audit.c78_audit.c77_audit_v2.load_repair_protocol()
    c80_audit.c78_audit_v2.load_repair_protocol()
    with c80_audit.c78_audit.c77_audit_v2._temporary_runtime_version_binding():
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        experiment_root.expanduser().resolve().glob("*_campaign081_no_return_audit.json")
    )
    return {
