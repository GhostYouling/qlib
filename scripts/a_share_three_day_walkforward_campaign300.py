#!/usr/bin/env python3
"""Run Campaign300's frozen Alpha158 conditional-return magnitude campaign."""
...
from scripts import a_share_three_day_walkforward_campaign299 as prior


REPO_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_300_preregistration_20260825.json"
)
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_300_implementation_freeze_20260825.json"
)
CONCEPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_300_concept_scouting_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign300.py"
)
...
C297_MANIFEST_PATH = prior.C297_MANIFEST_PATH
C298_MANIFEST_PATH = prior.C298_MANIFEST_PATH
C299_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
SIGNAL_LEDGER_PATH = prior.SIGNAL_LEDGER_PATH
EXECUTION_LEDGER_PATH = prior.EXECUTION_LEDGER_PATH

OUTPUT_ROOT = (
    REPO_ROOT
    / ".local-research/campaign_300/alpha158_conditional_return_magnitude_asymmetry_20d_v1"
)
...
FACTOR_NAME = "alpha158_conditional_return_magnitude_asymmetry_20d"
TRIAL_ID = "wf300_alpha158_conditional_return_magnitude_asymmetry_20d_single_higher"
POSITIVE_SHARE_FEATURE = "SUMP20"
NEGATIVE_SHARE_FEATURE = "SUMN20"
POSITIVE_COUNT_FEATURE = "CNTP20"
NEGATIVE_COUNT_FEATURE = "CNTN20"
FEATURE_COUNT = 158
MINIMUM_ALPHA158_FINITE_FEATURES = 119
EXPECTED_COMPARATOR_COUNT = 152
...
CAMPAIGN297_FACTOR = prior.CAMPAIGN297_FACTOR
CAMPAIGN298_FACTOR = prior.CAMPAIGN298_FACTOR
CAMPAIGN299_FACTOR = prior.FACTOR_NAME
...
class Campaign300Error(RuntimeError):
    """Fail closed when a frozen Campaign300 invariant changes."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Campaign300Error(f"JSON binding unreadable: {path}") from exc
    if not isinstance(value, dict):
        raise Campaign300Error(f"JSON binding is not an object: {path}")
    return value


def require_file(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or file_sha256(path) != expected:
        raise Campaign300Error(f"{label} fingerprint changed")
...
def validate_protocol() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        protocol.get("kind")
        == "a_share_three_day_walkforward_campaign300_preregistration"
        and protocol.get("status")
        == "fully_frozen_before_campaign300_candidate_comparator_daily_price_or_forward_return_values"
        and len(protocol.get("factor_library") or []) == 1
        and factor.get("name") == FACTOR_NAME
        and factor.get("direction") == "higher"
        and parameters.get("positive_share_feature") == POSITIVE_SHARE_FEATURE
        and parameters.get("negative_share_feature") == NEGATIVE_SHARE_FEATURE
        and parameters.get("positive_count_feature") == POSITIVE_COUNT_FEATURE
        and parameters.get("negative_count_feature") == NEGATIVE_COUNT_FEATURE
        and parameters.get("fixed_horizon_sessions") == 20
        and parameters.get("score")
        ...
        and uniqueness.get("all_152_must_pass") is True
        ...
        == {"complete_definitions": 171, "numeric_comparators": 152}
        and boundary.get(
            "campaign300_candidate_or_comparator_values_read_before_protocol"
        )
        is False
        and boundary.get(
        ...
        raise Campaign300Error("Campaign300 protocol semantics changed")
    ...
    required_features = {
        POSITIVE_SHARE_FEATURE,
        NEGATIVE_SHARE_FEATURE,
        POSITIVE_COUNT_FEATURE,
        NEGATIVE_COUNT_FEATURE,
    }
    if not (
        ...
        raise Campaign300Error("Campaign300 source or Candidate49 semantics changed")
...
def validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign300_implementation_freeze"
        and freeze.get("status")
        == "fully_frozen_before_campaign300_candidate_comparator_or_return_values"
        ...
        raise Campaign300Error("Campaign300 implementation freeze changed")
    return freeze


def conditional_return_magnitude_asymmetry(
    *,
    keys: np.ndarray,
    ...
    if not (
        ...
        raise Campaign300Error("Alpha158 row support semantics changed")
    finite_inputs = (
        np.isfinite(positive_share)
        & np.isfinite(negative_share)
        & np.isfinite(positive_count)
        & np.isfinite(negative_count)
    )
    valid_inputs = (
        finite_inputs
        & (positive_share >= 0.0)
    ...
    positive_magnitude = np.full(len(keys), np.nan, dtype=np.float64)
    negative_magnitude = np.full(len(keys), np.nan, dtype=np.float64)
    positive_magnitude[calculable] = (
        positive_share[calculable] / positive_count[calculable]
    )
    negative_magnitude[calculable] = (
        negative_share[calculable] / negative_count[calculable]
    )
    magnitude_sum = positive_magnitude + negative_magnitude
    calculable &= (
        np.isfinite(positive_magnitude)
        & np.isfinite(negative_magnitude)
        & (positive_magnitude >= 0.0)
        & (negative_magnitude >= 0.0)
        & np.isfinite(magnitude_sum)
        & (magnitude_sum > 0.0)
    )
    score[calculable] = positive_magnitude[calculable] / magnitude_sum[calculable]
    ...
    input_counts = (
        np.isfinite(positive_share[selected]).astype(np.uint8)
        + np.isfinite(negative_share[selected]).astype(np.uint8)
        + np.isfinite(positive_count[selected]).astype(np.uint8)
        + np.isfinite(negative_count[selected]).astype(np.uint8)
    )
...
def build_candidate_snapshot(alpha: Mapping[str, Any]) -> dict[str, Any]:
    if CANDIDATE_ROOT.exists():
        raise Campaign300Error("candidate output root already exists; refuse overwrite")
    columns = [
        "stock_day_key",
        POSITIVE_SHARE_FEATURE,
        NEGATIVE_SHARE_FEATURE,
    ...
    for year in YEARS:
        ...
        for _, frame in base.iter_session_frames(source_path, columns):
            parts.append(
                conditional_return_magnitude_asymmetry(
                    keys=frame["stock_day_key"].to_numpy(dtype=np.int64),
                    positive_share=frame[POSITIVE_SHARE_FEATURE].to_numpy(dtype=np.float64),
                    negative_share=frame[NEGATIVE_SHARE_FEATURE].to_numpy(dtype=np.float64),
                    positive_count=frame[POSITIVE_COUNT_FEATURE].to_numpy(dtype=np.float64),
                    negative_count=frame[NEGATIVE_COUNT_FEATURE].to_numpy(dtype=np.float64),
        ...
        if len(np.unique(annual["stock_day_key"])) != len(annual):
            raise Campaign300Error(f"candidate annual keys duplicated: {year}")
    ...
    if [item["sessions"] for item in records] != [244, 243, 243, 242, 242]:
        raise Campaign300Error("candidate session counts changed")
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign300_candidate_snapshot",
        "status": "immutable_candidate_ready_for_coverage_before_comparator_values",
        "created_at": base.utc_now(),
        "factor_name": FACTOR_NAME,
        "direction": "higher",
        "formula_parameters": {
            "positive_share_feature": POSITIVE_SHARE_FEATURE,
            "negative_share_feature": NEGATIVE_SHARE_FEATURE,
            "positive_count_feature": POSITIVE_COUNT_FEATURE,
            "negative_count_feature": NEGATIVE_COUNT_FEATURE,
            "fixed_horizon_sessions": 20,
            "score": "(SUMP20 / CNTP20) / ((SUMP20 / CNTP20) + (SUMN20 / CNTN20))",
...
def load_candidate_snapshot(
    ...
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign300_candidate_snapshot"
        ...
        raise Campaign300Error("candidate manifest semantics changed")
    ...
    if canonical_sha256(digest_rows) != manifest.get("dataset_sha256"):
        raise Campaign300Error("candidate dataset digest changed")
    keys = np.concatenate(key_parts)
    values = np.concatenate(value_parts)
    eligible = np.concatenate(eligible_parts)
    if len(np.unique(keys)) != len(keys) or np.any(keys[1:] < keys[:-1]):
        raise Campaign300Error("candidate snapshot key order changed")
...
def comparison_definitions(numeric_manifest: Mapping[str, Any]) -> list[str]:
    ...
    definitions.append(CAMPAIGN299_FACTOR)
    if len(definitions) != EXPECTED_COMPARATOR_COUNT:
        raise Campaign300Error("comparator name order changed")
    ...
    if observed != EXPECTED_COMPARATOR_ORDER_SHA256:
        raise Campaign300Error("comparator order digest changed")
    return definitions
    ...
    *,
    ...
    for year in YEARS:
        ...
        if not np.array_equal(source_keys, candidate_keys):
            raise Campaign300Error(f"{factor} candidate alignment changed: {year}")
        finite = values[np.isfinite(values)]
        if finite.size and minimum is not None and (finite < minimum).any():
            raise Campaign300Error(f"{factor} comparator minimum changed")
        if finite.size and maximum is not None and (finite > maximum).any():
            raise Campaign300Error(f"{factor} comparator maximum changed")
    ...
    *, numeric_manifest: Mapping[str, Any], candidate_manifest: Mapping[str, Any]
    ...
    prior_specs = [
        ...
        (
            C298_MANIFEST_PATH,
            CAMPAIGN298_FACTOR,
            "campaign298_verified_candidate_snapshot",
            0.0,
            None,
        ),
        (
            C299_MANIFEST_PATH,
            CAMPAIGN299_FACTOR,
            "campaign299_verified_candidate_snapshot",
            None,
            None,
        ),
    ]
    ...
    return {
        "status": (
            "all_152_uniqueness_gates_passed"
            if all_passed
...
def write_trial_ledger(
    *, no_return: Mapping[str, Any], terminal: Mapping[str, Any]
    ...
    entries.append(
        {
            "attempt_id": "campaign300_factor_001",
            "phase": "coverage_uniqueness_and_conditional_development",
            "name": FACTOR_NAME,
            "formula": "(SUMP20 / CNTP20) / ((SUMP20 / CNTP20) + (SUMN20 / CNTN20))",
            "direction": "higher",
            "parameters": {
                "fixed_horizon_sessions": 20,
                "fit": "none",
    ...
    ledger = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign300_terminal_trial_ledger",
...
def run_campaign(*, batch_size: int) -> dict[str, Any]:
    ...
    all_unique = bool(uniqueness and uniqueness["all_152_passed"])
    no_return = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign300_no_return_audit",
        "status": (
    ...
    if all_unique:
        ...
        for fold in protocol["development_trial"]["walkforward_folds"]:
            ...
            binding = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign300_prevalidation_scores",
    ...
    terminal = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign300_terminal_result",
        "status": (
        ...
        "prior_definition_count": 171,
        "prior_numeric_comparator_count": 152,
...
def verify_terminal() -> dict[str, Any]:
    ...
    for ordinal, entry in enumerate(ledger.get("entries", []), start=1):
        ...
        if not (
            ...
            raise Campaign300Error("terminal trial ledger chain changed")
        previous = observed
    if not (
        terminal.get("kind")
        == "a_share_three_day_walkforward_campaign300_terminal_result"
        ...
        and ledger.get("infrastructure_failure_count") == 0
        ...
        raise Campaign300Error("Campaign300 terminal semantics changed")
    return {
        "status": "verified_campaign300_terminal",
...
def plan() -> dict[str, Any]:
    ...
    for manifest_path, manifest, label in (
        ...
        for year in YEARS:
            ...
            path = resolve_record(manifest_path, record)
            if not path.is_file():
                raise Campaign300Error(f"{label} annual partition missing: {year}")
            schemas.append(
                {
                    "source": label,
                    "year": year,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        if not args.confirm_run:
            raise Campaign300Error("run requires --confirm-run")
        print(
            json.dumps(
