#!/usr/bin/env python3
"""Run Campaign084's frozen coverage-first no-return uniqueness audit."""
...
from scripts import (
    a_share_three_day_walkforward_campaign083_no_return_audit as c83_audit,
)
from scripts import a_share_three_day_walkforward_campaign084_features as candidate
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_084/no_return"
)
SNAPSHOT_MANIFEST_PATH = (
    candidate.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
)
SNAPSHOT_MANIFEST_SHA256 = (
    "68a547c9d85f6437382a16704f601074264c8206dd5fe4d23adaff20079f5994"
)
...
SNAPSHOT_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_084_feature_snapshot_binding_v2_20260806.json"
)
SNAPSHOT_BINDING_SHA256 = (
    "45e4dde42acb65ed25993f0f83f18b8430fc93fd424b5d3eb53aecd296dc48b0"
)
AUDIT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_084_no_return_audit_implementation_freeze_20260806.json"
)
AUDIT_ACTIVATION_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_084_no_return_audit_activation_binding_20260806.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign084_no_return_audit.py"
)
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 6_321_289
EXPECTED_COMPARISON_COUNT = 113
EXPECTED_COMPLETE_DEFINITION_COUNT = 115
...
C83_SNAPSHOT_MANIFEST_PATH = candidate.C83_MANIFEST_PATH
...
class Campaign084NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign084 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign084NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign084NoReturnAuditError("Campaign084 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign084NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign084_no_return_audit_implementation_freeze"
        ...
        raise Campaign084NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign084NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign084_no_return_audit_activation_binding"
        ...
        raise Campaign084NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int = 4) -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign084NoReturnAuditError("candidate snapshot verification changed")
    return result


def verify_campaign083_snapshot(*, workers: int = 4) -> dict[str, Any]:
    return c83_audit.verify_candidate_snapshot(workers=workers)
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(SNAPSHOT_MANIFEST_PATH, SNAPSHOT_MANIFEST_SHA256, "candidate snapshot")
    _require(
        C83_SNAPSHOT_MANIFEST_PATH,
        candidate.C83_MANIFEST_SHA256,
        "Campaign083 snapshot",
    )
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign084NoReturnAuditError("snapshot binding validation failed")
    ...
    if not (
        ...
        raise Campaign084NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    c83_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign084NoReturnAuditError("conflicting Campaign084 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c83_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    helper = (
        c83_audit.c82_audit.c81_audit.c80_audit.c78_audit.c77_audit.c76_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    )
    result, receipt = helper(
        manifest_path=C83_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C83_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: verify_campaign083_snapshot(
            workers=workers
        ),
    )
    comparisons.append(result)
    receipts["campaign083_snapshot"] = receipt
    receipts.pop("all_112_sources_loaded_in_frozen_order", None)
    receipts["all_113_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign084NoReturnAuditError("Campaign084 comparison count changed")
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign084NoReturnAuditError("Campaign084 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign084_no_return_audit.json")):
        raise Campaign084NoReturnAuditError(
            "Campaign084 no-return audit already exists"
        )
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    run_id = f"{timestamp}_campaign084_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign084_no_return_audit",
        "status": (
        ...
        "next_action": (
            "freeze and run the exact one Campaign084 development trial"
            if admitted
            else "record this no-return rejection and begin only a genuinely new campaign"
        ),
        "candidate_source_fields_read": list(candidate.EVENT_FIELDS),
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    repair = c83_audit.c82_audit.c81_audit.c80_audit.c78_audit.c77_audit_v2
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        ...
        .resolve()
        .glob("*_campaign084_no_return_audit.json")
    )
    return {
