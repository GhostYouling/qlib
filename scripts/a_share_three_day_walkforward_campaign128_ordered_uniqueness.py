#!/usr/bin/env python3
"""Run Campaign128's frozen all-139 ordered uniqueness audit without returns."""
...
from scripts import a_share_three_day_walkforward_campaign121_features as c121_candidate
from scripts import (
    a_share_three_day_walkforward_campaign121_ordered_uniqueness as prior,
)
from scripts import a_share_three_day_walkforward_campaign128_coverage_audit as coverage
from scripts import a_share_three_day_walkforward_campaign128_features as candidate
from scripts import (
    a_share_three_day_walkforward_campaign128_snapshot_verify_recovery as candidate_recovery,
)
...
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_128_ordered_numeric_uniqueness_protocol_20260814.json"
)
PROTOCOL_SHA256 = "b6dab3dd821bad4b03888bfd97c357b6a2c20f6467cae6e33b0647bf44ac3acd"
COVERAGE_PATH = coverage.OUTPUT_PATH
COVERAGE_SHA256 = "8ccfbc0fb76b5c4187febbf8659fb29c864259efd4e3a8be3bc5d0db8a980605"
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_128_ordered_uniqueness_implementation_freeze_v2_20260814.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign128_ordered_uniqueness.py"
)
OUTPUT_PATH = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_128/uniqueness/campaign128_ordered_uniqueness_audit.json"
)
PRIOR_RUNNER_PATH = Path(prior.__file__).resolve()
PRIOR_RUNNER_SHA256 = "34a3818b8f582eaa82c10835ec23e11a68e3533ee0238479b769a4d6c1a5193d"
CANDIDATE_MANIFEST_PATH = candidate_recovery.MANIFEST_PATH
CANDIDATE_MANIFEST_SHA256 = coverage.SNAPSHOT_MANIFEST_SHA256
CANDIDATE_DATASET_SHA256 = coverage.SNAPSHOT_DATASET_SHA256
C121_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
C121_MANIFEST_SHA256 = prior.CANDIDATE_MANIFEST_SHA256
C121_DATASET_SHA256 = prior.CANDIDATE_DATASET_SHA256
...
EXPECTED_COMPARISON_COUNT = 139
MINIMUM_PAIRWISE_NAMES = 50
MINIMUM_PAIRWISE_SESSIONS = 100
MAXIMUM_ABSOLUTE_MEDIAN_CORRELATION = 0.8
EXPECTED_QUALITY_LISTING_CANDIDATE_ROWS = 1319946


class Campaign128OrderedUniquenessError(RuntimeError):
    """Fail closed when the frozen all-139 comparison boundary changes."""
...
def load_protocol() -> dict[str, Any]:
    if not PROTOCOL_PATH.is_file() or _sha256(PROTOCOL_PATH) != PROTOCOL_SHA256:
        raise Campaign128OrderedUniquenessError("uniqueness protocol changed")
    ...
    if not (
        spec.get("kind")
        == "a_share_three_day_walkforward_campaign128_ordered_numeric_uniqueness_protocol"
        and spec.get("status")
        == "all_139_ordered_comparators_frozen_after_coverage_pass_before_any_comparator_value"
        ...
        and definitions[-1]
        == {"name": c121_candidate.FACTOR_NAME, "score_direction": "higher"}
        ...
        and gate.get("all_139_must_pass") is True
        ...
        raise Campaign128OrderedUniquenessError("uniqueness protocol semantics changed")
    return spec


def _validate_implementation_freeze() -> dict[str, Any]:
    if not IMPLEMENTATION_FREEZE_PATH.is_file():
        raise Campaign128OrderedUniquenessError(
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign128_ordered_uniqueness_implementation_freeze"
        and record.get("status")
        == "all_139_loader_and_gate_runner_frozen_before_comparator_values"
        ...
        and frozen.get("prior_138_runner_sha256") == PRIOR_RUNNER_SHA256
        ...
        and frozen.get("comparator_139_manifest_sha256") == C121_MANIFEST_SHA256
        and frozen.get("comparator_139_dataset_sha256") == C121_DATASET_SHA256
        and boundary.get("candidate_values_reopened_during_failed_run_before_v2")
        is True
...
def validate_static_bindings() -> dict[str, Any]:
    ...
    for path, expected, label in (
        ...
        (PRIOR_RUNNER_PATH, PRIOR_RUNNER_SHA256, "prior 138 runner"),
        (C121_MANIFEST_PATH, C121_MANIFEST_SHA256, "comparator 139 manifest"),
        (
        ...
        if not path.is_file() or _sha256(path) != expected:
            raise Campaign128OrderedUniquenessError(f"{label} changed: {path}")
    ...
    if not (
        ...
        raise Campaign128OrderedUniquenessError("coverage authority changed")
    candidate._validate_implementation_freeze()
    prior_bindings = prior.validate_static_bindings()
    prior.candidate_verifier.validate_static_bindings()
    return {
        ...
        "prior_138_loader_binding": prior_bindings,
        "comparator_139_manifest_sha256": C121_MANIFEST_SHA256,
        "comparator_139_dataset_sha256": C121_DATASET_SHA256,
...
def build_plan() -> dict[str, Any]:
    ...
    return {
        "kind": "a_share_three_day_walkforward_campaign128_ordered_uniqueness_plan",
        "ready": not blockers,
        "blockers": blockers,
...
def eligible_candidate_panel(frame: Any, eligible_keys: Any) -> Any:
    required = {
        "trade_date",
        "symbol",
    ...
    if not required.issubset(frame.columns):
        raise Campaign128OrderedUniquenessError("candidate panel columns changed")
    eligible = (
        frame[f"{candidate.FACTOR_NAME}_eligible"]
        .astype("boolean")
        .fillna(False)
        .astype(bool)
    )
    values = frame[candidate.FACTOR_NAME]
    selected = frame.loc[
        eligible & values.notna(),
        ["trade_date", "symbol", candidate.FACTOR_NAME],
    ]
    ...
    if quality[candidate.FACTOR_NAME].isna().any():
        raise Campaign128OrderedUniquenessError("eligible candidate panel is invalid")
    return quality
...
def _candidate_arrays() -> tuple[np.ndarray, np.ndarray, Any, dict[str, Any]]:
    ...
    gc.collect()
    if (
        len(quality) != EXPECTED_QUALITY_LISTING_CANDIDATE_ROWS
        or quality[candidate.FACTOR_NAME].isna().any()
    ):
        raise Campaign128OrderedUniquenessError("candidate quality panel changed")
    ...
    engine, comparison_engine = context[2], context[5]
...
def _load_campaign121_comparator(
    *,
    ...
    verification = prior.candidate_verifier.verify_snapshot(workers=8)
    if not (
        verification.get("manifest_sha256") == C121_MANIFEST_SHA256
        and verification.get("dataset_sha256") == C121_DATASET_SHA256
        and verification.get("rows") == candidate.EXPECTED_ROWS
    ):
        raise Campaign128OrderedUniquenessError("comparator 139 snapshot changed")
    manifest = json.loads(C121_MANIFEST_PATH.read_text(encoding="utf-8"))
    from scripts import (  # noqa: PLC0415
        a_share_three_day_walkforward_campaign105_no_return_audit as c105_audit,
    )
    ...
    ranges[c121_candidate.FACTOR_NAME] = (0.0, 1.0)
    engine.FACTOR_RANGES = ranges
    frame = engine.load_factor_frame(
        C121_MANIFEST_PATH, manifest, c121_candidate.FACTOR_NAME
    )
    adapter = prior.c120_base.c119_base.c118_base.c117_base.c110_v1.adapter
    source_keys, source_values = adapter.sorted_raw_comparator_arrays(
        frame,
        factor=c121_candidate.FACTOR_NAME,
        value_range=(0.0, 1.0),
    ...
    result = comparison_engine._aligned_comparison_result(
        ...
        comparison=c121_candidate.FACTOR_NAME,
        direction="higher",
        gate=gate,
    )
    result["source_score_direction"] = "higher"
    result["comparison_value_semantics"] = "frozen Campaign121 raw factor value"
    result["aligned_legal_nan_count_before_pairwise_overlap"] = legal_nan_count
    return result, {
        "manifest_path": str(C121_MANIFEST_PATH),
        "manifest_sha256": C121_MANIFEST_SHA256,
        "dataset_sha256": C121_DATASET_SHA256,
...
def run_ordered_uniqueness(*, confirm: bool) -> Path:
    ...
    if plan["ready"] is not True:
        raise Campaign128OrderedUniquenessError("ordered uniqueness plan is not ready")
    definitions = candidate.reconstruct_comparisons()
    ...
    candidate_recovery.verify_snapshot(workers=8)
    keys, values, comparison_engine, candidate_receipt = _candidate_arrays()
    first_results, receipts = (
        prior.c120_base.c119_base.c118_base.c117_base.c110_v1._load_comparisons_after_coverage(
    ...
    if [item["comparison_factor"] for item in first_results] != expected_order[:133]:
        raise Campaign128OrderedUniquenessError("first 133 comparator order changed")
    c110_result, c110_receipt = (
        prior.c120_base.c119_base.c118_base.c117_base._load_final_comparator(
            candidate_keys=keys,
            candidate_values=values,
            gate=gate,
            comparison_engine=comparison_engine,
    ...
    c117_result, c117_receipt = (
        prior.c120_base.c119_base.c118_base._load_campaign117_comparator(
            candidate_keys=keys,
            candidate_values=values,
            gate=gate,
            comparison_engine=comparison_engine,
    ...
    c118_result, c118_receipt = prior.c120_base.c119_base._load_campaign118_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    )
    c119_result, c119_receipt = prior.c120_base._load_campaign119_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    )
    c120_result, c120_receipt = prior._load_campaign120_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    )
    c121_result, c121_receipt = _load_campaign121_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign128_ordered_uniqueness_audit",
        "status": (
            "all_139_uniqueness_gates_passed_ready_to_freeze_one_development_trial"
            if admitted
        ...
        "comparison_source_verification": {
            ...
            "comparator_139": c121_receipt,
            "all_139_sources_loaded_in_frozen_order": True,
        },
        ...
        "next_action": (
            ...
            if admitted
            else "terminalize Campaign128 and begin only a genuinely new campaign"
        ),
    }
