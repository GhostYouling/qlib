#!/usr/bin/env python3
"""Run Campaign087's exact one-trial historical walk-forward campaign."""
...
ADMITTED_FACTOR = "intraday_range_amount_profile_alignment_js_240m"
FROZEN_TRIAL_ID = "wf087_intraday_range_amount_profile_alignment_js_240m_single_higher"
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_087_preregistration_v2.json"
)
CANDIDATE_MANIFEST = Path(
    "/Volumes/DIsk/qlib-a-share-tushare-1m/derived/a_share/rich/tushare/"
    "minute_walkforward_campaign087_feature_library/"
    "tushare_stk_mins_1m_2019_2025_ea0cbb8f_walkforward_campaign087_feature_library_v1/"
...
for _old, _new in (
    ("Campaign083", "Campaign087"),
    ("campaign083", "campaign087"),
    ("campaign_083", "campaign_087"),
    ("wf083", "wf087"),
...
_generated: dict[str, Any] = {
    ...
    "__name__": "a_share_three_day_walkforward_campaign087_generated",
}
...
Campaign087Error = _generated["Campaign087Error"]
...
def _decode_stock_day_keys(keys: np.ndarray) -> tuple[pd.DatetimeIndex, pd.Series]:
    ...
    if compact.ndim != 1 or len(compact) == 0:
        raise Campaign087Error("Campaign087 compact stock-day keys changed")
    ...
    if not np.isin(exchanges, np.array([1, 2, 3], dtype=np.int64)).all():
        raise Campaign087Error("Campaign087 compact exchange code changed")
    ...
    if dates.isna().any() or instruments.isna().any():
        raise Campaign087Error("Campaign087 compact identity decode failed")
...
def _load_compact_factor_panel(
    ...
    if not (
        ...
        and factors[0].get("dataset_group") == "campaign087_new_factors"
        ...
        raise Campaign087Error("Campaign087 compact factor request changed")
    if not CANDIDATE_MANIFEST.is_file() or _sha256(CANDIDATE_MANIFEST) != (
        CANDIDATE_MANIFEST_SHA256
    ):
        raise Campaign087Error("Campaign087 candidate manifest changed")
    ...
    if not (
        ...
        raise Campaign087Error("Campaign087 candidate snapshot semantics changed")
    frames: list[pd.DataFrame] = []
    for year in requested_years:
        record = records[year]
        path = (CANDIDATE_MANIFEST.parent / str(record["path"])).resolve()
        if not path.is_file() or _sha256(path) != str(record["sha256"]):
            raise Campaign087Error(f"Campaign087 candidate partition changed: {year}")
        ...
        if not (dates.year == year).all():
            raise Campaign087Error(
                f"Campaign087 candidate partition year changed: {year}"
            )
        frames.append(
            pd.DataFrame(
                {
    ...
    if panel.duplicated(["trade_date", "instrument"]).any():
        raise Campaign087Error("Campaign087 decoded factor keys are not unique")
...
def _temporary_compact_factor_loader() -> Iterator[None]:
    ...
    if original is _load_compact_factor_panel:
        raise Campaign087Error("Campaign087 compact factor loader was prepatched")
