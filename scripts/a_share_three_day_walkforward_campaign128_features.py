#!/usr/bin/env python3
"""Build Campaign128's frozen transaction-price dispersion snapshot.
...
DEFAULT_PROTOCOL = formula.PROTOCOL_PATH
DEFAULT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_128_source_bound_implementation_freeze_20260814.json"
)
FEATURE_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign128_features.py"
)
RECOVERY_PATH = (
    REPO_ROOT
    / "scripts/a_share_three_day_walkforward_campaign128_snapshot_verify_recovery.py"
)
RECOVERY_TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign128_snapshot_verify_recovery.py"
)

PROTOCOL_SHA256 = formula.PROTOCOL_SHA256
FORMULA_SHA256 = "6d3285060a656f75ab184a303b0c0dca0006de78d78bf2c0294284a64a96f57f"
FORMULA_FREEZE_SHA256 = (
    "0185789fe9418a9585d50122fdcdcf3f29ac9e72e4079ead16d266cbe82864de"
)
NUMERIC_POLICY_V137_SHA256 = (
    "8c9d3b47e2b10cc983a8097d95cd38f6621ccfb4e5ee93fd6441b1b093d38c2d"
)
NUMERIC_COMPARATOR_COUNT = 139
NUMERIC_COMPARATOR_ORDER_SHA256 = (
    "9c4de054ca1b83eced67eece256b024ebec3a6e1925de17a4cedfa7d36265161"
)
COMPLETE_DEFINITION_COUNT = 151
...
FACTOR_SHORT_NAME = "transaction_price_dispersion_resolution"
FACTOR_FORMULA = (
    "(morning volume-weighted IQR of log(amount/volume) - afternoon "
    "volume-weighted IQR)/(morning IQR + afternoon IQR), using stable "
    "left-continuous Q25/Q75; each half requires at least 60 active bars "
    "and a strictly positive IQR"
)
OUTPUT_RUN_ID = (
...
class Campaign128FeatureError(RuntimeError):
    """Fail closed when a frozen Campaign128 feature invariant changes."""
...
def _require(path: Path, expected: str, label: str) -> None:
    ...
    if not resolved.is_file() or _sha256(resolved) != expected:
        raise Campaign128FeatureError(f"Campaign128 {label} changed: {resolved}")


def reconstruct_comparisons() -> list[dict[str, str]]:
    items = [dict(item) for item in c121.reconstruct_comparisons()]
    items.append({"name": c121.FACTOR_NAME, "score_direction": "higher"})
    if (
        len(items) != NUMERIC_COMPARATOR_COUNT
        or _order_digest(items) != NUMERIC_COMPARATOR_ORDER_SHA256
    ):
        raise Campaign128FeatureError("Campaign128 numeric comparison order changed")
    return items


def reconstruct_complete_definitions() -> list[dict[str, str]]:
    items = [dict(item) for item in c121.reconstruct_complete_definitions()]
    items.extend(
        [
            {
    ...
    if (
        ...
        raise Campaign128FeatureError("Campaign128 complete definition order changed")
    return items


def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    """Validate the exact pre-value contract without reading a source row."""

    target = path.expanduser().resolve()
    try:
        spec = formula.load_protocol(target)
    except formula.Campaign128FormulaError as exc:
        raise Campaign128FeatureError(str(exc)) from exc
    report = bindings.validate_record(target, data_root=DEFAULT_DATA_ROOT)
    if report.get("all_bindings_passed") is not True:
        raise Campaign128FeatureError("Campaign128 protocol binding failed")
    ...
    if not (
        snapshot.get("data_root") == str(DEFAULT_DATA_ROOT)
        and snapshot.get("raw_manifest_sha256") == RAW_MANIFEST_SHA256
        ...
        raise Campaign128FeatureError("Campaign128 protocol semantics changed")
    return spec


def compute_transaction_price_dispersion_resolution(
    volumes: np.ndarray, amounts: np.ndarray
) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, dict[str, int]]:
    try:
        return formula.compute_transaction_price_dispersion_resolution(volumes, amounts)
    except formula.Campaign128FormulaError as exc:
        raise Campaign128FeatureError(str(exc)) from exc


def extract_transaction_price_dispersion_resolution(
    raw: pd.DataFrame, *, symbol: str
) -> tuple[pd.DataFrame, dict[str, int]]:
    """Validate the exact 241-row source grid and compute 240 selected bars."""

    if tuple(raw.columns) != RAW_COLUMNS:
        raise Campaign128FeatureError(
    ...
    empty_quality = {
        "raw_source_rows": 0,
        "source_sessions": 0,
        "valid_dispersion_resolution_sessions": 0,
        "invalid_raw_sessions": 0,
        "one_sided_zero_sessions": 0,
        "insufficient_morning_active_sessions": 0,
        "insufficient_afternoon_active_sessions": 0,
        "nonpositive_morning_dispersion_sessions": 0,
        "nonpositive_afternoon_dispersion_sessions": 0,
        "active_bars": 0,
        "joint_zero_bars": 0,
    }
    if raw.empty:
    ...
    if (
        ...
        raise Campaign128FeatureError(f"raw minute identity changed for {symbol}")
    ...
    if (
        ...
        raise Campaign128FeatureError(f"raw minute grid changed for {symbol}")
    selected = work.loc[
    ...
    if len(selected) != len(dates) * SELECTED_BAR_COUNT:
        raise Campaign128FeatureError(f"continuous minute grid changed for {symbol}")
    volume = selected["volume"].to_numpy(dtype=np.float64).reshape(len(dates), 240)
    amount = selected["amount"].to_numpy(dtype=np.float64).reshape(len(dates), 240)
    values, eligible, _morning, _afternoon, formula_quality = (
        compute_transaction_price_dispersion_resolution(volume, amount)
    )
    ...
    return pd.DataFrame({"trade_date": dates, FACTOR_SHORT_NAME: values}), {
        "raw_source_rows": int(len(work)),
        "source_sessions": int(len(dates)),
        "valid_dispersion_resolution_sessions": int(eligible.sum()),
        "invalid_raw_sessions": formula_quality["invalid_raw_rows"],
        "one_sided_zero_sessions": formula_quality["one_sided_zero_rows"],
        "insufficient_morning_active_sessions": formula_quality[
            "insufficient_morning_active_rows"
        ],
        "insufficient_afternoon_active_sessions": formula_quality[
            "insufficient_afternoon_active_rows"
        ],
        "nonpositive_morning_dispersion_sessions": formula_quality[
            "nonpositive_morning_dispersion_rows"
        ],
        "nonpositive_afternoon_dispersion_sessions": formula_quality[
            "nonpositive_afternoon_dispersion_rows"
        ],
        "active_bars": int(active.sum()),
        "joint_zero_bars": int(joint_zero.sum()),
    }


def attach_dispersion_resolution_values(
    identity: pd.DataFrame, values: pd.DataFrame, *, symbol: str
) -> pd.DataFrame:
    if tuple(identity.columns) != IDENTITY_COLUMNS:
        raise Campaign128FeatureError(f"identity projection changed for {symbol}")
    base = identity.copy()
    ...
    if base.empty:
        base[FACTOR_SHORT_NAME] = pd.Series(dtype="float64")
        return base
    if (
        ...
        raise Campaign128FeatureError(f"stock-day identity changed for {symbol}")
...
def finalize_feature_frame(frame: pd.DataFrame) -> pd.DataFrame:
    required = (*IDENTITY_COLUMNS, FACTOR_SHORT_NAME)
    if not set(required).issubset(frame.columns):
        raise Campaign128FeatureError("Campaign128 input columns changed")
    work = frame.copy()
...
def validate_value_semantics(frame: pd.DataFrame) -> tuple[int, int]:
    if tuple(frame.columns) != OUTPUT_COLUMNS:
        raise Campaign128FeatureError("Campaign128 output columns changed")
    ...
    if (
        ...
        raise Campaign128FeatureError("Campaign128 value semantics changed")
...
def output_root(data_root: Path) -> Path:
    return (
        ...
        / "derived/a_share/rich/tushare/minute_walkforward_campaign128_feature_library"
        / OUTPUT_RUN_ID
    )
...
def _validate_implementation_freeze() -> dict[str, Any]:
    if not DEFAULT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign128FeatureError("Campaign128 implementation freeze is absent")
    ...
    expected_tests = {
        ...
        str(RECOVERY_TEST_PATH.relative_to(REPO_ROOT)): _sha256(RECOVERY_TEST_PATH),
    }
    observed_tests = {str(item.get("path")): str(item.get("sha256")) for item in tests}
    boundary = record.get("research_boundary") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign128_source_bound_implementation_freeze"
        and record.get("status")
        == "frozen_before_campaign128_historical_minute_candidate_comparator_or_return_values"
        ...
        and frozen.get("numeric_policy_v137_sha256") == NUMERIC_POLICY_V137_SHA256
        and frozen.get("feature_builder_sha256") == _sha256(Path(__file__).resolve())
        and frozen.get("recovery_verifier_sha256") == _sha256(RECOVERY_PATH)
        and observed_tests == expected_tests
        and frozen.get("source_projection") == list(RAW_COLUMNS)
        and frozen.get("output_run_id") == OUTPUT_RUN_ID
        and frozen.get("expected_partitions") == EXPECTED_PARTITIONS
        and frozen.get("expected_rows") == EXPECTED_ROWS
        and boundary.get("historical_minute_source_rows_read_before_freeze") is False
        ...
        raise Campaign128FeatureError("Campaign128 implementation freeze changed")
    return record
...
def _require_input_metadata(
    ...
    if not (
        ...
        raise Campaign128FeatureError("Campaign128 source aggregate changed")
    return raw, clean, freeze


def plan_build(data_root: Path = DEFAULT_DATA_ROOT) -> dict[str, Any]:
    resolved = data_root.expanduser().resolve()
    if resolved != DEFAULT_DATA_ROOT.resolve():
        raise Campaign128FeatureError("Campaign128 data root changed")
    raw, clean, freeze = _require_input_metadata(resolved)
    manifest_path = output_root(resolved) / "snapshot_manifest.json"
    ready = not output_root(resolved).exists()
    return {
        "status": "ready_for_confirmed_historical_build" if ready else "output_exists",
        "ready": ready,
        "raw_manifest_files": len(raw["files"]),
        "joint_clean_manifest_files": len(clean["files"]),
        "expected_rows": EXPECTED_ROWS,
        "source_projection": list(RAW_COLUMNS),
        "snapshot_manifest": str(manifest_path),
        "implementation_freeze_status": freeze["status"],
        "historical_minute_source_rows_read_by_plan": False,
        "candidate_or_comparator_values_read_by_plan": False,
        "historical_daily_price_or_forward_return_values_read_by_plan": False,
        "provider_request_issued_by_plan": False,
    }
...
def _raw_file_map(raw: dict[str, Any]) -> dict[tuple[str, int], dict[str, Any]]:
    ...
    for item in raw.get("files") or []:
        ...
        if key in result:
            raise Campaign128FeatureError(f"duplicate raw partition: {key}")
        result[key] = item
    if len(result) != EXPECTED_PARTITIONS:
        raise Campaign128FeatureError("raw partition count changed")
    return result
...
def _load_attached(
    ...
    if (
        ...
        or _sha256(clean_path) != str(clean_item["output_byte_sha256"])
        or not raw_path.is_file()
        or pq.ParquetFile(raw_path).metadata.num_rows != int(raw_item["rows"])
        or _sha256(raw_path) != str(raw_item["byte_sha256"])
    ):
        raise Campaign128FeatureError(f"source partition changed for {symbol}")
    identity = pd.read_parquet(clean_path, columns=list(IDENTITY_COLUMNS))
    raw = pd.read_parquet(raw_path, columns=list(RAW_COLUMNS))
    values, quality = extract_transaction_price_dispersion_resolution(
        raw, symbol=symbol
    )
    out = attach_dispersion_resolution_values(identity, values, symbol=symbol)
...
def build_snapshot(
    *, data_root: Path, workers: int = 4, confirm_build: bool = False
    ...
    if not confirm_build:
        raise Campaign128FeatureError("Campaign128 build requires --confirm-build")
    if workers < 1 or workers > 16:
        raise Campaign128FeatureError("--workers must be between 1 and 16")
    resolved = data_root.expanduser().resolve()
    plan = plan_build(resolved)
    if plan.get("ready") is not True:
        raise Campaign128FeatureError("Campaign128 plan is not ready")
    raw, clean, freeze = _require_input_metadata(resolved)
    ...
    final_root = output_root(resolved)
    ...
    quality_keys = (
        "raw_source_rows",
        "source_sessions",
        "valid_dispersion_resolution_sessions",
        "invalid_raw_sessions",
        "one_sided_zero_sessions",
        "insufficient_morning_active_sessions",
        "insufficient_afternoon_active_sessions",
        "nonpositive_morning_dispersion_sessions",
        "nonpositive_afternoon_dispersion_sessions",
        "active_bars",
        "joint_zero_bars",
    )
    totals = {key: 0 for key in quality_keys}
    totals.update({"eligible": 0, "missing": 0})
    try:
        for year in sorted(by_year):
            ...
            for index, item in by_year[year]:
                ...
                if raw_item is None:
                    raise Campaign128FeatureError(f"raw partition missing: {key}")
            ...
            if not pieces:
                raise Campaign128FeatureError(f"Campaign128 year {year} is empty")
            ...
            for index, item in by_year[year]:
                ...
                records[index] = {
                    "schema_version": 1,
                    "kind": "a_share_three_day_walkforward_campaign128_feature_partition",
            ...
            print(
                f"Campaign128 built year={year} partitions={len(by_year[year])} "
        ...
        if (
            ...
            raise Campaign128FeatureError("Campaign128 publication totals changed")
        digest_rows = [
            [
        ...
        manifest = {
            ...
            "quality": {
                ...
                **{key: totals[key] for key in quality_keys},
            },
            "source_fields_read": list(RAW_COLUMNS),
            ...
            "source_half_bar_count": formula.HALF_BAR_COUNT,
            "minimum_active_bars_per_half": formula.MINIMUM_ACTIVE_BARS_PER_HALF,
            "source_minute_grid": (
                "09:30 plus 09:31-11:30 and 13:01-15:00; formula excludes 09:30"
            ),
            "weighted_quantile_rule": (
                "stable ascending log(amount/volume), smallest value whose inclusive "
                "cumulative volume reaches 25% or 75% of half-session active volume"
            ),
...
def validate_manifest_metadata(manifest: dict[str, Any]) -> None:
    ...
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign128_feature_snapshot"
        ...
        and manifest.get("source_half_bar_count") == formula.HALF_BAR_COUNT
        and manifest.get("minimum_active_bars_per_half")
        == formula.MINIMUM_ACTIVE_BARS_PER_HALF
        and manifest.get("joint_zero_volume_amount_policy") == "inactive"
        and manifest.get("one_sided_zero_policy") == "stock_day_invalid"
        and manifest.get("nonpositive_half_iqr_policy")
        == "missing_without_epsilon_or_fallback"
        ...
        raise Campaign128FeatureError("Campaign128 manifest semantics changed")
...
def verify_snapshot_files(manifest_path: Path, *, workers: int = 4) -> dict[str, Any]:
    ...
    if (
        ...
        raise Campaign128FeatureError("Campaign128 aggregate identity changed")
    return {
        "status": "verified",
...
def main() -> int:
    ...
    if args.command == "plan":
        payload = plan_build()
        exit_code = 0 if payload.get("ready") is True else 2
