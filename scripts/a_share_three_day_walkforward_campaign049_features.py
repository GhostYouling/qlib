#!/usr/bin/env python3
"""Build and audit Campaign049 morning/afternoon amount-profile similarity."""
...
try:
    ...
    import scripts.a_share_three_day_walkforward_campaign048_features as campaign048_reference
    ...
    import a_share_three_day_walkforward_campaign048_features as campaign048_reference
...
FACTOR_NAME = "intraday_morning_afternoon_amount_profile_similarity_120b"
FACTOR_FORMULA = (
    "Use exactly 120 morning amounts and 120 afternoon amounts. Normalize "
    "each half independently to p_i=a_morning_i/sum(a_morning) and "
    "q_i=a_afternoon_i/sum(a_afternoon). With m_i=(p_i+q_i)/2, return "
    "1-[0.5*sum(p_i*log(p_i/m_i))+0.5*sum(q_i*log(q_i/m_i))]/log(2), "
    "using 0*log(0/m)=0."
)
MECHANISM_AUDIT_SHA256 = "fa7eac593d03eaa52a5ae608474a444f4eb373d9bc810b46a5c5c7f712886979"
PROTOCOL_SHA256 = "b1cddf5168229877d57b58f556c52cb2f9799f4e387c4ec8184fb230e30f6588"
...
COMPARISON_COUNT = 72
INHERITED_COMPARISON_COUNT = 71
INHERITED_COMPARISON_ORDER_SHA256 = "271fb3b58605799788a3b966ab9b19f2348cd5533ffd18b140363ba497061c64"
COMPARISON_ORDER_SHA256 = "6bbe43d7f1077d0f76af53149ab40d048e4e4e299bab73b1562be54e974083f5"
SELECTED_BAR_COUNT = 240
BINS_PER_HALF = 120
MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF = 60
ENDPOINT_TOLERANCE = 1e-12
LOWER_BOUND = 0.0
UPPER_BOUND = 1.0
OUTPUT_RUN_ID = (
    "tushare_stk_mins_1m_2019_2025_ea0cbb8f_"
    "walkforward_campaign049_feature_library_v1"
)
DEFAULT_PROTOCOL = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_049_no_return_preregistration.json"
DEFAULT_IMPLEMENTATION_FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_049_feature_implementation_freeze_20260801.json"
DEFAULT_SNAPSHOT_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_049_snapshot_publication_binding_20260801.json"
DEFAULT_EXPERIMENT_ROOT = REPO_ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_049/no_return"
...
C48_SNAPSHOT_PATH = Path(
    "/Volumes/DIsk/qlib-a-share-tushare-1m/derived/a_share/rich/tushare/"
    "minute_walkforward_campaign048_feature_library/"
    "tushare_stk_mins_1m_2019_2025_ea0cbb8f_"
    "walkforward_campaign048_feature_library_v1/snapshot_manifest.json"
)
C48_SNAPSHOT_SHA256 = "49e5f7f83e8a5a4d34deb494602c8582da9dc645a0f7e5c8aef6ef824a3bc017"
C48_DATASET_SHA256 = "d294401092beb8e8cd0b84c1991e3ba5a158318cdab4b63c4920120fafe9db1b"
C48_FACTOR_NAME = "intraday_two_sided_wick_absorption_balance_240m"


class Campaign049FeatureError(RuntimeError):
    """Fail-closed Campaign049 feature boundary error."""
...
if _sha256(BASE_FEATURE_RUNNER) != BASE_FEATURE_RUNNER_SHA256:
    raise Campaign049FeatureError("frozen Campaign047 feature orchestration changed")

_spec = importlib.util.spec_from_file_location(
    "a_share_three_day_walkforward_campaign049_inherited_engine",
    BASE_FEATURE_RUNNER,
)
if _spec is None or _spec.loader is None:
    raise Campaign049FeatureError("Campaign047 inherited engine could not load")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not IMPLEMENTATION_FREEZE_SHA256:
        raise Campaign049FeatureError("Campaign049 implementation freeze is not bound")
    if _sha256(DEFAULT_IMPLEMENTATION_FREEZE) != IMPLEMENTATION_FREEZE_SHA256:
        raise Campaign049FeatureError("Campaign049 implementation freeze changed")
    ...
    if not (
        record.get("kind") == "a_share_three_day_walkforward_campaign049_feature_implementation_freeze"
        and record.get("status") == "frozen_before_campaign049_candidate_values"
        ...
        raise Campaign049FeatureError("Campaign049 implementation freeze semantics changed")
    return record


def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    """Validate every Campaign049 preregistration binding and exact semantic."""

    path = path.expanduser().resolve()
    if _sha256(path) != PROTOCOL_SHA256:
        raise Campaign049FeatureError("Campaign049 no-return protocol changed")
    validation = bindings.validate_record(path, data_root=DEFAULT_DATA_ROOT)
    if validation.get("all_bindings_passed") is not True:
        raise Campaign049FeatureError("Campaign049 protocol has a failed binding")
    ...
    inherited_spec = campaign048_reference.load_protocol()
    ...
    if not (
        delta.get("version") == 1
        and delta.get("kind") == "a_share_three_day_walkforward_campaign049_no_return_preregistration"
        and delta.get("status") == "frozen_before_campaign049_minute_candidate_comparison_daily_price_or_return_values"
        ...
        and candidate.get("forbidden_source_columns") == ["open", "high", "low", "close", "volume"]
        and candidate.get("selected_bar_count") == SELECTED_BAR_COUNT
        and candidate.get("bins_per_half") == BINS_PER_HALF
        ...
        and candidate.get("minimum_positive_amount_bars_per_half") == MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF
        and candidate.get("jensen_shannon_equal_mixture_weight") == 0.5
        and candidate.get("jensen_shannon_log_base") == "natural"
        ...
        and uniqueness.get("inherited_campaign048_comparison_factor_count") == INHERITED_COMPARISON_COUNT
        and uniqueness.get("inherited_campaign048_comparison_factor_order_sha256") == INHERITED_COMPARISON_ORDER_SHA256
        ...
        and uniqueness.get("all_72_must_pass") is True
        and comparisons[-1] == {"name": C48_FACTOR_NAME, "score_direction": "higher"}
        and finite.get("trial_id") == "wf049_intraday_morning_afternoon_amount_profile_similarity_120b_single_higher"
        ...
        raise Campaign049FeatureError("Campaign049 protocol semantics changed")
    ...
    spec.update({
        ...
        "candidates": [{
            ...
            "bins_per_half": BINS_PER_HALF,
            "minimum_positive_amount_bars_per_half": MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF,
        ...
        "finite_post_admissibility_search": {
            ...
            "development_interval": {
                "start": "2019-01-01",
                "end": "2023-12-31",
                "folds": copy.deepcopy(inherited_spec["finite_post_admissibility_search"]["development_interval"]["folds"]),
...
def compute_factor_values(
    *, amounts: np.ndarray
    ...
    """Compute the frozen equal-clock half-session amount JSD similarity."""

    amounts = np.asarray(amounts, dtype=float)
    if amounts.ndim != 2 or amounts.shape[1] != SELECTED_BAR_COUNT:
        raise Campaign049FeatureError("Campaign049 amount shape is invalid")
    rows = len(amounts)
    finite = np.isfinite(amounts).all(axis=1)
    nonnegative = (amounts >= 0.0).all(axis=1)
    morning = amounts[:, :BINS_PER_HALF]
    afternoon = amounts[:, BINS_PER_HALF:]
    morning_positive_count = (morning > 0.0).sum(axis=1)
    afternoon_positive_count = (afternoon > 0.0).sum(axis=1)
    support = (
        (morning_positive_count >= MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF)
        & (afternoon_positive_count >= MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF)
    )
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        morning_total = np.sum(morning, axis=1)
        afternoon_total = np.sum(afternoon, axis=1)
        p = np.divide(
            morning,
            morning_total[:, None],
            out=np.full_like(morning, np.nan),
            where=morning_total[:, None] > 0.0,
        )
        q = np.divide(
            afternoon,
            afternoon_total[:, None],
            out=np.full_like(afternoon, np.nan),
            where=afternoon_total[:, None] > 0.0,
        )
        mixture = 0.5 * (p + q)
        p_terms = np.where(p > 0.0, p * np.log(p / mixture), 0.0)
        q_terms = np.where(q > 0.0, q * np.log(q / mixture), 0.0)
        divergence = 0.5 * np.sum(p_terms, axis=1) + 0.5 * np.sum(
            q_terms, axis=1
        )
        raw_values = 1.0 - divergence / np.log(2.0)
    finite_components = (
    ...
    positive_totals = (morning_total > 0.0) & (afternoon_total > 0.0)
    ...
    required_valid = finite & nonnegative
    eligible = (
        required_valid
        & support
        & positive_totals
    ...
    quality = {
        ...
        f"{FACTOR_NAME}__nonfinite_amount_rows": int((~finite).sum()),
        f"{FACTOR_NAME}__negative_amount_rows": int((finite & ~nonnegative).sum()),
        f"{FACTOR_NAME}__exact_zero_amount_positions": int(
            (amounts == 0.0).sum()
        ),
        f"{FACTOR_NAME}__below_minimum_positive_morning_bars_rows": int(
            (required_valid & (morning_positive_count < MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF)).sum()
        ),
        f"{FACTOR_NAME}__below_minimum_positive_afternoon_bars_rows": int(
            (required_valid & (afternoon_positive_count < MINIMUM_POSITIVE_AMOUNT_BARS_PER_HALF)).sum()
        ),
        f"{FACTOR_NAME}__nonpositive_morning_total_rows": int(
            (required_valid & ~(morning_total > 0.0)).sum()
        ),
        f"{FACTOR_NAME}__nonpositive_afternoon_total_rows": int(
            (required_valid & ~(afternoon_total > 0.0)).sum()
        ),
        f"{FACTOR_NAME}__invalid_probability_or_divergence_rows": int(
            (required_valid & support & positive_totals & ~finite_components).sum()
        ),
...
def compute_partition_frame(
    ...
    *,
    ...
    if tuple(raw.columns) != RAW_COLUMNS:
        raise Campaign049FeatureError(f"unexpected raw columns for {symbol}: {tuple(raw.columns)}")
    if tuple(base_frame.columns) != BASE_COLUMNS:
        raise Campaign049FeatureError(f"unexpected joint-base columns for {symbol}: {tuple(base_frame.columns)}")
    ...
    if base_work["trade_date"].isna().any() or set(base_work["symbol"].unique()) != {symbol.upper()} or base_work.duplicated(["trade_date", "symbol"]).any():
        raise Campaign049FeatureError(f"joint-base identity changed for {symbol}")
    ...
    if work["datetime"].isna().any() or set(work["symbol"].unique()) != {symbol.upper()} or set(work["provider"].unique()) != {"tushare"} or work.duplicated(["datetime"]).any():
        raise Campaign049FeatureError(f"raw identity changed for {symbol}")
    ...
    if not source_counts.eq(241).all():
        raise Campaign049FeatureError(f"every source stock-day must retain 241 rows for {symbol}")
    ...
    if not source_codes.eq(market.SOURCE_MINUTE_CODE_SET).all():
        raise Campaign049FeatureError(f"source minute grid changed for {symbol}")
    ...
    if not base_work["trade_date"].reset_index(drop=True).equals(expected_dates):
        raise Campaign049FeatureError(f"source and joint-base dates changed for {symbol}")
    continuous = work.loc[
    ...
    if len(continuous) != len(base_work) * SELECTED_BAR_COUNT:
        raise Campaign049FeatureError(f"continuous minute grid changed for {symbol}")
    amounts = continuous["amount"].to_numpy(dtype=float).reshape(
        -1, SELECTED_BAR_COUNT
    )
    values, eligible, quality = compute_factor_values(amounts=amounts)
    frame = pd.DataFrame({
...
def _validate_snapshot_manifest(manifest: dict[str, Any], *, require_fingerprint_constants: bool) -> None:
    if not require_fingerprint_constants:
        manifest["kind"] = "a_share_three_day_walkforward_campaign049_feature_snapshot"
        ...
        evidence = {
            ...
            if not key.startswith("campaign047_")
            and not key.startswith("campaign048_")
            and not key.startswith("campaign049_")
        }
        evidence["campaign049_mechanism_overlap_audit_sha256"] = MECHANISM_AUDIT_SHA256
        evidence["campaign049_no_return_preregistration_sha256"] = PROTOCOL_SHA256
        evidence["campaign049_implementation_freeze_sha256"] = IMPLEMENTATION_FREEZE_SHA256
    ...
    if not (
        ...
        and manifest.get("kind") == "a_share_three_day_walkforward_campaign049_feature_snapshot"
        ...
        and evidence.get("campaign049_mechanism_overlap_audit_sha256") == MECHANISM_AUDIT_SHA256
        and evidence.get("campaign049_no_return_preregistration_sha256") == PROTOCOL_SHA256
        and evidence.get("campaign049_implementation_freeze_sha256") == IMPLEMENTATION_FREEZE_SHA256
        ...
        raise Campaign049FeatureError("Campaign049 snapshot semantics changed")
    if require_fingerprint_constants and not (
        ...
        raise Campaign049FeatureError("Campaign049 snapshot fingerprint is not bound")
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    def write_with_truth(value: dict[str, Any], path: Path) -> None:
        if value.get("kind") == "a_share_three_day_walkforward_campaign049_feature_snapshot" and value.get("output_run_id") == OUTPUT_RUN_ID:
            value = dict(value)
            value.update({"source_open_high_low_read": False, "source_close_read": False, "source_volume_read": False, "source_amount_read": True})
        original(value, path)
...
def run_no_return_audit(*, data_root: Path, experiment_root: Path, workers: int) -> Path:
    ...
    if not SNAPSHOT_MANIFEST_SHA256 or not SNAPSHOT_DATASET_SHA256 or not SNAPSHOT_PUBLICATION_BINDING_SHA256:
        raise Campaign049FeatureError("bind Campaign049 snapshot and publication record before audit")
    if _sha256(DEFAULT_SNAPSHOT_BINDING) != SNAPSHOT_PUBLICATION_BINDING_SHA256:
        raise Campaign049FeatureError("Campaign049 snapshot publication binding changed")
    ...
    if _sha256(manifest_path) != SNAPSHOT_MANIFEST_SHA256:
        raise Campaign049FeatureError("Campaign049 snapshot manifest changed")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    _validate_snapshot_manifest(manifest, require_fingerprint_constants=True)
    existing = sorted(experiment_root.glob("*_campaign049_no_return_audit.json"))
    if existing:
        if len(existing) != 1 or not NO_RETURN_AUDIT_SHA256 or _sha256(existing[0]) != NO_RETURN_AUDIT_SHA256:
            raise Campaign049FeatureError("existing Campaign049 audit is ambiguous or unbound")
        return existing[0]
    ...
    if coverage["gate_passed_before_comparison_values"]:
        ...
        if len(comparisons) != TERMINAL_LIBRARY_COUNT:
            raise Campaign049FeatureError("complete 66-factor catalog changed")
        ...
        for label, path, expected_sha, expected_dataset, factor, module in (
            ...
            ("campaign048", C48_SNAPSHOT_PATH, C48_SNAPSHOT_SHA256, C48_DATASET_SHA256, C48_FACTOR_NAME, campaign048_reference),
        ):
            if _sha256(path) != expected_sha:
                raise Campaign049FeatureError(f"{label} snapshot manifest changed")
            ...
            if expected_dataset is not None and source_manifest.get("dataset_sha256") != expected_dataset:
                raise Campaign049FeatureError(f"{label} snapshot dataset changed")
    ...
    run_id = f"{research._timestamp()}_campaign049_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign049_no_return_audit",
        ...
        "next_action": "freeze the exact one-trial Campaign049 walk-forward catalog before reading 2019-2023 returns" if admitted else "record the no-return rejection and design a genuinely new campaign",
        "source_fields_read": list(RAW_COLUMNS),
        "minute_open_high_low_close_fields_read": [],
        "minute_volume_fields_read": [],
...
def status(data_root: Path, experiment_root: Path) -> dict[str, Any]:
    ...
    audits = sorted(experiment_root.expanduser().resolve().glob("*_campaign049_no_return_audit.json"))
    result = {
