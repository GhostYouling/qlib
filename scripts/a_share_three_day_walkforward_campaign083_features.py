#!/usr/bin/env python3
"""Build Campaign083's close-frontier-innovation snapshot without returns."""
...
from scripts import a_share_three_day_walkforward_campaign082_features as c82

REPO_ROOT = Path(__file__).resolve().parents[1]
DEFAULT_DATA_ROOT = c74.DEFAULT_DATA_ROOT
DEFAULT_PROTOCOL = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_083_no_return_preregistration.json"
)
DEFAULT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_083_feature_implementation_freeze_20260806.json"
)

FACTOR_NAME = "intraday_close_frontier_innovation_share_238p"
FACTOR_FORMULA = (
    "within each 120-close half count strict new running highs or lows after "
    "the first close; sum the 238 indicators and divide by 238"
)
PROTOCOL_SHA256 = "31271774531c1bd761d3963740be959fe329974b2bb1c8d6b6038fd62ede35af"
MECHANISM_AUDIT_SHA256 = (
    "fab8ec22717b83e444923e4d74a88878b11d4ab239e1c37eb3658e3c37d5c0fa"
)
NUMERIC_POLICY_SHA256 = (
    "05258e51c1e0a7c9805c1b68c1f936504c390f43646f3e71c67f1d5e4fc8db9c"
)
CURRENT_STATE_SHA256 = (
    "6c3ce9276b1d3d9530868e766d96a855bb625f00910baf585339e519f5e401fb"
)
...
FULL_DEFINITION_COUNT = 114
FULL_DEFINITION_ORDER_SHA256 = (
    "bc8ecccee5fa8278342253908e34167313041a23ae153bb3d64d3f60c285e04f"
)
COMPARISON_COUNT = 112
COMPARISON_ORDER_SHA256 = (
    "78656b7dcd0df95703ccf663d261c61cb5f596ca368c3523907464fe25ed1f2b"
)
OUTPUT_RUN_ID = (
    "tushare_stk_mins_1m_2019_2025_ea0cbb8f_"
    "walkforward_campaign083_feature_library_v1"
)
...
C82_FACTOR = c82.FACTOR_NAME
C82_BINDING_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_082_feature_snapshot_binding_20260806.json"
)
C82_BINDING_SHA256 = "041faa2e08a0563d792bca119f46fd8378d9132a55497aea24f728d29f3c6933"
C82_MANIFEST_PATH = c82.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
C82_MANIFEST_SHA256 = "928d1466963c55605ad446bfa44f75997b9a967fe510c454cc42684ae3db1b65"
C82_DATASET_SHA256 = "e1a310820c6551d02eb1a09611c798fad78e14a66ddb4e7f85f4d6495497ad27"


class Campaign083FeatureError(RuntimeError):
    """Fail-closed Campaign083 feature error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign083FeatureError(f"Campaign083 {label} changed: {path}")


def reconstruct_comparisons() -> list[dict[str, str]]:
    items = c82.reconstruct_comparisons()
    items.append({"name": C82_FACTOR, "score_direction": "higher"})
    if (
        len(items) != COMPARISON_COUNT
        or _comparison_order_digest(items) != COMPARISON_ORDER_SHA256
    ):
        raise Campaign083FeatureError("Campaign083 numeric comparison order changed")
    return items


def reconstruct_complete_definitions() -> list[dict[str, str]]:
    items = c82.reconstruct_complete_definitions()
    items.append({"name": C82_FACTOR, "score_direction": "higher"})
    if (
        len(items) != FULL_DEFINITION_COUNT
        or _comparison_order_digest(items) != FULL_DEFINITION_ORDER_SHA256
    ):
        raise Campaign083FeatureError("Campaign083 complete definition order changed")
    return items
...
def load_protocol(path: Path = DEFAULT_PROTOCOL) -> dict[str, Any]:
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign083FeatureError("Campaign083 protocol binding failed")
    _require(C82_BINDING_PATH, C82_BINDING_SHA256, "Campaign082 snapshot binding")
    _require(C82_MANIFEST_PATH, C82_MANIFEST_SHA256, "Campaign082 snapshot manifest")
    previous = json.loads(C82_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    c82_link = chain.get("campaign082_terminal_numeric_comparator") or {}
    if not (
        spec.get("kind")
        == "a_share_three_day_walkforward_campaign083_no_return_preregistration"
        and spec.get("status")
        == "frozen_before_campaign083_minute_source_candidate_comparison_daily_price_or_return_values"
        and (chain.get("numeric_comparator_policy_v18") or {}).get("sha256")
        == NUMERIC_POLICY_SHA256
        and (chain.get("authoritative_iteration_state") or {}).get("sha256")
        == CURRENT_STATE_SHA256
        and c82_link.get("factor") == C82_FACTOR
        and c82_link.get("score_direction") == "higher"
        and c82_link.get("dataset_sha256") == C82_DATASET_SHA256
        and previous.get("dataset_sha256") == C82_DATASET_SHA256
        and previous.get("factor_names") == [C82_FACTOR]
        ...
        and unique.get("all_112_numeric_comparators_must_pass") is True
        ...
        == "wf083_intraday_close_frontier_innovation_share_238p_single_higher"
        ...
        raise Campaign083FeatureError("Campaign083 protocol semantics changed")
    return spec


def extract_close_frontier_innovation_share(
    raw: pd.DataFrame, *, symbol: str
) -> tuple[pd.DataFrame, dict[str, int]]:
    if tuple(raw.columns) != RAW_COLUMNS:
        raise Campaign083FeatureError(
            f"unexpected raw columns for {symbol}: {tuple(raw.columns)}"
        )
    empty = pd.DataFrame(
        {
            "trade_date": pd.Series(dtype="datetime64[ns]"),
            "close_frontier_innovation_share": pd.Series(dtype="float64"),
        }
    )
    quality = {
        "source_rows": 0,
        "source_sessions": 0,
        "valid_close_frontier_innovation_sessions": 0,
        "invalid_close_grid_sessions": 0,
        "invalid_innovation_score_sessions": 0,
        "strict_new_high_events": 0,
        "strict_new_low_events": 0,
    }
    if raw.empty:
    ...
    if (
        ...
        raise Campaign083FeatureError(f"raw minute identity changed for {symbol}")
    ...
    if (
        ...
        raise Campaign083FeatureError(f"raw minute grid changed for {symbol}")
    continuous = work.loc[
    ...
    if len(continuous) != len(dates) * SELECTED_BAR_COUNT:
        raise Campaign083FeatureError(f"continuous minute grid changed for {symbol}")
    closes = (
        continuous["close"]
    ...
    high_events = np.zeros(len(dates), dtype=np.int64)
    low_events = np.zeros(len(dates), dtype=np.int64)
    for row_index in np.flatnonzero(close_valid):
        row = closes[row_index]
        innovation_count = 0
        for start in (0, HALF_SESSION_BAR_COUNT):
            half = row[start : start + HALF_SESSION_BAR_COUNT]
            running_min = float(half[0])
            running_max = float(half[0])
            for value in half[1:]:
                current = float(value)
                if current > running_max:
                    high_events[row_index] += 1
                    innovation_count += 1
                elif current < running_min:
                    low_events[row_index] += 1
                    innovation_count += 1
                running_max = max(running_max, current)
                running_min = min(running_min, current)
        candidate = innovation_count / float(POOLED_OPPORTUNITY_COUNT)
        if np.isfinite(candidate) and 0.0 <= candidate <= 1.0:
            score[row_index] = candidate
    valid_score = np.isfinite(score) & (score >= 0.0) & (score <= 1.0)
    score[~valid_score] = np.nan
    return pd.DataFrame(
        {"trade_date": dates, "close_frontier_innovation_share": score}
    ), {
        "source_rows": len(work),
        "source_sessions": len(dates),
        "valid_close_frontier_innovation_sessions": int(valid_score.sum()),
        "invalid_close_grid_sessions": int((~close_valid).sum()),
        "invalid_innovation_score_sessions": int((close_valid & ~valid_score).sum()),
        "strict_new_high_events": int(high_events.sum()),
        "strict_new_low_events": int(low_events.sum()),
    }
...
def attach_values(
    ...
    if tuple(identity.columns) != IDENTITY_COLUMNS:
        raise Campaign083FeatureError(f"identity projection changed for {symbol}")
    base = identity.copy()
    ...
    if base.empty:
        base["close_frontier_innovation_share"] = pd.Series(dtype="float64")
        return base
    if (
        ...
        raise Campaign083FeatureError(f"joint-clean identity changed for {symbol}")
    if values.empty or values.duplicated(["trade_date"]).any():
        raise Campaign083FeatureError(
            f"Campaign083 value identity changed for {symbol}"
        )
...
def finalize_feature_frame(frame: pd.DataFrame) -> pd.DataFrame:
    required = (*IDENTITY_COLUMNS, "close_frontier_innovation_share")
    if not set(required).issubset(frame.columns):
        raise Campaign083FeatureError("Campaign083 input columns changed")
    work = frame.copy()
    values = pd.to_numeric(work["close_frontier_innovation_share"], errors="coerce")
...
def validate_value_semantics(frame: pd.DataFrame) -> tuple[int, int]:
    if tuple(frame.columns) != OUTPUT_COLUMNS:
        raise Campaign083FeatureError("Campaign083 output columns changed")
    ...
    if (
        ...
        raise Campaign083FeatureError("Campaign083 value semantics changed")
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_walkforward_campaign083_feature_library"
        / OUTPUT_RUN_ID
    )
...
def _validate_implementation_freeze() -> dict[str, Any]:
    if not DEFAULT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign083FeatureError("Campaign083 implementation freeze is absent")
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign083_feature_implementation_freeze"
        and freeze.get("status") == "frozen_before_campaign083_minute_source_values"
        ...
        raise Campaign083FeatureError("Campaign083 implementation freeze changed")
    return freeze
...
def _load_attached(
    ...
    if not raw_path.is_file() or pq.ParquetFile(raw_path).metadata.num_rows != int(
        ...
        raise Campaign083FeatureError(f"raw partition changed: {raw_path}")
    raw = pd.read_parquet(raw_path, columns=list(RAW_COLUMNS))
    values, quality = extract_close_frontier_innovation_share(raw, symbol=symbol)
...
def build_snapshot(*, data_root: Path, workers: int = 4) -> Path:
    ...
    if manifest_path.exists():
        raise Campaign083FeatureError(
            "Campaign083 snapshot already exists; verify instead"
        )
    ...
    for year in sorted(by_year):
        ...
        for index, item in by_year[year]:
            ...
            if raw_item is None:
                raise Campaign083FeatureError("raw partition missing")
        ...
        for index, item in by_year[year]:
            ...
            records[index] = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign083_feature_partition",
        ...
        print(
            f"Campaign083 built year={year} partitions={len(by_year[year])} "
    ...
    if (
        ...
        raise Campaign083FeatureError("Campaign083 publication totals changed")
    digest_rows = [
        [
    ...
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign083_feature_snapshot",
        ...
        "close_rule": "finite strictly positive float64 close for every selected bar",
        "tie_rule": "strict less-than or greater-than preceding running frontier; equality is not an innovation",
        "half_session_bar_count": HALF_SESSION_BAR_COUNT,
        "within_half_opportunity_count": WITHIN_HALF_OPPORTUNITY_COUNT,
        "pooled_opportunity_count": POOLED_OPPORTUNITY_COUNT,
        "half_session_reset_rule": "restart running minimum and maximum at 13:01",
        "close_frontier_innovation_rule": FACTOR_FORMULA,
...
def _validate_manifest(manifest: dict[str, Any]) -> None:
    ...
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign083_feature_snapshot"
        ...
        and manifest.get("half_session_bar_count") == HALF_SESSION_BAR_COUNT
        and manifest.get("within_half_opportunity_count")
        == WITHIN_HALF_OPPORTUNITY_COUNT
        and manifest.get("pooled_opportunity_count") == POOLED_OPPORTUNITY_COUNT
        and manifest.get("half_session_reset_rule")
        == "restart running minimum and maximum at 13:01"
        and manifest.get("close_frontier_innovation_rule") == FACTOR_FORMULA
        ...
        raise Campaign083FeatureError("Campaign083 manifest semantics changed")
...
def verify_snapshot_files(manifest_path: Path, *, workers: int = 4) -> dict[str, Any]:
    ...
    if manifest_path != expected:
        raise Campaign083FeatureError("Campaign083 manifest path changed")
    ...
    def verify(item: dict[str, Any]) -> tuple[int, int]:
        ...
        if _sha256(path) != item["output_byte_sha256"]:
            raise Campaign083FeatureError(f"partition byte hash changed: {path}")
        frame = pd.read_parquet(path)
        if (
            len(frame) != item["rows"]
            or c74._frame_sha256(frame) != item["output_frame_sha256"]
        ):
            raise Campaign083FeatureError(f"partition frame changed: {path}")
    ...
    if (
        ...
        raise Campaign083FeatureError("Campaign083 aggregate identity changed")
    return {
        "status": "verified",
