#!/usr/bin/env python3
"""Build and audit the preregistered intraday upside-semivariance share.

The candidate reads only minute identity, timestamp, and close from the
immutable Tushare source.  It measures the share of full-session adjacent
close-to-close realized variance contributed by positive log returns.  Build
and no-return audit never read a daily price or forward return, and terminal
...
import a_share_tushare_intraday_amount_profile_serial_persistence as previous
...
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_upside_semivariance_share_no_return_preregistration.json"
)
PREREGISTRATION_SHA256 = (
    "9fc26c9625e8223e9091042d6e07745780be4d4b570dbd51d742d556c295de14"
)
...
DIAGNOSTIC_PURPOSE = (
    "development_only_preregistered_tushare_intraday_upside_semivariance_"
    "share_three_session_diagnostic_research_not_investment_advice"
)
CONSUMPTION_FILENAME = (
    "tushare_intraday_upside_semivariance_share_v1_consumption.json"
)
...
AMOUNT_PROFILE_PERSISTENCE_MANIFEST_SHA256 = previous.CANDIDATE_MANIFEST_SHA256
...
OUTPUT_RUN_ID = f"{SOURCE_RUN_ID}_intraday_upside_semivariance_share_v1"
FACTOR_NAME = "intraday_upside_semivariance_share_239m"
FACTOR_FORMULA = (
    "sum(max(r_t,0)^2 for the 239 adjacent continuous-session close log "
    "returns) / sum(r_t^2 for the same 239 returns), where r_t=log(c_t/"
    "c_(t-1)) across 09:31-11:30 and 13:01-15:00"
)
...
class UpsideSemivarianceShareError(RuntimeError):
...
def load_preregistration(path: Path = DEFAULT_PREREGISTRATION) -> dict[str, Any]:
    ...
    _require_file(path, PREREGISTRATION_SHA256, "upside-semivariance protocol")
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_upside_semivariance_share_no_return_preregistration",
    )
    ...
    if not (
        ...
        and current.get("terminal_mechanism_count_before_this_candidate") == 32
        ...
        and grid.get("lunch_break_pair_included") == "11:30_to_13:01"
        and candidate.get("formula") == FACTOR_FORMULA
        and validity.get("all_240_required_close_values_finite_and_strictly_positive")
        is True
        and validity.get("total_realized_squared_log_return_strictly_positive")
        is True
        ...
        == 0.8
        and uniqueness.get("all_eight_comparisons_must_pass") is True
        ...
        raise UpsideSemivarianceShareError(
            "upside-semivariance-share protocol no longer matches its frozen definition"
        )
    return spec
...
def _validate_profile_persistence_manifest(
    ...
    if not (
        ...
        raise UpsideSemivarianceShareError(
            "amount-profile-persistence manifest identity is rejected"
        )
...
def validate_external_chain(
    ...
    profile, profile_path = _validate_profile_persistence_manifest(spec, data_root)
    return (
        raw,
        joint,
        raw_path,
...
def compute_partition_frame(
    ...
    *,
    ...
    """Compute the upside share of continuous-session realized semivariance."""
    ...
    if (
        ...
        raise UpsideSemivarianceShareError(f"joint base identity is invalid for {symbol}")
    ...
    if raw_work["datetime"].isna().any():
        raise UpsideSemivarianceShareError(f"raw timestamps are invalid for {symbol}")
    ...
    if set(raw_work["symbol"]) != {symbol} or set(raw_work["provider"]) != {"tushare"}:
        raise UpsideSemivarianceShareError(f"raw identity is invalid for {symbol}")
    ...
    if not np.array_equal(codes, np.broadcast_to(SOURCE_MINUTE_CODES, codes.shape)):
        raise UpsideSemivarianceShareError(f"raw source grid order changed for {symbol}")
    if len(CONTINUOUS_MINUTE_CODES) != 240:
        raise UpsideSemivarianceShareError("continuous close support changed")
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        ...
        total_variance = squared_returns.sum(axis=1)
        upside_semivariance = np.where(returns > 0.0, squared_returns, 0.0).sum(
            axis=1
        )
        values = upside_semivariance / total_variance
    positive_variance = (
        np.isfinite(total_variance)
        & (total_variance > 0.0)
        & np.isfinite(upside_semivariance)
    )
    canonicalizable = (
        required_valid
        & positive_variance
    ...
    if range_violation.any():
        raise UpsideSemivarianceShareError(
            f"upside-semivariance share escaped [0, 1] beyond tolerance for {symbol}"
        )
    eligible = (
        required_valid
        & positive_variance
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_intraday_upside_semivariance_share"
        / OUTPUT_RUN_ID
    )
...
def _load_checkpoint(
    ...
    valid = (
        record.get("kind")
        == "a_share_tushare_intraday_upside_semivariance_share_partition"
    ...
    if not valid:
        raise UpsideSemivarianceShareError(
            f"completed upside-semivariance checkpoint changed: {paths.partial_sidecar}"
        )
    ...
    if len(output) != int(record.get("rows", -1)) or foundation.frame_digest(output) != record.get(
        ...
        raise UpsideSemivarianceShareError(
            f"completed upside-semivariance frame changed: {paths.partial_data}"
        )
    dates = Counter(
...
def _process_partition(
    ...
    *,
    ...
    if foundation.file_digest(raw_path) != raw_record.get("byte_sha256"):
        raise UpsideSemivarianceShareError(f"raw partition changed: {raw_path}")
    if foundation.file_digest(base_path) != joint_record.get("output_byte_sha256"):
        raise UpsideSemivarianceShareError(f"joint-base partition changed: {base_path}")
    ...
    if len(raw) != int(raw_record.get("rows", -1)):
        raise UpsideSemivarianceShareError(f"raw row count changed: {raw_path}")
    if len(base) != int(joint_record.get("rows", -1)):
        raise UpsideSemivarianceShareError(f"joint-base row count changed: {base_path}")
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_upside_semivariance_share_partition",
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    if final_root.exists():
        if not final_manifest.is_file():
            raise UpsideSemivarianceShareError(
                f"published upside-semivariance root has no manifest: {final_root}"
            )
        manifest = research.load_json_record(
            final_manifest,
        ...
        if not (
            ...
            raise UpsideSemivarianceShareError(
                f"published upside-semivariance snapshot changed: {final_root}"
            )
        return final_manifest
    if shutil.disk_usage(data_root).free < 5 * 1024**3:
        raise UpsideSemivarianceShareError("external data root has less than 5 GiB free")
    ...
    lock_path = data_root / ".a_share_tushare_intraday_upside_semivariance_share.lock"
    with foundation.ProcessLock(lock_path):
        ...
        completed_symbols = 0
        print(
            f"building {len(joint_records):,} upside-semivariance partitions "
        ...
        if len(all_records) != 33_015:
            raise UpsideSemivarianceShareError(
                "not every source partition produced an upside-semivariance checkpoint"
            )
...
def verify_snapshot_files(
    ...
    if len(records) != 33_015:
        raise UpsideSemivarianceShareError("candidate snapshot partition count changed")
    ...
    if rows != manifest.get("rows") or eligible != manifest.get("eligible_rows"):
        raise UpsideSemivarianceShareError("candidate snapshot aggregate counts changed")
    return {
...
def load_candidate_frame(manifest_path: Path, manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(frame) != int(manifest.get("rows", -1)):
        raise UpsideSemivarianceShareError("candidate frame row count changed")
    ...
    if (
        ...
        raise UpsideSemivarianceShareError("candidate frame values or keys are invalid")
...
def uniqueness_audit(
    ...
    print("coverage passed; loading eight terminal comparison factors", flush=True)
    ...
    profile_verification = previous.verify_snapshot_files(
    ...
    if (
        ...
        raise UpsideSemivarianceShareError("base comparison frame identity changed")
    ...
    profile_frame = previous.load_candidate_frame(
        profile_manifest_path, profile_manifest
    )[
        ["trade_date", "symbol", previous.FACTOR_NAME]
    ]
    for frame in (efficiency, recovery_frame, entropy_frame, profile_frame):
    ...
    del efficiency, recovery_frame, entropy_frame, profile_frame
    ...
    all_passed = len(results) == 8 and all(item["gate_passed"] for item in results)
    return {
        ...
        "all_eight_comparisons_passed": all_passed,
    }
...
def _find_existing_audit(experiment_root: Path, manifest_sha256: str) -> Path | None:
    for path in sorted(
        experiment_root.glob(
            "*_intraday_upside_semivariance_share_no_return_audit.json"
        )
    ):
        record = research.load_json_record(path)
        if (
            record.get("kind")
            == "a_share_tushare_intraday_upside_semivariance_share_no_return_audit"
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if not (
        ...
        raise UpsideSemivarianceShareError("candidate snapshot identity is rejected")
    ...
    uniqueness: dict[str, Any] = {
        ...
        "all_eight_comparisons_passed": False,
        "comparisons": [],
    }
    ...
    passed = bool(
        ...
        and uniqueness["all_eight_comparisons_passed"]
    )
    status = (
    ...
    audit = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_upside_semivariance_share_no_return_audit",
        "status": status,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "purpose": "ordered_candidate_coverage_capacity_then_eight_terminal_factor_uniqueness_without_daily_prices_or_forward_returns",
        "preregistration": {
    ...
    path = (
        experiment_root
        / f"{run_id}_intraday_upside_semivariance_share_no_return_audit.json"
    )
...
def load_diagnostic_preregistration(
    ...
    _require_file(
        path,
        DIAGNOSTIC_PREREGISTRATION_SHA256,
        "upside-semivariance diagnostic protocol",
    )
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_upside_semivariance_share_diagnostic_preregistration",
    )
    ...
    if not (
        ...
        raise UpsideSemivarianceShareError(
            "upside-semivariance diagnostic protocol no longer matches its frozen definition"
        )
    return spec
...
def require_diagnostic_unconsumed(experiment_root: Path) -> None:
    ...
    if marker.exists():
        raise UpsideSemivarianceShareError(
            f"upside-semivariance historical diagnostic is already consumed: {marker}"
        )
    for path in sorted(experiment_root.glob("*_factor_diagnostic.json")):
        record = research.load_json_record(path)
        if record.get("purpose") == DIAGNOSTIC_PURPOSE:
            raise UpsideSemivarianceShareError(
                f"upside-semivariance historical diagnostic already exists: {path}"
            )
...
def run_diagnostic(args: argparse.Namespace) -> Path:
    ...
    marker = {
        "kind": "a_share_tushare_intraday_upside_semivariance_share_historical_consumption",
