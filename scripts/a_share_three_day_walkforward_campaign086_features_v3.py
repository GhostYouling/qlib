#!/usr/bin/env python3
"""Run Campaign086's additive v3 dispatch repair without changing v1/v2."""
...
DEFAULT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_086_feature_implementation_freeze_v3_20260807.json"
)
REPAIR_PROTOCOL = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_086_v3_dispatch_repair_protocol_20260807.json"
)
REPAIR_PROTOCOL_SHA256 = (
    "22bed279ee8ba94149725cc9ef20cbbd68559d2a121bc1b2a6930bd7035e38f0"
)
V1_RUNNER_SHA256 = v2.V1_RUNNER_SHA256
V1_FREEZE_SHA256 = v2.V1_FREEZE_SHA256
V2_RUNNER_SHA256 = (
    "5d5047d8d13963ac5e2aa2b6629c3d6e34ac5c0cc6b4e268bc943d96d57ad57e"
)
V2_FREEZE_SHA256 = (
    "083772ea9eec1d26cf02cbc915b7ca21296b1b141a9aa831a3274a997d8522ba"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign086_features_v3.py"
)
_ORIGINAL_V1_ATTACH_VALUES = v1.attach_values


class Campaign086FeatureV3Error(RuntimeError):
    """Fail-closed Campaign086 v3 dispatch repair error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign086FeatureV3Error(f"Campaign086 v3 {label} changed: {path}")
...
def attach_values(
    ...
    if tuple(identity.columns) != v1.IDENTITY_COLUMNS:
        raise Campaign086FeatureV3Error(
            f"Campaign086 v3 identity projection changed for {symbol}"
        )
    if identity.empty:
        return v2.empty_attached_frame()
    return _ORIGINAL_V1_ATTACH_VALUES(identity, values, symbol=symbol)
...
def _load_implementation_freeze() -> dict[str, Any]:
    ...
    if not DEFAULT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign086FeatureV3Error("Campaign086 v3 freeze is absent")
    record = json.loads(DEFAULT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign086_feature_implementation_freeze_v3"
        and record.get("status")
        == "dispatch_repair_frozen_before_campaign086_feature_build_retry"
        ...
        raise Campaign086FeatureV3Error("Campaign086 v3 freeze changed")
    return record
...
def _dataset_material(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "v3_implementation_freeze_sha256": manifest["implementation_freeze"][
            "sha256"
        ],
        "repair_protocol_sha256": repair["repair_protocol_sha256"],
...
def _activate_v3_manifest(manifest_path: Path) -> Path:
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    freeze_sha = _sha256(DEFAULT_IMPLEMENTATION_FREEZE)
    manifest["kind"] = "a_share_three_day_walkforward_campaign086_feature_snapshot_v3"
    ...
    manifest["runtime_repair"] = {
        "scope": "capture frozen v1 nonempty dispatch plus v2 exact empty schema",
...
def build_snapshot(
    *, data_root: Path, workers: int = 8, confirm_build: bool = False
    ...
    if not confirm_build:
        raise Campaign086FeatureV3Error("Campaign086 v3 build requires --confirm-build")
    _load_implementation_freeze()
    if v1.output_root(data_root.expanduser().resolve()).exists():
        raise Campaign086FeatureV3Error("Campaign086 output already exists")
    ...
    return _activate_v3_manifest(manifest_path)
...
def verify_snapshot_files(manifest_path: Path) -> dict[str, Any]:
    ...
    if path != expected.resolve() or not path.is_file():
        raise Campaign086FeatureV3Error("Campaign086 v3 manifest path changed")
    manifest = json.loads(path.read_text(encoding="utf-8"))
    if not (
        manifest.get("kind")
        == "a_share_three_day_walkforward_campaign086_feature_snapshot_v3"
        ...
        raise Campaign086FeatureV3Error("Campaign086 v3 manifest semantics changed")
    rows = 0
    eligible_count = 0
    ...
    for record in manifest.get("files") or []:
        ...
        if _sha256(partition) != record.get("sha256"):
            raise Campaign086FeatureV3Error("Campaign086 v3 partition bytes changed")
        frame = pd.read_parquet(partition, columns=list(v1.OUTPUT_COLUMNS))
        if c85._frame_sha256(frame) != record.get("frame_sha256"):
            raise Campaign086FeatureV3Error("Campaign086 v3 partition frame changed")
        ...
        if not (
            ...
            raise Campaign086FeatureV3Error("Campaign086 v3 values changed")
        rows += len(frame)
    ...
    if not (
        ...
        raise Campaign086FeatureV3Error("Campaign086 v3 aggregate identity changed")
    return {
        "status": "verified",
...
def status(data_root: Path = DEFAULT_DATA_ROOT) -> dict[str, Any]:
    ...
    return {
        "status": "snapshot_present" if path.is_file() else "snapshot_absent_pre_v3_retry",
