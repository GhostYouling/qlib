#!/usr/bin/env python3
"""Run Campaign119's frozen all-136 ordered uniqueness audit without returns."""
...
from scripts import a_share_three_day_walkforward_campaign118_features as c118_candidate
from scripts import (
    a_share_three_day_walkforward_campaign118_ordered_uniqueness as c118_base,
)
from scripts import (
    a_share_three_day_walkforward_campaign118_snapshot_verify_recovery as c118_verifier,
)
from scripts import a_share_three_day_walkforward_campaign119_features as candidate
from scripts import (
    a_share_three_day_walkforward_campaign119_no_return_audit as coverage,
)
...
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_119_ordered_numeric_uniqueness_protocol_20260813.json"
)
PROTOCOL_SHA256 = "32e3741f7fee8dec2e4b9eef281762864cdb9d66a71c2daaeaecca156f9e1d8a"
COVERAGE_PATH = coverage.OUTPUT_PATH
COVERAGE_SHA256 = "c7c023c096421f3ada44de3ad2012162b4c877ae4e4c3247603749e47356ab30"
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_119_ordered_uniqueness_implementation_freeze_20260813.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign119_ordered_uniqueness.py"
)
OUTPUT_PATH = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_119/uniqueness/campaign119_ordered_uniqueness_audit.json"
)
BASE118_RUNNER_PATH = Path(c118_base.__file__).resolve()
BASE118_RUNNER_SHA256 = (
    "c15c0dab13c4b3b78fb805acb1861975be591ddc9c62ab3b432542ec10f5be39"
)
...
C118_MANIFEST_PATH = (
    c118_candidate.output_root(c118_candidate.DEFAULT_DATA_ROOT)
    / "snapshot_manifest.json"
)
C118_MANIFEST_SHA256 = (
    "194c2ef2f81c1906c3e94a85e26ce97d7d82dba567b54128cf6e9086e3a027bd"
)
C118_DATASET_SHA256 = "19495eb562683e1e14fbe5e9a9730e6d614b481f7642c3da824ea36fdb4d5e2b"
C118_BINDING_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_118_feature_snapshot_binding_20260813.json"
)
C118_BINDING_SHA256 = "6aed914825a6c01951c3f05510bc964e2fb46da7077d9deb07aead18a9324fc4"
C118_VERIFIER_SHA256 = (
...
EXPECTED_COMPARISON_COUNT = 136
MINIMUM_PAIRWISE_NAMES = 50
MINIMUM_PAIRWISE_SESSIONS = 100
MAXIMUM_ABSOLUTE_MEDIAN_CORRELATION = 0.8


class Campaign119OrderedUniquenessError(RuntimeError):
    """Fail closed when the frozen all-136 comparison boundary changes."""
...
def load_protocol() -> dict[str, Any]:
    if not PROTOCOL_PATH.is_file() or _sha256(PROTOCOL_PATH) != PROTOCOL_SHA256:
        raise Campaign119OrderedUniquenessError("uniqueness protocol changed")
    ...
    expected_tail = [
        {
            "name": c118_base.c117_base.c110.FACTOR_NAME,
            "score_direction": "higher",
        },
        {
            "name": c118_base.c117_candidate.FACTOR_NAME,
            "score_direction": "higher",
        },
        {"name": c118_candidate.FACTOR_NAME, "score_direction": "higher"},
    ]
    if not (
        spec.get("kind")
        == "a_share_three_day_walkforward_campaign119_ordered_numeric_uniqueness_protocol"
        and spec.get("status")
        == "all_136_ordered_comparators_frozen_after_coverage_pass_before_any_comparator_value"
        ...
        and definitions[-3:] == expected_tail
        ...
        and gate.get("all_136_must_pass") is True
        ...
        raise Campaign119OrderedUniquenessError("uniqueness protocol semantics changed")
    return spec


def _validate_implementation_freeze() -> dict[str, Any]:
    if not IMPLEMENTATION_FREEZE_PATH.is_file():
        raise Campaign119OrderedUniquenessError(
    ...
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign119_ordered_uniqueness_implementation_freeze"
        and record.get("status")
        == "all_136_loader_and_gate_runner_frozen_before_comparator_values"
        ...
        and frozen.get("base118_runner_sha256") == BASE118_RUNNER_SHA256
...
def validate_static_bindings() -> dict[str, Any]:
    ...
    for path, expected, label in (
        ...
        (BASE118_RUNNER_PATH, BASE118_RUNNER_SHA256, "Campaign118 base runner"),
        (C118_MANIFEST_PATH, C118_MANIFEST_SHA256, "comparator 136 manifest"),
        (C118_BINDING_PATH, C118_BINDING_SHA256, "comparator 136 binding"),
        (
            Path(c118_verifier.__file__).resolve(),
            C118_VERIFIER_SHA256,
            "comparator 136 verifier",
        ),
        (
            CANDIDATE49_SIGNAL_LEDGER,
            CANDIDATE49_SIGNAL_LEDGER_SHA256,
        ...
        if not path.is_file() or _sha256(path) != expected:
            raise Campaign119OrderedUniquenessError(f"{label} changed: {path}")
    ...
    if not (
        ...
        raise Campaign119OrderedUniquenessError("coverage authority changed")
    prior = c118_base.validate_static_bindings()
    c118_verifier.validate_static_bindings()
    return {
        "implementation_freeze_sha256": _sha256(IMPLEMENTATION_FREEZE_PATH),
        "coverage_sha256": COVERAGE_SHA256,
        "candidate_manifest_sha256": CANDIDATE_MANIFEST_SHA256,
        "prior_135_loader_binding": prior,
        "comparator_136_manifest_sha256": C118_MANIFEST_SHA256,
        "comparator_136_dataset_sha256": C118_DATASET_SHA256,
...
def build_plan() -> dict[str, Any]:
    ...
    return {
        "kind": "a_share_three_day_walkforward_campaign119_ordered_uniqueness_plan",
        "ready": not blockers,
        "blockers": blockers,
...
def _candidate_arrays() -> tuple[np.ndarray, np.ndarray, Any, dict[str, Any]]:
    ...
    if len(quality) != 1_330_171 or quality[candidate.FACTOR_NAME].isna().any():
        raise Campaign119OrderedUniquenessError("candidate quality panel changed")
...
def _load_campaign118_comparator(
    *,
    ...
    verification = c118_verifier.verify_snapshot(workers=8)
    if not (
        verification.get("manifest_sha256") == C118_MANIFEST_SHA256
        and verification.get("dataset_sha256") == C118_DATASET_SHA256
        and verification.get("rows") == candidate.EXPECTED_ROWS
    ):
        raise Campaign119OrderedUniquenessError("comparator 136 snapshot changed")
    manifest = json.loads(C118_MANIFEST_PATH.read_text(encoding="utf-8"))
    from scripts import (  # noqa: PLC0415
        a_share_three_day_walkforward_campaign105_no_return_audit as c105_audit,
    )
    ...
    ranges[c118_candidate.FACTOR_NAME] = (0.0, 1.0)
    engine.FACTOR_RANGES = ranges
    frame = engine.load_factor_frame(
        C118_MANIFEST_PATH, manifest, c118_candidate.FACTOR_NAME
    )
    source_keys, source_values = (
        c118_base.c117_base.c110_v1.adapter.sorted_raw_comparator_arrays(
            frame,
            factor=c118_candidate.FACTOR_NAME,
            value_range=(0.0, 1.0),
    ...
    del frame
    gc.collect()
    aligned = c118_base.c117_base.c110_v1.adapter.align_raw_comparator_values(
    ...
    result = comparison_engine._aligned_comparison_result(
        ...
        comparison=c118_candidate.FACTOR_NAME,
        direction="higher",
        gate=gate,
    )
    result["source_score_direction"] = "higher"
    result["comparison_value_semantics"] = "frozen Campaign118 raw factor value"
    result["aligned_legal_nan_count_before_pairwise_overlap"] = legal_nan_count
    return result, {
        "manifest_path": str(C118_MANIFEST_PATH),
        "manifest_sha256": C118_MANIFEST_SHA256,
        "dataset_sha256": C118_DATASET_SHA256,
        "binding_sha256": C118_BINDING_SHA256,
...
def run_ordered_uniqueness(*, confirm: bool) -> Path:
    ...
    if plan["ready"] is not True:
        raise Campaign119OrderedUniquenessError("ordered uniqueness plan is not ready")
    definitions = candidate.reconstruct_comparisons()
    ...
    first_results, receipts = (
        c118_base.c117_base.c110_v1._load_comparisons_after_coverage(
    ...
    if [item["comparison_factor"] for item in first_results] != expected_order[:133]:
        raise Campaign119OrderedUniquenessError("first 133 comparator order changed")
    c110_result, c110_receipt = c118_base.c117_base._load_final_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    )
    c117_result, c117_receipt = c118_base._load_campaign117_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    )
    c118_result, c118_receipt = _load_campaign118_comparator(
        candidate_keys=keys,
        candidate_values=values,
        gate=gate,
        comparison_engine=comparison_engine,
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign119_ordered_uniqueness_audit",
        "status": (
            "all_136_uniqueness_gates_passed_ready_to_freeze_one_development_trial"
            if admitted
        ...
        "comparison_source_verification": {
            ...
            "all_136_sources_loaded_in_frozen_order": True,
        },
        ...
        "next_action": (
            ...
            if admitted
            else "terminalize Campaign119 and begin only a genuinely new campaign"
        ),
    }
