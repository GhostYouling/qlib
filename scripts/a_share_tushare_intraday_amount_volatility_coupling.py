#!/usr/bin/env python3
"""Build and audit the preregistered intraday amount-volatility coupling.

The candidate reads only minute identity, close, and amount from the immutable
Tushare source.  It correlates destination-bar log1p amount with absolute
within-half log return across 238 synchronous pairs.  The build and ordered
no-return audit never read a daily price or forward return, and prior terminal
factor values load only after coverage and capacity pass.
"""
...
import a_share_tushare_intraday_diffusive_variation_ratio as previous  # noqa: E402


foundation = previous.foundation
amount_center = previous.amount_center
amount_lead = previous.amount_lead
...
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_volatility_coupling_no_return_preregistration.json"
)
PREREGISTRATION_SHA256 = (
    "71de0d8214daaed1025149747c16b7dd94e682a8881318fef98899eff89a5142"
)
DEFAULT_DIAGNOSTIC_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_volatility_coupling_diagnostic_preregistration.json"
)
DIAGNOSTIC_PREREGISTRATION_SHA256 = (
    "4afd6fc54f42609a71a235831569ade0423ce883b59dc595780ecc9cd6c614db"
)
...
CANDIDATE_MANIFEST_SHA256 = (
    "ffe853163c8acfe7cb7accd8dde69ad15768160d8346386a849978f845215600"
)
...
CURRENT_STATUS_SHA256 = (
    "c33cbdabfc43ad0559548b8be0ea6ad251230314e040ce3709ff1f7c53861f7a"
)
...
AMOUNT_CENTER_OF_MASS_MANIFEST_SHA256 = previous.AMOUNT_CENTER_OF_MASS_MANIFEST_SHA256
AMOUNT_CENTER_OF_MASS_DATASET_SHA256 = previous.AMOUNT_CENTER_OF_MASS_DATASET_SHA256
DIFFUSIVE_VARIATION_MANIFEST_SHA256 = previous.CANDIDATE_MANIFEST_SHA256
DIFFUSIVE_VARIATION_DATASET_SHA256 = previous.CANDIDATE_DATASET_SHA256
UP_MOVE_AMOUNT_SHARE_MANIFEST_SHA256 = previous.UP_MOVE_AMOUNT_SHARE_MANIFEST_SHA256
UP_MOVE_AMOUNT_SHARE_DATASET_SHA256 = previous.UP_MOVE_AMOUNT_SHARE_DATASET_SHA256
...
OUTPUT_RUN_ID = f"{SOURCE_RUN_ID}_intraday_amount_volatility_coupling_v1"
FACTOR_NAME = "intraday_amount_volatility_coupling_238p"
FACTOR_FORMULA = (
    "PearsonCorr({log1p(amount_hj)}, {|log(close_hj/close_h,j-1)|}) "
    "over h in {morning,afternoon}, j=2..120"
)
DIAGNOSTIC_PURPOSE = (
    "single_preregistered_intraday_amount_volatility_coupling_three_session_diagnostic"
)
CONSUMPTION_FILENAME = "intraday_amount_volatility_coupling_238p_historical_consumption.json"
...
class IntradayAmountVolatilityCouplingError(RuntimeError):
...
def _require_file(path: Path, expected_sha256: str, label: str) -> None:
    ...
    if observed != expected_sha256:
        raise IntradayAmountVolatilityCouplingError(
...
def load_preregistration(path: Path = DEFAULT_PREREGISTRATION) -> dict[str, Any]:
    ...
    _require_file(path, PREREGISTRATION_SHA256, "amount-volatility-coupling protocol")
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_volatility_coupling_no_return_preregistration",
    )
    ...
    if not (
        ...
        and current.get("terminal_mechanism_count_before_this_candidate") == 40
        ...
        and grid.get("return_amount_pairs") == 238
        and grid.get("destination_amount_bar_ends")
        == "09:32-11:30 and 13:02-15:00"
        and grid.get("cross_lunch_return_or_pair_included") is False
        ...
        is True
        and validity.get("all_238_destination_amounts_finite_and_nonnegative")
        is True
        and validity.get("all_238_log_returns_and_log1p_amounts_finite") is True
        and validity.get(
            "strictly_positive_finite_variance_required_for_both_vectors"
        )
        is True
        and validity.get("pearson_correlation_finite_required") is True
        and validity.get("theoretical_closed_interval") == [-1, 1]
        and validity.get("constant_return_or_amount_vector_policy") == "missing"
        and validity.get("numerical_endpoint_canonicalization_tolerance") == 1e-12
        and validity.get(
            "statistical_clipping_imputation_winsorization_rank_transform_or_daily_substitution_allowed"
        )
        is False
        ...
        == 0.8
        and uniqueness.get("all_sixteen_comparisons_must_pass") is True
        ...
        raise IntradayAmountVolatilityCouplingError(
            "amount-volatility-coupling protocol no longer matches its frozen definition"
        )
    return spec
...
def validate_repository_chain(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    evidence["terminal_diffusive_variation_ratio_record"] = {
...
def load_terminal_record_if_present() -> dict[str, Any] | None:
    ...
    _require_file(
        DEFAULT_TERMINAL_RECORD,
        TERMINAL_RECORD_SHA256,
        "intraday-amount-volatility-coupling research record",
    )
    record = research.load_json_record(
        DEFAULT_TERMINAL_RECORD,
        kind="a_share_tushare_intraday_amount_volatility_coupling_research_record",
    )
    ...
    if not (
        ...
        raise IntradayAmountVolatilityCouplingError(
            "intraday-amount-volatility-coupling research record is inconsistent"
        )
    return record


def _validate_diffusive_variation_manifest(
    spec: dict[str, Any], data_root: Path
) -> tuple[dict[str, Any], Path]:
    link = (spec.get("source_chain") or {}).get(
        "diffusive_variation_ratio_comparison_manifest"
    ) or {}
    path = (data_root / str(link.get("path_below_data_root"))).resolve()
    _require_file(
        path,
        DIFFUSIVE_VARIATION_MANIFEST_SHA256,
        "diffusive-variation-ratio manifest",
    )
    manifest = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_diffusive_variation_ratio_snapshot",
    )
    ...
    if not (
        ...
        and manifest.get("dataset_sha256") == DIFFUSIVE_VARIATION_DATASET_SHA256
        ...
        and quality.get("nonfinite_realized_variance_or_bipower_rows") == 0
        and quality.get("nonfinite_ratio_rows") == 0
        and quality.get("numerical_endpoint_canonicalization_rows") == 0
        and quality.get("diffusive_variation_ratio_range_violation_rows") == 0
        ...
        and manifest.get("cross_lunch_bipower_pair_included") is False
        and manifest.get("comparison_factor_values_read") is False
        and manifest.get("forward_return_fields_read") is False
    ):
        raise IntradayAmountVolatilityCouplingError(
            "diffusive-variation-ratio manifest identity is rejected"
        )
...
def validate_external_chain(spec: dict[str, Any], data_root: Path) -> tuple[Any, ...]:
    ...
    diffusive, diffusive_path = _validate_diffusive_variation_manifest(
        spec, data_root
    )
    return (*chain, diffusive, diffusive_path)
...
def compute_partition_frame(
    ...
    *,
    ...
    """Compute the frozen synchronous amount and absolute-return correlation."""
    ...
    empty_quality = {
        ...
        "invalid_destination_amount_rows": 0,
        "invalid_required_value_rows": 0,
        "nonfinite_log_return_or_log1p_amount_rows": 0,
        "constant_absolute_return_rows": 0,
        "constant_log1p_amount_rows": 0,
        "nonfinite_correlation_rows": 0,
        "numerical_endpoint_canonicalization_rows": 0,
        "amount_volatility_coupling_range_violation_rows": 0,
    }
    ...
    destination_amounts_valid = np.isfinite(destination_amounts).all(
        axis=(1, 2)
    ) & (destination_amounts >= 0.0).all(axis=(1, 2))
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        absolute_returns = np.abs(np.diff(np.log(closes), axis=2)).reshape(-1, 238)
        log1p_amounts = np.log1p(destination_amounts).reshape(-1, 238)
        return_means = absolute_returns.mean(axis=1, keepdims=True)
        amount_means = log1p_amounts.mean(axis=1, keepdims=True)
        centered_returns = absolute_returns - return_means
        centered_amounts = log1p_amounts - amount_means
        return_ss = np.square(centered_returns).sum(axis=1)
        amount_ss = np.square(centered_amounts).sum(axis=1)
        denominator = np.sqrt(return_ss * amount_ss)
        values = (centered_returns * centered_amounts).sum(axis=1) / denominator
    paired_values_finite = np.isfinite(absolute_returns).all(axis=1) & np.isfinite(
        log1p_amounts
    ).all(axis=1)
    required_valid = (
        required_closes_valid & destination_amounts_valid & paired_values_finite
    )
    return_nonconstant = np.isfinite(return_ss) & (return_ss > 0.0)
    amount_nonconstant = np.isfinite(amount_ss) & (amount_ss > 0.0)
    correlation_finite = np.isfinite(values)
    tolerance = 1e-12
    near_low = (
        required_valid
        & return_nonconstant
    ...
    if range_violation.any():
        raise IntradayAmountVolatilityCouplingError(
            f"intraday amount-volatility coupling escaped [-1, 1] for {symbol}"
        )
    eligible = (
        required_valid
        & return_nonconstant
    ...
    invalid_required = ~required_closes_valid | ~destination_amounts_valid
    return output, {
        "base_rows": int(len(output)),
        "eligible_rows": int(eligible.sum()),
        "invalid_required_close_rows": int((~required_closes_valid).sum()),
        "invalid_destination_amount_rows": int((~destination_amounts_valid).sum()),
        "invalid_required_value_rows": int(invalid_required.sum()),
        "nonfinite_log_return_or_log1p_amount_rows": int(
            ((~paired_values_finite) & ~invalid_required).sum()
        ),
        "constant_absolute_return_rows": int(
            (required_valid & ~return_nonconstant).sum()
        ),
        "constant_log1p_amount_rows": int(
            (required_valid & ~amount_nonconstant).sum()
        ),
        ...
        "amount_volatility_coupling_range_violation_rows": int(
            range_violation.sum()
        ),
    }
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_intraday_amount_volatility_coupling"
        / OUTPUT_RUN_ID
    )
...
def _load_checkpoint(
    ...
    valid = (
        record.get("kind") == "a_share_tushare_intraday_amount_volatility_coupling_partition"
    ...
    if not valid:
        raise IntradayAmountVolatilityCouplingError(
            f"completed amount-volatility-coupling checkpoint changed: {paths.partial_sidecar}"
        )
    ...
    if len(output) != int(record.get("rows", -1)) or foundation.frame_digest(
        ...
        raise IntradayAmountVolatilityCouplingError(
            f"completed amount-volatility-coupling frame changed: {paths.partial_data}"
        )
    dates = Counter(
...
def _process_partition(
    ...
    *,
    ...
    if foundation.file_digest(raw_path) != raw_record.get("byte_sha256"):
        raise IntradayAmountVolatilityCouplingError(f"raw partition changed: {raw_path}")
    ...
    if len(raw) != int(raw_record.get("rows", -1)):
        raise IntradayAmountVolatilityCouplingError(f"raw row count changed: {raw_path}")
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_amount_volatility_coupling_partition",
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    if final_root.exists():
        if not final_manifest.is_file():
            raise IntradayAmountVolatilityCouplingError(
                f"published amount-volatility-coupling root has no manifest: {final_root}"
            )
        _require_file(
            final_manifest,
            CANDIDATE_MANIFEST_SHA256,
            "published amount-volatility-coupling manifest",
        )
        load_terminal_record_if_present()
        manifest = research.load_json_record(
            final_manifest,
            kind="a_share_tushare_intraday_amount_volatility_coupling_snapshot",
        )
        if not (
            ...
            and manifest.get("rows") == 7_724_498
            and manifest.get("eligible_rows") == 7_695_088
            and (manifest.get("quality") or {}).get("constant_absolute_return_rows")
            == 29_410
            and (manifest.get("quality") or {}).get("constant_log1p_amount_rows")
            == 0
            and (manifest.get("quality") or {}).get("invalid_required_close_rows") == 0
            and (manifest.get("quality") or {}).get("invalid_destination_amount_rows")
            == 0
            and (manifest.get("quality") or {}).get("invalid_required_value_rows") == 0
            and (manifest.get("quality") or {}).get(
                "nonfinite_log_return_or_log1p_amount_rows"
            )
            == 0
            and (manifest.get("quality") or {}).get(
            ...
            == 0
            and (manifest.get("quality") or {}).get(
                "amount_volatility_coupling_range_violation_rows"
            )
            == 0
            and manifest.get("source_close_read") is True
            ...
            raise IntradayAmountVolatilityCouplingError(
                f"published amount-volatility-coupling snapshot changed: {final_root}"
            )
    ...
    lock_path = data_root / ".a_share_tushare_intraday_amount_volatility_coupling.lock"
    with foundation.ProcessLock(lock_path):
        ...
        completed_symbols = 0
        print(
            f"building {len(joint_records):,} amount-volatility-coupling partitions "
        ...
        if len(all_records) != 33_015:
            raise IntradayAmountVolatilityCouplingError(
                "not every source partition produced a amount-volatility-coupling checkpoint"
            )
...
def load_candidate_frame(manifest_path: Path, manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(frame) != int(manifest.get("rows", -1)):
        raise IntradayAmountVolatilityCouplingError("candidate frame row count changed")
...
def uniqueness_audit(
    ...
    up_move_manifest: dict[str, Any],
    up_move_manifest_path: Path,
    diffusive_manifest: dict[str, Any],
    diffusive_manifest_path: Path,
    spec: dict[str, Any],
    workers: int,
) -> dict[str, Any]:
    print("coverage passed; loading sixteen terminal comparison factors", flush=True)
    ...
    amount_center_verification = amount_center.verify_snapshot_files(
        amount_center_manifest, amount_center_manifest_path, workers
    )
    up_move_verification = previous.previous.verify_snapshot_files(
        up_move_manifest, up_move_manifest_path, workers
    )
    diffusive_verification = previous.verify_snapshot_files(
        diffusive_manifest, diffusive_manifest_path, workers
    )
    dataset = pa_dataset.dataset(
    ...
    if (
        ...
        raise IntradayAmountVolatilityCouplingError("base comparison frame identity changed")
    ...
    amount_center_frame = amount_center.load_candidate_frame(
        amount_center_manifest_path, amount_center_manifest
    )[["trade_date", "symbol", amount_center.FACTOR_NAME]]
    up_move_frame = previous.previous.load_candidate_frame(
        up_move_manifest_path, up_move_manifest
    )[["trade_date", "symbol", previous.previous.FACTOR_NAME]]
    diffusive_frame = previous.load_candidate_frame(
        diffusive_manifest_path, diffusive_manifest
    ...
    all_passed = len(results) == 16 and all(item["gate_passed"] for item in results)
    return {
        ...
        "prior_candidate_snapshot_file_verification": {
            ...
            "up_move_amount_share": up_move_verification,
            "diffusive_variation_ratio": diffusive_verification,
        },
        ...
        "all_sixteen_comparisons_passed": all_passed,
    }


def _find_existing_audit(experiment_root: Path, manifest_sha256: str) -> Path | None:
    for path in sorted(
        experiment_root.glob("*_intraday_amount_volatility_coupling_no_return_audit.json")
    ):
        record = research.load_json_record(path)
        if (
            record.get("kind")
            == "a_share_tushare_intraday_amount_volatility_coupling_no_return_audit"
    ...
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if terminal_record is not None:
        ...
        audit = research.load_json_record(
            audit_path,
            kind="a_share_tushare_intraday_amount_volatility_coupling_no_return_audit",
        )
        if not (
            audit.get("status")
            == "passed_no_return_coverage_capacity_and_uniqueness_pending_separate_return_diagnostic_preregistration"
    ...
    if not (
        ...
        and (manifest.get("quality") or {}).get("constant_absolute_return_rows")
        == 29_410
        and (manifest.get("quality") or {}).get("constant_log1p_amount_rows") == 0
        and (manifest.get("quality") or {}).get("invalid_required_close_rows") == 0
        and (manifest.get("quality") or {}).get("invalid_destination_amount_rows") == 0
        and (manifest.get("quality") or {}).get("invalid_required_value_rows") == 0
        and (manifest.get("quality") or {}).get(
            "nonfinite_log_return_or_log1p_amount_rows"
        )
        == 0
        and (manifest.get("quality") or {}).get("nonfinite_correlation_rows") == 0
        and (manifest.get("quality") or {}).get(
            "numerical_endpoint_canonicalization_rows"
        )
        == 0
        ...
        raise IntradayAmountVolatilityCouplingError("candidate snapshot identity is rejected")
    ...
    uniqueness: dict[str, Any] = {
        ...
        "all_sixteen_comparisons_passed": False,
        "comparisons": [],
    }
    ...
    passed = bool(
        ...
        and uniqueness["all_sixteen_comparisons_passed"]
    )
    status = (
        "passed_no_return_coverage_capacity_and_uniqueness_pending_separate_return_diagnostic_preregistration"
        if passed
    ...
    audit = {
        ...
        "status": status,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "purpose": "ordered_candidate_coverage_capacity_then_sixteen_terminal_factor_uniqueness_without_daily_prices_or_forward_returns",
        "preregistration": {
        ...
        "source_chain": {
            ...
            "up_move_amount_share_manifest_sha256": UP_MOVE_AMOUNT_SHARE_MANIFEST_SHA256,
            "diffusive_variation_ratio_manifest_sha256": DIFFUSIVE_VARIATION_MANIFEST_SHA256,
            "repository_evidence": repository_evidence,
    ...
    path = (
        experiment_root / f"{run_id}_intraday_amount_volatility_coupling_no_return_audit.json"
    )
...
def load_diagnostic_preregistration(
    ...
    _require_file(
        path,
        DIAGNOSTIC_PREREGISTRATION_SHA256,
        "intraday amount-volatility-coupling diagnostic protocol",
    )
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_volatility_coupling_diagnostic_preregistration",
    )
    ...
    if not (
        ...
        raise IntradayAmountVolatilityCouplingError(
            "intraday amount-volatility-coupling diagnostic protocol no longer matches its frozen definition"
        )
    return spec
...
def require_diagnostic_unconsumed(experiment_root: Path) -> None:
    ...
    if marker.exists():
        raise IntradayAmountVolatilityCouplingError(
            f"intraday amount-volatility-coupling historical diagnostic is already consumed: {marker}"
        )
    for path in sorted(experiment_root.glob("*_factor_diagnostic.json")):
        record = research.load_json_record(path)
        if record.get("purpose") == DIAGNOSTIC_PURPOSE:
            raise IntradayAmountVolatilityCouplingError(
                f"intraday amount-volatility-coupling historical diagnostic already exists: {path}"
            )
...
def run_diagnostic(args: argparse.Namespace) -> Path:
    ...
    marker = {
        "kind": "a_share_tushare_intraday_amount_volatility_coupling_historical_consumption",
    ...
    diagnostic = {
        ...
        "strategy_timing": {
            ...
            "signal_time": "full-session intraday amount-volatility-coupling factor known after signal-session close",
