#!/usr/bin/env python3
"""Run Campaign088's frozen coverage-first no-return uniqueness audit."""
...
from scripts import (
    a_share_three_day_walkforward_campaign087_no_return_audit_v2 as c87_audit,
)
from scripts import a_share_three_day_walkforward_campaign088_features as definitions
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_088/no_return"
)
...
AUDIT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_088_no_return_audit_implementation_freeze_20260807.json"
)
AUDIT_ACTIVATION_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_088_no_return_audit_activation_binding_20260807.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign088_no_return_audit.py"
)
EXPECTED_ROWS = 1_331_759
EXPECTED_PARTITIONS = 7
EXPECTED_ELIGIBLE_ROWS = 1_328_065
EXPECTED_SESSIONS = 1_632
EXPECTED_COMPARISON_COUNT = 117
EXPECTED_COMPLETE_DEFINITION_COUNT = 119
...
C87_MANIFEST_PATH = c87_audit.v1.SNAPSHOT_MANIFEST_PATH
C87_MANIFEST_SHA256 = c87_audit.v1.SNAPSHOT_MANIFEST_SHA256
C87_FACTOR_NAME = c87_audit.v1.FACTOR_NAME


class Campaign088NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign088 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign088NoReturnAuditError(f"Campaign088 {label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        and comparisons[-1] == {"name": C87_FACTOR_NAME, "score_direction": "higher"}
    ):
        raise Campaign088NoReturnAuditError("Campaign088 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign088NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign088_no_return_audit_implementation_freeze"
        and record.get("status")
        == "frozen_before_campaign088_coverage_or_comparison_values"
        ...
        raise Campaign088NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign088NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign088_no_return_audit_activation_binding"
        and record.get("status")
        == "frozen_before_campaign088_coverage_or_comparison_values"
        ...
        raise Campaign088NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign088NoReturnAuditError("candidate snapshot verification changed")
    return result
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign088NoReturnAuditError("snapshot binding validation failed")
    return {
...
def load_candidate_arrays() -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    if not (
        ...
        raise Campaign088NoReturnAuditError("candidate arrays changed")
...
def coverage_and_capacity(
    ...
    return c87_audit.v1.coverage_and_capacity(keys, values, years, spec)


def _install_frozen_ranges(engine: Any) -> None:
    c87_audit.v1._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign088NoReturnAuditError("conflicting Campaign088 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c87_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    if len(comparisons) != 116:
        raise Campaign088NoReturnAuditError("first 116 comparison order changed")
    result, receipt = c87_audit._compact_snapshot_comparison(
        manifest_path=C87_MANIFEST_PATH,
        expected_manifest_sha256=C87_MANIFEST_SHA256,
        factor=C87_FACTOR_NAME,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        comparison_engine=comparison_engine,
        verifier=c87_audit.v1.verify_candidate_snapshot,
    )
    comparisons.append(result)
    receipts["campaign087_compact_snapshot"] = receipt
    receipts.pop("all_116_sources_loaded_in_frozen_order", None)
    receipts["all_117_sources_loaded_in_frozen_order"] = True
    if not (
        len(comparisons) == EXPECTED_COMPARISON_COUNT
        and comparisons[-1]["comparison_factor"] == C87_FACTOR_NAME
    ):
        raise Campaign088NoReturnAuditError("Campaign088 comparison order changed")
    del result
    gc.collect()
...
def _run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int, confirm_run: bool
    ...
    if not confirm_run:
        raise Campaign088NoReturnAuditError("Campaign088 audit requires --confirm-run")
    static = verify_static_bindings()
    spec = load_protocol()
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign088NoReturnAuditError("Campaign088 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign088_no_return_audit.json")):
        raise Campaign088NoReturnAuditError("Campaign088 audit already exists")
    ...
    payload = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign088_no_return_audit",
        "status": (
        ...
        "run_id": f"{run_id}_campaign088_no_return_audit",
        "protocol": {
    ...
    return {
        ...
        "audit_count": len(
            list(
                ...
                .resolve()
                .glob("*_campaign088_no_return_audit.json")
            )
        ),
