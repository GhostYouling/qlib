#!/usr/bin/env python3
"""Build Campaign096's frozen intrabar close-location total-variation snapshot."""
...
FACTOR_FORMULA = (
    ...
    "log(high/low), retain adjacent pairs separately within each half when "
    "both states exist, require at least 120 pairs, and return the unweighted "
    "mean absolute transition abs(x_t-x_t-1)"
)
RAW_COLUMNS = ("datetime", "symbol", "provider", "high", "low", "close")
SELECTED_BAR_COUNT = 240
PAIR_OPPORTUNITY_COUNT = 238
MINIMUM_INFORMATIVE_PAIRS = 120
ENDPOINT_TOLERANCE = 1e-12
C95_SEMANTIC_DEFINITION = {
    "name": "intraday_own_bar_close_location_entropy_10b_240m",
...
for _old, _new in (
    ...
    (
        ...
        "97d8cdd6cc3731de8e8d4db101db1b4b0ee62353c9c39ac71a8753fea4501924",
    ),
    (
        "c0f50dd19cfb983f08998fbafa80b9661872afc78074e806eefc8e5282019225",
        "032fe9770629f926f66aaae88caffd3e947cd8e19771d0c0f30b5b3d263362f6",
    ),
    (
        "0a6a79e05eb12c0303ba1c186d31adeca0d45abdea48b9de50101cde2523a57b",
        "b100ba8d347727e71f17799dbeeb1d7e2e1b34770148d217cf4ae4014c193b47",
    ),
    (
        "8e2d23139dbe5f423a5b96fb1230a2f5f0f3c04e3bced33753aa38ac5d7554f8",
        "26af71f841cf9d1880821b61853a347dfff9bc42652b2af09220aebf72de9bb6",
    ),
    (
    ...
    ("numeric_comparator_policy_v36", "numeric_comparator_policy_v39"),
    ("v36", "v39"),
    ("close_transition_count", "informative_pair_count"),
    (
        "valid_close_transition_range_efficiency_sessions",
        "valid_close_location_total_variation_sessions",
    ),
    (
        "nonpositive_quadratic_mass_sessions",
        "insufficient_informative_pair_sessions",
    ),
    ("zero_close_transition_pairs", "zero_close_location_transitions"),
    ("positive_close_transition_pairs", "positive_close_location_transitions"),
    ...
    (
        ...
        "intrabar close-location total variation",
    ),
):
...
_namespace: dict[str, Any] = {
    ...
    "__name__": "a_share_three_day_walkforward_campaign096_features_generated",
}
exec(compile(_source, str(BASE_RUNNER), "exec"), _namespace)  # noqa: S102

_generated = _namespace["_generated"]
Campaign096FeatureError = _namespace["Campaign096FeatureError"]
_base_complete_definitions = tuple(
    (item["name"], item["score_direction"])
    for item in _generated["reconstruct_complete_definitions"]()
)
...
def reconstruct_complete_definitions() -> list[dict[str, str]]:
    definitions = [
        ...
        for name, direction in _base_complete_definitions
    ]
    definitions.append(dict(C95_SEMANTIC_DEFINITION))
    return definitions


_generated["FULL_DEFINITION_COUNT"] = 127
_generated["FULL_DEFINITION_ORDER_SHA256"] = (
    "17ad9beb53d236171155211ea4cc4b9ec05b8edea7ebc51ca7705f84adc9e030"
)
_generated["reconstruct_complete_definitions"] = reconstruct_complete_definitions


def load_protocol(
    path: Path = _generated["DEFAULT_PROTOCOL"],
) -> dict[str, Any]:
    """Validate every frozen Campaign096 protocol binding and search boundary."""

    path = path.expanduser().resolve()
    if not path.is_file() or _sha256(path) != _generated["PROTOCOL_SHA256"]:
        raise Campaign096FeatureError(f"Campaign096 protocol changed: {path}")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign096FeatureError("Campaign096 protocol binding failed")
    ...
    if (
        ...
        raise Campaign096FeatureError("Campaign096 predecessor snapshot changed")
    previous = json.loads(previous_path.read_text(encoding="utf-8"))
    comparisons = _generated["reconstruct_comparisons"]()
    complete = reconstruct_complete_definitions()
    if not (
        spec.get("kind")
        == "a_share_three_day_walkforward_campaign096_no_return_preregistration"
        and spec.get("status")
        == "frozen_before_campaign096_minute_source_candidate_comparison_daily_price_or_return_values"
        ...
        and (chain.get("numeric_comparator_policy_v39") or {}).get("sha256")
        ...
        and candidate.get("within_half_adjacent_opportunity_count")
        ...
        == "wf096_intraday_intrabar_close_location_total_variation_238p_single_higher"
        ...
        raise Campaign096FeatureError("Campaign096 protocol semantics changed")
    return spec


def compute_intrabar_close_location_total_variation(
    highs: np.ndarray,
    lows: np.ndarray,
    closes: np.ndarray,
) -> tuple[
    ...
    """Return score, eligibility, states, informative-pair mask, and L1 edges."""

    high = np.asarray(highs, dtype=np.float64)
    low = np.asarray(lows, dtype=np.float64)
    close = np.asarray(closes, dtype=np.float64)
    if high.ndim != 2 or high.shape[1] != SELECTED_BAR_COUNT:
        raise Campaign096FeatureError("Campaign096 requires an n-by-240 high array")
    if low.shape != high.shape or close.shape != high.shape:
        raise Campaign096FeatureError("Campaign096 requires matching n-by-240 HLC arrays")
    source_valid = (
    ...
    with np.errstate(divide="ignore", invalid="ignore", over="ignore"):
        states = (np.log(close) - np.log(low)) / (np.log(high) - np.log(low))
    ...
    invalid_positive_range_state = positive_range & ~informative_states
    previous = np.concatenate((states[:, :119], states[:, 120:239]), axis=1)
    following = np.concatenate((states[:, 1:120], states[:, 121:240]), axis=1)
    informative_pairs = np.isfinite(previous) & np.isfinite(following)
    transitions = np.full(previous.shape, np.nan, dtype=np.float64)
    transitions[informative_pairs] = np.abs(
        following[informative_pairs] - previous[informative_pairs]
    )
    pair_counts = informative_pairs.sum(axis=1, dtype=np.int64)
    transition_sums = np.nansum(transitions, axis=1, dtype=np.float64)
    with np.errstate(divide="ignore", invalid="ignore"):
        raw_score = transition_sums / pair_counts
    raw_score[np.isfinite(raw_score) & (np.abs(raw_score) <= ENDPOINT_TOLERANCE)] = 0.0
    raw_score[
        np.isfinite(raw_score) & (np.abs(raw_score - 1.0) <= ENDPOINT_TOLERANCE)
    ] = 1.0
    support = (
        source_valid
        & ~invalid_positive_range_state.any(axis=1)
        & (pair_counts >= MINIMUM_INFORMATIVE_PAIRS)
    ...
    return result, eligible, states, informative_pairs, transitions
...
def extract_intrabar_close_location_total_variation(
    ...
    quality = {
        "source_rows": 0,
        "source_sessions": 0,
        "valid_close_location_total_variation_sessions": 0,
        "invalid_selected_source_sessions": 0,
        "insufficient_informative_pair_sessions": 0,
        "invalid_ordered_hlc_sessions": 0,
        "zero_close_location_transitions": 0,
        "positive_close_location_transitions": 0,
        "zero_range_bars": 0,
    ...
    if (
        ...
        raise Campaign096FeatureError(f"raw minute identity changed for {symbol}")
    ...
    if (
        ...
        raise Campaign096FeatureError(f"raw minute grid changed for {symbol}")
    continuous = work.loc[
    ...
    if len(continuous) != len(dates) * SELECTED_BAR_COUNT:
        raise Campaign096FeatureError(f"continuous minute grid changed for {symbol}")
    arrays = {
    ...
    values, eligible, states, informative_pairs, transitions = (
    ...
    return pd.DataFrame({"trade_date": dates, FACTOR_NAME: values}), {
        ...
        "valid_close_location_total_variation_sessions": int(eligible.sum()),
        "invalid_selected_source_sessions": int((~finite_positive).sum()),
        "insufficient_informative_pair_sessions": int(
            (ordered & (pair_counts < MINIMUM_INFORMATIVE_PAIRS)).sum()
        ),
        "invalid_ordered_hlc_sessions": int((finite_positive & ~ordered).sum()),
        "zero_close_location_transitions": int(
            ((transitions == 0.0) & ordered[:, None]).sum()
        ),
        "positive_close_location_transitions": int(
            ((transitions > 0.0) & ordered[:, None]).sum()
        ),
        "zero_range_bars": int(
