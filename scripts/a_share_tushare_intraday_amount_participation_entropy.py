#!/usr/bin/env python3
"""Build and audit the preregistered intraday amount-participation entropy.

The candidate build reads only minute identity, timestamp, and amount from the
immutable Tushare source, and restricts output to stock-days already present in
the immutable joint-clean layer.  The ordered audit applies coverage/capacity
before loading six terminal comparison factors.  Neither operation reads a
minute price, a daily price, or a forward return.
"""
...
import a_share_tushare_afternoon_drawdown_recovery_resilience as previous
...
DEFAULT_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_participation_entropy_no_return_preregistration.json"
)
PREREGISTRATION_SHA256 = (
    "66f5925b2883151fc051d20388d927646cd6a058744f07ae26253bf848454f5b"
)
DEFAULT_DIAGNOSTIC_PREREGISTRATION = (
    REPO_ROOT
    / "docs"
    / "a_share_tushare_intraday_amount_participation_entropy_diagnostic_preregistration.json"
)
DIAGNOSTIC_PREREGISTRATION_SHA256 = (
    "ec59d180c4d8c76dfe0183b45ec37eeb350e43756b9aefde473d0dbd17a20055"
)
NO_RETURN_AUDIT_SHA256 = (
    "6cf5d5d16681c542dc915ebbdfa0c5a5ace62e370f2cf71c44f10840fd9ab6cd"
)
CANDIDATE_MANIFEST_SHA256 = (
    "58c9a9c8f3991fa94a3e5289ed0a72333dfc037dae1e7f318de29eca4f207631"
)
DIAGNOSTIC_PURPOSE = (
    "development_only_preregistered_tushare_intraday_amount_participation_"
    "entropy_three_session_diagnostic_research_not_investment_advice"
)
CONSUMPTION_FILENAME = "tushare_intraday_amount_participation_entropy_v1_consumption.json"
...
AFTERNOON_RECOVERY_MANIFEST_SHA256 = previous.CANDIDATE_MANIFEST_SHA256
AFTERNOON_RECOVERY_DATASET_SHA256 = (
...
OUTPUT_RUN_ID = f"{SOURCE_RUN_ID}_intraday_amount_participation_entropy_v1"
FACTOR_NAME = "intraday_amount_participation_entropy_240m"
FACTOR_FORMULA = (
    "-sum(p_t * log(p_t) for positive p_t) / log(240), where p_t is each "
    "continuous-session minute amount divided by the positive total amount "
    "across 09:31-11:30 and 13:01-15:00"
)
...
CONTINUOUS_MINUTE_CODES = SOURCE_MINUTE_CODES[1:]
DEVELOPMENT_START = foundation.DEVELOPMENT_START
DEVELOPMENT_END = foundation.DEVELOPMENT_END


class AmountParticipationEntropyError(RuntimeError):
...
def _require_file(path: Path, expected_sha256: str, label: str) -> None:
    ...
    if observed != expected_sha256:
        raise AmountParticipationEntropyError(
...
def load_preregistration(path: Path = DEFAULT_PREREGISTRATION) -> dict[str, Any]:
    ...
    _require_file(path, PREREGISTRATION_SHA256, "amount-entropy protocol")
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_participation_entropy_no_return_preregistration",
    )
    ...
    if not (
        ...
        and current.get("terminal_mechanism_count_before_this_candidate") == 30
        ...
        is True
        and validity.get("total_continuous_session_amount_strictly_positive")
        is True
        and validity.get("zero_amount_term_contribution") == 0.0
        ...
        == 0.8
        and uniqueness.get("all_six_comparisons_must_pass") is True
        ...
        raise AmountParticipationEntropyError(
            "amount-participation-entropy protocol no longer matches its frozen definition"
        )
    return spec


def validate_repository_chain(spec: dict[str, Any]) -> dict[str, Any]:
    """Fingerprint-bind repository state without reading prices or returns."""
...
def _validate_prior_manifest(
    *,
    ...
    _require_file(path, expected_sha256, label)
    manifest = research.load_json_record(path, kind=kind)
    if not (
        manifest.get("status")
        ...
        raise AmountParticipationEntropyError(f"{label} identity is rejected")
...
def validate_external_chain(
    ...
    """Validate raw, joint-clean, and both prior-factor manifests."""

    raw, joint, raw_path, joint_path = foundation.validate_external_chain(
        spec, data_root
    )
    source = spec.get("source_chain") or {}
    efficiency, efficiency_path = _validate_prior_manifest(
        data_root=data_root,
        link=source.get("afternoon_efficiency_comparison_manifest") or {},
        expected_sha256=AFTERNOON_EFFICIENCY_MANIFEST_SHA256,
        expected_dataset_sha256=AFTERNOON_EFFICIENCY_DATASET_SHA256,
        kind="a_share_tushare_afternoon_signed_amount_efficiency_snapshot",
        factor_name=foundation.FACTOR_NAME,
        label="afternoon-efficiency comparison manifest",
    )
    recovery, recovery_path = _validate_prior_manifest(
        data_root=data_root,
        link=source.get("afternoon_recovery_comparison_manifest") or {},
        expected_sha256=AFTERNOON_RECOVERY_MANIFEST_SHA256,
        expected_dataset_sha256=AFTERNOON_RECOVERY_DATASET_SHA256,
        kind="a_share_tushare_afternoon_drawdown_recovery_resilience_snapshot",
        factor_name=previous.FACTOR_NAME,
        label="afternoon-recovery comparison manifest",
    )
    return (
        raw,
        joint,
...
def compute_partition_frame(
    ...
    *,
    ...
    """Compute normalized Shannon entropy for one symbol-year using amount only."""
    ...
    if set(raw_work["symbol"]) != {symbol} or set(raw_work["provider"]) != {
        "tushare"
    }:
        raise AmountParticipationEntropyError(f"raw identity is invalid for {symbol}")
    ...
    if (
        len(CONTINUOUS_MINUTE_CODES) != 240
        or int(SOURCE_MINUTE_CODES[0]) != 9 * 60 + 30
        or int(CONTINUOUS_MINUTE_CODES[0]) != 9 * 60 + 31
        or int(CONTINUOUS_MINUTE_CODES[-1]) != 15 * 60
    ):
        raise AmountParticipationEntropyError("continuous amount support changed")

    amounts = raw_work["amount"].to_numpy(dtype=float).reshape(-1, 241)[:, 1:]
    required_valid = np.isfinite(amounts).all(axis=1) & (amounts >= 0.0).all(
        axis=1
    )
    totals = amounts.sum(axis=1)
    positive_total = np.isfinite(totals) & (totals > 0.0)
    with np.errstate(divide="ignore", invalid="ignore"):
        probabilities = amounts / totals[:, None]
        terms = np.where(
            probabilities > 0.0,
            probabilities * np.log(probabilities),
            0.0,
        )
        values = -terms.sum(axis=1) / math.log(240.0)
    range_violation = (
        required_valid
        & positive_total
        & np.isfinite(values)
    ...
    if range_violation.any():
        raise AmountParticipationEntropyError(
            f"normalized entropy escaped [0, 1] for {symbol}"
        )
    eligible = (
        required_valid
        & positive_total
    ...
    return output, {
        ...
        "zero_total_amount_rows": int((required_valid & ~positive_total).sum()),
        "invalid_required_value_rows": int((~required_valid).sum()),
        "entropy_range_violation_rows": int(range_violation.sum()),
    }
...
def output_root(data_root: Path) -> Path:
    return (
        data_root
        / "derived/a_share/rich/tushare/minute_intraday_amount_participation_entropy"
        / OUTPUT_RUN_ID
    )
...
def _load_checkpoint(
    ...
    valid = (
        record.get("kind")
        == "a_share_tushare_intraday_amount_participation_entropy_partition"
    ...
    if not valid:
        raise AmountParticipationEntropyError(
            f"completed amount-entropy checkpoint changed: {paths.partial_sidecar}"
        )
    ...
    if len(output) != int(record.get("rows", -1)) or foundation.frame_digest(
        ...
        raise AmountParticipationEntropyError(
            f"completed amount-entropy frame changed: {paths.partial_data}"
        )
    dates = Counter(
...
def _process_partition(
    ...
    *,
    ...
    if foundation.file_digest(raw_path) != raw_record.get("byte_sha256"):
        raise AmountParticipationEntropyError(f"raw partition changed: {raw_path}")
    ...
    if len(raw) != int(raw_record.get("rows", -1)):
        raise AmountParticipationEntropyError(f"raw row count changed: {raw_path}")
    ...
    record = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_amount_participation_entropy_partition",
...
def build_snapshot(*, data_root: Path, workers: int) -> Path:
    ...
    if final_root.exists():
        if not final_manifest.is_file():
            raise AmountParticipationEntropyError(
                f"published amount-entropy root has no manifest: {final_root}"
            )
        ...
        if not (
            ...
            raise AmountParticipationEntropyError(
                f"published amount-entropy snapshot changed: {final_root}"
            )
    ...
    lock_path = data_root / ".a_share_tushare_intraday_amount_entropy.lock"
    with foundation.ProcessLock(lock_path):
        ...
        completed_symbols = 0
        print(
            f"building {len(joint_records):,} amount-entropy partitions "
        ...
        if len(all_records) != 33_015:
            raise AmountParticipationEntropyError(
                "not every source partition produced an amount-entropy checkpoint"
            )
...
def verify_snapshot_files(
    ...
    """Verify every candidate, raw-source, and joint-base byte fingerprint."""
...
def load_candidate_frame(manifest_path: Path, manifest: dict[str, Any]) -> pd.DataFrame:
    ...
    if len(frame) != int(manifest.get("rows", -1)):
        raise AmountParticipationEntropyError("candidate frame row count changed")
...
def uniqueness_audit(
    ...
    """Load six terminal values only after coverage passes and gate synonyms."""

    print("coverage passed; loading six terminal comparison factors", flush=True)
    efficiency_verification = foundation.verify_snapshot_files(
        efficiency_manifest, efficiency_manifest_path, workers
    )
    recovery_verification = previous.verify_snapshot_files(
    ...
    table = dataset.to_table(
        columns=["trade_date", "symbol", *previous.BASE_COMPARISON_FACTORS],
        use_threads=True,
    )
    ...
    recovery = previous.load_candidate_frame(
        recovery_manifest_path, recovery_manifest
    )[["trade_date", "symbol", previous.FACTOR_NAME]]
    efficiency["symbol"] = efficiency["symbol"].astype(str)
    recovery["symbol"] = recovery["symbol"].astype(str)
    ...
    all_passed = len(results) == 6 and all(item["gate_passed"] for item in results)
    return {
        ...
        "all_six_comparisons_passed": all_passed,
    }


def _find_existing_audit(experiment_root: Path, manifest_sha256: str) -> Path | None:
    for path in sorted(
        experiment_root.glob("*_intraday_amount_participation_entropy_no_return_audit.json")
    ):
        record = research.load_json_record(path)
        if (
            record.get("kind")
            == "a_share_tushare_intraday_amount_participation_entropy_no_return_audit"
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    """Run coverage/capacity and conditional six-factor uniqueness."""
    ...
    if not (
        ...
        raise AmountParticipationEntropyError("candidate snapshot identity is rejected")
    ...
    uniqueness: dict[str, Any] = {
        ...
        "all_six_comparisons_passed": False,
        "comparisons": [],
    }
    ...
    passed = bool(
        ...
        and uniqueness["all_six_comparisons_passed"]
    )
    status = (
    ...
    audit = {
        "schema_version": 1,
        "kind": "a_share_tushare_intraday_amount_participation_entropy_no_return_audit",
        "status": status,
        "created_at": dt.datetime.now(dt.timezone.utc).isoformat(),
        "purpose": "ordered_candidate_coverage_capacity_then_six_terminal_factor_uniqueness_without_prices_or_forward_returns",
        "preregistration": {
    ...
    path = (
        experiment_root
        / f"{run_id}_intraday_amount_participation_entropy_no_return_audit.json"
    )
...
def load_diagnostic_preregistration(
    ...
    _require_file(
        path,
        DIAGNOSTIC_PREREGISTRATION_SHA256,
        "amount-entropy diagnostic protocol",
    )
    spec = research.load_json_record(
        path,
        kind="a_share_tushare_intraday_amount_participation_entropy_diagnostic_preregistration",
    )
    ...
    if not (
        ...
        == "8d34ac56836fe1cc22157ac07f29e6a3ef94429008376e22e851730992b3891d"
        ...
        and snapshot.get("entropy_range_violation_rows") == 0
        ...
        == 0.10844868387614219
        and uniqueness.get("all_six_comparisons_passed") is True
        ...
        raise AmountParticipationEntropyError(
            "amount-entropy diagnostic protocol no longer matches its frozen definition"
        )
    return spec
...
def validate_diagnostic_source_chain(
    ...
    if not (
        ...
        is True
        and (audit.get("uniqueness") or {}).get("all_six_comparisons_passed")
        is True
...
def require_diagnostic_unconsumed(experiment_root: Path) -> None:
    ...
    if marker.exists():
        raise AmountParticipationEntropyError(
            f"amount-entropy historical diagnostic is already consumed: {marker}"
        )
    for path in sorted(experiment_root.glob("*_factor_diagnostic.json")):
        record = research.load_json_record(path)
        if record.get("purpose") == DIAGNOSTIC_PURPOSE:
            raise AmountParticipationEntropyError(
                f"amount-entropy historical diagnostic already exists: {path}"
            )
...
def run_diagnostic(args: argparse.Namespace) -> Path:
    ...
    marker = {
        "kind": "a_share_tushare_intraday_amount_participation_entropy_historical_consumption",
    ...
    diagnostic = {
        ...
        "strategy_timing": {
            ...
            "signal_time": "full-session amount factor known after signal-session close",
