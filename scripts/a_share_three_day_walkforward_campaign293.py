#!/usr/bin/env python3
"""Run Campaign293's frozen Alpha158 multi-horizon smoothness campaign."""
...
from scripts import a_share_three_day_walkforward_campaign292 as prior


REPO_ROOT = Path(__file__).resolve().parents[1]
PROTOCOL_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_preregistration_20260825.json"
)
ORIGINAL_IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_implementation_freeze_20260825.json"
)
IMPLEMENTATION_FREEZE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_implementation_freeze_v2_20260825.json"
)
CONCEPT_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_concept_scouting_20260825.json"
)
PATH_PROBE_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_recovery_worktree_candidate49_path_probe_failure_20260825.json"
)
OUTPUT_PATH_FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_293_output_path_failure_20260825.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign293.py"
)
...
C292_MANIFEST_PATH = prior.CANDIDATE_MANIFEST_PATH
SIGNAL_LEDGER_PATH = prior.SIGNAL_LEDGER_PATH
EXECUTION_LEDGER_PATH = prior.EXECUTION_LEDGER_PATH
OUTPUT_ROOT = (
    REPO_ROOT
    / ".local-research/campaign_293/"
    "alpha158_multi_horizon_peer_rank_smoothness_v2"
)
...
FACTOR_NAME = "alpha158_multi_horizon_peer_rank_smoothness_29g"
TRIAL_ID = "wf293_alpha158_multi_horizon_peer_rank_smoothness_29g_single_higher"
CAMPAIGN290_FACTOR = prior.CAMPAIGN290_FACTOR
CAMPAIGN291_FACTOR = prior.CAMPAIGN291_FACTOR
CAMPAIGN292_FACTOR = prior.FACTOR_NAME
FEATURE_COUNT = 158
...
MINIMUM_COMPLETE_FAMILIES = 22
EXPECTED_COMPARATOR_COUNT = 146
...
class Campaign293Error(RuntimeError):
    """Fail closed when a frozen Campaign293 invariant changes."""
...
def load_json(path: Path) -> dict[str, Any]:
    try:
        ...
        raise Campaign293Error(f"JSON binding unreadable: {path}") from exc
    if not isinstance(value, dict):
        raise Campaign293Error(f"JSON binding is not an object: {path}")
    return value


def require_file(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or file_sha256(path) != expected:
        raise Campaign293Error(f"{label} fingerprint changed")


def family_names() -> list[list[str]]:
    return [[f"{prefix}{horizon}" for horizon in HORIZONS] for prefix in FAMILY_PREFIXES]


def family_indices(feature_names: Sequence[str]) -> np.ndarray:
    positions = {name: index for index, name in enumerate(feature_names)}
    groups = family_names()
    if len(groups) != 29 or any(name not in positions for group in groups for name in group):
        raise Campaign293Error("Alpha158 multi-horizon family inventory changed")
    indices = np.asarray([[positions[name] for name in group] for group in groups])
    if len(np.unique(indices)) != 145:
        raise Campaign293Error("Alpha158 multi-horizon family coordinates changed")
    return indices
...
def validate_protocol() -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    if not (
        protocol.get("kind") == "a_share_three_day_walkforward_campaign293_preregistration"
        and protocol.get("status")
        == "fully_frozen_before_campaign293_candidate_comparator_daily_price_or_forward_return_values"
        ...
        and parameters.get("minimum_complete_families") == MINIMUM_COMPLETE_FAMILIES
        and parameters.get("family_support") == "all five horizons finite"
        ...
        and uniqueness.get("all_146_must_pass") is True
        ...
        and boundary.get("campaign293_candidate_or_comparator_values_read_before_protocol")
        is False
        ...
        raise Campaign293Error("Campaign293 protocol semantics changed")
    ...
    require_file(SIGNAL_LEDGER_PATH, str(candidate49["signal_ledger_sha256"]), "signal ledger")
    require_file(
    ...
    if not (
        ...
        raise Campaign293Error("Campaign293 source manifest semantics changed")
    family_indices(list(alpha["feature_names"]))
    for label, path, factor_name in (
        ...
        if not (
            ...
            and [int(item["year"]) for item in manifest.get("files", [])] == list(YEARS)
        ):
            raise Campaign293Error(f"{label} comparator semantics changed")
...
def validate_implementation_freeze() -> dict[str, Any]:
    ...
    if not (
        freeze.get("kind")
        == "a_share_three_day_walkforward_campaign293_implementation_freeze_v2"
        and freeze.get("status")
        == "scientifically_identical_output_path_refrozen_before_v2_return_values"
        ...
        and frozen.get("path_probe_failure_sha256")
        == file_sha256(PATH_PROBE_FAILURE_PATH)
        and frozen.get("output_path_failure_sha256")
        == file_sha256(OUTPUT_PATH_FAILURE_PATH)
        ...
        and frozen.get("family_count") == len(FAMILY_PREFIXES)
        and frozen.get("minimum_complete_families") == MINIMUM_COMPLETE_FAMILIES
        ...
        raise Campaign293Error("Campaign293 implementation freeze changed")
    return freeze


def session_peer_rank_smoothness(
    *,
    keys: np.ndarray,
    matrix: np.ndarray,
    ...
    if not (
        ...
        and groups.shape == (len(FAMILY_PREFIXES), len(HORIZONS))
        ...
        raise Campaign293Error("Alpha158 session support semantics changed")
    score_by_row = np.full(len(keys), np.nan, dtype=np.float64)
    complete_by_row = np.zeros(len(keys), dtype=np.uint8)
    ...
    if len(supported):
        ...
        family_values = percentiles[:, groups]
        complete = np.isfinite(family_values).all(axis=2)
        complete_count = complete.sum(axis=1)
        segment_differences = np.abs(np.diff(family_values, axis=2))
        segment_differences[~complete] = np.nan
        valid = complete_count >= MINIMUM_COMPLETE_FAMILIES
        supported_scores = np.full(len(supported), np.nan, dtype=np.float64)
        supported_scores[valid] = 1.0 - np.nanmean(
            segment_differences[valid], axis=(1, 2)
        )
        score_by_row[model_support] = supported_scores
        complete_by_row[model_support] = complete_count.astype(np.uint8)
    selected = np.flatnonzero(quality_listing)
    scores = score_by_row[selected]
    complete_counts = complete_by_row[selected]
    eligible = (
        model_support[selected]
        & (complete_counts >= MINIMUM_COMPLETE_FAMILIES)
        & np.isfinite(scores)
    )
    return pd.DataFrame(
        {
            ...
            FACTOR_NAME: scores,
            "complete_horizon_family_count": complete_counts,
...
def build_candidate_snapshot(
    ...
    if output_root.exists():
        raise Campaign293Error("candidate output root already exists; refuse overwrite")
    feature_names = list(alpha["feature_names"])
    groups = family_indices(feature_names)
    columns = [
        "stock_day_key",
        *feature_names,
    ...
    totals = {
        ...
        "complete_family_count_minimum_eligible": len(FAMILY_PREFIXES),
        "complete_family_count_maximum_eligible": 0,
    }
    for year in YEARS:
        ...
        observed_sessions = 0
        for _, frame in prior.base.iter_session_frames(source_path, columns):
            output = session_peer_rank_smoothness(
        ...
        if len(np.unique(annual["stock_day_key"])) != len(annual):
            raise Campaign293Error(f"candidate annual keys duplicated: {year}")
        ...
        counts = annual["complete_horizon_family_count"].to_numpy(dtype=np.uint8)
        totals["quality_listing_rows"] += len(annual)
        totals["candidate_eligible_rows"] += int(eligible.sum())
        if eligible.any():
            totals["complete_family_count_minimum_eligible"] = min(
                totals["complete_family_count_minimum_eligible"],
                int(counts[eligible].min()),
            )
            totals["complete_family_count_maximum_eligible"] = max(
                totals["complete_family_count_maximum_eligible"],
    ...
    if [item["sessions"] for item in records] != [244, 243, 243, 242, 242]:
        raise Campaign293Error("candidate session counts changed")
    manifest = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign293_candidate_snapshot",
        "status": "immutable_candidate_ready_for_coverage_before_comparator_values",
        "created_at": prior.base.utc_now(),
        "factor_name": FACTOR_NAME,
        "direction": "higher",
        "formula_parameters": {
            "family_count": len(FAMILY_PREFIXES),
            "ordered_family_prefixes": list(FAMILY_PREFIXES),
            ...
            "score": "1-mean absolute adjacent-horizon peer-percentile difference",
        },
...
def load_candidate_snapshot(
    ...
    for record in manifest.get("files", []):
        ...
        keys = frame["stock_day_key"].to_numpy(dtype=np.int64)
        values = pd.to_numeric(frame[FACTOR_NAME], errors="coerce").to_numpy(dtype=np.float64)
        ...
        if not (
            ...
            raise Campaign293Error("candidate snapshot values changed")
    ...
    if len(np.unique(keys)) != len(keys) or np.any(keys[1:] < keys[:-1]):
        raise Campaign293Error("candidate snapshot key order changed")
...
def run_ordered_uniqueness(
    *, numeric_manifest: Mapping[str, Any], candidate_manifest: Mapping[str, Any]
    ...
    if len(definitions) != EXPECTED_COMPARATOR_COUNT:
        raise Campaign293Error("comparator name order changed")
    results: list[dict[str, Any]] = []
    receipts: list[dict[str, Any]] = []
    failed_ordinal: int | None = None
    for ordinal, name in enumerate(definitions[:140], start=1):
        result = prior.base.numeric140_comparison(
            name=name,
        ...
        print(json.dumps({"ordinal": ordinal, "factor": name, "absolute_median": result["absolute_median_daily_rank_correlation"], "passed": result["gate_passed"]}), flush=True)
        if result["gate_passed"] is not True:
            failed_ordinal = ordinal
            break
    snapshot_specs = [
        (C136_MANIFEST_PATH, load_json(C136_MANIFEST_PATH), definitions[140], 0.0, None, "campaign136_verified_snapshot"),
        (C146_MANIFEST_PATH, load_json(C146_MANIFEST_PATH), definitions[141], -1.0, 1.0, "campaign146_verified_snapshot"),
        (C263_MANIFEST_PATH, load_json(C263_MANIFEST_PATH), definitions[142], 0.0, 1.0, "campaign263_verified_snapshot"),
    ]
    if failed_ordinal is None:
        for ordinal, (path, manifest, factor, minimum, maximum, source) in enumerate(snapshot_specs, start=141):
            aligned, receipt = prior.base.fill_snapshot_aligned(
                manifest_path=path,
                manifest=manifest,
                factor=factor,
            ...
            rows = prior.base.daily_rank_rows(candidate_keys, candidate_values, aligned)
            result = prior.base.comparison_result(factor, rows, source)
            results.append(result)
            receipts.append({"ordinal": ordinal, **receipt, "source": source})
            print(json.dumps({"ordinal": ordinal, "factor": factor, "absolute_median": result["absolute_median_daily_rank_correlation"], "passed": result["gate_passed"]}), flush=True)
            if result["gate_passed"] is not True:
                failed_ordinal = ordinal
                break
            del aligned
    ...
    if failed_ordinal is None:
        ...
        for ordinal, (manifest_path, factor, source) in enumerate(prior_specs, start=144):
            result, receipt = prior.prior_campaign_comparison(
            ...
            if result["gate_passed"] is not True:
                failed_ordinal = ordinal
                break
    absolute = [float(item["absolute_median_daily_rank_correlation"]) for item in results if item["absolute_median_daily_rank_correlation"] is not None]
    all_passed = failed_ordinal is None and len(results) == EXPECTED_COMPARATOR_COUNT
    return {
        "status": "all_146_uniqueness_gates_passed" if all_passed else "ordered_uniqueness_failed_closed",
        ...
        "failed_factor": results[-1]["comparison_factor"] if failed_ordinal is not None else None,
        "all_146_passed": all_passed,
        "maximum_observed_absolute_median_daily_rank_correlation": max(absolute) if absolute else None,
        "results": results,
...
def evaluate_validation(
    ...
    dates, instruments = prior.base.decode_keys(keys)
    identities = pd.DataFrame({"trade_date": dates, "instrument": instruments})
    market, calendar = prior.base.engine.load_market_context(
        prior.base.model_context(), validation_end, validation_start, batch_size
    )
    schedule = prior.base.engine.global_signal_schedule(calendar)
    factors = identities.copy()
    factors[prior.base.engine.factor_score_column(TRIAL_ID)] = scores
    panel = prior.base.engine.build_signal_panel(market, factors, schedule)
    quotes = prior.base.engine.quote_lookup(market)
    return prior.base.engine.evaluate_trial_period(
        panel,
        quotes,
        calendar,
        schedule,
...
def write_trial_ledger(*, no_return: Mapping[str, Any], terminal: Mapping[str, Any]) -> dict[str, Any]:
    ...
    failure = load_json(PATH_PROBE_FAILURE_PATH)
    entries.append(
        {
            "attempt_id": failure["attempt_id"],
            "phase": "infrastructure_failure",
            "name": "recovery_worktree_candidate49_and_test_path_probe",
            "outcome": "corrected_from_frozen_path_bindings",
            "reason": failure["observed_failure"],
            "failure_record_sha256": file_sha256(PATH_PROBE_FAILURE_PATH),
            "historical_forward_return_values_read": False,
        }
    )
    entries.append(
        {
            "attempt_id": "campaign293_factor_001",
            "phase": "coverage_uniqueness_and_development",
            "name": FACTOR_NAME,
            "formula": "One minus mean absolute adjacent-horizon peer-percentile difference across complete Alpha158 operator families.",
            "direction": "higher",
            "parameters": {
                "family_count": len(FAMILY_PREFIXES),
                "ordered_horizons": list(HORIZONS),
                "minimum_complete_families": MINIMUM_COMPLETE_FAMILIES,
                "weights": "equal family segments",
                "fit": "none",
            },
            "filters": "quality/listing and model support; all five horizons per included family",
            ...
            "outcome": "survived" if terminal.get("survivor_count") == 1 else "terminated",
            "historical_forward_return_values_read": bool(terminal.get("development_return_values_read")),
        }
    )
    chained = chain_entries(entries)
    ledger = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign293_terminal_trial_ledger",
...
def verify_source_files(alpha: Mapping[str, Any], numeric: Mapping[str, Any]) -> None:
    prior.verify_annual_source_files(alpha, numeric)
    manifest = load_json(C292_MANIFEST_PATH)
    for year in YEARS:
        record = record_for_year(manifest, year)
        path = resolve_record(C292_MANIFEST_PATH, record)
        require_file(path, str(record["sha256"]), f"Campaign292 partition {year}")
        required = {"stock_day_key", CAMPAIGN292_FACTOR, f"{CAMPAIGN292_FACTOR}_eligible"}
        if not required.issubset(pq.read_schema(path).names):
            raise Campaign293Error(f"Campaign292 partition schema changed: {year}")


def run_campaign(*, batch_size: int) -> dict[str, Any]:
    if batch_size != 8192:
        raise Campaign293Error("Campaign293 batch size changed")
    protocol, alpha, numeric = validate_protocol()
    validate_implementation_freeze()
    verify_source_files(alpha, numeric)
    if OUTPUT_ROOT.exists():
        raise Campaign293Error("Campaign293 output root already exists; refuse overwrite")
    candidate_manifest = build_candidate_snapshot(alpha)
    keys, values, eligible, _ = load_candidate_snapshot(candidate_manifest)
    coverage = prior.base.coverage_result(keys, values, eligible, protocol)
    uniqueness = None
    if coverage["gate_passed"]:
        uniqueness = run_ordered_uniqueness(
            numeric_manifest=numeric, candidate_manifest=candidate_manifest
        )
    all_unique = bool(uniqueness and uniqueness["all_146_passed"])
    no_return = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign293_no_return_audit",
        "status": "passed_ready_for_exact_one_no_fit_development_trial" if all_unique else "failed_closed_before_historical_daily_price_or_forward_return_values",
        "created_at": prior.base.utc_now(),
        "protocol_sha256": file_sha256(PROTOCOL_PATH),
        "candidate_manifest": {"path": str(CANDIDATE_MANIFEST_PATH), "sha256": file_sha256(CANDIDATE_MANIFEST_PATH), "dataset_sha256": candidate_manifest["dataset_sha256"]},
        "coverage": coverage,
    ...
    if all_unique:
        ...
        for fold in protocol["development_trial"]["walkforward_folds"]:
            ...
            binding = {
                "schema_version": 1,
                "kind": "a_share_three_day_walkforward_campaign293_prevalidation_scores",
                ...
                "candidate_partition_sha256": next(item["sha256"] for item in candidate_manifest["files"] if int(item["year"]) == year),
            ...
            prevalidation_bindings.append({"path": str(binding_path), "sha256": file_sha256(binding_path)})
            validation_metrics.append(evaluate_validation(fold, candidate_keys, candidate_scores, batch_size=batch_size))
            gc.collect()
    decision = prior.base.survivor_decision(validation_metrics)
    survivor_count = int(decision.get("passed") is True)
    terminal = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign293_terminal_result",
        "status": "development_survivor_frozen_lockbox_closed_pending_new_source_freeze" if survivor_count else "terminal_no_survivor_lockbox_closed",
        "created_at": prior.base.utc_now(),
        "protocol_sha256": file_sha256(PROTOCOL_PATH),
        "implementation_freeze_sha256": file_sha256(IMPLEMENTATION_FREEZE_PATH),
        ...
        "lockbox_2024_2025_returns_open": False,
        "lockbox_reason": "Alpha158 2024-2025 source is absent and must be separately completed and frozen" if survivor_count else "zero development survivors or no-return gate failed",
        "definition_library_append_eligible": all_unique,
        "prior_definition_count": 165,
        "prior_numeric_comparator_count": 146,
    ...
    terminal["terminal_trial_ledger"] = {"path": str(TRIAL_LEDGER_PATH), "sha256": file_sha256(TRIAL_LEDGER_PATH), "entry_count": ledger["entry_count"], "terminal_entry_sha256": ledger["terminal_entry_sha256"]}
    atomic_json(TERMINAL_RESULT_PATH, terminal)
    return terminal
...
def verify_terminal() -> dict[str, Any]:
    ...
    for record in manifest.get("files", []):
        require_file(resolve_record(CANDIDATE_MANIFEST_PATH, record), str(record["sha256"]), f"candidate output {record['year']}")
    previous = CHAIN_GENESIS
    for ordinal, entry in enumerate(ledger.get("entries", []), start=1):
        body = dict(entry)
        observed = str(body.pop("entry_sha256"))
        if not (int(body.get("ordinal", 0)) == ordinal and body.get("previous_entry_sha256") == previous and canonical_sha256(body) == observed):
            raise Campaign293Error("terminal trial ledger chain changed")
        previous = observed
    if not (
        terminal.get("kind") == "a_share_three_day_walkforward_campaign293_terminal_result"
        and terminal.get("lockbox_2024_2025_returns_open") is False
        and terminal.get("candidate49_ledgers_changed") is False
        and (terminal.get("no_return_audit") or {}).get("sha256") == file_sha256(NO_RETURN_PATH)
        and (terminal.get("terminal_trial_ledger") or {}).get("sha256") == file_sha256(TRIAL_LEDGER_PATH)
        and ledger.get("terminal_entry_sha256") == previous
        and no_return.get("historical_daily_price_or_forward_return_values_read") is False
        and file_sha256(SIGNAL_LEDGER_PATH) == terminal.get("candidate49_signal_ledger_sha256")
        and file_sha256(EXECUTION_LEDGER_PATH) == terminal.get("candidate49_execution_ledger_sha256")
    ):
        raise Campaign293Error("Campaign293 terminal semantics changed")
    return {"status": "verified_campaign293_terminal", "survivor_count": terminal["survivor_count"], "validation_fold_count": terminal["validation_fold_count"], "no_return_status": no_return["status"], "trial_ledger_entries": ledger["entry_count"], "lockbox_2024_2025_returns_open": False, "candidate49_ledgers_changed": False}
...
def plan() -> dict[str, Any]:
    ...
    validate_implementation_freeze()
    schemas = []
    for manifest_path, manifest, label in ((ALPHA158_MANIFEST_PATH, alpha, "alpha158"), (NUMERIC140_MANIFEST_PATH, numeric, "numeric140")):
        for year in YEARS:
            record = record_for_year(manifest, year)
            path = resolve_record(manifest_path, record)
            if not path.is_file():
                raise Campaign293Error(f"{label} annual partition missing: {year}")
            schemas.append({"source": label, "year": year, "columns": len(pq.read_schema(path).names)})
    return {
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command == "run":
        if not args.confirm_run:
            raise Campaign293Error("run requires --confirm-run")
        print(json.dumps(run_campaign(batch_size=args.batch_size), ensure_ascii=False, sort_keys=True, indent=2))
        return 0
