#!/usr/bin/env python3
"""Run Campaign074's frozen coverage-first, no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign073_no_return_audit as base
from scripts import a_share_three_day_walkforward_campaign074_features as candidate
...
DEFAULT_EXPERIMENT_ROOT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_074/no_return"
)
SNAPSHOT_MANIFEST_PATH = candidate.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
SNAPSHOT_MANIFEST_SHA256 = "5a993ce8b71c541a0b2d2402627d5f2b7f562cf1266bebcafe0606945086a3cd"
SNAPSHOT_DATASET_SHA256 = "d4fbb9eb8135674df16a50c96b5817f63e6461e99ee48df96537cd8ed930a4b5"
SNAPSHOT_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_074_feature_snapshot_binding_20260806.json"
)
SNAPSHOT_BINDING_SHA256 = "1c404dde2e3f6856dde88c2ba7c95b45c9bbc5ea34dc6bed130662fa5154eb81"
AUDIT_IMPLEMENTATION_FREEZE = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_074_no_return_audit_implementation_freeze_20260806.json"
)
AUDIT_ACTIVATION_BINDING = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_074_no_return_audit_activation_binding_20260806.json"
)
TEST_PATH = (
    REPO_ROOT
    / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign074_no_return_audit.py"
)
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 7_724_498
EXPECTED_COMPARISON_COUNT = 104
EXPECTED_COMPLETE_DEFINITION_COUNT = 105
...
C73_SNAPSHOT_MANIFEST_PATH = c73.output_root(DEFAULT_DATA_ROOT) / "snapshot_manifest.json"
STRUCTURALLY_NONNUMERIC_FACTOR = (
    "intraday_cross_sectional_standardized_return_state_stability_236p"
)


class Campaign074NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign074 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign074NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign074NoReturnAuditError("Campaign074 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign074NoReturnAuditError("audit implementation freeze is absent")
    record = json.loads(AUDIT_IMPLEMENTATION_FREEZE.read_text(encoding="utf-8"))
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign074_no_return_audit_implementation_freeze"
        ...
        raise Campaign074NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign074NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign074_no_return_audit_activation_binding"
        ...
        raise Campaign074NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(C73_SNAPSHOT_MANIFEST_PATH, candidate.C73_MANIFEST_SHA256, "Campaign073 snapshot")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign074NoReturnAuditError("snapshot binding validation failed")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    c73_manifest = json.loads(C73_SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    if not (
        ...
        and c73_manifest.get("dataset_sha256") == candidate.C73_DATASET_SHA256
        and len(candidate.reconstruct_comparisons()) == EXPECTED_COMPARISON_COUNT
    ):
        raise Campaign074NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    ...
    expected = {FACTOR_NAME: (0.0, 1.0), candidate.C73_FACTOR: (0.0, 1.0)}
    for name, value_range in expected.items():
        if name in ranges and tuple(ranges[name]) != value_range:
            raise Campaign074NoReturnAuditError(f"conflicting range for {name}")
...
def _load_comparisons_after_coverage(
    *,
    ...
    result, receipt = base._append_snapshot_comparison(
        manifest_path=C73_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C73_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=c73.verify_snapshot_files,
    )
    comparisons.append(result)
    receipts["campaign073_snapshot"] = receipt
    receipts.pop("all_103_sources_loaded_in_frozen_order", None)
    receipts["all_104_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign074NoReturnAuditError("Campaign074 comparison count changed")
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root != DEFAULT_DATA_ROOT.resolve():
        raise Campaign074NoReturnAuditError("Campaign074 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign074_no_return_audit.json")):
        raise Campaign074NoReturnAuditError("Campaign074 no-return audit already exists")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    if coverage["gate_passed_before_comparison_values"]:
        ...
        uniqueness = {
            ...
            "campaign068_to_campaign073_comparator_values_read": False,
            "comparisons": [],
    ...
    run_id = f"{timestamp}_campaign074_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign074_no_return_audit",
        ...
        "next_action": "freeze and run the exact one Campaign074 development trial"
        if admitted
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        experiment_root.expanduser().resolve().glob("*_campaign074_no_return_audit.json")
    )
    return {
