#!/usr/bin/env python3
"""Run Campaign077's frozen coverage-first no-return uniqueness audit."""
...
from scripts import a_share_three_day_walkforward_campaign076_no_return_audit as c76_audit
from scripts import a_share_three_day_walkforward_campaign077_features as candidate
...
SNAPSHOT_MANIFEST_SHA256 = "2449976e3bc95b66668496cee513277da21eff301677969f17053baf36fa18c8"
SNAPSHOT_DATASET_SHA256 = "2b40ea3cb07a75b8dbddae124da86d32578120b74e616ff4ada5446d77010fc9"
SNAPSHOT_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_077_feature_snapshot_binding_20260806.json"
SNAPSHOT_BINDING_SHA256 = "e26726129d1abeafdbe6333a083ac4a83e5ca37197d589f2b3cfac8ee03c4e96"
AUDIT_IMPLEMENTATION_FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_077_no_return_audit_implementation_freeze_20260806.json"
AUDIT_ACTIVATION_BINDING = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_077_no_return_audit_activation_binding_20260806.json"
TEST_PATH = REPO_ROOT / "tests/data_collector_tests/test_a_share_three_day_walkforward_campaign077_no_return_audit.py"
EXPECTED_ROWS = 7_724_498
EXPECTED_PARTITIONS = 33_015
EXPECTED_ELIGIBLE_ROWS = 7_724_498
EXPECTED_COMPARISON_COUNT = 107
EXPECTED_COMPLETE_DEFINITION_COUNT = 108
...
C76_SNAPSHOT_MANIFEST_PATH = candidate.C76_MANIFEST_PATH
STRUCTURALLY_NONNUMERIC_FACTOR = "intraday_cross_sectional_standardized_return_state_stability_236p"


class Campaign077NoReturnAuditError(RuntimeError):
    """Fail-closed Campaign077 no-return audit error."""
...
def _require(path: Path, expected: str, label: str) -> None:
    if not path.is_file() or _sha256(path) != expected:
        raise Campaign077NoReturnAuditError(f"{label} changed: {path}")
...
def load_protocol() -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign077NoReturnAuditError("Campaign077 comparison order changed")
...
def _load_implementation_freeze() -> dict[str, Any]:
    if not AUDIT_IMPLEMENTATION_FREEZE.is_file():
        raise Campaign077NoReturnAuditError("audit implementation freeze is absent")
    ...
    if not (
        ...
        raise Campaign077NoReturnAuditError("audit implementation freeze changed")
    return record
...
def _load_activation_binding() -> dict[str, Any]:
    ...
    if not AUDIT_ACTIVATION_BINDING.is_file():
        raise Campaign077NoReturnAuditError("audit activation binding is absent")
    record = json.loads(AUDIT_ACTIVATION_BINDING.read_text(encoding="utf-8"))
    snapshot = record.get("candidate_snapshot") or {}
    if not (
        record.get("kind")
        == "a_share_three_day_walkforward_campaign077_no_return_audit_activation_binding"
        ...
        raise Campaign077NoReturnAuditError("audit activation binding changed")
    return record
...
def verify_candidate_snapshot(*, workers: int = 4) -> dict[str, Any]:
    ...
    if not (
        ...
        raise Campaign077NoReturnAuditError("candidate snapshot verification changed")
    return result


def verify_campaign076_snapshot(*, workers: int = 4) -> dict[str, Any]:
    return c76_audit.verify_candidate_snapshot(C76_SNAPSHOT_MANIFEST_PATH, workers=workers)
...
def verify_static_bindings() -> dict[str, Any]:
    ...
    _require(SNAPSHOT_MANIFEST_PATH, SNAPSHOT_MANIFEST_SHA256, "candidate snapshot")
    _require(C76_SNAPSHOT_MANIFEST_PATH, candidate.C76_MANIFEST_SHA256, "Campaign076 snapshot")
    ...
    if report.get("all_bindings_passed") is not True:
        raise Campaign077NoReturnAuditError("snapshot binding validation failed")
    ...
    if not (
        ...
        raise Campaign077NoReturnAuditError("static snapshot identity changed")
    return {
...
def _install_frozen_ranges(engine: Any) -> None:
    c76_audit._install_frozen_ranges(engine)
    ranges = dict(getattr(engine, "FACTOR_RANGES", {}))
    if FACTOR_NAME in ranges and tuple(ranges[FACTOR_NAME]) != (0.0, 1.0):
        raise Campaign077NoReturnAuditError("conflicting Campaign077 range")
...
def _load_comparisons_after_coverage(
    *,
    ...
    comparisons, receipts = c76_audit._load_comparisons_after_coverage(
        coverage=coverage,
    ...
    helper = c76_audit.c75_v5.v4.v3.v1.base.base._append_snapshot_comparison
    result, receipt = helper(
        manifest_path=C76_SNAPSHOT_MANIFEST_PATH,
        factor=candidate.C76_FACTOR,
        candidate_keys=candidate_keys,
        candidate_values=candidate_values,
        gate=gate,
        engine=engine,
        comparison_engine=comparison_engine,
        workers=workers,
        verifier=lambda _path, workers=workers: verify_campaign076_snapshot(
            workers=workers
        ),
    )
    comparisons.append(result)
    receipts["campaign076_snapshot"] = receipt
    receipts.pop("all_106_sources_loaded_in_frozen_order", None)
    receipts["all_107_sources_loaded_in_frozen_order"] = True
    if len(comparisons) != EXPECTED_COMPARISON_COUNT:
        raise Campaign077NoReturnAuditError("Campaign077 comparison count changed")
...
def run_no_return_audit(
    *, data_root: Path, experiment_root: Path, workers: int
    ...
    if data_root.expanduser().resolve() != DEFAULT_DATA_ROOT.resolve():
        raise Campaign077NoReturnAuditError("Campaign077 data root changed")
    experiment_root = experiment_root.expanduser().resolve()
    if list(experiment_root.glob("*_campaign077_no_return_audit.json")):
        raise Campaign077NoReturnAuditError("Campaign077 no-return audit already exists")
    manifest = json.loads(SNAPSHOT_MANIFEST_PATH.read_text(encoding="utf-8"))
    ...
    run_id = f"{timestamp}_campaign077_no_return_audit"
    record = {
        "schema_version": 1,
        "kind": "a_share_three_day_walkforward_campaign077_no_return_audit",
        ...
        "next_action": "freeze and run the exact one Campaign077 development trial"
        if admitted
...
def status(experiment_root: Path = DEFAULT_EXPERIMENT_ROOT) -> dict[str, Any]:
    audits = sorted(
        experiment_root.expanduser().resolve().glob("*_campaign077_no_return_audit.json")
    )
    return {
