"""Current terminal and reporting tests for Campaign059."""
...
RECORD = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_research_record.json"
LEDGER = REPO_ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_059/research_attempt_ledger_v2.json"
FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_terminal_completion_freeze_20260804.json"
SUPERSESSION = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_unified_report_supersession_20260804.json"
...
def test_campaign059_current_records_have_live_bindings() -> None:
    paths = (
...
def test_campaign059_result_and_append_only_accounting_are_exact() -> None:
    record = _load(RECORD)
    no_return = record["no_return_result"]
    development = record["development_result"]
    accounting = record["append_only_attempt_accounting"]
    ledger = _load(LEDGER)
    assert no_return["comparison_factor_count"] == 90
    assert no_return["uniqueness_gate_passed"] is True
    assert no_return["maximum_absolute_median_daily_rank_correlation"] == (
        0.7148903759224717
    )
    assert [item["mean_rank_ic"] for item in development["validation_folds"]] == [
        -0.010330795383273883,
        0.013325929435016594,
        0.01056626775172599,
    ]
    assert [
        item["normalized_return"] for item in development["validation_folds"]
    ] == [
        0.2197368198808547,
        -0.37502953669720074,
    ...
    assert [
        item["pilot_10bp_return"] for item in development["validation_folds"]
    ] == [
        0.020980317053472586,
        -0.07662733787933296,
        -0.011706923718404427,
    ]
    assert development["development_aggregate"]["pilot_20bp_return"] == (
        -0.13017742332606042
    )
    assert development["development_survivor_count"] == 0
    assert record["decision"]["2024_2025_stress_opened"] is False
    assert record["decision"]["2024_2025_returns_read"] is False
    assert ledger["campaign059_ledger_entry_count"] == 5
    assert ledger["campaign059_attempt_count"] == 4
    assert ledger["campaign059_infrastructure_only_failure_count"] == 3
    assert ledger["campaign059_complete_factor_attempt_count"] == 1
    assert accounting["cumulative_historical_research_attempt_count"] == 337
    assert accounting["cumulative_return_reading_development_trial_count"] == 269


def test_campaign059_current_status_is_post_audit_and_stress_closed() -> None:
...
def test_campaign059_reports_and_generator_are_current(tmp_path: Path) -> None:
    assert _sha256(REPORT) == (
        "21f11ef3ad2e78cc7937dc227884d11da0c8a41c603afc06f4ec00eb9c9ecf1e"
    )
    assert _sha256(PIPELINE_DOC) == (
        "e76b54562babbbcc269199cb13d61c3bbac190fff2f26e5f8c70dd6507b08053"
    )
    assert _sha256(SKILL) == (
        "9cc859cd7f14b1bd3944dcdc92e7321b93738b9388fe8cf8db719361a51e35db"
    )
...
def test_campaign059_preserves_candidate49_isolation() -> None:
