...
from scripts import a_share_three_day_walkforward_campaign085 as campaign
...
def test_preregistration_bindings_and_single_trial_catalog() -> None:
    ...
    assert report["record_sha256"] == (
        "fd2bf11241f1051c5c676271393d1df6b1a61359d626f4bc5d1e02853c99c908"
    )
...
def test_pre_retry_status_preserves_infrastructure_failure_and_closed_stress() -> None:
    ...
    assert result["campaign_id"] == "a_share_three_day_walkforward_campaign_085"
    assert result["expected_trial_count"] == 1
    assert result["ledger_entry_count"] == 1
    assert result["ledger_chain_tip_sha256"] == (
        "852a4625c5735f91cc3b3f22b408dc602dc0327615ea28f7a38723ee3d55fb93"
    )
    ledger = json.loads((OUTPUT_ROOT / "trial_ledger.json").read_text(encoding="utf-8"))
    assert [entry["phase"] for entry in ledger["entries"]] == ["infrastructure_failure"]
    assert not [entry for entry in ledger["entries"] if entry["phase"] == "development"]
...
def test_admission_and_output_boundaries_remain_closed() -> None:
