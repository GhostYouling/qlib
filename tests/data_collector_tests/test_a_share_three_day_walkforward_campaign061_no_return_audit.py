...
def test_protocol_reconstructs_exact_92_factor_order() -> None:
...
def test_campaign060_terminal_snapshot_identity_is_frozen() -> None:
    manifest = json.loads(audit.C60_SNAPSHOT_PATH.read_text(encoding="utf-8"))
    assert audit._local_sha256(audit.C60_SNAPSHOT_PATH) == audit.C60_SNAPSHOT_SHA256
    assert manifest["dataset_sha256"] == audit.C60_DATASET_SHA256
    assert manifest["factor_names"] == [audit.C60_FACTOR]


def test_audit_identities_have_no_sentinels() -> None:
    audit._require_finalized_identities()
    assert audit.EXPECTED_ELIGIBLE_ROWS == 7_671_540
