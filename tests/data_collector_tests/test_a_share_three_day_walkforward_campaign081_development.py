...
from scripts import a_share_three_day_walkforward_campaign081 as campaign


ROOT = Path(__file__).resolve().parents[2]
FREEZE = ROOT / "docs/a_share_three_day_walkforward_campaign_081_development_implementation_freeze_20260806.json"
...
def test_development_preregistration_bindings_and_single_trial_catalog() -> None:
    ...
    assert digest == "8bd457d19de554c3e4cd19d258c0fe2294a3e735a867f88265d05ad64f89fe6a"
...
def test_predevelopment_status_has_zero_ledger_and_closed_stress() -> None:
    status = campaign.status(
        argparse.Namespace(
            ...
            output_root=str(
                ROOT
                / "data/experiments/short_horizon/historical_walkforward/campaign_081/walkforward"
            ),
        )
    )
    assert status["campaign_id"] == "a_share_three_day_walkforward_campaign_081"
...
def test_development_implementation_freeze_is_live() -> None:
    ...
    assert record["status"] == "frozen_before_campaign081_2019_2023_development_return_read"
