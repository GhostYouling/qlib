...
def test_campaign117_gate_and_protocol_are_frozen() -> None:
...
def test_campaign117_synthetic_panel_passes_coverage_and_variation() -> None:
...
def test_campaign117_constant_cross_section_fails_variation_only() -> None:
    candidate, keys = _synthetic_panel(sessions=1300)
...
def test_campaign117_invalid_or_undeclared_values_fail_closed() -> None:
    candidate, keys = _synthetic_panel(sessions=10)
    candidate.loc[0, audit.FACTOR_NAME] = 2.0
    with pytest.raises(audit.Campaign117NoReturnAuditError):
