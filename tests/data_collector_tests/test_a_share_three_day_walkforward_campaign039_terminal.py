"""Post-result immutable evidence tests for terminal Campaign039."""
...
import scripts.a_share_three_day_walkforward_campaign039 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
CAMPAIGN_SHA256 = (
    "328bf78dacea06224b731b17dae5fbfd2ebe014e38325f0889402e03e16b3861"
)


def test_campaign039_status_is_terminal_with_closed_stress() -> None:
...
def test_campaign039_terminal_record_preserves_all_rejections() -> None:
    root = (
        REPO_ROOT
        / "data/experiments/short_horizon/historical_walkforward/campaign_039"
        / "walkforward"
    )
    record_path = (
        REPO_ROOT
        / "docs/a_share_three_day_walkforward_campaign_039_research_record.json"
    )
...
def test_unified_report_contains_campaign039_terminal_result() -> None:
    report = (
        REPO_ROOT
        / "data/experiments/short_horizon/three_day_research_report.md"
    ).read_text()
    assert "## 历史滚动 Campaign039 权威追加" in report
    assert "最大绝对中位日秩相关为 0.308938" in report
    assert (
        "零成本、5bp、10bp、20bp 整手收益为 "
        "+1.311107%/-3.852654%/-7.934397%/-16.734396%"
    ) in report
    assert "累计历史开发试验推进到 259" in report
