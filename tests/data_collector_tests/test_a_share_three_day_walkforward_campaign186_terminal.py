...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_186_shipping_fleet_charter_offhire_disposal_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_186/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_186_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_186_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v275_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign186_bindings_and_finite_frontier() -> None:
    for path in (FRONTIER, TERMINAL, POLICY):
        record = _load(path)
...
def test_campaign186_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_vessel_fleet_orderbook_charter_offhire_drydock_disposal_factor_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1642
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign186_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v274"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v274"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 187
    assert policy["future_campaign_boundary"][
        "2024_2025_source_or_returns_remain_closed"
    ]
    assert (
        policy["deferred_source_boundary"][
            "successful_delivery_continuous_charter_or_profitable_disposal_cases_may_be_selected_without_cancellations_offhire_failures_and_losses"
        ]
        is False
    )
    assert (
...
def test_campaign186_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign186 航运船队与租船履约来源前沿值前终止"
    ...
    for path in (
        ...
        assert "Campaign186 最终有效会计为 7 次科学尝试、0 次基础设施失败" in text


def test_campaign186_records_use_reached_wall_clock_timestamps() -> None:
    for path in (FRONTIER, LEDGER, TERMINAL, POLICY):
