...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_170_customer_experience_quality_kpi_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_170/research_attempt_ledger.json"
)
EFFECTIVE_LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_170/research_attempt_ledger_v2.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_170_terminal_result_v2_20260815.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_170_terminal_report_v2_20260815.md"
)
FAILURE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_170_report_append_failure_20260815.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v248_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign170_bindings_and_finite_frontier() -> None:
    ...
    assert summary["incident_or_operator_route_rejections"] == 3
    assert summary["selected_candidate_count"] == 0


def test_campaign170_source_gate_and_effective_attempt_chain() -> None:
    ...
    for ledger_path in (LEDGER, EFFECTIVE_LEDGER):
    ...
    assert effective["campaign_effective_entry_count"] == 8
    assert effective["campaign_effective_prevalue_scientific_attempt_count"] == 7
    assert effective["campaign_effective_infrastructure_failure_attempt_count"] == 1
    assert effective["cumulative_historical_research_attempt_count"] == 1495
    assert effective["cumulative_return_reading_development_trial_count"] == 314
    assert _load(FAILURE)["write_result"]["atomic_patch_applied"] is False


def test_campaign170_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v247"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v247"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 171
...
def test_campaign170_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign170 客户体验、质量与留存 KPI 来源前沿值前终止"
    assert "8 次尝试（7 科学、1 基础设施）" in REPORT.read_text(encoding="utf-8")
    for report_path in (
        ...
        assert "Campaign170 有效会计为 8 次尝试（7 科学、1 基础设施）" in text


def test_campaign170_records_use_reached_wall_clock_timestamps() -> None:
    for record_path in (
        FRONTIER,
        LEDGER,
        EFFECTIVE_LEDGER,
