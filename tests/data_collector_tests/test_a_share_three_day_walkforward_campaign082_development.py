...
from scripts import a_share_three_day_walkforward_campaign082 as campaign


ROOT = Path(__file__).resolve().parents[2]
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_082_development_implementation_freeze_20260806.json"
)
...
def test_development_preregistration_bindings_and_single_trial_catalog() -> None:
    ...
    assert digest == "a8950c130a579ce85a6a36f1f95c2094db63e905e01926fa8ce01efb5827295d"
...
def test_predevelopment_status_has_zero_ledger_and_closed_stress() -> None:
    status = campaign.status(
        argparse.Namespace(
            ...
            output_root=str(
                ROOT
                / "data/experiments/short_horizon/historical_walkforward/campaign_082/walkforward"
            ),
        )
    )
    assert status["campaign_id"] == "a_share_three_day_walkforward_campaign_082"
...
def test_development_implementation_freeze_is_live() -> None:
    ...
    assert (
        record["status"]
        == "frozen_before_campaign082_2019_2023_development_return_read"
    )
