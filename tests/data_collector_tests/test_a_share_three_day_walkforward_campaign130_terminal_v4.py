...
LEDGER_V4_PATH = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_130/research_attempt_ledger_v4.json"
)
RESULT_V4_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_130_terminal_result_v4_20260814.json"
)
POLICY_V151_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v151_20260814.json"
)
FAILURE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_campaign_130_legacy_report_boundary_test_failure_20260814.json"
)
REPORT_PATHS = [
...
def test_v4_appends_legacy_report_test_failure_without_scientific_change() -> None:
    ...
    assert v4["chain_tip_sha256"] == entry["entry_sha256"]
...
def test_v4_result_v151_reports_and_boundaries_are_current() -> None:
    ledger = load_json(LEDGER_V4_PATH)
    result = load_json(RESULT_V4_PATH)
    policy = load_json(POLICY_V151_PATH)
    expected = {
        "campaign130_attempt_count": 7,
        "campaign130_ledger_entry_count": 7,
        "campaign130_infrastructure_failure_count": 6,
        "campaign130_prevalue_scientific_attempt_count": 1,
        "cumulative_historical_research_attempt_count": 1064,
    ...
    assert result["authoritative_inputs"]["campaign130_attempt_ledger_v4"][
        "sha256"
    ] == sha256(LEDGER_V4_PATH)
    assert policy["authoritative_inputs"]["campaign130_terminal_result_v4"][
        "sha256"
    ] == sha256(RESULT_V4_PATH)
    assert policy["version"] == 151
    assert (
