"""Pre-return tests for the single frozen Campaign065 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign065 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "c6384ebd8c37de2f6ec918156a72a61aac6d20e37610517ffa723476b5dd8d07"
)
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_065/no_return/"
    "20260805T065707Z_campaign065_no_return_audit.json"
)


def test_campaign065_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign065_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign065_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "0e04aec29c4cd4b2bced6ba57d6556350a0fba4318de3e339d63ce8ef19401c0"
    )
    assert audit["status"] == (
...
def test_campaign065_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "scripts.a_share_three_day_walkforward_campaign065",
            "status",
        ],
        cwd=REPO_ROOT,
        check=True,
...
def test_campaign065_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            ...
            "docs/a_share_three_day_walkforward_campaign_065_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
