...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_232_commercial_parking_"
    "reservation_access_occupancy_charging_payment_enforcement_incident_"
    "maintenance_turnover_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_232"
...
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_232_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v353_20260816.json"
)
...
def test_campaign232_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c232_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger_v3["cumulative_historical_research_attempt_count"] == 2186
    assert ledger_v3["cumulative_return_reading_development_trial_count"] == 314


def test_campaign232_reports_candidate49_and_research_boundaries():
    heading = "## Campaign232 商业停车预订、进出占用、收费结算、违规处置与设施周转生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign232_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2186`" in campaign232_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign232_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign232_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 233
