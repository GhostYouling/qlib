...
from scripts import a_share_three_day_walkforward_campaign081_features as features
from scripts import a_share_tushare_candidate49_future_session_workflow as future_workflow


ROOT = Path(__file__).resolve().parents[2]
STATE = ROOT / "docs/a_share_three_day_iteration_status_20260806_campaign081_verified.json"
RECORD = ROOT / "docs/a_share_three_day_walkforward_campaign_081_research_record.json"
POLICY = ROOT / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v17_20260806.json"
FREEZE = ROOT / "docs/a_share_three_day_walkforward_campaign_081_terminal_completion_freeze_20260806.json"
TRIAL = ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_081/walkforward/trial_ledger.json"
SURVIVORS = ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_081/walkforward/development_survivors.json"
STRESS = ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_081/walkforward/exposed_stress_consumption_record.json"
ATTEMPTS = ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_081/research_attempt_ledger_v3.json"
REPORT = ROOT / "docs/a_share_three_day_walkforward_campaign_081_report.md"
...
def test_state_and_terminal_freeze_bindings_are_live() -> None:
    ...
    state = _load(STATE)
    assert state["status"] == "campaign081_terminal_verified_zero_survivors"
    assert state["campaign081_result"]["development_survivor_count"] == 0
    assert state["campaign081_result"]["stress_2024_2025_opened"] is False


def test_research_record_matches_frozen_no_return_and_development_results() -> None:
    assert bindings.validate_record(RECORD)["all_bindings_passed"] is True
    record = _load(RECORD)
    assert record["no_return_admission"]["uniqueness"]["comparisons_passed"] == 110
...
def test_trial_survivor_stress_and_attempt_records_are_terminal() -> None:
    trial = _load(TRIAL)
    ...
    assert attempts["campaign081_infrastructure_failure_count"] == 7
    assert attempts["campaign081_return_reading_development_trial_count"] == 1
    assert attempts["cumulative_historical_research_attempt_count"] == 499
    assert attempts["cumulative_return_reading_development_trial_count"] == 282


def test_v17_complete_and_numeric_orders_reconstruct_exactly() -> None:
...
def test_report_states_terminal_result_without_current_action() -> None:
    ...
    assert "110/110" in report
    assert "−16.21%" in report
    assert "−23.07%" in report
