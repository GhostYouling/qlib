...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_253_commercial_laundry_"
    "equipment_service_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_253"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_253_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v385_20260816.json"
)
...
def test_campaign253_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c253_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 112
    assert not frontier["repository_evidence"][
        "dedicated_commercial_laundry_equipment_service_definition_present"
    ]
    assert (
        frontier["repository_evidence"][
            "commercial_laundry_equipment_coin_operated_laundry_equipment_laundromat_equipment_stack_washer_stack_dryer_tumble_dryer_or_washing_machine_service_exact_phrase_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    assert frontier["repository_evidence"][
        "adjacent_generic_laundry_washer_or_dryer_mentions_present"
    ]
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2413
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign253_reports_candidate49_and_research_boundaries():
    heading = "## Campaign253：商用洗衣设备服务生命周期值前终止（2026-08-16）"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign253_section = report[report.index(heading) :]
        assert "累计历史尝试为 2413" in campaign253_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign253_section
        assert (
    ...
    assert result["effective_accounting"]["campaign_attempt_count"] == 7
    assert (
        policy["effective_accounting"]["campaign253_infrastructure_failure_count"] == 0
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 254
    ...
    assert len(carried) == 46
    assert (
        carried[-1]
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign252_terminal.py::test_campaign252_reports_candidate49_and_research_boundaries"
    )
    assert policy["cumulative_regression_boundary"]["deselection_node_count"] == 68
