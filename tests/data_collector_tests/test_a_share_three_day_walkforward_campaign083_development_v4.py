...
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_083_development_implementation_freeze_v4_20260806.json"
)
OUTPUT_ROOT = (
    ROOT
...
def test_repair_status_preserves_two_failures_zero_complete_trials_and_closed_stress() -> (
    None
):
    ...
    assert (
        ...
        == "0b91948fff668b1c11f728e95e1e3dd416760c818427287f827e96484ee0b215"
    )
    ...
    assert [entry["trial_id"] for entry in ledger["entries"]] == [
        "wf083_infrastructure__development_load_001",
        "wf083_infrastructure__development_load_002",
    ]
    assert all(
        entry["development_aggregate_metrics"] == {} for entry in ledger["entries"]
    )
    assert all(
        entry["candidate49_historical_return_read"] is False
...
def test_development_implementation_freeze_v4_is_live() -> None:
    record = json.loads(FREEZE.read_text(encoding="utf-8"))
    assert (
        record["status"]
        == "frozen_before_campaign083_conda_compatible_full_development_retry"
    )
