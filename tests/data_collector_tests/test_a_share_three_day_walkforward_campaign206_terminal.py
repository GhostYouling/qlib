...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_206_waste_management_"
    "environmental_services_customer_collection_transfer_sorting_recycling_"
    "treatment_disposal_manifest_billing_compliance_lifecycle_source_"
    "frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_206"
    / "research_attempt_ledger_v1.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_206_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v307_20260815.json"
)
...
def test_campaign206_bindings_frontier_safety_and_append_only_chain():
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_issuer_as_waste_management_or_environmental_services_provider_lifecycle_frontier_found"
        ]
        is False
    )
    assert (
        frontier["research_boundary"][
            "customer_identity_address_precise_route_hazardous_material_security_location_identifiable_manifest_or_sensitive_facility_control_record_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1889
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign206_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign206 废弃物管理与环境服务客户收运、分选回收、处理处置和合规结算"
        "生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert "Campaign206 最终有效会计为 9 次尝试（7 科学、2 基础设施）" in report
        assert "累计历史尝试 `1889`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 207
