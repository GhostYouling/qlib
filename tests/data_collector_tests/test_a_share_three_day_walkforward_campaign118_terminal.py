...
from scripts import a_share_three_day_walkforward_campaign118_features as features


ROOT = Path(__file__).resolve().parents[2]
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_118_terminal_result_20260813.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v107_20260813.json"
)
ATTEMPTS = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_118/research_attempt_ledger_v2.json"
)
WALKFORWARD = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_118/walkforward"
)
...
def test_frozen_rejection_reasons_and_metrics_are_preserved() -> None:
    ...
    assert decision["development_aggregate_pilot_return_20bp"] == -0.158867992390459
    assert decision["worst_validation_normalized_drawdown"] == -0.25634789928169766
...
def test_attempt_ledger_advances_once_without_opening_stress() -> None:
    ...
    assert attempts["attempt_count"] == 16
    assert attempts["infrastructure_failure_count"] == 8
    ...
    assert attempts["cumulative_historical_research_attempt_count"] == 914
    assert attempts["cumulative_return_reading_development_trial_count"] == 304
...
def test_v107_appends_definition_and_numeric_comparator_once() -> None:
    policy = _load(POLICY)
    ...
    assert (
        policy["complete_historical_feature_library"]["factor_definition_count"] == 144
    )
...
def test_unified_reports_record_campaign118_terminal_result() -> None:
    for relative in (
        "data/experiments/short_horizon/current_research_report.md",
        "data/experiments/short_horizon/three_day_research_report.md",
        "docs/a_share_three_day_walkforward_campaign_118_terminal_report.md",
    ):
        text = (ROOT / relative).read_text(encoding="utf-8")
        assert "Campaign118" in text
        assert "-15.886799%" in text
