...
RESEARCH = ROOT / "docs/a_share_three_day_walkforward_campaign_083_research_record.json"
POLICY = (
    ROOT
...
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_083_terminal_completion_freeze_20260806.json"
)
STATE = (
    ROOT / "docs/a_share_three_day_iteration_status_20260806_campaign083_verified.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_083/research_attempt_ledger_v11.json"
)
...
def test_attempt_accounting_and_candidate49_boundaries() -> None:
    ...
    assert ledger["campaign083_attempt_count"] == 22
    assert ledger["campaign083_infrastructure_failure_count"] == 21
    assert ledger["campaign083_return_reading_development_trial_count"] == 1
    assert accounting["cumulative_historical_research_attempts"] == 528
