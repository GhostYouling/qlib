...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_209_freight_parcel_logistics_"
    "shipment_booking_waybill_pickup_sortation_linehaul_last_mile_delivery_"
    "return_claim_billing_settlement_lifecycle_source_frontier_20260815.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_209"
    / "research_attempt_ledger_v1.json"
)
LEDGER_V2 = LEDGER_V1.with_name("research_attempt_ledger_v2.json")
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_209_terminal_result_v2_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v316_20260815.json"
)
...
def test_campaign209_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c209_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert len(entries) == ledger_v2["effective_entry_count"] == 13
    ...
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 1933
    assert ledger_v2["cumulative_return_reading_development_trial_count"] == 314


def test_campaign209_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign209 货运快递包裹下单、运单、揽收、分拣、干线、末端投递、"
        "退回与理赔结算生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 210
