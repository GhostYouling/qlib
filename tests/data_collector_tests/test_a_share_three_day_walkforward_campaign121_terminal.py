...
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_121_terminal_result_20260814.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v117_20260814.json"
)
ATTEMPTS = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_121/research_attempt_ledger_v20.json"
)
WALKFORWARD = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_121/walkforward"
)
...
def test_frozen_rejection_reasons_and_metrics_are_preserved() -> None:
    ...
    assert decision["development_aggregate_pilot_return_20bp"] == -0.1718500339925697
    assert decision["worst_validation_normalized_drawdown"] == -0.2761310248574681
...
def test_attempt_ledger_advances_once_without_opening_stress() -> None:
    ...
    assert attempts["attempt_count"] == 36
    assert attempts["infrastructure_failure_count"] == 26
    ...
    assert attempts["cumulative_historical_research_attempt_count"] == 992
...
def test_v117_preserves_v116_definition_and_numeric_comparator_append() -> None:
    policy = _load(POLICY)
...
def test_unified_reports_record_campaign121_terminal_result() -> None:
    for relative in (
        "data/experiments/short_horizon/current_research_report.md",
        "data/experiments/short_horizon/three_day_research_report.md",
        "docs/a_share_three_day_walkforward_campaign_121_terminal_report.md",
    ):
        text = (ROOT / relative).read_text(encoding="utf-8")
        assert "Campaign121" in text
        assert "-17.185003%" in text
