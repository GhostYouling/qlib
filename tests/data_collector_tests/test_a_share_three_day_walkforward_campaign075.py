"""Pre-return tests for the single frozen Campaign075 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign075 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "945c4b3a062cd85342955f72e1fc6f22f5b5d9928177504fab17832a548e5015"
)
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_075/no_return/"
    "20260806T015958Z_campaign075_no_return_audit.json"
)


def test_campaign075_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign075_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign075_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "83fcd1907e1aaa56bae9bd03e4051025167402bc20bf46a4bfdad800cc57646b"
    )
...
def test_campaign075_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "scripts.a_share_three_day_walkforward_campaign075", "status"],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_campaign075_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            ...
            "docs/a_share_three_day_walkforward_campaign_075_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
