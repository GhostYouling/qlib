...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_202_education_provider_student_"
    "application_admission_enrollment_course_delivery_progression_completion_"
    "tuition_aid_refund_collection_lifecycle_source_frontier_20260815.json"
)
LEDGERS = tuple(
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_202"
    / f"research_attempt_ledger_v{version}.json"
    for version in range(1, 2)
)
LEDGER = LEDGERS[-1]
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_202_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v300_20260815.json"
)
...
def test_campaign202_bindings_frontier_privacy_and_append_only_chain():
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_issuer_as_education_provider_student_and_tuition_"
    ...
    assert (
        frontier["research_boundary"][
            "directly_identifiable_or_protected_education_student_record_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1844
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign202_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign202 教育服务提供者申请入学、课程进展、毕业与学费资助结算"
        "生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert "Campaign202 最终有效会计为 11 次尝试（7 科学、4 基础设施）" in report
        assert "累计历史尝试 `1844`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 203
