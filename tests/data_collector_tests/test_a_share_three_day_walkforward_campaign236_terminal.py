...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_236_commercial_janitorial_"
    "contract_cleaning_scope_shift_task_inspection_consumables_quality_"
    "incident_billing_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_236"
    / "research_attempt_ledger_v3.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_236_terminal_result_v3_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v360_20260816.json"
)
...
def test_campaign236_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c236_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2231
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign236_reports_candidate49_and_research_boundaries():
    heading = "## Campaign236 商业保洁合同、清洁范围、班次任务、质检、耗材、异常与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign236_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2231`" in campaign236_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign236_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign236_infrastructure_failure_count"] == 6
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 237
