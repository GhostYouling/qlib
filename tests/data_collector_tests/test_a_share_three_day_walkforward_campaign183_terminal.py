...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_183_ip_license_technology_transfer_royalty_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_183/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_183_terminal_result_v2_20260815.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_183_terminal_report_v2_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v272_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign183_bindings_and_finite_frontier() -> None:
...
def test_campaign183_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_ip_agreement_filing_patent_family_royalty_factor_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1616
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign183_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v271"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v271"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 184
    assert policy["future_campaign_boundary"][
        "2024_2025_source_or_returns_remain_closed"
    ]
    assert (
        policy["deferred_source_boundary"][
            "successful_milestone_renewal_settlement_or_commercialization_cases_may_be_selected_without_failures_terminations_and_withdrawals"
        ]
        is False
    )
...
def test_campaign183_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign183 知识产权许可与技术转让来源前沿值前终止"
    assert "最终有效会计为 8 次尝试（7 科学、1 基础设施）" in REPORT.read_text(
        encoding="utf-8"
    )
    for report_path in (
        ...
        assert "Campaign183 共 7 次科学尝试、0 次基础设施失败" in text
        assert text.count("### Campaign183 累计回归节点前缀失败会计追加") == 1
        assert "Campaign183 最终有效会计为 8 次尝试（7 科学、1 基础设施）" in text


def test_campaign183_records_use_reached_wall_clock_timestamps() -> None:
