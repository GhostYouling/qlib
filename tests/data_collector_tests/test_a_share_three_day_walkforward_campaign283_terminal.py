...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v3.json"
)
STATE = (
    ROOT / "docs/a_share_three_day_iteration_status_20260825_campaign283_terminal.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_283_terminal_prevalue_report.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign283.md"
...
def test_catalog_freezes_seven_routes_before_targeted_inventory() -> None:
    ...
    assert [route["concept_id"] for route in routes] == [
        "public_insolvency_restructuring_administrator_plan_confirmation_discharge_state",
        "public_cross_border_data_security_assessment_transfer_authorization_state",
        "public_education_accreditation_enrollment_capacity_restoration_state",
        "public_hazardous_material_transport_route_escort_clearance_state",
        "public_dam_reservoir_safety_drawdown_recommissioning_state",
        "public_protected_species_trade_quota_seizure_release_state",
        "public_postal_universal_service_route_suspension_restoration_state",
    ]
    for route in routes:
        for field in (
            "formula",
...
def test_result_closes_all_routes_before_formula_direction_or_values() -> None:
    ...
    assert decision["campaign283_terminalized"] is True


def test_effective_attempt_chain_has_exact_accounting() -> None:
    assert _sha256(LEDGER) == (
        "437db9d75f3381dfa22182156f6ded7f04172e7f886d1ad87fa9680c8ee25093"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2890
...
def test_reports_and_handoff_publish_one_terminal_result() -> None:
    ...
    assert "正式破产/法院监督重整" in handoff
    assert "累计历史尝试 2,890" in handoff
    assert "持续目标保持 `active`" in handoff
    assert "不构成投资建议" in report
    title = "## Campaign283：破产来源缺口与六条机制重叠值前终止（2026-08-25）"
    for path in UNIFIED_REPORTS:
        text = path.read_text(encoding="utf-8")
        assert text.count(title) == 1
        section = text.split(title, maxsplit=1)[1]
        assert "累计历史尝试 2,890" in section
        assert "Candidate49 保持唯一前瞻候选且账本 0/0" in section
        assert "Campaign284" in section


def test_library_and_historical_boundaries_remain_frozen() -> None:
    state = _load(STATE)
    library = state["library_state"]
    assert library["changed_by_campaign283"] is False
