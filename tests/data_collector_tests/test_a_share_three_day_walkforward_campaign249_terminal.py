...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_249_commercial_vehicle_wash_"
    "equipment_service_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_249"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_249_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v381_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_249_git_status_output_"
    "truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_249_wrong_unified_report_"
    "paths_hash_failure_20260816.json",
]
...
def test_campaign249_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c249_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 108
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_vehicle_wash_equipment_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "commercial_vehicle_wash_car_wash_wash_bay_gantry_wash_tunnel_wash_in_bay_wash_water_reclaim_chemical_dosing_or_wash_equipment_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2378
...
def test_campaign249_reports_candidate49_and_research_boundaries():
    heading = "## Campaign249：商用车辆清洗设备服务生命周期值前终止（2026-08-16）"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign249_section = report[report.index(heading) :]
        assert "累计历史尝试为 2378" in campaign249_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign249_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign249_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 250
    ...
    assert len(carried) == 42
    assert (
        carried[-1]
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign248_terminal.py::test_campaign248_reports_candidate49_and_research_boundaries"
    )
    assert policy["cumulative_regression_boundary"]["deselection_node_count"] == 64
