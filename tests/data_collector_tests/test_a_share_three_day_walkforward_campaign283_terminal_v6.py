...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v8.json"
)
STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260825_campaign283_terminal_v6.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_terminal_prevalue_report_v6.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign283_v6.md"
...
FAILURES = (
    ...
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_v4_unified_next_campaign_marker_failure_20260825.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_v5_cross_stage_deselect_prefix_failure_20260825.json",
)
UNIFIED = (
...
def test_effective_v8_ledger_replays_all_entries() -> None:
    assert (
        _sha(LEDGER)
        == "53ee6056f4263dc459c942c4e51f34836340039dc0fe97303c287f99b4a4fdd9"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2895
    assert ledger["cumulative_return_reading_development_trial_count"] == 315


def test_v6_reports_and_unified_accounting_correction() -> None:
    report = REPORT.read_text(encoding="utf-8")
    handoff = HANDOFF.read_text(encoding="utf-8")
    assert "累计历史尝试 2,895" in report
    assert "持续目标保持 `active`" in handoff
    assert "不构成投资建议" in report
    for report_path in UNIFIED:
        text = report_path.read_text(encoding="utf-8")
        assert text.count("### Campaign283 验证会计更正 v5") == 1
        correction = text.split("### Campaign283 验证会计更正 v5", maxsplit=1)[1]
        assert "累计历史尝试 2,895" in correction
        assert "Campaign284" in correction


def test_seven_failures_are_value_free_and_not_consumed() -> None:
...
def test_v6_state_binds_current_artifacts_and_active_goal() -> None:
    state = _load(STATE)
    ...
    assert state["effective_accounting"]["campaign283_attempt_count"] == 14
    assert (
        state["effective_accounting"]["campaign283_infrastructure_failure_count"] == 7
    )
