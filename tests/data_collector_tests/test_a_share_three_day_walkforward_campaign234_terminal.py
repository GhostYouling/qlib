...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_234_commercial_pest_"
    "management_contract_inspection_detection_treatment_followup_compliance_"
    "materials_billing_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_234"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_234_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v356_20260816.json"
)
...
def test_campaign234_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c234_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2210
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign234_reports_candidate49_and_research_boundaries():
    heading = "## Campaign234 商业虫害管理合同、巡检监测、处理复查、合规与物料生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign234_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2210`" in campaign234_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign234_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign234_infrastructure_failure_count"] == 6
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 235
