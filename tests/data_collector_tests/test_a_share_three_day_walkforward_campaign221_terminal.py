...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_221_testing_inspection_"
    "certification_sample_test_calibration_certificate_lifecycle_source_"
    "frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_221"
...
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_221_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v335_20260816.json"
)
...
def test_campaign221_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c221_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 2062
    assert ledger_v2["cumulative_return_reading_development_trial_count"] == 314


def test_campaign221_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    for key, path in {
        ...
        campaign221_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2062`" in campaign221_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign221_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign221_infrastructure_failure_count"] == 3
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 222
