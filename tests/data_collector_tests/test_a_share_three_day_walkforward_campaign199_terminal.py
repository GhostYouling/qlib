...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_199_service_support_case_"
    "entitlement_ticket_backlog_response_resolution_escalation_sla_lifecycle_"
    "source_frontier_20260815.json"
)
LEDGER = ROOT / (
    "data/experiments/short_horizon/historical_walkforward/campaign_199/"
    "research_attempt_ledger_v1.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_199_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v293_20260815.json"
)
...
def test_campaign199_bindings_frontier_and_append_only_chain():
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_service_support_case_entitlement_ticket_sla_lifecycle_"
            "frontier_found"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1806
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign199_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    for key, file_path in {
        ...
        assert "Campaign199 最终有效会计为 12 次尝试（7 科学、5 基础设施）" in report
        assert "累计历史尝试 `1806`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 200
