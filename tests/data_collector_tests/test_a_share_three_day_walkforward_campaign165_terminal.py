...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_165_environmental_operating_externality_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_165/research_attempt_ledger.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_165_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_165_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v236_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign165_all_authoritative_bindings_resolve() -> None:
...
def test_campaign165_frontier_is_finite_and_distinguishes_deferral() -> None:
    ...
    assert (
        frontier["scientific_interpretation"][
            "environmental_operating_externality_declared_ineffective"
        ]
        is False
    )


def test_campaign165_source_gate_and_research_boundary_are_closed() -> None:
    frontier = _load(FRONTIER)
    ...
    assert (
        boundary[
            "annual_esg_sustainability_document_pdf_table_source_row_candidate_comparator_price_or_return_value_read"
        ]
        is False
    )
...
def test_campaign165_attempt_chain_and_accounting_are_exact() -> None:
    ledger = _load(LEDGER)
    predecessor = ledger["authoritative_predecessor"]
    assert _sha(ROOT / predecessor["path"]) == predecessor["sha256"]
    assert predecessor["cumulative_historical_research_attempt_count"] == 1443
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1449
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign165_preserves_library_orders_and_future_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v235"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v235"]
    boundary = policy["future_campaign_boundary"]
    assert boundary["next_campaign"] == 166
...
def test_campaign165_candidate49_and_prospective_boundaries_hold() -> None:
...
def test_campaign165_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign165 环境经营外部性来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign165 共 6 次科学尝试、0 次基础设施失败" in text


def test_campaign165_records_use_reached_wall_clock_timestamps() -> None:
