...
from scripts import (
    a_share_three_day_walkforward_campaign152_ordered_uniqueness as audit,
)
...
def test_candidate_panel_accepts_positive_unbounded_values() -> None:
    frame = pd.DataFrame(
        {
            "trade_date": pd.to_datetime(["2023-01-03", "2023-01-03"]),
            "symbol": ["SH600000", "SZ000001"],
            audit.candidate.FACTOR_NAME: [18.5, 23.0],
            f"{audit.candidate.FACTOR_NAME}_eligible": [True, True],
        }
    )
    keys = frame[["trade_date", "symbol"]].copy()
    result = audit.eligible_candidate_panel(frame, keys)
    assert result[audit.candidate.FACTOR_NAME].tolist() == [18.5, 23.0]
...
def test_source_snapshot_comparison_uses_intersection_without_fill() -> None:
    ...
    candidate_values = np.arange(len(source), dtype=np.float64) % 47 + 10.0
...
def test_output_path_is_campaign_scoped() -> None:
    assert "campaign_152/uniqueness" in str(audit.OUTPUT_PATH)
