...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_175_enterprise_lease_commitment_source_frontier_20260815.json"
)
FAILURE_PATH_GUESS = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_175_authority_policy_path_guess_failure_20260815.json"
)
FAILURE_PATTERN = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_175_policy_filename_pattern_failure_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_175/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_175_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_175_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v263_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign175_bindings_and_finite_frontier() -> None:
    for record_path in (
        FRONTIER,
        FAILURE_PATH_GUESS,
        FAILURE_PATTERN,
...
def test_campaign175_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_filing_document_lease_liability_right_of_use_asset_facility_candidate_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1550
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign175_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v262"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v262"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 176
...
def test_campaign175_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign175 企业租赁承诺来源前沿值前终止"
    assert "9 次尝试（7 次科学、2 次基础设施）" in REPORT.read_text(encoding="utf-8")
    for report_path in (
        ...
        assert "Campaign175 共 9 次尝试（7 科学、2 基础设施）" in text


def test_campaign175_records_use_reached_wall_clock_timestamps() -> None:
    for record_path in (
        FRONTIER,
        FAILURE_PATH_GUESS,
        FAILURE_PATTERN,
