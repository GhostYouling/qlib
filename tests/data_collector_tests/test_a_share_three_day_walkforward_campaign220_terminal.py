...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_220_renewable_power_grid_"
    "interconnection_generation_certificate_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_220"
    / "research_attempt_ledger_v2.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_220_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v334_20260816.json"
)
...
def test_campaign220_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c220_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2052
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign220_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    for key, path in {
        ...
        assert "累计历史尝试改为 `2052`" in report
    ...
    assert (
        policy["effective_accounting"]["campaign220_infrastructure_failure_count"] == 1
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 221
