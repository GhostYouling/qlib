...
def test_three_day_iteration_status_binds_latest_tushare_minute_terminal_result():
    ...
    assert len(terminal) == 42
    assert len({item["mechanism"] for item in terminal}) == 42
    ...
    assert "前沿之后另有 42 条机制已到达终止门禁" in report
    assert "当前禁止聚合、评分、选股、定仓和下单" in report
    assert "Tushare 原五因子来源覆盖门仍未通过" in report
    assert "字段级清洗层保留 4 个因子并全部通过覆盖门" in report
    assert "前四因子和之后十三个完成收益诊断的独立分钟机制均没有产生双门禁合格因子" in report
    assert "终点收盘位置另在无收益近同义门停止" in report
    assert "日内收益方差熵通过覆盖和十七因子唯一性门" in report
    assert "0.49458" in report
    assert "平均 Rank IC 为 +0.02085" in report
    assert "七个年份均为正" in report
    assert "完整关联稳定性门通过" in report
    assert "执行感知累计 +13.66%、最大回撤 -45.27%" in report
    assert "20 万元整手、双边 10bp 滑点累计为 -8.50%" in report
    assert "最大成交额参与率仅 0.0680%" in report
...
def test_eastmoney_related_party_sparsity_diagnostic_is_frozen_before_returns():
    spec = (
        RESEARCH.load_eastmoney_related_party_transaction_sparsity_diagnostic_preregistration()
    )
    assert spec["no_return_evidence"]["potential_complete_cohorts"] == 352
    assert spec["no_return_evidence"]["both_no_return_gates_passed"] is True
    assert spec["no_return_evidence"]["price_fields_loaded"] == []
    assert spec["no_return_evidence"]["forward_return_fields_read"] is False
    assert spec["factor"]["raw_direction"] == "higher_is_better"
    assert spec["factor"]["maximum_event_age_calendar_days"] == 3
    assert spec["run_contract"]["holding_period_trading_days"] == 3
    assert spec["run_contract"]["topk"] == 3
    assert spec["run_contract"]["minimum_valid_names_per_factor_cohort"] == 6
    assert spec["execution_policies"]["pilot_initial_capital_cny"] == 200000
    assert spec["execution_policies"]["buy_lot_size_shares"] == 100
    assert spec["execution_policies"]["commission_rate_each_side"] == 0.0001
    assert spec["execution_policies"]["stamp_duty_rate_sell_only"] == 0.0005
    assert spec["price_fields_loaded"] == []
    assert spec["forward_return_fields_read"] is False
    assert spec["selection_or_promotion_allowed"] is False


def test_eastmoney_related_party_sparsity_diagnostic_guard_precedes_source_reuse(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(
        RESEARCH,
    ...
    consumed = tmp_path / "20260720T000000Z_factor_diagnostic.json"
    consumed.write_text(
        json.dumps(
            {
                "status": "completed",
                "purpose": (
                    RESEARCH.EASTMONEY_RELATED_PARTY_TRANSACTION_SPARSITY_DIAGNOSTIC_PURPOSE
                ),
            }
        ),
        encoding="utf-8",
    ...
    monkeypatch.setattr(
        ...
        lambda: (_ for _ in ()).throw(
            AssertionError("consumption guard must run before the source chain")
        ),
    )
    args = SimpleNamespace(
        provider_uri=str(tmp_path / "provider"),
    ...
    with pytest.raises(ValueError, match="diagnostic is already consumed"):
        RESEARCH.run_eastmoney_related_party_transaction_sparsity_diagnostic(args)


def test_eastmoney_related_party_sparsity_terminal_diagnostic_record_is_frozen():
    assert (
        RESEARCH.file_sha256(
            RESEARCH.DEFAULT_EASTMONEY_RELATED_PARTY_TRANSACTION_SPARSITY_DIAGNOSTIC_RECORD
        )
        == RESEARCH.EASTMONEY_RELATED_PARTY_TRANSACTION_SPARSITY_DIAGNOSTIC_RECORD_SHA256
    )
    record = (
        RESEARCH.load_eastmoney_related_party_transaction_sparsity_diagnostic_record()
    )
    ...
    assert record["gate_decisions"]["prospective_execution_gate_passed"] is False
    assert record["gate_decisions"]["pilot_execution_gate_passed"] is False
...
def test_eastmoney_related_party_sparsity_terminal_record_stops_diagnostic_runner(
    tmp_path,
):
    args = SimpleNamespace(
        provider_uri=str(tmp_path / "provider"),
...
def test_eastmoney_related_party_sparsity_terminal_record_stops_generic_gate_reruns(
    tmp_path,
):
...
def test_eastmoney_monetary_funds_no_return_source_chain_is_frozen():
    spec = (
    ...
    assert spec["capacity_contract"]["maximum_factor_age_calendar_days"] == 3
    assert spec["capacity_contract"]["minimum_required_cohorts"] == 200
    assert spec["uniqueness_contract"]["comparison_factor_count"] == 7
    assert tuple(spec["uniqueness_contract"]["comparison_factors"]) == (
        RESEARCH.EASTMONEY_MONETARY_FUNDS_ASSET_INTENSITY_COMPARISON_FIELDS
    )
    assert record["published_snapshot"]["partition_count"] == 28
    assert record["published_snapshot"]["total_rows"] == 114418
    assert len(events) == 114418
    assert evidence["manifest"]["partitions"] == 28
    assert evidence["comparison_fields_loaded"] == []
    assert evidence["price_fields_loaded"] == []
    assert evidence["forward_return_fields_read"] is False
...
def test_eastmoney_monetary_funds_materialization_uses_announcement_age_and_newest_report():
    ...
    materialized, audit = (
        RESEARCH.materialize_eastmoney_monetary_funds_asset_intensity_sessions(
            events,
            calendar,
            calendar,
    ...
    assert materialized["log1p_monetary_funds"].tolist() == pytest.approx(
        [np.log1p(20.0)] * 3
    )
    assert audit["older_report_rows_blocked_after_newer_report_availability"] == 3
...
def test_eastmoney_monetary_funds_uniqueness_rejects_raw_cash_synonym():
    dates = pd.bdate_range("2025-01-02", periods=6)
    symbols = [f"SZ{index:06d}" for index in range(1, 9)]
    ...
    for date in dates:
        random_values = {
            field: rng.normal(size=len(symbols))
            for field in RESEARCH.EASTMONEY_MONETARY_FUNDS_ASSET_INTENSITY_COMPARISON_FIELDS
        }
        for position, symbol in enumerate(symbols, start=1):
    ...
    independent = (
        RESEARCH.summarize_eastmoney_monetary_funds_asset_intensity_uniqueness(
            candidate, comparison, contract=contract
        )
    )
    assert independent["fields_passing_all_gates"] == 7
    assert independent["uniqueness_gate_passed"] is True
    synonym = comparison.copy()
    ...
    result = next(
        item
        for item in rejected["field_results"]
        if item["comparison_field"] == "log1p_monetary_funds"
    )
...
def test_eastmoney_monetary_funds_capacity_failure_skips_comparisons(
    ...
    spec["local_context"]["holding_universe"]["sha256"] = RESEARCH.file_sha256(
        holding_path
    )
    spec["local_context"]["quarterly_quality"]["path"] = str(quality_path)
    spec["local_context"]["quarterly_quality"]["sha256"] = RESEARCH.file_sha256(
        quality_path
    )
    failed_capacity = {
        "factor": RESEARCH.EASTMONEY_MONETARY_FUNDS_ASSET_INTENSITY_FACTOR_NAME,
        "potential_complete_cohorts": 199,
        "potential_complete_cohorts_by_year": {"2025": 199},
        "observed_calendar_years": 1,
        "capacity_gate_passed": False,
    ...
    calendar = pd.DatetimeIndex(pd.to_datetime(["2025-01-02", "2025-01-03"]))
    monkeypatch.setattr(
        RESEARCH,
        "local_market_capacity_context",
        lambda *args, **kwargs: (
            calendar,
            calendar,
            {"SH600519": [(calendar[0], calendar[-1])]},
        ),
    )
    monkeypatch.setattr(RESEARCH, "load_fundamentals", lambda path: pd.DataFrame())
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: (pd.DataFrame(), {"price_fields_loaded": []}),
    )
    monkeypatch.setattr(
        RESEARCH,
        "eastmoney_monetary_funds_asset_intensity_capacity",
        lambda *args, **kwargs: failed_capacity,
    )
    monkeypatch.setattr(
        RESEARCH,
        "load_eastmoney_monetary_funds_asset_intensity_comparisons",
        lambda *args, **kwargs: (_ for _ in ()).throw(
            AssertionError("comparisons must not load after capacity failure")
        ),
    )
    args = SimpleNamespace(
        manifest=str(tmp_path / "full.json"),
    ...
    assert audit["capacity_gate_passed"] is False
    assert audit["uniqueness"] is None
    assert audit["data"]["comparison_fields_loaded"] == []
    assert audit["price_fields_loaded"] == []
    assert audit["forward_return_fields_read"] is False
...
def test_tushare_cash_conversion_no_return_preregistration_is_fingerprint_frozen():
    assert (
        RESEARCH.file_sha256(RESEARCH.DEFAULT_TUSHARE_CASH_CONVERSION_NO_RETURN_SPEC)
        == RESEARCH.TUSHARE_CASH_CONVERSION_NO_RETURN_SPEC_SHA256
    )
    spec = RESEARCH.load_tushare_cash_conversion_no_return_preregistration()
    assert spec["source_protocol"]["factor"] == (
        RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME
    )
    assert spec["capacity_contract"]["holding_period_trading_days"] == 3
    assert spec["capacity_contract"]["minimum_required_cohorts"] == 200
    assert spec["uniqueness_contract"]["comparison_factor_count"] == 54
...
def test_tushare_cash_conversion_partition_recomputes_formula_and_signal_year():
    ...
    frame = pd.DataFrame(
        [
            {
                "announcement_date": pd.Timestamp("2024-04-29"),
                "income_actual_announcement_date": pd.Timestamp("2024-04-28"),
                "cashflow_actual_announcement_date": pd.Timestamp("2024-04-29"),
                "report_period": pd.Timestamp("2024-03-31"),
                "instrument": "SZ000001",
                "n_income_attr_p": 2.0,
                "n_cashflow_act": -1.0,
                RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME: -0.5,
                "provider": "tushare",
            }
        ],
        columns=RESEARCH.TUSHARE_CASH_CONVERSION_COLUMNS,
    )
    validated = RESEARCH._validate_tushare_cash_conversion_partition(
        frame,
        signal_year=2024,
        calendar=calendar,
    )
    assert validated["event_effective_date"].tolist() == [pd.Timestamp("2024-04-30")]
    changed = frame.copy()
    changed[RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME] = 0.5
    with pytest.raises(ValueError, match="formula"):
        RESEARCH._validate_tushare_cash_conversion_partition(
            changed,
            signal_year=2024,
            calendar=calendar,
        )
...
def test_tushare_cash_conversion_capacity_excludes_same_announcement_collisions():
    full_calendar = pd.bdate_range("2023-10-02", periods=80)
    research_calendar = full_calendar[-10:]
    rebalances = research_calendar[:-3:3]
    symbols = [f"SZ{index:06d}" for index in range(1, 8)]
    intervals = {
        symbol: [(full_calendar[0], research_calendar[-1])] for symbol in symbols
    }
    rows = []
            ...
            rows.append(
                {
                    ...
                    "income_actual_announcement_date": announcement,
                    "cashflow_actual_announcement_date": announcement,
                    "report_period": pd.Timestamp("2023-09-30"),
                    "instrument": symbol,
                    "n_income_attr_p": 10.0,
                    "n_cashflow_act": float(position),
                    RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME: (position / 10.0),
                    "provider": "tushare",
                }
            )
    first_announcement = full_calendar[int(full_calendar.get_loc(rebalances[0])) - 1]
    rows.append(
        {
            "announcement_date": first_announcement,
            "income_actual_announcement_date": first_announcement,
            "cashflow_actual_announcement_date": first_announcement,
            "report_period": pd.Timestamp("2023-06-30"),
            "instrument": symbols[-1],
            "n_income_attr_p": 10.0,
            "n_cashflow_act": 8.0,
            RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME: 0.8,
            "provider": "tushare",
        }
    )
    factor_frame = pd.DataFrame(rows, columns=RESEARCH.TUSHARE_CASH_CONVERSION_COLUMNS)
    fundamentals = pd.DataFrame(
        [
            {
                "instrument": symbol,
    ...
    capacity = RESEARCH.tushare_cash_conversion_capacity(
        factor_frame,
        fundamentals,
        full_calendar,
        research_calendar,
    ...
    canonicalization = capacity["event_canonicalization"]
    assert (
        canonicalization[
    ...
    assert canonicalization["collision_period_selection_performed"] is False
    assert capacity["potential_complete_cohorts"] == 3
    assert capacity["capacity_gate_passed"] is True
    assert capacity["price_fields_loaded"] == []
    assert capacity["forward_return_fields_read"] is False
...
def test_tushare_cash_conversion_uniqueness_rejects_a_synonym():
    ...
    comparison_rows = []
    all_comparisons = (
        *RESEARCH.TUSHARE_CASH_CONVERSION_COMPARISON_FIELDS,
        *RESEARCH.TUSHARE_CASH_CONVERSION_ADDITIONAL_SEMANTIC_FIELDS,
    )
    for date in dates:
        random_values = {
            field: rng.normal(size=len(symbols)) for field in all_comparisons
        }
        for position, symbol in enumerate(symbols, start=1):
            factor_rows.append(
                {
    ...
    contract = {
        "screen_start": dates[0].date().isoformat(),
        "screen_end": dates[-1].date().isoformat(),
        "comparison_factors": list(RESEARCH.TUSHARE_CASH_CONVERSION_COMPARISON_FIELDS),
        "additional_composite_semantic_checks_not_counted_twice": list(
            RESEARCH.TUSHARE_CASH_CONVERSION_ADDITIONAL_SEMANTIC_FIELDS
        ),
        "minimum_pairwise_names_per_session": 6,
    ...
    factor_frame = pd.DataFrame(factor_rows)
    comparison_frame = pd.DataFrame(comparison_rows)
    independent = RESEARCH.summarize_tushare_cash_conversion_uniqueness(
        factor_frame,
        comparison_frame,
        contract=contract,
    )
    assert independent["comparison_field_count"] == 54
    assert independent["additional_composite_semantic_check_count"] == 2
    assert independent["uniqueness_gate_passed"] is True

    synonym = comparison_frame.copy()
    synonym["quality_score"] = factor_frame[
        RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME
    ].to_numpy()
    rejected = RESEARCH.summarize_tushare_cash_conversion_uniqueness(
        factor_frame,
        synonym,
        contract=contract,
    )
    quality_score = next(
        item
        for item in rejected["additional_composite_semantic_results"]
        if item["comparison_field"] == "quality_score"
    )
    assert quality_score["median_daily_rank_correlation"] == pytest.approx(1.0)
    assert quality_score["uniqueness_gate_passed"] is False
    assert rejected["uniqueness_gate_passed"] is False
    assert rejected["forward_return_fields_read"] is False


def test_tushare_cash_conversion_capacity_failure_stops_before_comparison_load(
    tmp_path, monkeypatch
):
    provider = tmp_path / "provider"
    calendar_path = provider / "calendars" / "day.txt"
    ...
    spec = {
        "preregistered_at": "2026-07-16T14:25:16Z",
        "point_in_time_context": {
            "local_calendar": {"file_sha256": RESEARCH.file_sha256(calendar_path)},
            "holding_universe": {"file_sha256": RESEARCH.file_sha256(universe_path)},
            "accepted_price_basis": {"sha256": RESEARCH.file_sha256(price_basis_path)},
    ...
    capacity = {
        "factor": RESEARCH.TUSHARE_CASH_CONVERSION_FACTOR_NAME,
        "potential_complete_cohorts": 199,
        "minimum_required_cohorts": 200,
        "observed_calendar_years": 7,
        "minimum_observed_calendar_years": 5,
    ...
    monkeypatch.setattr(
        RESEARCH,
        "load_tushare_cash_conversion_no_return_preregistration",
        lambda: spec,
    )
    monkeypatch.setattr(
        RESEARCH,
    ...
    monkeypatch.setattr(
        RESEARCH,
        "tushare_cash_conversion_capacity",
        lambda *args, **kwargs: capacity,
    )
    monkeypatch.setattr(
        RESEARCH,
    ...
    result = RESEARCH.run_tushare_cash_conversion_no_return_audit(args)
    audit = json.loads(Path(result["audit_path"]).read_text())
    assert audit["capacity_gate_passed"] is False
    assert audit["uniqueness"] is None
    assert audit["data"]["close_known_comparison_fields_loaded"] == []
