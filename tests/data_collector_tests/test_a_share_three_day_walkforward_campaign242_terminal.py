...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_242_commercial_water_treatment_"
    "water_hygiene_service_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_242"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_242_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v373_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_242_skill_lines_201_300_"
    "combined_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_242_authority_shasum_missing_"
    "path_failure_20260816.json",
]
...
def test_campaign242_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c242_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 101
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_water_treatment_or_water_hygiene_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "commercial_water_treatment_water_hygiene_legionella_cooling_tower_water_boiler_water_treatment_or_reverse_osmosis_service_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2307
...
def test_campaign242_reports_candidate49_and_research_boundaries():
    heading = "## Campaign242 商业水处理与水卫生服务合同、系统台账、采样检测、加药过滤、整改复测、耗材合规与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign242_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2307`" in campaign242_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign242_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign242_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 243
    ...
    assert (
        ...
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign241_terminal.py::test_campaign241_reports_candidate49_and_research_boundaries"
    )
