...
from scripts import a_share_three_day_walkforward_campaign081_no_return_audit as audit
...
def test_protocol_and_orders_are_frozen_before_coverage() -> None:
    ...
    assert gate["numeric_comparator_count"] == 110
    assert gate["complete_definition_count"] == 112
    assert len(gate["comparison_factors"]) == 110
    assert gate["comparison_factors"][-1] == {
        "name": "intraday_above_median_amount_longest_run_240m",
        "score_direction": "higher",
    }
    assert gate["all_110_numeric_comparators_must_pass"] is True
...
def test_snapshot_identity_is_frozen_without_loading_values() -> None:
    ...
    assert manifest["factor_eligible_rows"][audit.FACTOR_NAME] == 7724498
...
def test_audit_implementation_and_activation_bindings_are_live() -> None:
    ...
    assert activation["candidate_snapshot"]["dataset_sha256"] == audit.SNAPSHOT_DATASET_SHA256
