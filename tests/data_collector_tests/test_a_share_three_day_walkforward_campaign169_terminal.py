...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_169_commercial_pricing_terms_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_169/research_attempt_ledger.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_169_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_169_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v247_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign169_bindings_and_finite_frontier() -> None:
...
def test_campaign169_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_document_title_body_business_price_candidate_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1487
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign169_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v246"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v246"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 170
...
def test_campaign169_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign169 商业定价、费率与销售条款来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign169 共 7 次科学尝试、0 次基础设施失败" in text


def test_campaign169_records_use_reached_wall_clock_timestamps() -> None:
