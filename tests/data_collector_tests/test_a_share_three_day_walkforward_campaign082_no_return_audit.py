...
from scripts import a_share_three_day_walkforward_campaign082_no_return_audit as audit
...
def test_protocol_and_orders_are_frozen_before_coverage() -> None:
    ...
    assert gate["numeric_comparator_count"] == 111
    assert gate["complete_definition_count"] == 113
    assert len(gate["comparison_factors"]) == 111
    assert gate["comparison_factors"][-1] == {
        "name": "intraday_amount_path_efficiency_238p",
        "score_direction": "higher",
    }
    assert gate["all_111_numeric_comparators_must_pass"] is True
...
def test_snapshot_identity_is_frozen_without_loading_values() -> None:
    ...
    assert manifest["factor_eligible_rows"][audit.FACTOR_NAME] == 7699914
