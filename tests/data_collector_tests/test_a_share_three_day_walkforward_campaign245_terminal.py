...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_245_commercial_building_"
    "access_door_loading_dock_equipment_service_lifecycle_source_frontier_"
    "20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_245"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_245_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v376_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_245_skill_lines_361_720_"
    "combined_output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_245_skill_lines_601_660_"
...
def test_campaign245_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c245_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 104
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_building_access_door_and_loading_dock_equipment_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "commercial_building_access_door_loading_dock_equipment_dock_leveler_dock_seal_dock_shelter_vehicle_restraint_sectional_door_or_rolling_door_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2335
...
def test_campaign245_reports_candidate49_and_research_boundaries():
    heading = "## Campaign245 商业建筑实体门与装卸月台设备服务合同、资产台账、巡检维保、故障修复、配件合规与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign245_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2335`" in campaign245_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign245_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign245_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 246
    ...
    assert (
        ...
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign244_terminal.py::test_campaign244_reports_candidate49_and_research_boundaries"
    )
