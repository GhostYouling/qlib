...
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_146_development_implementation_freeze_20260813.json"
)
...
def test_no_return_admission_is_bound_to_all_141_authority() -> None:
    ...
    assert interface["all_141_numeric_comparisons_passed"] is True
    assert authority["factor"] == campaign.ADMITTED_FACTOR
    assert authority["summary"]["comparison_factor_count"] == 141
...
def test_minute_snapshot_adapter_preserves_negative_and_missing_without_ohlcv(
    ...
    output = campaign._campaign146_load_factor_panel(
        spec,
        [2019],
    ...
    assert output[factor].iloc[0] == -0.25
...
def test_development_activation_fails_before_return_loader_when_absent(
    ...
    with pytest.raises(campaign.Campaign146Error, match="activation is absent"):
...
def test_implementation_freeze_is_absent_or_live() -> None:
    ...
    assert record["status"] == (
        "frozen_before_campaign146_2019_2023_development_return_read"
    )
