...
from scripts import a_share_three_day_walkforward_campaign080_no_return_audit as audit
...
def test_protocol_and_orders_are_frozen_before_coverage() -> None:
    ...
    assert gate["numeric_comparator_count"] == 109
    assert gate["complete_definition_count"] == 111
    assert len(gate["comparison_factors"]) == 109
    assert gate["comparison_factors"][-1] == {
        "name": "intraday_amount_local_peak_density_238p",
        "score_direction": "higher",
    }
    assert gate["all_109_numeric_comparators_must_pass"] is True
