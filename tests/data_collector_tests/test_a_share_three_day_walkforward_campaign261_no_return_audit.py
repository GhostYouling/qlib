...
from scripts import a_share_three_day_walkforward_campaign261_no_return_audit as audit
...
def test_campaign261_gate_and_protocol_are_frozen() -> None:
...
def test_campaign261_synthetic_panel_passes_coverage_and_variation() -> None:
...
def test_campaign261_constant_cross_section_fails_variation_only() -> None:
    candidate, keys = _synthetic_panel()
...
def test_campaign261_out_of_range_or_undeclared_values_fail_closed() -> None:
    candidate, keys = _synthetic_panel(sessions=10)
    candidate.loc[0, audit.FACTOR_NAME] = 2.0
    with pytest.raises(audit.Campaign261NoReturnAuditError):
