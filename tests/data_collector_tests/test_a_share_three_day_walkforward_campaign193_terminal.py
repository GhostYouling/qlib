...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_193_asset_management_fiduciary_aum_subscription_redemption_lifecycle_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_193/research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_193_terminal_result_20260815.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v282_20260815.json"
)
...
def test_campaign193_bindings_frontier_and_append_only_chain():
    ...
    assert len(ledger["delta_entries"]) == ledger["effective_entry_count"] == 10
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1721
    ...
    for accounting in (result["effective_accounting"], policy["effective_accounting"]):
        assert accounting["campaign193_attempt_count"] == 10
        assert accounting["campaign193_infrastructure_failure_count"] == 3
        assert accounting["cumulative_historical_research_attempt_count"] == 1721


def test_campaign193_reports_candidate49_and_research_boundaries():
    heading = "## Campaign193 资产管理受托 AUM、申购赎回与产品生命周期来源前沿值前终止"
    policy = load(POLICY)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 194
