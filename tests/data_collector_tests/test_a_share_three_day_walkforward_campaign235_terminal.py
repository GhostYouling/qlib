...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_235_commercial_landscaping_"
    "grounds_maintenance_contract_work_order_irrigation_plant_tree_health_"
    "materials_billing_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_235"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_235_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v357_20260816.json"
)
...
def test_campaign235_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c235_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert len(ledger["delta_entries"]) == 8
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2218
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign235_reports_candidate49_and_research_boundaries():
    heading = "## Campaign235 商业园林绿化与场地养护合同、工单、灌溉、植株树木健康与物料生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign235_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2218`" in campaign235_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign235_section
        assert (
    ...
    assert result["effective_accounting"]["campaign_attempt_count"] == 8
    assert (
        policy["effective_accounting"]["campaign235_infrastructure_failure_count"] == 1
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 236
