...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_191_agricultural_biological_asset_production_cycle_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_191/research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_191_terminal_result_20260815.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v280_20260815.json"
)
...
def test_campaign191_bindings_frontier_and_append_only_chain():
    ...
    assert sum(route["new_issuer_state"] for route in routes) == 4
    assert (
        len(
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1698
    ...
    for accounting in (result["effective_accounting"], policy["effective_accounting"]):
        assert accounting["campaign191_attempt_count"] == 13
        assert accounting["campaign191_infrastructure_failure_count"] == 6
        assert accounting["cumulative_historical_research_attempt_count"] == 1698


def test_campaign191_reports_candidate49_and_research_boundaries():
    heading = "## Campaign191 农业生物资产生产周期来源前沿值前终止"
    policy = load(POLICY)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 192
