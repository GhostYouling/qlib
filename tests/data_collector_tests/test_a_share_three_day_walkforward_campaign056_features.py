...
from scripts import a_share_three_day_walkforward_campaign056_features as runner
...
def test_protocol_and_comparison_catalog_are_fingerprint_bound() -> None:
    ...
    assert len(comparisons) == runner.COMPARISON_COUNT == 79
...
def test_similarity_retains_zero_bins_and_rejects_invalid_totals() -> None:
    current = np.stack([_profile(), np.zeros(240), _profile(), _profile()])
    ...
    assert quality["nonpositive_current_total_rows"] == 1
    assert quality["negative_current_amount_rows"] == 1
    assert quality["nonfinite_prior_amount_rows"] == 1


def test_extract_amount_profiles_enforces_exact_241_source_grid() -> None:
    raw = pd.concat(
        [_raw_session("2020-01-02"), _raw_session("2020-01-03")],
        ignore_index=True,
    )
    profiles, quality = runner.extract_amount_profiles(raw, symbol="SH600000")
    assert list(profiles) == [pd.Timestamp("2020-01-02"), pd.Timestamp("2020-01-03")]
    assert all(value.shape == (240,) for value in profiles.values())
    assert quality["source_sessions"] == 2
    bad = raw.drop(index=0).reset_index(drop=True)
    with pytest.raises(runner.Campaign056FeatureError, match="241 rows"):
        runner.extract_amount_profiles(bad, symbol="SH600000")
...
def test_output_uses_exact_previous_market_session_without_bridging() -> None:
    ...
    assert (
        suspended_quality[f"{runner.FACTOR_NAME}__missing_prior_stock_session_rows"]
        == 1
    )


def test_preregistration_records_state_no_value_and_no_return_boundary() -> None:
    concept = json.loads(
        (
            runner.REPO_ROOT
            / "docs/a_share_three_day_walkforward_campaign_056_concept_scouting.json"
        ).read_text(encoding="utf-8")
    )
    mechanism = json.loads(
        (
            runner.REPO_ROOT
            / "docs/a_share_three_day_walkforward_campaign_056_mechanism_overlap_audit.json"
        ).read_text(encoding="utf-8")
    )
    protocol = json.loads(runner.DEFAULT_PROTOCOL.read_text(encoding="utf-8"))
    assert concept["research_boundary"]["external_campaign056_minute_partitions_read"] is False
