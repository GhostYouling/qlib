...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_182_credit_facility_covenant_liquidity_backstop_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_182/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_182_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_182_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v270_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign182_bindings_and_finite_frontier() -> None:
...
def test_campaign182_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_credit_agreement_filing_facility_covenant_collateral_factor_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1608
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign182_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v269"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v269"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 183
    assert policy["future_campaign_boundary"][
        "2024_2025_source_or_returns_remain_closed"
    ]
    assert (
        policy["deferred_source_boundary"][
            "successful_waiver_cure_syndication_refinancing_or_takeout_cases_may_be_selected_without_failures_and_withdrawals"
        ]
        is False
    )
...
def test_campaign182_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign182 企业授信额度与债务契约来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign182 共 7 次科学尝试、0 次基础设施失败" in text


def test_campaign182_records_use_reached_wall_clock_timestamps() -> None:
