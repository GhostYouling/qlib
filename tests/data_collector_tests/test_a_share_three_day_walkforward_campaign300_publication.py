...
STATE_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_iteration_status_20260825_campaign300_terminal.json"
)
POLICY_PATH = (
    REPO_ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v430_20260825.json"
)
...
def test_campaign300_terminal_publication_bindings_and_semantics() -> None:
    ...
    assert uniqueness["all_152_passed"] is True
    assert uniqueness["comparators_evaluated"] == 152
    ...
    assert ledger["infrastructure_failure_count"] == 0
    assert documented["effective_accounting"]["campaign300_total_attempts"] == 12
    ...
    assert documented["development_result"]["operationally_admissible"] is False

    library = policy["complete_historical_feature_library"]
    comparators = policy["numerical_comparator_eligibility"]
    assert library["factor_definition_count"] == 172
    assert library["campaign300_definition_appended"] is True
    assert library["order_sha256"] == (
        "ba884534eca5c6ec0812271ed5b78d6a4587c5ffa2a00c52d5c0f6892aaf0df1"
    )
    assert comparators["eligible_numeric_comparator_count"] == 153
    assert comparators["campaign300_numeric_comparator_appended"] is True
    assert comparators["order_sha256"] == (
        "41b425635ddb9986d9e6d37f83220caca213d6437ac1ccbc02c29072610b499d"
    )


def test_campaign300_candidate49_lockbox_and_reports_are_unchanged() -> None:
    state = _load(STATE_PATH)
    policy = _load(POLICY_PATH)
    ...
    campaign_handoff = (
        REPO_ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign300.md"
    ).read_text(encoding="utf-8")
    ...
    assert "Campaign300 离线历史终局" in pipeline
    assert "历史滚动 Campaign300 权威追加" in rendered
