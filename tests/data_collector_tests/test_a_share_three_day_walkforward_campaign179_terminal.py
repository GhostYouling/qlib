...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_179_operating_license_concession_right_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_179/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_179_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_179_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v267_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign179_bindings_and_finite_frontier() -> None:
...
def test_campaign179_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_registry_filing_license_concession_tenure_quota_candidate_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1584
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign179_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v266"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v266"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 180
...
def test_campaign179_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign179 经营牌照与特许经营权来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign179 共 7 次科学尝试、0 次基础设施失败" in text


def test_campaign179_records_use_reached_wall_clock_timestamps() -> None:
