...
from scripts import a_share_three_day_walkforward_campaign262_no_return_audit as audit


def _synthetic_panel(
    *, sessions: int = 1300, names: int = 60
    ...
    ordinal = np.tile(np.arange(names, dtype=float), sessions)
    candidate = keys.copy()
    candidate[audit.FACTOR_NAME] = ordinal / float(names)
    candidate[f"{audit.FACTOR_NAME}_eligible"] = True
    return candidate, keys


def test_campaign262_gate_and_protocol_are_frozen() -> None:
    spec = audit.load_protocol()
    assert spec["candidate"]["name"] == audit.FACTOR_NAME
    assert "all 142" in spec["ordered_no_return_gates"][2]["rule"]
    assert audit.expected_gate()["minimum_nonconstant_cross_sectional_sessions"] == 200


def test_campaign262_synthetic_panel_passes_coverage_and_variation() -> None:
    candidate, keys = _synthetic_panel()
...
def test_campaign262_constant_cross_section_fails_variation_only() -> None:
    candidate, keys = _synthetic_panel()
...
def test_campaign262_out_of_range_or_undeclared_values_fail_closed() -> None:
    candidate, keys = _synthetic_panel(sessions=10)
    candidate.loc[0, audit.FACTOR_NAME] = 2.0
    with pytest.raises(audit.Campaign262NoReturnAuditError):
