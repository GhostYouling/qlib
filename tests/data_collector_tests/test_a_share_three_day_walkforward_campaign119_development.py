...
from scripts import a_share_three_day_walkforward_campaign119 as campaign


ROOT = Path(__file__).resolve().parents[2]
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_119_development_implementation_freeze_20260813.json"
)
...
def test_no_return_admission_is_interface_only_and_bound_to_authority() -> None:
    ...
    assert interface["all_136_numeric_comparisons_passed"] is True
    assert authority["admissible_factor_names"] == [campaign.ADMITTED_FACTOR]
    assert authority["comparison_summary"]["comparison_factor_count"] == 136
...
def test_development_activation_fails_before_return_loader_when_absent(
    ...
    with pytest.raises(campaign.Campaign119Error, match="activation is absent"):
...
def test_implementation_freeze_is_absent_or_live() -> None:
    ...
    assert record["status"] == (
        "frozen_before_campaign119_2019_2023_development_return_read"
    )
