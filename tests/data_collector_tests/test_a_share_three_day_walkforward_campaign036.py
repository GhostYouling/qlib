"""Pre-return boundary tests for Campaign036 development."""
...
import scripts.a_share_three_day_walkforward_campaign036_features as features
...
def test_campaign036_preregistration_and_single_trial_are_frozen() -> None:
    ...
    assert campaign.build_trial_catalog(spec) == [
        {
            "trial_id": (
                "wf036_single__"
                "intraday_two_sided_range_frontier_balance_238m"
            ),
...
def test_campaign036_folds_search_and_stress_boundary_are_closed() -> None:
    ...
    assert (
        spec["research_output_boundary"][
            "campaign035_rescue_or_reweight_allowed"
        ]
        is False
    )
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign036 development trial is "
...
def test_campaign036_survivor_reporting_uses_one_feature_complexity(
    monkeypatch,
) -> None:
    monkeypatch.setitem(
