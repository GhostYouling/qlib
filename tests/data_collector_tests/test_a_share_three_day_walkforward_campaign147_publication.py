...
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v198_20260814.json"
)
STATUS = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260814_campaign147_prevalue_terminal.json"
)
VALIDATION = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_147_terminal_validation_20260814.json"
)
...
def test_campaign147_policy_advances_without_changing_library_orders() -> None:
    assert bindings.validate_record(POLICY)["all_bindings_passed"] is True
    policy = _load(POLICY)
    assert policy["version"] == 198
    assert policy["complete_historical_feature_library"] == {
        ...
        "unchanged_from_v197": True,
        "campaign147_definition_appended": False,
    }
    ...
    assert eligibility["campaign147_numeric_series_appended"] is False
    assert policy["future_campaign_boundary"]["next_campaign"] == 148


def test_campaign147_latest_status_keeps_goal_and_candidate49_boundaries() -> None:
    ...
    assert status["goal"]["campaign148_offline_scouting_authorized"] is True
    assert status["campaign147"]["terminal"] is True
    assert status["campaign147"]["selected_candidate_count"] == 0
