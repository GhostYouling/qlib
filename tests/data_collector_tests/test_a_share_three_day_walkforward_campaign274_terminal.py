...
CATALOG = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_274_concept_scouting_20260824.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_274_prevalue_mechanism_frontier_result_20260824.json"
)
LEDGERS = tuple(
    ROOT
    / f"data/experiments/short_horizon/historical_walkforward/campaign_274/research_attempt_ledger_v{version}.json"
    for version in (1, 2, 3, 4)
)
STATE = (
    ROOT / "docs/a_share_three_day_iteration_status_20260824_campaign274_terminal.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_274_terminal_prevalue_report.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260824_campaign274.md"
HANDOFF_V2 = ROOT / "docs/a_share_three_day_strategy_handoff_20260824_campaign274_v2.md"
VALIDATION = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_274_terminal_validation_20260824.json"
)
FINAL_STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260824_campaign274_final_validated.json"
)
...
def test_catalog_freezes_seven_priority_routes_before_targeted_inventory() -> None:
    assert _sha256(CATALOG) == (
        "acad4130fbc50b8c509fa9a0ef3e39ca251cc8487fd889a223627e08343718ea"
    )
    ...
    assert [route["concept_id"] for route in routes] == [
        "public_vat_invoice_counterparty_activity_state",
        "public_corporate_registry_branch_subsidiary_lifecycle_state",
        "public_tax_credit_arrears_enforcement_state",
        "public_land_title_mortgage_encumbrance_state",
        "public_environmental_permit_emissions_compliance_state",
        "public_product_certification_accreditation_state",
        "public_employer_social_insurance_contribution_state",
    ]
    for route in routes:
        for field in (
            "formula",
...
def test_result_closes_all_routes_before_formula_source_rows_or_values() -> None:
    assert _sha256(RESULT) == (
        "d4fa0b5763b2563296987532a800270164ab68b9af0adeb810d599e4d12730b3"
    )
    result = _load(RESULT)
    ...
    assert decision["campaign274_terminalized"] is True
...
def test_append_only_attempt_chain_and_accounting_are_exact() -> None:
    assert tuple(_sha256(path) for path in LEDGERS) == (
        "34a2b0933c9099280d0df770df23095f1195bfc88377f182a94919a0d1da293a",
        "126cd780fb797d0dd7ead68e598bf87962d8374ba0cce880699a5f254225ce06",
        "9ae0111acfe420fca7d6166ba43020f3fde76acfc1a4d2212aa30e48f4740b26",
        "652b200753d54b2ab0a8155dad15b665b20a07a351726126a6fc33101c41f4df",
    )
    ...
    assert (
        previous == "1eac7ab9da557c6ede9b73efdf5db481c8ac3675feca9f5e7c06042a949750f7"
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2793
...
def test_state_keeps_goal_candidate49_and_credentials_boundary_unchanged() -> None:
    assert _sha256(STATE) == (
        "c77c09da73b6a87a8bae297cdd9ac9346359e1558548c2ad1d880fd745bc1d4e"
    )
    state = _load(STATE)
    _assert_bindings(
        {
    ...
    assert state["goal"]["current_campaign"] == 274
    assert "Campaign275" in state["goal"]["next_safe_work"]
    ...
    assert candidate49["campaign274_plan_or_run_executed"] is False
...
def test_optional_validation_and_final_state_bind_current_artifacts() -> None:
    if VALIDATION.exists():
        ...
        assert validation["cross_stage_result"]["passed"] == 239
        assert validation["cross_stage_result"]["deselected"] == 7
    if FINAL_STATE.exists():
        ...
        assert final_state["goal"]["current_campaign"] == 274
        assert "Campaign275" in final_state["goal"]["next_safe_work"]
        assert (
            _sha256(HANDOFF_V2)
            == final_state["authoritative_inputs"]["campaign274_handoff_v2"]["sha256"]
        )
