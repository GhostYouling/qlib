...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v7.json"
)
STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260825_campaign283_terminal_v5.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_terminal_prevalue_report_v5.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign283_v5.md"
...
def test_effective_v7_ledger_replays_all_entries() -> None:
    assert (
        _sha(LEDGER)
        == "9316e27c2312df2a6fac7b9ee7d6bc235eb2c959f13aba652d108223f97ba9c3"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2894
    assert ledger["cumulative_return_reading_development_trial_count"] == 315


def test_v5_reports_and_unified_accounting_correction() -> None:
    report = REPORT.read_text(encoding="utf-8")
    handoff = HANDOFF.read_text(encoding="utf-8")
    assert "累计历史尝试 2,894" in report
    assert "持续目标保持 `active`" in handoff
    assert "不构成投资建议" in report
    for report_path in UNIFIED:
        text = report_path.read_text(encoding="utf-8")
        assert text.count("### Campaign283 验证会计更正 v4") == 1
        correction = text.split("### Campaign283 验证会计更正 v4", maxsplit=1)[1]
        assert "累计历史尝试 2,894" in correction
        assert "Campaign284" in correction


def test_six_failures_are_value_free_and_not_consumed() -> None:
...
def test_v5_state_binds_current_artifacts_and_active_goal() -> None:
    state = _load(STATE)
    ...
    assert state["effective_accounting"]["campaign283_attempt_count"] == 13
    assert (
        state["effective_accounting"]["campaign283_infrastructure_failure_count"] == 6
    )
