...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_212_manufacturing_execution_"
    "shop_floor_work_order_bom_routing_material_lot_genealogy_quality_inspection_"
    "nonconformance_quarantine_rework_scrap_finished_goods_release_lifecycle_"
    "source_frontier_20260815.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_212"
...
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_212_terminal_result_v2_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v322_20260815.json"
)
...
def test_campaign212_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c212_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert observed_entries == ledger["effective_entry_count"] == 13
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1969
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign212_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign212 制造执行工单、物料批次追溯、质量检验、隔离返工与"
        "成品放行生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
        ...
        assert report.count("### Campaign212 累计回归文件筛选失败会计追加") == 1
        assert "累计历史尝试 `1969`" in report
    ...
    assert (
        policy["effective_accounting"]["campaign212_infrastructure_failure_count"] == 6
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 213
