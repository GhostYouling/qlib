...
from scripts import a_share_three_day_walkforward_campaign110 as campaign


ROOT = Path(__file__).resolve().parents[2]
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_110_development_implementation_freeze_20260808.json"
)
...
def test_development_preregistration_bindings_and_single_trial_catalog() -> None:
    ...
    assert report["binding_count"] == 16
...
def test_no_return_admission_is_interface_only_and_bound_to_authority() -> None:
    ...
    assert interface["all_133_numeric_comparisons_passed"] is True
    assert authority["admissible_factor_names"] == [campaign.ADMITTED_FACTOR]
    assert authority["uniqueness"][campaign.ADMITTED_FACTOR][
...
def test_folds_purge_survivor_and_execution_policy_are_inherited_exactly() -> None:
    ...
    assert [item["validation"]["start"][:4] for item in effective["walkforward_folds"]] == [
        "2021",
        "2022",
        "2023",
    ]
...
def test_predevelopment_status_has_zero_ledger_and_closed_stress(tmp_path: Path) -> None:
...
def test_development_activation_fails_before_return_loader_when_absent(
    ...
    with pytest.raises(campaign.Campaign110Error, match="activation is absent"):
...
def test_implementation_freeze_is_absent_or_live() -> None:
    ...
    assert record["status"] == (
        "frozen_before_campaign110_2019_2023_development_return_read"
    )
