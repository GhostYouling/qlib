...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_185_real_estate_presale_sales_delivery_land_bank_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_185/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_185_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_185_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v274_20260815.json"
)
TAX_REJECTION = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_185_enterprise_tax_domain_overlap_rejection_20260815.json"
)
EQUITY_REJECTION = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_185_equity_incentive_terminated_source_family_rejection_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign185_bindings_and_finite_frontier() -> None:
    for record_path in (FRONTIER, TERMINAL, POLICY, TAX_REJECTION, EQUITY_REJECTION):
    ...
    assert (
        _load(TAX_REJECTION)["scientific_decision"][
            "selected_for_campaign185_finite_catalog"
        ]
        is False
    )
    assert (
        _load(EQUITY_REJECTION)["scientific_decision"][
            "terminal_source_family_rewritten_or_retried"
        ]
        is False
    )


def test_campaign185_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_real_estate_project_land_parcel_presale_contracted_sales_collection_delivery_factor_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1635
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign185_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v273"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v273"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 186
    assert policy["future_campaign_boundary"][
        "2024_2025_source_or_returns_remain_closed"
    ]
    assert (
        policy["deferred_source_boundary"][
            "successful_sales_delivery_or_disposal_projects_may_be_selected_without_cancellations_delays_failures_and_remediation"
        ]
        is False
    )
    assert (
        policy["deferred_source_boundary"][
            "terminal_cninfo_equity_incentive_source_family_may_be_retried_or_keyword_rescued"
        ]
        is False
    )
...
def test_campaign185_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign185 房地产项目预售、销售与交付来源前沿值前终止"
    assert "9 次科学尝试、0 次基础设施失败" in REPORT.read_text(encoding="utf-8")
    for report_path in (
        ...
        assert "Campaign185 最终有效会计为 9 次科学尝试、0 次基础设施失败" in text


def test_campaign185_records_use_reached_wall_clock_timestamps() -> None:
    for record_path in (
        FRONTIER,
        LEDGER,
        TERMINAL,
