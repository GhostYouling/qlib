...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_205_telecommunications_"
    "subscriber_service_provisioning_network_session_rating_billing_dunning_"
    "suspension_reconnection_churn_lifecycle_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_205"
    / "research_attempt_ledger_v1.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_205_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v306_20260815.json"
)
...
def test_campaign205_bindings_frontier_privacy_and_append_only_chain():
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_issuer_as_telecommunications_provider_service_lifecycle_frontier_found"
        ]
        is False
    )
    assert (
        frontier["research_boundary"][
            "directly_identifiable_subscriber_communications_content_precise_location_or_sensitive_network_control_record_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1880
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign205_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign205 通信服务商订户开通、网络会话、计费催收与停复机生命周期"
        "来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert "Campaign205 最终有效会计为 10 次尝试（7 科学、3 基础设施）" in report
        assert "累计历史尝试 `1880`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 206
