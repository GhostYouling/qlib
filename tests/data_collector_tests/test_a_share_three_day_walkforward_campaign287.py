...
def test_weighted_mlp_is_deterministic_and_reduces_loss() -> None:
    rng = np.random.default_rng(4)
    matrix = rng.normal(size=(256, campaign.FEATURE_COUNT)).astype(np.float32)
    target = (matrix[:, 0] - 0.5 * matrix[:, 1] + 0.2).astype(np.float64)
    weights = np.full(len(matrix), 1.0 / len(matrix), dtype=np.float64)
    parameters = {**campaign.MODEL_PARAMETERS, "epochs": 4, "batch_size": 64}

    first = campaign.WeightedMLPRegressor(parameters).fit(matrix, target, weights)
    second = campaign.WeightedMLPRegressor(parameters).fit(matrix, target, weights)

    assert first.loss_curve_ is not None
    assert first.loss_curve_[-1] < first.loss_curve_[0]
    assert np.array_equal(first.predict(matrix), second.predict(matrix))
...
def test_deterministic_sampler_selects_exactly_96_per_retained_session() -> None:
    ...
    counts = identities.iloc[first].groupby("trade_date").size()
    assert counts.eq(96).all()


def test_preprocessing_is_training_only_finite_and_bounded() -> None:
    rng = np.random.default_rng(8)
    matrix = rng.normal(size=(600, campaign.FEATURE_COUNT)).astype(np.float32)
    matrix[0, 0] = np.nan
    matrix[1, 1] = np.inf

    centers, scales, stats = campaign.preprocessing_statistics(matrix)
    transformed = campaign.transform_features(matrix, centers, scales)

    assert stats["minimum_finite_observations_per_feature"] >= 512
