"""Versioned current terminal and reporting tests for final Campaign059 accounting."""
...
RECORD = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_research_record_v4.json"
LEDGER = REPO_ROOT / "data/experiments/short_horizon/historical_walkforward/campaign_059/research_attempt_ledger_v5.json"
...
FAILURE_3 = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_terminal_test_workdir_import_failure_20260804.json"
FAILURE_4 = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_terminal_subprocess_import_path_failure_20260804.json"
FREEZE = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_terminal_completion_freeze_v4_20260804.json"
SUPERSESSION = REPO_ROOT / "docs/a_share_three_day_walkforward_campaign_059_unified_report_supersession_v4_20260804.json"
...
def test_campaign059_v3_current_records_have_live_bindings() -> None:
    paths = (
...
def test_campaign059_v3_result_and_append_only_accounting_are_exact() -> None:
    record = _load(RECORD)
    ...
    assert ledger["campaign059_ledger_entry_count"] == 9
    assert ledger["campaign059_attempt_count"] == 8
    assert ledger["campaign059_infrastructure_only_failure_count"] == 7
    assert ledger["campaign059_complete_factor_attempt_count"] == 1
    assert accounting["cumulative_historical_research_attempt_count"] == 341
    assert accounting["cumulative_return_reading_development_trial_count"] == 269
    assert record["successful_current_suite"] == {
        "explicit_pythonpath": "/Volumes/DIsk/Disk-Coding/qlib",
        "passed": 33,
        "failed": 0,
        "deselected": 1,
    }
...
def test_campaign059_v3_current_status_is_post_audit_and_stress_closed() -> None:
...
def test_campaign059_v3_reports_and_generator_are_current(tmp_path: Path) -> None:
    assert _sha256(REPORT) == (
        "a7a93b023f6bd33bc72fee3e34a3b5b31eb73bb703ef024c555fa8d55a211565"
    )
    assert _sha256(PIPELINE_DOC) == (
        "638f2020e536e73165ead6c77c5823604dc41dd1ce11675006297f12c70e537f"
    )
    assert _sha256(SKILL) == (
        "03a8990b752af9ba24215831c1258a08233f279e8b6f4b7a1673a40e7b2c6087"
    )
...
def test_campaign059_v3_preserves_candidate49_isolation() -> None:
