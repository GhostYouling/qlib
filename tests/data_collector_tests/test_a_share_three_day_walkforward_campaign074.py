"""Pre-return tests for the single frozen Campaign074 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign074 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "18989edcebc21b9e211c4a830a98d61eaeedd37c4e5fcb00d0ea67a02ba7d27c"
)
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_074/no_return/"
    "20260806T001358Z_campaign074_no_return_audit.json"
)


def test_campaign074_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign074_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign074_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "6256ca2db8ff21dcfb3e101e019957c2d1241fd893f290bd449ed462ace17009"
    )
...
def test_campaign074_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "scripts.a_share_three_day_walkforward_campaign074", "status"],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_campaign074_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            ...
            "docs/a_share_three_day_walkforward_campaign_074_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
