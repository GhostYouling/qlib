...
from scripts import (
    a_share_three_day_walkforward_campaign118_snapshot_verify_recovery as recovery,
)
...
def test_recovery_accepts_exact_truthful_manifest_metadata() -> None:
    ...
    assert manifest["amount_magnitude_used_after_event_selection"] is False
    assert manifest["comparison_factor_values_read"] is False


def test_recovery_rejects_old_campaign105_event_count_expectation() -> None:
    manifest = copy.deepcopy(_manifest())
    manifest["event_count"] = 0
    with pytest.raises(recovery.Campaign118SnapshotRecoveryError):
        recovery.validate_manifest_metadata(manifest)
