...
from scripts import a_share_three_day_walkforward_campaign077_no_return_audit as audit
...
def test_protocol_and_orders_are_frozen_before_coverage() -> None:
    ...
    assert gate["numeric_comparator_count"] == 107
    assert gate["complete_definition_count"] == 108
    assert len(gate["comparison_factors"]) == 107
    assert gate["comparison_factors"][-1] == {
        "name": "intraday_peak_amount_bar_recency_240m",
        "score_direction": "higher",
    }
    assert gate["all_107_numeric_comparators_must_pass"] is True
