...
from scripts import a_share_three_day_walkforward_campaign105_no_return_audit as audit


def test_runtime_protocol_preserves_exact_coverage_first_131_order() -> None:
    ...
    assert len(uniqueness["comparison_factors"]) == 131
    assert uniqueness["comparison_factors"][-1]["name"] == audit.c103.FACTOR_NAME
...
def test_comparator_loader_cannot_run_before_coverage_pass() -> None:
    with pytest.raises(audit.Campaign105NoReturnAuditError, match="before"):
...
def test_yearly_comparator_alignment_is_exact_and_subset_safe(tmp_path) -> None:
    path = tmp_path / "2019.parquet"
    frame = pd.DataFrame(
        {
            "stock_day_key": np.array([10, 11, 12, 13], dtype=np.int64),
            "factor_a": [0.1, 0.2, 0.3, 0.4],
            "factor_b": [1.0, np.nan, 0.5, 0.0],
        }
    )
    ...
    aligned = audit._aligned_columns(
        manifest_path=tmp_path / "snapshot_manifest.json",
        records=[record],
        names=["factor_a", "factor_b"],
        candidate_keys=np.array([10, 12], dtype=np.int64),
    )
    np.testing.assert_allclose(aligned[:, 0], [0.1, 0.3])
    np.testing.assert_allclose(aligned[:, 1], [1.0, 0.5])
...
def test_status_is_value_blind_and_audit_requires_confirmation(tmp_path) -> None:
    ...
    with pytest.raises(audit.Campaign105NoReturnAuditError, match="confirm-run"):
...
def test_shared_implementation_freeze_is_live() -> None:
    ...
    assert freeze["numeric_comparator_count"] == 131
