...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_252_commercial_playground_"
    "recreation_equipment_service_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_252"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_252_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v384_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_252_skill_lines_201_300_"
    "output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_252_combined_skill_"
    "reread_lines_261_300_output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_252_combined_skill_lines_"
    "376_400_output_truncated_failure_20260816.json",
]
...
def test_campaign252_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c252_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 111
    assert not frontier["repository_evidence"][
        "dedicated_commercial_playground_recreation_equipment_service_definition_present"
    ]
    assert (
        frontier["repository_evidence"][
            "playground_equipment_play_structure_playground_swing_set_slide_climber_rope_net_spring_rider_seesaw_outdoor_recreation_equipment_impact_attenuation_or_safety_surfacing_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2406
    assert ledger["cumulative_return_reading_development_trial_count"] == 314
    for failure in FAILURES:
        assert all(
            value is False for value in load(failure)["research_boundary"].values()
        )


def test_campaign252_reports_candidate49_and_research_boundaries():
    heading = "## Campaign252：商用户外游乐设施设备服务生命周期值前终止（2026-08-16）"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign252_section = report[report.index(heading) :]
        assert "累计历史尝试为 2406" in campaign252_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign252_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign252_infrastructure_failure_count"] == 3
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 253
    ...
    assert len(carried) == 45
    assert (
        carried[-1]
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign251_terminal.py::test_campaign251_reports_candidate49_and_research_boundaries"
    )
    assert policy["cumulative_regression_boundary"]["deselection_node_count"] == 67
