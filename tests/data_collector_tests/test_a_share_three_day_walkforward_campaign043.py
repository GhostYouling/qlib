"""Frozen development-boundary tests for Campaign043."""
...
import scripts.a_share_three_day_walkforward_campaign043_features as features
...
def test_campaign043_preregistration_and_single_trial_are_frozen() -> None:
    ...
    assert campaign.build_trial_catalog(spec) == [
        {
            "trial_id": (
                "wf043_single__intraday_transaction_vwap_consensus_240m"
            ),
...
def test_campaign043_folds_search_and_stress_boundary_are_closed() -> None:
    ...
    assert (
        spec["research_output_boundary"][
            "campaign042_rescue_or_reweight_allowed"
        ]
        is False
    )
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign043 development trial is "
...
def test_campaign043_survivor_reporting_uses_one_feature_complexity(
    monkeypatch,
) -> None:
    monkeypatch.setitem(
