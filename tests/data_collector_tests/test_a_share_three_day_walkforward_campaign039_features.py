"""Synthetic and pre-value tests for Campaign039 time irreversibility."""
...
MODULE = importlib.import_module(
    "scripts.a_share_three_day_walkforward_campaign039_features"
)
...
def test_time_reversal_asymmetry_detects_an_ordered_cubic_sequence():
    reversible = np.full(119, 0.001)
    irreversible = np.resize(np.array([0.003, 0.001, 0.0]), 119)

    zero, zero_eligible, _ = MODULE.compute_factor_values(
        closes=_closes_from_half_returns(reversible, reversible),
    )
    positive, positive_eligible, _ = MODULE.compute_factor_values(
        closes=_closes_from_half_returns(irreversible, irreversible),
    )

    assert zero_eligible[MODULE.FACTOR_NAME].tolist() == [True]
    assert positive_eligible[MODULE.FACTOR_NAME].tolist() == [True]
    assert zero[MODULE.FACTOR_NAME][0] == pytest.approx(0.0, abs=1e-12)
    assert positive[MODULE.FACTOR_NAME][0] == pytest.approx(0.5, abs=1e-12)
    assert positive[MODULE.FACTOR_NAME][0] > zero[MODULE.FACTOR_NAME][0]


def test_asymmetry_is_time_reversal_magnitude_invariant_and_excludes_lunch():
    returns = np.resize(np.array([0.003, 0.001, 0.0]), 119)
    first, first_eligible, _ = MODULE.compute_factor_values(
        closes=_closes_from_half_returns(returns, returns),
    )
    reversed_path, reversed_eligible, _ = MODULE.compute_factor_values(
    ...
    assert reversed_eligible[MODULE.FACTOR_NAME].tolist() == [True]
    assert first[MODULE.FACTOR_NAME][0] == pytest.approx(
        reversed_path[MODULE.FACTOR_NAME][0],
        abs=1e-10,
    )


def test_zero_normalization_is_missing_but_one_informative_half_is_allowed():
    ...
    assert quality[f"{MODULE.FACTOR_NAME}__zero_normalization_rows"] == 1

    morning = np.zeros(119)
    afternoon = np.resize(np.array([0.003, 0.001, 0.0]), 119)
    ...
    assert values[MODULE.FACTOR_NAME][0] == pytest.approx(0.5, abs=1e-12)
    assert (
        quality[f"{MODULE.FACTOR_NAME}__adjacent_return_pair_observations"]
        == MODULE.ADJACENT_RETURN_PAIR_COUNT
    )
    assert (
...
def test_invalid_closes_and_bad_shape_fail_closed():
    with pytest.raises(MODULE.Campaign039FeatureError):
...
def test_protocol_materializes_60_unique_comparisons_in_frozen_order():
    ...
    assert len({item["name"] for item in comparisons}) == 60
    assert comparisons[-1] == {
        "name": "intraday_half_session_chord_adherence_240m",
...
def test_partition_builder_projects_close_only_and_uses_v1_output_root():
    ...
    assert MODULE.output_root(MODULE.DEFAULT_DATA_ROOT).name.endswith(
        "campaign039_feature_library_v1"
    )
