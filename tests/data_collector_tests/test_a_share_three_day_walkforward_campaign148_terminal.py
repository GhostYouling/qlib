...
SCOUTING = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_148_concept_scouting_20260814.json"
)
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_148_mechanism_frontier_audit_20260814.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_148/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_148_terminal_result_20260814.json"
)
...
def test_campaign148_finite_catalog_records_every_choice_before_values() -> None:
    ...
    assert boundary["campaign148_candidate_values_computed_or_read"] is False
    assert boundary["campaign148_comparator_values_read"] is False
...
def test_campaign148_frontier_rejects_consumed_mechanism_routes() -> None:
...
def test_campaign148_preserves_complete_and_numeric_library_orders() -> None:
...
def test_campaign148_append_only_attempt_chain_and_accounting() -> None:
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1286
...
def test_campaign148_terminal_result_and_candidate49_boundary() -> None:
    ...
    assert (
        terminal["library_after_campaign148"]["complete_factor_definition_count"] == 155
    )
    assert (
        terminal["library_after_campaign148"]["eligible_numeric_comparator_count"]
        == 142
    )
    assert terminal["effective_accounting"]["campaign148_attempt_count"] == 6
    assert (
...
def test_campaign148_bindings_and_unified_reports() -> None:
    ...
    heading = "## Campaign148 残余分钟机制前沿值前终止"
