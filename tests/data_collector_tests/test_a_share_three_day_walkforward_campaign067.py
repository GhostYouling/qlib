"""Pre-return tests for the single frozen Campaign067 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign067 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "221c9ff4f5acc0f46bf2df6565fc1194a175cefbd8adb6626b964b850c100d31"
)
AUDIT = (
    REPO_ROOT
...
def test_campaign067_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign067_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign067_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "daeac2c2364d00c09247d0e721435b81f587c0f215a5a93cba30b7661f205d54"
    )
    assert audit["status"] == (
    ...
    assert uniqueness["comparison_factor_count"] == 97
...
def test_campaign067_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "scripts.a_share_three_day_walkforward_campaign067", "status"],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_campaign067_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            ...
            "docs/a_share_three_day_walkforward_campaign_067_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
