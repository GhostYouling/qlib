...
SCOUTING = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_149_concept_scouting_20260814.json"
)
AUDIT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_149_mechanism_frontier_audit_20260814.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_149/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_149_terminal_result_20260814.json"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v202_20260814.json"
)
STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260814_campaign149_prevalue_terminal.json"
)
REPORT = ROOT / "docs/a_share_three_day_walkforward_campaign_149_terminal_report.md"
SIGNAL_LEDGER = (
...
def test_campaign149_finite_catalog_stops_before_values() -> None:
    ...
    assert boundary["campaign149_candidate_values_computed_or_read"] is False
    assert boundary["campaign149_comparator_values_read"] is False
...
def test_campaign149_mechanism_gate_rejects_every_proposal() -> None:
    audit = _load(AUDIT)
...
def test_campaign149_attempt_chain_and_accounting() -> None:
    ledger = _load(LEDGER)
    previous = "0" * 64
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1297
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign149_preserves_library_orders() -> None:
...
def test_campaign149_publication_bindings_and_candidate49_boundary() -> None:
    ...
    assert state["campaign149"]["terminal_result"]["sha256"] == _sha(TERMINAL)
    assert state["campaign149"]["attempt_ledger"]["sha256"] == _sha(LEDGER)
...
def test_campaign149_records_have_reached_wall_clock_timestamps() -> None:
...
def test_campaign149_reports_are_append_only_once() -> None:
    heading = "## Campaign149 估计器与模型残余前沿值前终止"
    assert "Campaign149" in REPORT.read_text(encoding="utf-8")
    for path in (
