...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_218_aerospace_aircraft_"
    "airworthiness_maintenance_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_218"
    / "research_attempt_ledger_v1.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_218_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v329_20260816.json"
)
...
def test_campaign218_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c218_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert len(ledger["delta_entries"]) == 9
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2032
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign218_reports_candidate49_and_research_boundaries():
    heading = "## Campaign218 航空航天飞机项目、交付注册、适航指令与全寿命维修生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
    ...
    assert (
        policy["effective_accounting"]["campaign218_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 219
