...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_203_passenger_transportation_"
    "booking_ticketing_checkin_boarding_trip_completion_disruption_"
    "reaccommodation_refund_compensation_lifecycle_source_frontier_20260815.json"
)
LEDGERS = tuple(
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_203"
...
RESULTS = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v2_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v3_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v4_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v5_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v6_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_203_terminal_result_v7_20260815.json",
)
RESULT = RESULTS[-1]
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v304_20260815.json"
)
...
def test_campaign203_bindings_frontier_privacy_and_append_only_chain():
    for record in (
        load(FRONTIER),
        *(load(result) for result in RESULTS),
        load(POLICY),
    ):
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_issuer_as_passenger_transportation_provider_journey_"
            "lifecycle_frontier_found"
        ]
        is False
    )
    assert (
        frontier["research_boundary"][
            "directly_identifiable_or_precise_passenger_travel_record_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1859
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign203_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign203 客运服务商订票、乘行、行李、中断改签与退款赔付生命周期"
        "来源前沿值前终止"
    )
    corrections = (
    ...
    for key, file_path in {
        ...
        assert all(report.count(correction) == 1 for correction in corrections)
        assert (
            "Campaign203 最终有效会计更正为 15 次尝试（7 科学、8 基础设施）" in report
        )
        assert "累计历史尝试 `1859`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 204
