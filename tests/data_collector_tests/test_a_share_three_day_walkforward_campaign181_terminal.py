...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_181_regulated_financial_capital_solvency_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_181/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_181_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_181_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v269_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign181_bindings_and_finite_frontier() -> None:
...
def test_campaign181_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_regulatory_return_filing_capital_liquidity_solvency_factor_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1601
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign181_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v268"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v268"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 182
    assert policy["future_campaign_boundary"][
        "2024_2025_source_or_returns_remain_closed"
    ]
    assert (
        policy["deferred_source_boundary"][
            "nonfinancial_issuer_may_be_filled_zero_missing_or_cross_ranked_with_regulated_financial_entities"
        ]
        is False
    )
...
def test_campaign181_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign181 金融机构监管资本与偿付能力来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign181 共 7 次科学尝试、0 次基础设施失败" in text


def test_campaign181_records_use_reached_wall_clock_timestamps() -> None:
