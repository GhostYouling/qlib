...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_204_utility_customer_"
    "connection_metering_consumption_billing_arrears_disconnection_"
    "reconnection_outage_restoration_lifecycle_source_frontier_20260815.json"
)
LEDGERS = tuple(
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_204"
    / f"research_attempt_ledger_v{version}.json"
    for version in range(1, 3)
)
RESULTS = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_204_terminal_result_20260815.json",
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_204_terminal_result_v2_20260815.json",
)
RESULT = RESULTS[-1]
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v305_20260815.json"
)
...
def test_campaign204_bindings_frontier_privacy_and_append_only_chain():
    for record in (
        load(FRONTIER),
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_issuer_as_utility_provider_customer_service_lifecycle_"
            "frontier_found"
        ]
        is False
    )
    assert (
        frontier["research_boundary"][
            "directly_identifiable_customer_precise_consumption_or_critical_"
            "infrastructure_control_record_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1870
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign204_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign204 公用事业客户接入、计量、账单欠费与停复供生命周期"
        "来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert "Campaign204 最终有效会计为 11 次尝试（7 科学、4 基础设施）" in report
        assert "累计历史尝试 `1870`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 205
