...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_214_media_content_"
    "commissioning_production_rights_windowing_release_distribution_"
    "audience_subscription_royalty_lifecycle_source_frontier_20260815.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_214"
...
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_214_terminal_result_v2_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v325_20260815.json"
)
...
def test_campaign214_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c214_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert observed_entries == ledger["effective_entry_count"] == 11
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1989
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign214_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    assert (
        policy["effective_accounting"]["campaign214_infrastructure_failure_count"] == 4
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 215
