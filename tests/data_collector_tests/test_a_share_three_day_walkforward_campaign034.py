"""Pre-return boundary tests for Campaign034 development."""
...
import scripts.a_share_three_day_walkforward_campaign034_features as features
...
def test_campaign034_preregistration_and_single_trial_are_frozen() -> None:
    ...
    assert campaign.build_trial_catalog(spec) == [
        {
            "trial_id": (
                "wf034_single__"
                "intraday_round_tenth_close_avoidance_240m"
            ),
...
def test_campaign034_folds_search_and_stress_boundary_are_closed() -> None:
    ...
    assert (
        spec["research_output_boundary"][
            "campaign033_rescue_or_reweight_allowed"
        ]
        is False
    )
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign034 development trial is "
...
def test_campaign034_survivor_reporting_uses_one_feature_complexity(
    monkeypatch,
) -> None:
    monkeypatch.setitem(
