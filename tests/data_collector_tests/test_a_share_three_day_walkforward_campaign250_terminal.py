...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_250_commercial_vending_"
    "unattended_retail_equipment_service_lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_250"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_250_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v382_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_250_skill_lines_241_320_"
    "output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_250_wrong_campaign249_"
    "frontier_path_failure_20260816.json",
]
...
def test_campaign250_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c250_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 109
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_vending_unattended_retail_equipment_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "commercial_vending_unattended_retail_vending_machine_smart_cabinet_micro_market_kiosk_or_automatic_merchandising_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2387
...
def test_campaign250_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    for key, path in {
        ...
        campaign250_section = report[report.index(heading) :]
        assert "累计历史尝试为 2387" in campaign250_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign250_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign250_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 251
    ...
    assert len(carried) == 43
    assert (
        carried[-1]
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign249_terminal.py::test_campaign249_reports_candidate49_and_research_boundaries"
    )
    assert policy["cumulative_regression_boundary"]["deselection_node_count"] == 65
