...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_197_loyalty_rewards_points_miles_"
    "issuance_outstanding_redemption_expiration_breakage_partner_settlement_"
    "lifecycle_source_frontier_20260815.json"
)
REJECTION = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_197_regulated_utility_rate_case_"
    "cost_recovery_domain_overlap_rejection_20260815.json"
)
LEDGER = ROOT / (
    "data/experiments/short_horizon/historical_walkforward/campaign_197/"
    "research_attempt_ledger_v1.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_197_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v290_20260815.json"
)
...
def test_campaign197_bindings_frontier_and_append_only_chain():
    ...
    assert rejection["bounded_repository_search"][
        "dedicated_prior_regulated_tariff_route_found"
    ]
    assert rejection["bounded_repository_search"]["campaign169_catalog_id"] == "c169_02"
    assert "Campaign169" in rejection["reason"]
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_loyalty_rewards_obligation_lifecycle_frontier_found"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1782
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign197_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign197 客户忠诚积分、里程、兑换、到期与 breakage "
        "生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert "Campaign197 最终有效会计为 17 次尝试（8 科学、9 基础设施）" in report
        assert "累计历史尝试 `1782`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 198
