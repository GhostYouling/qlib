"""Pre-return tests for the single frozen Campaign066 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign066 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "3d6a09af392371f49013884903b564db1bd9762f3c5a55ce2a3110a93e73533d"
)
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_066/no_return/"
    "20260805T093555Z_campaign066_no_return_audit.json"
)


def test_campaign066_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign066_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign066_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "4d3b8436bc55eb0066458ccd73daf632daf0afae1a373822f20ddd5723b74bfa"
    )
    assert audit["status"] == (
    ...
    assert uniqueness["comparison_factor_count"] == 96
...
def test_campaign066_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [sys.executable, "-m", "scripts.a_share_three_day_walkforward_campaign066", "status"],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
        text=True,
...
def test_campaign066_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "-m",
            "scripts.a_share_three_day_preregistration_binding_validator",
            "--data-root",
            "/Volumes/DIsk/qlib-a-share-tushare-1m",
            "docs/a_share_three_day_walkforward_campaign_066_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
