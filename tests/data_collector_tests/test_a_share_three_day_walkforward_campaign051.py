"""Frozen pre-return development-boundary tests for Campaign051."""
...
CAMPAIGN_SHA256 = (
    "e8bdb4a655b875af96aacdfa35a7a49f4a866cd94ad277ac2651bbf9a1a4da00"
)
DATA_ROOT = Path("/Volumes/DIsk/qlib-a-share-tushare-1m")


def test_campaign051_preregistration_and_exact_pre_value_trial_are_frozen() -> None:
...
def test_campaign051_folds_search_and_stress_boundary_are_closed() -> None:
    spec, _ = campaign.load_campaign(campaign.engine_namespace["DEFAULT_CAMPAIGN"])
    ...
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign051 development trial is "
        "retained and the survivor record is frozen."
    )


def test_campaign051_status_has_exactly_one_trial_and_terminal_closed_stress() -> None:
...
def test_campaign051_survivor_reporting_uses_one_feature_complexity(monkeypatch) -> None:
    function = campaign._generated["_survivor_decision_with_canonical_complexity"]
    monkeypatch.setitem(
