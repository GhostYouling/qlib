...
from scripts import a_share_three_day_walkforward_campaign261_features as features
...
def test_definition_and_comparator_orders_are_exact() -> None:
    assert len(features.reconstruct_prior_complete_definitions()) == 159
    assert len(features.reconstruct_complete_definitions()) == 160
...
def test_protocol_binds_source_without_values() -> None:
    ...
    assert (
        spec["research_boundary"][
            "campaign261_candidate_values_computed_or_read_before_freeze"
        ]
        is False
    )
...
def test_extract_matches_pure_formula_and_excludes_0930() -> None:
    raw = _raw()
    values, quality = features.extract_amount_lorenz_gini(raw, symbol="SH600000")
    expected, _, _ = features.formula.compute_amount_lorenz_gini(
        np.arange(2.0, 242.0)[None, :]
    )
    assert values.loc[0, features.FACTOR_SHORT_NAME] == pytest.approx(expected[0])
    changed = raw.copy()
    changed.loc[0, "amount"] = 1e15
    second, _ = features.extract_amount_lorenz_gini(changed, symbol="SH600000")
    assert second.loc[0, features.FACTOR_SHORT_NAME] == pytest.approx(expected[0])
    assert quality["source_rows"] == 241
    assert quality["valid_gini_sessions"] == 1


def test_temporal_permutation_does_not_change_extracted_score() -> None:
    raw = _raw()
    first, _ = features.extract_amount_lorenz_gini(raw, symbol="SH600000")
    changed = raw.copy()
    continuous = changed.loc[1:, "amount"].to_numpy()
    changed.loc[1:, "amount"] = continuous[::-1]
    second, _ = features.extract_amount_lorenz_gini(changed, symbol="SH600000")
    assert first.loc[0, features.FACTOR_SHORT_NAME] == pytest.approx(
...
def test_invalid_or_nonpositive_amount_session_is_missing() -> None:
    for bad in (np.nan, -1.0):
        ...
        amount[7] = bad
        values, quality = features.extract_amount_lorenz_gini(
            _raw(amount), symbol="SH600000"
        )
        assert np.isnan(values.loc[0, features.FACTOR_SHORT_NAME])
        assert quality["valid_gini_sessions"] == 0
    values, quality = features.extract_amount_lorenz_gini(
...
def test_attach_finalize_and_validate_semantics() -> None:
    values, _ = features.extract_amount_lorenz_gini(_raw(), symbol="SH600000")
    attached = features.attach_gini_values(_identity(), values, symbol="SH600000")
...
def test_wrong_projection_or_grid_fails_closed() -> None:
    with pytest.raises(features.Campaign261FeatureError):
        features.extract_amount_lorenz_gini(
            _raw().drop(columns=["amount"]), symbol="SH600000"
        )
    with pytest.raises(features.Campaign261FeatureError):
        features.extract_amount_lorenz_gini(_raw().iloc[:-1].copy(), symbol="SH600000")
