...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_223_cloud_data_center_resource_"
    "availability_metering_billing_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_223"
...
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_223_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v338_20260816.json"
)
...
def test_campaign223_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c223_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 2087
    ...
    assert ledger_v3["cumulative_historical_research_attempt_count"] == 2088
    assert ledger_v3["cumulative_return_reading_development_trial_count"] == 314


def test_campaign223_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign223 公有云与数据中心租户资源、容量可用性、计量计费与服务结算"
        "生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
        ...
        campaign223_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2088`" in campaign223_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign223_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign223_infrastructure_failure_count"] == 4
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 224
