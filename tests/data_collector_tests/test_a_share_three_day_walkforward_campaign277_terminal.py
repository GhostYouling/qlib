...
CATALOG = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_277_concept_scouting_20260824.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_277_prevalue_mechanism_frontier_result_20260824.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_277/research_attempt_ledger_v4.json"
)
STATE = (
    ROOT / "docs/a_share_three_day_iteration_status_20260824_campaign277_terminal.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_277_terminal_prevalue_report_v3.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260824_campaign277_v3.md"
...
def test_catalog_freezes_seven_routes_before_targeted_inventory() -> None:
    assert _sha256(CATALOG) == (
        "5a7c95ca49c94b67f2177a8d561ed9321a8d850b48196a1fdbb3f9eb02eb4de5"
    )
    ...
    assert [route["concept_id"] for route in routes] == [
        "public_cybersecurity_vulnerability_incident_remediation_state",
        "public_clinical_trial_site_enrollment_completion_state",
        "public_adverse_event_inspection_enforcement_remediation_state",
        "public_utility_connection_outage_restoration_state",
        "public_public_private_partnership_concession_performance_state",
        "public_standard_committee_association_participation_state",
        "public_physical_store_poi_open_close_operating_state",
    ]
    for route in routes:
        for field in (
            "formula",
...
def test_result_closes_overlaps_and_defers_three_source_contracts() -> None:
    assert _sha256(RESULT) == (
        "22aee0dde5f2b1de82742986f7772018271932e484afd4c5b1b4fc61e5ada900"
    )
    result = _load(RESULT)
    _assert_bindings(result["authoritative_inputs"])
    routes = result["route_results"]
    assert len(routes) == 7
    assert sum(route["economically_independent"] for route in routes) == 3
    ...
    assert decision["economically_independent_route_count"] == 3
    ...
    assert decision["campaign277_terminalized"] is True


def test_append_only_attempt_chain_and_accounting_are_exact() -> None:
    assert _sha256(LEDGER) == (
        "863988fc9e98fe575207ad98b089b6d7dfae864c64ff5bfa24ba60643eaf7b47"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2827
...
def test_state_keeps_goal_candidate49_and_credential_boundary() -> None:
    assert _sha256(STATE) == (
        "268f997a850b0d43ff74950654d349817c835ebaf8f77230c059dbb80124c822"
    )
    state = _load(STATE)
    immutable_bindings = {
        key: value
    ...
    assert state["goal"]["current_campaign"] == 277
    assert "Campaign278" in state["goal"]["next_safe_work"]
    ...
    assert candidate49["campaign277_plan_or_run_executed"] is False
