...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_224_consumer_lending_origination_"
    "servicing_collections_recovery_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_224"
...
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_224_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v339_20260816.json"
)
...
def test_campaign224_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c224_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 2099
    assert ledger_v2["cumulative_return_reading_development_trial_count"] == 314


def test_campaign224_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign224 消费信贷申请、授信放款、还款逾期与催收回收生命周期"
        "来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
        ...
        campaign224_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2099`" in campaign224_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign224_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign224_infrastructure_failure_count"] == 4
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 225
