"""Pre-return boundary tests for Campaign039 development."""
...
import scripts.a_share_three_day_walkforward_campaign039_features as features
...
def test_campaign039_preregistration_and_single_trial_are_frozen() -> None:
    ...
    assert campaign.build_trial_catalog(spec) == [
        {
            "trial_id": (
                "wf039_single__"
                "intraday_return_time_reversal_asymmetry_236p"
            ),
...
def test_campaign039_folds_search_and_stress_boundary_are_closed() -> None:
    ...
    assert (
        spec["research_output_boundary"][
            "campaign038_rescue_or_reweight_allowed"
        ]
        is False
    )
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign039 development trial is "
...
def test_campaign039_survivor_reporting_uses_one_feature_complexity(
    monkeypatch,
) -> None:
    monkeypatch.setitem(
