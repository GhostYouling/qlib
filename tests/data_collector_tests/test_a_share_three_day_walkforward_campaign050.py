"""Frozen development-boundary tests for Campaign050."""
...
CAMPAIGN_SHA256 = (
    "4188d30f59cdac6dc002fe59b61373786a5f4c64ce089bcdc0a2a86c2efc7660"
)
DATA_ROOT = Path("/Volumes/DIsk/qlib-a-share-tushare-1m")


def test_campaign050_preregistration_and_exact_pre_value_trial_are_frozen() -> None:
    ...
    protocol = json.loads(
        (
            campaign.REPO_ROOT
            / "docs/a_share_three_day_walkforward_campaign_050_no_return_preregistration.json"
        ).read_text()
    )
...
def test_campaign050_folds_search_and_stress_boundary_are_closed() -> None:
    ...
    assert spec["exposed_stress_replay"]["opening_rule"] == (
        "Open only after the single frozen Campaign050 development trial is "
        "retained and the survivor record is frozen."
    )


def test_campaign050_status_has_one_terminal_trial_with_closed_stress() -> None:
...
def test_campaign050_survivor_reporting_uses_one_feature_complexity(
    monkeypatch,
) -> None:
