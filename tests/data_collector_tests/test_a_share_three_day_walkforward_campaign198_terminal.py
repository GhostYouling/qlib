...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_198_software_product_release_"
    "installed_base_migration_support_patch_deprecation_compatibility_lifecycle_"
    "source_frontier_20260815.json"
)
LEDGER = ROOT / (
    "data/experiments/short_horizon/historical_walkforward/campaign_198/"
    "research_attempt_ledger_v2.json"
)
RESULT = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_198_terminal_result_v2_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v292_20260815.json"
)
...
def test_campaign198_bindings_frontier_and_append_only_chain():
    ...
    assert (
        frontier["repository_evidence"][
            "dedicated_software_product_version_installed_base_support_retirement_"
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1794
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign198_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign198 软件版本迁移、补丁支持、退役与兼容性" "生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, file_path in {
        ...
        assert (
            "Campaign198 最终有效会计追加校正为 12 次尝试（7 科学、5 基础设施）"
            in report
        )
        assert "累计历史尝试 `1794`" in report
        assert (
            sha(file_path)
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 199
