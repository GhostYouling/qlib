...
from scripts import a_share_three_day_walkforward_campaign291 as campaign
...
def test_session_state_consensus_is_sign_agnostic_and_counts_equals() -> None:
    output = campaign.session_state_consensus(**_session_inputs())
    assert output[f"{campaign.FACTOR_NAME}_eligible"].all()
    assert output[campaign.FACTOR_NAME].iloc[0] == pytest.approx(1.0)
    assert output[campaign.FACTOR_NAME].iloc[-1] == pytest.approx(1.0)
    assert output[campaign.FACTOR_NAME].iloc[2] == pytest.approx(0.0)
    assert (output["finite_feature_count"] == campaign.FEATURE_COUNT).all()


def test_session_state_consensus_requires_same_session_model_support() -> None:
    ...
    output = campaign.session_state_consensus(**inputs)
...
def test_session_state_consensus_rejects_changed_support_semantics() -> None:
    inputs = _session_inputs()
    inputs["finite_count"][0] = 157
    with pytest.raises(campaign.Campaign291Error):
        campaign.session_state_consensus(**inputs)
