...
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_083_development_implementation_freeze_v2_20260806.json"
)
OUTPUT_ROOT = (
    ROOT
...
def test_development_implementation_freeze_v2_is_live() -> None:
    record = json.loads(FREEZE.read_text(encoding="utf-8"))
    assert (
        record["status"]
        == "frozen_before_campaign083_full_development_retry_after_dependency_repair"
    )
