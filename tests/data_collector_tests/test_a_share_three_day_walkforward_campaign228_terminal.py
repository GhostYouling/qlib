...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_228_industrial_textile_rental_"
    "laundry_hygiene_service_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_228"
...
RESULT_V1 = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_228_terminal_result_20260816.json"
)
RESULT_V2 = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_228_terminal_result_v2_20260816.json"
)
RESULT_V3 = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_228_terminal_result_v3_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v347_20260816.json"
)
...
def test_campaign228_frontier_bindings_and_append_only_chain():
    for path in (
        FRONTIER,
        LEDGER_V1,
        LEDGER_V2,
    ...
    assert [route["route_id"] for route in routes] == [
        f"c228_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger_v2["effective_infrastructure_failure_attempt_count"] == 3
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 2144
    ...
    assert ledger_v3["effective_entry_count"] == 11
    assert ledger_v3["effective_prevalue_scientific_attempt_count"] == 7
    assert ledger_v3["effective_infrastructure_failure_attempt_count"] == 4
    assert ledger_v3["cumulative_historical_research_attempt_count"] == 2145
    assert ledger_v3["cumulative_return_reading_development_trial_count"] == 314


def test_campaign228_reports_candidate49_and_research_boundaries():
    heading = "## Campaign228 工业纺织品租赁、洗涤与卫生服务生命周期来源前沿" "值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign228_section = report[report.index(heading) :]
        assert "累计历史尝试更正为 `2145`" in campaign228_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign228_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign228_infrastructure_failure_count"] == 4
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 229
