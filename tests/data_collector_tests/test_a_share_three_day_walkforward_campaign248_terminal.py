...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_248_commercial_printing_"
    "labeling_finishing_equipment_service_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_248"
...
RESULT_V1 = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_248_terminal_result_20260816.json"
)
RESULT_V2 = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_248_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v380_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_248_skill_lines_601_660_"
    "output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_248_skill_tail_batch_output_"
    "truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_248_unified_report_patch_"
    "context_failure_20260816.json",
]
...
def validate_entries(entries, previous, expected_tip):
    ...
    assert previous == expected_tip


def test_campaign248_frontier_bindings_and_append_only_chain():
    for path in (
        FRONTIER,
        LEDGER_V1,
        LEDGER_V2,
    ...
    assert [route["route_id"] for route in routes] == [
        f"c248_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 107
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_printing_labeling_finishing_equipment_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "commercial_printing_printing_equipment_labeling_equipment_digital_press_offset_press_finishing_equipment_printhead_or_printer_service_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    validate_entries(
        v1["entries"],
        v1["authoritative_predecessor"]["chain_tip_sha256"],
        v1["chain_tip_sha256"],
    )
    validate_entries(
        v2["delta_entries"],
        v2["authoritative_predecessor"]["chain_tip_sha256"],
        v2["chain_tip_sha256"],
    )
    assert v1["effective_entry_count"] == 9
    assert v2["effective_entry_count"] == 10
    assert v2["effective_infrastructure_failure_attempt_count"] == 3
    assert v2["effective_prevalue_scientific_attempt_count"] == 7
    assert v2["cumulative_historical_research_attempt_count"] == 2369
    assert v2["cumulative_return_reading_development_trial_count"] == 314
    entries = v1["entries"] + v2["delta_entries"]
    assert [entry["ordinal"] for entry in entries] == list(range(1, 11))
...
def test_campaign248_reports_candidate49_and_research_boundaries():
    heading = "## Campaign248 商业印刷、标签与印后设备服务合同、资产台账、计量维保、故障修复、配件耗材、软件合规与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign248_section = report[report.index(heading) :]
        assert "累计历史尝试为 2369" in campaign248_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign248_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign248_infrastructure_failure_count"] == 3
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 249
    ...
    assert (
        ...
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign247_terminal.py::test_campaign247_reports_candidate49_and_research_boundaries"
    )
