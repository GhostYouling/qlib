...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v4.json"
)
STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260825_campaign283_terminal_v2.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_terminal_prevalue_report_v2.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign283_v2.md"
...
def test_effective_v4_ledger_replays_all_entries() -> None:
    assert (
        _sha(LEDGER)
        == "34fc42c10a22df2e175a1733eee281c479407a80f88a218ccd00b2ca3e350f16"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2891
    assert ledger["cumulative_return_reading_development_trial_count"] == 315


def test_v2_reports_and_unified_accounting_correction() -> None:
    report = REPORT.read_text(encoding="utf-8")
    handoff = HANDOFF.read_text(encoding="utf-8")
    assert "累计历史尝试 2,891" in report
    assert "持续目标保持 `active`" in handoff
    assert "不构成投资建议" in report
    for path in UNIFIED:
        text = path.read_text(encoding="utf-8")
        assert text.count("### Campaign283 验证会计更正") == 1
        correction = text.split("### Campaign283 验证会计更正", maxsplit=1)[1]
        assert "累计历史尝试 2,891" in correction
        assert "Campaign284" in correction


def test_three_failures_are_value_free_and_not_consumed() -> None:
...
def test_v2_state_binds_current_artifacts_and_active_goal() -> None:
    state = _load(STATE)
    ...
    assert state["effective_accounting"]["campaign283_attempt_count"] == 10
    assert (
        state["effective_accounting"]["campaign283_infrastructure_failure_count"] == 3
    )
