"""Pre-return tests for the single frozen Campaign056 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign056 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_056/"
    "20260803T211608Z_campaign056_no_return_audit.json"
)


def test_campaign056_preregistration_loads_exact_single_trial() -> None:
    spec, observed_sha = campaign.load_campaign(campaign.DEFAULT_PREREGISTRATION)
    catalog = campaign.build_trial_catalog(spec)
    assert observed_sha == (
        "d4a11a41e44e4c8e45c9a0874cc95622452d505e8c49d92880d09b1ebeaee298"
    )
    assert catalog == [
        {
...
def test_campaign056_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign056_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "2a0c05800af7343e22d20c84c7380407df48616cfe86585b8cc52fb6513fcf8a"
    )
    assert audit["status"] == (
    ...
    assert coverage["median_coverage"] == 0.9991718413721302
    assert coverage["p05_coverage"] == 0.9945110191453884
    assert uniqueness["comparison_factor_count"] == 79
...
def test_campaign056_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "scripts/a_share_three_day_walkforward_campaign056.py",
            "status",
        ],
        cwd=REPO_ROOT,
        check=True,
...
def test_campaign056_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            ...
            "docs/a_share_three_day_walkforward_campaign_056_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
    ...
    assert payload["reports"][0]["binding_count"] == 18
