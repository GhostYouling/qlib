...
FRONTIER = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_177_enterprise_ma_restructuring_source_frontier_20260815.json"
)
FAILURE_REPORT_APPEND = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_177_unified_report_append_patch_failure_20260815.json"
)
FAILURE_CUMULATIVE_IMPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_177_cumulative_regression_import_path_failure_20260815.json"
)
FAILURE_ACCOUNTING_SYNC = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_177_accounting_sync_patch_failure_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_177/research_attempt_ledger_v1.json"
)
TERMINAL = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_177_terminal_result_20260815.json"
)
REPORT = (
    ROOT / "docs/a_share_three_day_walkforward_campaign_177_terminal_report_20260815.md"
)
POLICY = (
    ROOT
    / "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_policy_v265_20260815.json"
)
SIGNAL_LEDGER = (
...
def test_campaign177_bindings_and_finite_frontier() -> None:
    for record_path in (
        FRONTIER,
        FAILURE_REPORT_APPEND,
        FAILURE_CUMULATIVE_IMPORT,
...
def test_campaign177_source_gate_and_attempt_chain() -> None:
    ...
    assert (
        frontier["research_boundary"][
            "source_row_filing_document_deal_consideration_valuation_earnout_integration_impairment_candidate_comparator_security_price_or_return_value_read"
        ]
        is False
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1570
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign177_library_policy_and_candidate49_boundaries() -> None:
    ...
    assert policy["complete_historical_feature_library"]["unchanged_from_v264"]
    assert policy["numerical_comparator_eligibility"]["unchanged_from_v264"]
    assert policy["future_campaign_boundary"]["next_campaign"] == 178
...
def test_campaign177_reports_are_appended_exactly_once() -> None:
    heading = "## Campaign177 企业并购重组来源前沿值前终止"
    ...
    for report_path in (
        ...
        assert "Campaign177 共 10 次尝试（7 科学、3 基础设施）" in text


def test_campaign177_records_use_reached_wall_clock_timestamps() -> None:
    for record_path in (
        FRONTIER,
        FAILURE_REPORT_APPEND,
        FAILURE_CUMULATIVE_IMPORT,
