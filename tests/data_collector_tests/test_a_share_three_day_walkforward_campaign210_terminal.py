...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_210_professional_services_"
    "client_opportunity_proposal_engagement_sow_staffing_timesheet_deliverable_"
    "acceptance_unbilled_invoice_collection_dispute_settlement_lifecycle_"
    "source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_210"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_210_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v317_20260815.json"
)
...
def test_campaign210_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c210_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1943
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign210_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign210 专业服务客户机会、提案、项目约定、人员工时、交付验收与"
        "未开票争议结算生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 211
