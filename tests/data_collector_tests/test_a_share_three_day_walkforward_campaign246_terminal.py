...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_246_commercial_material_handling_"
    "conveyor_sortation_palletizing_equipment_service_lifecycle_source_frontier_"
    "20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_246"
...
RESULT_V1 = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_246_terminal_result_20260816.json"
)
RESULT_V2 = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_246_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v377_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_246_authority_worktree_status_"
    "output_truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_246_report_patch_builder_"
    "javascript_syntax_failure_20260816.json",
]
...
def test_campaign246_frontier_bindings_and_append_only_chain():
    for path in (
        FRONTIER,
        LEDGER_V1,
        LEDGER_V2,
    ...
    assert [route["route_id"] for route in routes] == [
        f"c246_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 105
    assert (
        frontier["repository_evidence"][
            "dedicated_commercial_material_handling_conveyor_sortation_palletizing_equipment_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "conveyor_palletizer_or_palletiser_exact_keyword_line_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
...
def test_campaign246_reports_candidate49_and_research_boundaries():
    heading = "## Campaign246 商业物料搬运、输送、分拣与码垛设备服务合同、资产台账、巡检维保、故障修复、配件合规与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign246_section = report[report.index(heading) :]
        assert "累计历史尝试为 2344" in campaign246_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign246_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign246_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 247
    ...
    assert (
        ...
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign245_terminal.py::test_campaign245_reports_candidate49_and_research_boundaries"
    )
