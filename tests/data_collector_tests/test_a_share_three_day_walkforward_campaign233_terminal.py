...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_233_commercial_security_"
    "guarding_patrol_alarm_monitoring_incident_response_access_control_"
    "equipment_maintenance_contract_lifecycle_source_frontier_20260816.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_233"
    / "research_attempt_ledger_v1.json"
)
LEDGER_V2 = LEDGER_V1.with_name("research_attempt_ledger_v2.json")
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_233_terminal_result_v2_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v355_20260816.json"
)
...
def test_campaign233_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c233_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    expected = [
        (LEDGER_V1, 10, 10, 3, 2196),
        (LEDGER_V2, 1, 11, 4, 2197),
    ]
    for (
        path,
        delta_count,
        ...
        assert (
            ledger["effective_infrastructure_failure_attempt_count"]
            == infrastructure_count
        )
        assert ledger["effective_prevalue_scientific_attempt_count"] == 7
        assert ledger["cumulative_historical_research_attempt_count"] == cumulative
        assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign233_reports_candidate49_and_research_boundaries():
    heading = "## Campaign233 商业安保合同、值守巡逻、报警响应、事件闭环与设备维护生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign233_section = report[report.index(heading) :]
        assert "累计历史尝试更正为 `2197`" in campaign233_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign233_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign233_infrastructure_failure_count"] == 4
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 234
