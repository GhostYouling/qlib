...
from scripts import a_share_three_day_walkforward_campaign117 as campaign
...
def test_no_return_admission_is_interface_only_and_bound_to_authority() -> None:
    ...
    assert interface["all_134_numeric_comparisons_passed"] is True
    assert authority["admissible_factor_names"] == [campaign.ADMITTED_FACTOR]
    assert authority["comparison_summary"]["comparison_factor_count"] == 134
...
def test_development_activation_fails_before_return_loader_when_absent(
    ...
    with pytest.raises(campaign.Campaign117Error, match="activation is absent"):
...
def test_implementation_freeze_is_absent_or_live() -> None:
    ...
    assert record["status"] == (
        "frozen_before_campaign117_2019_2023_development_return_read"
    )
