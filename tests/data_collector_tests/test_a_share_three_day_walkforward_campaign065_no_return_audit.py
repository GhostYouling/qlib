...
from scripts import a_share_three_day_walkforward_campaign065_no_return_audit as audit
...
def test_static_snapshot_bindings_are_frozen() -> None:
    ...
    assert result["campaign064_manifest_sha256"] == audit.C64_SNAPSHOT_SHA256
    assert result["complete_definition_count"] == 96
...
def test_protocol_reconstructs_exact_full_and_numeric_orders() -> None:
    ...
    full = audit.candidate.reconstruct_complete_definitions(spec)
    assert len(full) == 96
    ...
    assert comparisons[-2:] == [
        ...
        {"name": audit.C64_FACTOR, "score_direction": "higher"},
    ]
...
def test_frozen_candidate_range_registration_preserves_and_fails_closed() -> None:
    ...
    with pytest.raises(audit.Campaign065NoReturnAuditError):
        audit._install_frozen_candidate_range(conflict)


def test_c62_verifier_columns_can_be_restored_without_reading_values() -> None:
    audit.c62_features._install_engine_globals()
    assert tuple(audit.c62_features.base._engine_globals["OUTPUT_COLUMNS"]) == tuple(
        audit.c62_features.OUTPUT_COLUMNS
    )


def test_campaign063_is_explicit_mechanism_challenge_not_numeric_gate() -> None:
    spec = audit.load_protocol()
    gate = spec["ordered_no_return_gates"]["uniqueness_after_coverage_only"]
    assert gate["structurally_nonnumeric_mechanism_challenges"] == [
        audit.STRUCTURALLY_NONNUMERIC_FACTOR
    ]
    ...
    assert gate["all_95_numeric_comparators_must_pass"] is True
