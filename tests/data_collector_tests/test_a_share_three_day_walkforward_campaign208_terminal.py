...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_208_hospitality_lodging_"
    "reservation_room_inventory_checkin_stay_housekeeping_checkout_folio_"
    "refund_dispute_settlement_lifecycle_source_frontier_20260815.json"
)
LEDGER_V1 = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_208"
    / "research_attempt_ledger_v1.json"
)
LEDGER_V2 = LEDGER_V1.with_name("research_attempt_ledger_v2.json")
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_208_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v314_20260815.json"
)
...
def test_campaign208_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c208_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    entries = ledger_v1["delta_entries"] + ledger_v2["delta_entries"]
    previous = ledger_v1["authoritative_predecessor"]["chain_tip_sha256"]
    for entry in entries:
        evidence = entry.get("artifact") or entry.get("evidence")
        if evidence:
            assert sha(rooted(evidence["path"])) == evidence["sha256"]
    ...
    assert len(entries) == ledger_v2["effective_entry_count"] == 12
    ...
    assert ledger_v2["cumulative_historical_research_attempt_count"] == 1920
    assert ledger_v2["cumulative_return_reading_development_trial_count"] == 314


def test_campaign208_reports_candidate49_and_research_boundaries():
    heading = (
        "## Campaign208 酒店住宿预订、客房库存、入住、清扫周转、退房与账单退款"
        "争议结算生命周期来源前沿值前终止"
    )
    policy = load(POLICY)
    for key, path in {
    ...
    assert policy["future_campaign_boundary"]["next_campaign"] == 209
