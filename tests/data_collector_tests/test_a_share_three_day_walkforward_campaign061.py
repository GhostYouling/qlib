"""Pre-return tests for the single frozen Campaign061 walk-forward trial."""
...
import scripts.a_share_three_day_walkforward_campaign061 as campaign


REPO_ROOT = Path(__file__).resolve().parents[2]
PREREGISTRATION_SHA256 = (
    "ecd3e55dff46a979fc782782e9e0486603fb5fd0dc6d68389d11c62bed156c90"
)
AUDIT = (
    REPO_ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_061/"
    "20260804T183906Z_campaign061_no_return_audit.json"
)


def test_campaign061_preregistration_loads_exact_single_trial() -> None:
...
def test_campaign061_inherits_exact_three_folds_and_survivor_gates() -> None:
...
def test_campaign061_no_return_audit_is_admissible_and_return_closed() -> None:
    ...
    assert hashlib.sha256(AUDIT.read_bytes()).hexdigest() == (
        "0d095077d10dd75668d4ce43f88f3b358aea0f815cbe9b7742a104eb913d16cb"
    )
    assert audit["status"] == (
    ...
    assert audit["provider_request_issued_by_campaign061_audit"] is False


def test_campaign061_stress_is_closed_before_development_survivor_freeze() -> None:
    result = subprocess.run(
        [
            sys.executable,
            "scripts/a_share_three_day_walkforward_campaign061.py",
            "status",
        ],
        cwd=REPO_ROOT,
        check=True,
...
def test_campaign061_development_preregistration_bindings_pass() -> None:
    result = subprocess.run(
        [
            sys.executable,
            ...
            "docs/a_share_three_day_walkforward_campaign_061_preregistration.json",
        ],
        cwd=REPO_ROOT,
        check=True,
        capture_output=True,
    ...
    assert payload["reports"][0]["binding_count"] == 23
