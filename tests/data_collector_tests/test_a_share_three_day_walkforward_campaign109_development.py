...
from scripts import a_share_three_day_walkforward_campaign109 as campaign


ROOT = Path(__file__).resolve().parents[2]
FREEZE = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_109_development_implementation_freeze_20260808.json"
)
...
def test_no_return_admission_is_interface_only_and_bound_to_authority() -> None:
    ...
    assert interface["all_132_numeric_comparisons_passed"] is True
...
def test_development_activation_fails_before_return_loader_when_absent(
    ...
    with pytest.raises(campaign.Campaign109Error, match="activation is absent"):
...
def test_implementation_freeze_is_absent_or_live() -> None:
    ...
    assert record["status"] == (
        "frozen_before_campaign109_2019_2023_development_return_read"
    )
