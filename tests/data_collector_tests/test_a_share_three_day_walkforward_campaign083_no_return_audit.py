...
from scripts import a_share_three_day_walkforward_campaign083_no_return_audit as audit
...
def test_protocol_and_orders_are_frozen_before_coverage() -> None:
    ...
    assert gate["numeric_comparator_count"] == 112
    assert gate["complete_definition_count"] == 114
    assert len(gate["comparison_factors"]) == 112
    assert gate["comparison_factors"][-1] == {
        "name": "intraday_range_amount_peak_timing_alignment_240m",
        "score_direction": "higher",
    }
    assert gate["all_112_numeric_comparators_must_pass"] is True
