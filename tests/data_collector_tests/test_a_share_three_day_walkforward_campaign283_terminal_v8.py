...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v10.json"
)
STATE = (
    DOCS / "a_share_three_day_iteration_status_20260825_campaign283_terminal_v8.json"
)
REPORT = (
    DOCS / "a_share_three_day_walkforward_campaign_283_terminal_prevalue_report_v8.md"
)
HANDOFF = DOCS / "a_share_three_day_strategy_handoff_20260825_campaign283_v8.md"
...
FAILURES = (
    ...
    "a_share_three_day_walkforward_campaign_283_v7_publication_staging_ignored_data_failure_20260825.json",
)
UNIFIED = (
...
def test_effective_v10_ledger_replays_all_entries() -> None:
    assert (
        _sha(LEDGER)
        == "7cee17cce3bb9f546222d9619945d11bfd47f01635fe27519e49065cb77c203d"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2897


def test_v8_reports_and_unified_accounting_correction() -> None:
    assert "累计历史尝试 2,897" in REPORT.read_text(encoding="utf-8")
    assert "持续目标保持 `active`" in HANDOFF.read_text(encoding="utf-8")
    for report_path in UNIFIED:
        text = report_path.read_text(encoding="utf-8")
        assert text.count("### Campaign283 验证会计更正 v7") == 1
        correction = text.split("### Campaign283 验证会计更正 v7", maxsplit=1)[1]
        assert "累计历史尝试 2,897" in correction
        assert "Campaign284" in correction


def test_nine_failures_are_value_free_and_not_consumed() -> None:
...
def test_v8_state_binds_current_artifacts_and_active_goal() -> None:
    state = _load(STATE)
    ...
    assert state["effective_accounting"]["campaign283_attempt_count"] == 16
    assert (
        state["effective_accounting"]["campaign283_infrastructure_failure_count"] == 9
    )
