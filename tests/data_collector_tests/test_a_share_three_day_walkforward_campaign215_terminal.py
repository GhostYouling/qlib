...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_215_restaurant_foodservice_"
    "menu_reservation_waitlist_table_order_kitchen_ticket_preparation_serve_"
    "food_safety_waste_refund_settlement_lifecycle_source_frontier_20260815.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_215"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_215_terminal_result_20260815.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v326_20260815.json"
)
...
def test_campaign215_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c215_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 1999
    assert ledger["cumulative_return_reading_development_trial_count"] == 314


def test_campaign215_reports_candidate49_and_research_boundaries():
    heading = (
    ...
    assert (
        policy["effective_accounting"]["campaign215_infrastructure_failure_count"] == 3
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 216
