...
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_283/research_attempt_ledger_v5.json"
)
STATE = (
    ROOT
    / "docs/a_share_three_day_iteration_status_20260825_campaign283_terminal_v3.json"
)
REPORT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_283_terminal_prevalue_report_v3.md"
)
HANDOFF = ROOT / "docs/a_share_three_day_strategy_handoff_20260825_campaign283_v3.md"
...
def test_effective_v5_ledger_replays_all_entries() -> None:
    assert (
        _sha(LEDGER)
        == "bae351c7fdfa36e12ad2e7e16c3c5f61de9451ab0d33d4fb4eb59ed3c010d0b6"
    )
    ledger = _load(LEDGER)
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2892
    assert ledger["cumulative_return_reading_development_trial_count"] == 315


def test_v3_reports_and_unified_accounting_correction() -> None:
    report = REPORT.read_text(encoding="utf-8")
    handoff = HANDOFF.read_text(encoding="utf-8")
    assert "累计历史尝试 2,892" in report
    assert "持续目标保持 `active`" in handoff
    assert "不构成投资建议" in report
    for report_path in UNIFIED:
        text = report_path.read_text(encoding="utf-8")
        assert text.count("### Campaign283 验证会计更正 v2") == 1
        correction = text.split("### Campaign283 验证会计更正 v2", maxsplit=1)[1]
        assert "累计历史尝试 2,892" in correction
        assert "Campaign284" in correction


def test_four_failures_are_value_free_and_not_consumed() -> None:
    ...
    for failure_path in FAILURES:
        ...
        assert (
            failure["failure_classification"][
                "result_consumed_as_validation_pass"
            ]
            is False
        )
...
def test_v3_state_binds_current_artifacts_and_active_goal() -> None:
    state = _load(STATE)
    ...
    assert state["effective_accounting"]["campaign283_attempt_count"] == 11
    assert (
        state["effective_accounting"]["campaign283_infrastructure_failure_count"]
        == 4
    )
