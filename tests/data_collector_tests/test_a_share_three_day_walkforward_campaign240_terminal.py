...
FRONTIER = ROOT / (
    "docs/a_share_three_day_walkforward_campaign_240_compressed_air_service_"
    "lifecycle_source_frontier_20260816.json"
)
LEDGER = (
    ROOT
    / "data/experiments/short_horizon/historical_walkforward/campaign_240"
    / "research_attempt_ledger_v1.json"
)
RESULT = (
    ROOT
    / "docs/a_share_three_day_walkforward_campaign_240_terminal_result_20260816.json"
)
POLICY = ROOT / (
    "docs/a_share_three_day_walkforward_future_numeric_comparison_eligibility_"
    "policy_v368_20260816.json"
)
FAILURES = [
    ROOT / "docs/a_share_three_day_walkforward_campaign_240_skill_lines_581_680_"
    "truncated_failure_20260816.json",
    ROOT / "docs/a_share_three_day_walkforward_campaign_240_combined_unified_report_"
...
def test_campaign240_frontier_bindings_and_append_only_chain():
    ...
    assert [route["route_id"] for route in routes] == [
        f"c240_{number:02d}" for number in range(1, 8)
    ]
    assert (
    ...
    assert frontier["repository_evidence"]["source_frontier_files_reviewed"] == 99
    assert (
        frontier["repository_evidence"][
            "dedicated_industrial_or_commercial_compressed_air_system_service_definition_present"
        ]
        is False
    )
    assert (
        frontier["repository_evidence"][
            "compressed_air_air_compressor_compressor_service_desiccant_dryer_or_air_receiver_keyword_match_count_in_prior_source_frontiers"
        ]
        == 0
    )
    ...
    assert ledger["cumulative_historical_research_attempt_count"] == 2281
...
def test_campaign240_reports_candidate49_and_research_boundaries():
    heading = "## Campaign240 工业与商业压缩空气系统服务合同、设备台账、维保审计、泄漏故障维修、配件能效、合规与计费生命周期来源前沿值前终止"
    policy = load(POLICY)
    for key, path in {
        ...
        campaign240_section = report[report.index(heading) :]
        assert "累计历史尝试为 `2281`" in campaign240_section
        assert "Candidate49 仍是唯一前瞻候选" in campaign240_section
        assert (
    ...
    assert (
        policy["effective_accounting"]["campaign240_infrastructure_failure_count"] == 2
    )
    assert policy["future_campaign_boundary"]["next_campaign"] == 241
    ...
    assert (
        ...
        == "data_collector_tests/test_a_share_three_day_walkforward_campaign239_terminal.py::test_campaign239_reports_candidate49_and_research_boundaries"
    )
